...
function initialState(c?: Channel | null): FormState {
  ...
  return {
    name: c?.name ?? "",
    allocation_group: c?.allocation_group ?? "",
    type: c?.type ?? "sub2api",
    site_url: c?.site_url ?? "",
    username: c?.username ?? "",
    ...
    self_operated: c?.self_operated ?? false,
    ...
    try {
      ...
      if (isEdit) {
        const body: Record<string, unknown> = {
          name: form.name,
          allocation_group: form.allocation_group.trim(),
          ...
          self_operated: form.self_operated,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="allocation-group">充值分配组（可选）</Label>
            <Input
              id="allocation-group"
              value={form.allocation_group}
              onChange={(e) => setForm({ ...form, allocation_group: e.target.value })}
              placeholder="如：ccmax上游、kiro上游、gpt上游"
              maxLength={64}
              disabled={submitting}
            />
            <p className="text-[11px] text-muted-foreground">
              仅用于面板归类和充值筛选，不会改变上游真实分组或倍率。
            </p>
          </div>
          ...
          <div className="flex items-center justify-between rounded-lg border border-border px-3 py-2">
            <div>
              <p className="text-sm font-medium">自营站</p>
              <p className="text-xs text-muted-foreground">自营站真实成本计 0，改由运营成本记账并计入利润</p>
            </div>
            <Switch
              checked={form.self_operated}
              onCheckedChange={(v) => setForm({ ...form, self_operated: v })}
              disabled={submitting}
            />
          </div>
