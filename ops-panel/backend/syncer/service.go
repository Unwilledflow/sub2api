...
const (
	defaultPoolModeRetryCount = 5
	maxPoolModeRetryCount     = 10
)

func normalizePoolModeRetryCount(value int) int {
	if value <= 0 {
		return defaultPoolModeRetryCount
	}
	if value > maxPoolModeRetryCount {
...
func (s *Service) CreateSyncGroup(in SyncGroupDTO) (*SyncGroupDTO, error) {
	...
	item := &storage.UpstreamSyncGroup{
		...
		PoolModeRetryCount:       normalizePoolModeRetryCount(in.PoolModeRetryCount),
...
func (s *Service) UpdateSyncGroup(id uint, in SyncGroupDTO) (*SyncGroupDTO, error) {
	...
	item.PoolModeRetryCount = normalizePoolModeRetryCount(in.PoolModeRetryCount)
...
func (s *Service) testManagedTargetAccount(
	...
	if model == "" {
		...
		if err != nil {
			if isTransientAccountTestError(err) {
				return fmt.Sprintf("拉取模型列表瞬态异常，保持调度：%s", err.Error()), "", nil
			}
	...
	if _, err := client.TestAccount(ctx, adminTarget, account.ID, model); err != nil {
		if isTransientAccountTestError(err) {
			return fmt.Sprintf("测试模型 %s 瞬态异常，保持调度：%s", model, err.Error()), "", nil
		}
...
// isTransientAccountTestError 判定账号测试错误是否为瞬态（限流/上游故障/网络）。
// 瞬态错误不摘调度：sub2api 对限流有到点自动恢复，摘除会把「到点恢复」
// 换成「等探测成功才回来」，高并发时凭空少容量（guardian 同款原则）。
func isTransientAccountTestError(err error) bool {
	code, ok := connector.HTTPStatusCode(err)
	if !ok {
		return true
	}
...
func (s *Service) buildAdminAccount(syncGroup *storage.UpstreamSyncGroup, syncAccount *storage.UpstreamSyncAccount, ch *storage.Channel, apiKey string, remoteGroupIDs []int64, priority int, rateMultiplier float64) sub2api.AdminAccount {
	...
	// Synced API keys are always placed in pool mode. This keeps imported keys
	// consistent across OpenAI, Gemini, Claude, and other target platforms even
	// when an older sync group was saved before the pool-mode default existed.
	credentials["pool_mode"] = true
	credentials["pool_mode_retry_count"] = normalizePoolModeRetryCount(syncGroup.PoolModeRetryCount)
...
func (s *Service) toSyncGroupDTO(item *storage.UpstreamSyncGroup, ids []uint, accounts []storage.UpstreamSyncAccount) SyncGroupDTO {
	return SyncGroupDTO{
		...
		PoolModeRetryCount:       normalizePoolModeRetryCount(item.PoolModeRetryCount),
