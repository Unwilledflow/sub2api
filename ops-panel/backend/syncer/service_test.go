...
func TestCreateSyncGroupUsesPoolDefaultsAndBoundsRetryCount(t *testing.T) {
	db := openSyncerTestDB(t)
	svc := newTestService(t, db, &fakeChannelService{})
	group, err := svc.CreateSyncGroup(SyncGroupDTO{
		NameTemplate: "sync-{同步分组ID}", TargetID: 1, Platform: "openai",
		PoolModeRetryCount: 0,
		Accounts: []SyncAccountDTO{{SourceChannelID: 1, Enabled: true}},
	})
	if err != nil {
		t.Fatalf("create sync group: %v", err)
	}
	if group.PoolModeRetryCount != defaultPoolModeRetryCount {
		t.Fatalf("retry count = %d, want %d", group.PoolModeRetryCount, defaultPoolModeRetryCount)
	}
	group, err = svc.UpdateSyncGroup(group.ID, SyncGroupDTO{PoolModeRetryCount: 99, Accounts: group.Accounts})
	if err != nil {
		t.Fatalf("update sync group: %v", err)
	}
	if group.PoolModeRetryCount != maxPoolModeRetryCount {
		t.Fatalf("bounded retry count = %d, want %d", group.PoolModeRetryCount, maxPoolModeRetryCount)
	}
}

func TestCreateTargetCreatesCanonicalConnectionShadow(t *testing.T) {
	db := openSyncerTestDB(t)
	svc := newTestService(t, db, &fakeChannelService{})
	target, err := svc.CreateTarget(context.Background(), TargetInput{
		Name: "canonical", BaseURL: "https://canonical.example", AdminAPIKey: "admin-key", Enabled: true,
	})
	if err != nil {
	...
	if err := db.First(&shadow, target.ID).Error; err != nil {
		t.Fatalf("find connection shadow: %v", err)
	}
	if shadow.ID != target.ID || shadow.Name != target.Name || shadow.BaseURL != target.BaseURL ||
		!shadow.Enabled || shadow.SyncMode != storage.ConnectionSyncModeCanonicalTarget {
		t.Fatalf("connection shadow = %#v", shadow)
	}
	if shadow.AdminAPIKey != "" {
		t.Fatalf("connection shadow duplicated canonical credential")
	}
}

func TestUpdateTargetChangesOnlyCanonicalConnectionShadow(t *testing.T) {
	db := openSyncerTestDB(t)
	svc := newTestService(t, db, &fakeChannelService{})
	target, err := svc.CreateTarget(context.Background(), TargetInput{
		Name: "before", BaseURL: "https://before.example", AdminAPIKey: "admin-key", Enabled: true,
	})
	if err != nil {
		t.Fatalf("create target: %v", err)
	}

	updated, err := svc.UpdateTarget(context.Background(), target.ID, TargetInput{
		Name: "after", BaseURL: "https://after.example", Enabled: false,
	})
	if err != nil {
	...
	if shadow.Name != updated.Name || shadow.BaseURL != updated.BaseURL || shadow.Enabled ||
		shadow.SyncMode != storage.ConnectionSyncModeCanonicalTarget || shadow.AdminAPIKey != "" {
		t.Fatalf("updated connection shadow = %#v", shadow)
	}
}

func TestUpdateImportedTargetLeavesLegacyConnectionUnchanged(t *testing.T) {
	db := openSyncerTestDB(t)
	svc := newTestService(t, db, &fakeChannelService{})
	target := storage.UpstreamSyncTarget{
		ID: 41, Name: "imported", BaseURL: "https://canonical.example", AdminAPIKeyCipher: "cipher", Enabled: true,
	}
	if err := db.Create(&target).Error; err != nil {
		t.Fatalf("create imported target: %v", err)
	}
	legacy := storage.Connection{
		ID: 41, Name: "legacy", BaseURL: "https://legacy.example", AdminAPIKey: "legacy-cipher",
		Enabled: true, SyncMode: "manual",
	}
	if err := db.Create(&legacy).Error; err != nil {
		t.Fatalf("create legacy connection: %v", err)
	}

	if _, err := svc.UpdateTarget(context.Background(), target.ID, TargetInput{
		Name: "updated", BaseURL: "https://updated.example", Enabled: false,
	}); err != nil {
		t.Fatalf("update imported target: %v", err)
	}
	var stored storage.Connection
	if err := db.First(&stored, legacy.ID).Error; err != nil {
		t.Fatalf("find legacy connection: %v", err)
	}
	if stored.Name != legacy.Name || stored.BaseURL != legacy.BaseURL || stored.AdminAPIKey != legacy.AdminAPIKey ||
		stored.Enabled != legacy.Enabled || stored.SyncMode != legacy.SyncMode {
		t.Fatalf("legacy connection changed: before=%#v after=%#v", legacy, stored)
	}
}

func TestDeleteTargetRemovesCanonicalConnectionShadow(t *testing.T) {
	db := openSyncerTestDB(t)
	svc := newTestService(t, db, &fakeChannelService{})
	target, err := svc.CreateTarget(context.Background(), TargetInput{
		Name: "canonical", BaseURL: "https://canonical.example", AdminAPIKey: "admin-key", Enabled: true,
	...
	if err := db.First(&storage.Connection{}, target.ID).Error; !errors.Is(err, gorm.ErrRecordNotFound) {
		t.Fatalf("connection shadow lookup error = %v", err)
	}
}

func TestDeleteImportedTargetLeavesLegacyConnection(t *testing.T) {
	db := openSyncerTestDB(t)
	svc := newTestService(t, db, &fakeChannelService{})
	target := storage.UpstreamSyncTarget{
		ID: 42, Name: "imported", BaseURL: "https://canonical.example", AdminAPIKeyCipher: "cipher", Enabled: true,
	}
	if err := db.Create(&target).Error; err != nil {
		t.Fatalf("create imported target: %v", err)
	}
	legacy := storage.Connection{
		ID: 42, Name: "legacy", BaseURL: "https://legacy.example", AdminAPIKey: "legacy-cipher",
		Enabled: true, SyncMode: "manual",
	}
	if err := db.Create(&legacy).Error; err != nil {
		t.Fatalf("create legacy connection: %v", err)
	...
	if err := svc.DeleteTarget(target.ID); err != nil {
		t.Fatalf("delete imported target: %v", err)
	}
	var stored storage.Connection
	if err := db.First(&stored, legacy.ID).Error; err != nil {
		t.Fatalf("legacy connection was deleted: %v", err)
	}
	if stored.SyncMode != "manual" || stored.AdminAPIKey != legacy.AdminAPIKey {
		t.Fatalf("legacy connection changed: %#v", stored)
	}
}

func TestCreateTargetConnectionCollisionRollsBack(t *testing.T) {
	db := openSyncerTestDB(t)
	svc := newTestService(t, db, &fakeChannelService{})
	legacy := storage.Connection{
		ID: 1, Name: "legacy", BaseURL: "https://legacy.example", AdminAPIKey: "legacy-cipher",
		Enabled: true, SyncMode: "manual",
	}
	if err := db.Create(&legacy).Error; err != nil {
		t.Fatalf("create legacy connection: %v", err)
	...
	if _, err := svc.CreateTarget(context.Background(), TargetInput{
		...
		t.Fatal("create target succeeded despite connection id collision")
	}
	var targetCount int64
	if err := db.Model(&storage.UpstreamSyncTarget{}).Count(&targetCount).Error; err != nil {
		t.Fatalf("count targets: %v", err)
	}
	if targetCount != 0 {
		t.Fatalf("target count = %d, want 0", targetCount)
	}
	var stored storage.Connection
	if err := db.First(&stored, legacy.ID).Error; err != nil {
		t.Fatalf("legacy connection missing: %v", err)
	}
	if stored.SyncMode != legacy.SyncMode || stored.AdminAPIKey != legacy.AdminAPIKey {
		t.Fatalf("legacy connection changed: %#v", stored)
	}
}
