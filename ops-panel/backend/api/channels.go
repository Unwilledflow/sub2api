...
type channelInput struct {
	...
	AllocationGroup             string                 `json:"allocation_group"`
...
func listChannels(c *gin.Context, d *Deps) {
	if c.Query("page") != "" || c.Query("page_size") != "" {
		...
		list, total, err := d.Channels.ListPage(page, pageSize, c.Query("allocation_group"))
		if err != nil {
...
func createChannel(c *gin.Context, d *Deps) {
	...
	created, err := d.ChannelSvc.Create(channel.CreateInput{
		Name:                        in.Name,
		AllocationGroup:             in.AllocationGroup,
		Type:                        in.Type,
		SiteURL:                     in.SiteURL,
