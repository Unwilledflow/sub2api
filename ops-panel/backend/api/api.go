...
func Register(r *gin.Engine, d *Deps) {
	...
	}
	{
		...
		registerAuth(api, d)
		api.POST("/client-error", func(c *gin.Context) {
			var in struct {
			...
			if d.Log != nil {
				d.Log.Error("client.render.error",
					"message", in.Message,
					"stack", in.Stack,
					"component_stack", in.ComponentStack,
				)
			} else {
				c.Error(fmt.Errorf("client render error: %s", in.Message))
			}
		...
		registerOperations(api, d)
		registerMobile(api, d)
		registerMobileOps(api, d)
	}
