...
func TestRefreshRatesSyncAnnouncementsAndNotify(t *testing.T) {
	...
	ch := &storage.Channel{
		...
		MonitorEnabled: true,
		// 显式配置倍率 1，避免倍率采集额外探测上游 /api/status，保持本用例只关注公告同步。
		RechargeMultiplier:     ptrFloat64(1),
		RechargeMultiplierMode: "divide",
	}
...
func TestRefreshRatesSkipsAnnouncementsWhenIgnored(t *testing.T) {
	...
	ch := &storage.Channel{
		...
		// 显式配置倍率 1，避免倍率采集额外探测上游 /api/status，保持 status calls 断言语义。
		RechargeMultiplier:     ptrFloat64(1),
		RechargeMultiplierMode: "divide",
	}
	if err := channels.Create(ch); err != nil {
...
func TestRefreshRatesAppliesRechargeMultiplierBeforePersistence(t *testing.T) {
	db := openTestDB(t)
	channels := storage.NewChannels(db)
	authSessions := storage.NewAuthSessions(db)
	captchas := storage.NewCaptchas(db)
	...
	apiSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/v1/groups/available":
			...
			_, _ = w.Write([]byte(`{"code":0,"message":"success","data":[{"id":7,"name":"normalized","rate_multiplier":` + jsonNumber(ratio) + `}]}`))
		case "/api/v1/groups/rates":
			_, _ = w.Write([]byte(`{"code":0,"message":"success","data":{}}`))
		default:
			http.NotFound(w, r)
	...
	if len(snapshots) != 1 || snapshots[0].Ratio != 3 {
		t.Fatalf("snapshots = %#v, want normalized ratio 3", snapshots)
	}
	changes, err := rates.ListChanges(ch.ID, 10)
	if err != nil {
		t.Fatalf("list changes: %v", err)
	}
	if len(changes) != 1 || changes[0].OldRatio == nil || *changes[0].OldRatio != 2 || changes[0].NewRatio != 3 {
		t.Fatalf("changes = %#v, want normalized 2 -> 3", changes)
	}
}
...
func TestRefreshRatesFollowsUpstreamRechargeMultiplier(t *testing.T) {
	db := openTestDB(t)
	channels := storage.NewChannels(db)
	authSessions := storage.NewAuthSessions(db)
	captchas := storage.NewCaptchas(db)
	...
	apiSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/v1/payment/checkout-info":
			checkoutCalls.Add(1)
			_, _ = w.Write([]byte(`{"code":0,"message":"success","data":{"balance_recharge_multiplier":10,"methods":{}}}`))
		case "/api/v1/groups/available":
			_, _ = w.Write([]byte(`{"code":0,"message":"success","data":[{"id":7,"name":"normalized","description":"","rate_multiplier":1}]}`))
		case "/api/v1/groups/rates":
			_, _ = w.Write([]byte(`{"code":0,"message":"success","data":{}}`))
		default:
			http.NotFound(w, r)
	...
	if got := checkoutCalls.Load(); got == 0 {
		t.Fatalf("checkout-info calls = %d, want > 0", got)
	}
	snapshots, err := rates.ListByChannel(ch.ID)
	if err != nil {
		t.Fatalf("list snapshots: %v", err)
	}
	if len(snapshots) != 1 || snapshots[0].Ratio != 0.1 {
		t.Fatalf("snapshots = %#v, want normalized ratio 0.1 (1 / 10)", snapshots)
	}
}
