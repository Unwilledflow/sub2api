...
func rateScanDue(channel storage.Channel, now time.Time) bool {
	if channel.RateIntervalMinutes <= 0 || channel.LastRateScanAt == nil {
		return true
	}
	return !now.Before(channel.LastRateScanAt.Add(time.Duration(channel.RateIntervalMinutes) * time.Minute))
}
...
func (s *Service) RefreshBalance(ctx context.Context, c *storage.Channel) error {
	...
	if err != nil {
		if status, ok := connector.HTTPStatusCode(err); ok && status == 404 {
			// Some compatible upstreams expose balance but not dashboard stats.
			// Keep the successful balance sample and retain the previous cost data.
			s.log.Warn("optional cost endpoint unavailable; balance retained", "channel", c.Name, "err", err)
			progress.OK(ctx, progress.StageCost, "上游未提供消费统计，保留历史消费数据", map[string]any{"available": false})
			costRes = nil
		} else {
...
func (s *Service) RefreshRates(ctx context.Context, c *storage.Channel) error {
	...
	multiplier := c.RechargeMultiplier
	if multiplier == nil {
		if provider, ok := conn.(connector.RechargeMultiplierProvider); ok {
			if m, mErr := provider.ResolveRechargeMultiplier(ctx, resolved, session); mErr == nil && m != nil {
				multiplier = m
			}
		}
	}
	for _, r := range results {
		r.Ratio = connector.ApplyRechargeMultiplier(r.Ratio, multiplier, c.RechargeMultiplierMode)
	...
	if err := s.channels.UpdateLastRateScan(c.ID, now); err != nil {
		return err
	}
