...
func TestChannelAllocationGroupPersistsAndFilters(t *testing.T) {
	db := openTestDB(t)
	channels := NewChannels(db)
	...
	items := []*Channel{
		{
			Name:            "ccmax-source",
			AllocationGroup: "ccmax upstream",
			Type:            ChannelTypeSub2API,
			SiteURL:         "https://ccmax.example.com",
			Username:        "ccmax-user",
			PasswordCipher:  "cipher-1",
		},
		{
			Name:            "kiro-source",
			AllocationGroup: "kiro upstream",
			Type:            ChannelTypeSub2API,
			SiteURL:         "https://kiro.example.com",
			Username:        "kiro-user",
			PasswordCipher:  "cipher-2",
		},
		{
			Name:            "ccmax-source-2",
			AllocationGroup: "ccmax upstream",
			Type:            ChannelTypeSub2API,
			SiteURL:         "https://ccmax-2.example.com",
			Username:        "ccmax-user-2",
			PasswordCipher:  "cipher-3",
		},
	}
	for _, item := range items {
		if err := channels.Create(item); err != nil {
			t.Fatalf("create channel %q: %v", item.Name, err)
		}
	}

	got, err := channels.FindByID(items[0].ID)
	if err != nil {
		t.Fatalf("find channel: %v", err)
	}
	if got.AllocationGroup != "ccmax upstream" {
		t.Fatalf("allocation_group = %q, want %q", got.AllocationGroup, "ccmax upstream")
	}

	got.AllocationGroup = ""
	if err := channels.Update(got); err != nil {
		t.Fatalf("clear allocation group: %v", err)
	}
	cleared, err := channels.FindByID(items[0].ID)
	if err != nil {
		t.Fatalf("find cleared channel: %v", err)
	}
	if cleared.AllocationGroup != "" {
		t.Fatalf("cleared allocation_group = %q, want empty", cleared.AllocationGroup)
	}

	list, total, err := channels.ListPage(1, 1, "ccmax upstream")
	if err != nil {
		t.Fatalf("filter channels: %v", err)
	}
	if total != 1 || len(list) != 1 || list[0].Name != "ccmax-source-2" {
		t.Fatalf("filtered page = %#v total=%d, want one remaining ccmax channel", list, total)
	}

	list, total, err = channels.ListPage(2, 1, "ccmax upstream")
	if err != nil {
		t.Fatalf("filter second page: %v", err)
	}
	if total != 1 || len(list) != 0 {
		t.Fatalf("second filtered page = %#v total=%d, want empty page with total 1", list, total)
	}
}
