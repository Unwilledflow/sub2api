...
type Channel struct {
	...
	AllocationGroup        string         `gorm:"size:64;not null;default:'';index" json:"allocation_group"`
	...
	SelfOperated           bool           `gorm:"default:false" json:"self_operated"`
	...
	RateIntervalMinutes    int            `gorm:"not null;default:0" json:"rate_interval_minutes"`
...
const ConnectionSyncModeCanonicalTarget = "canonical_target"

// Connection is the retained legacy ownership row used by extension tables.
// Canonical targets only mirror identity fields here; credentials stay in
// upstream_sync_targets using the canonical APP_SECRET encryption format.
type Connection struct {
	ID          uint       `gorm:"type:integer;primaryKey;autoIncrement" json:"id"`
	Name        string     `gorm:"not null" json:"name"`
	BaseURL     string     `gorm:"not null" json:"base_url"`
	AdminAPIKey string     `gorm:"not null" json:"-"`
	Enabled     bool       `gorm:"not null;default:true" json:"enabled"`
	SyncMode    string     `gorm:"not null;default:'manual'" json:"sync_mode"`
	LastCheckAt *time.Time `json:"last_check_at,omitempty"`
	CreatedAt   time.Time  `json:"created_at"`
	UpdatedAt   time.Time  `json:"updated_at"`
}
...
type UpstreamSyncGroup struct {
	...
	PoolModeRetryCount       int        `gorm:"default:5" json:"pool_mode_retry_count"`
