...
type Config struct {
	...
	DefaultTarget DefaultTargetConfig `mapstructure:"defaultTarget" yaml:"-" json:"-"`
...
type DatabaseConfig struct {
	...
	URL          string `mapstructure:"url" yaml:"-" json:"-"`
...
// DefaultTargetConfig imports the legacy single-target environment into the
// canonical target store on first startup. It is never persisted to config.yaml.
type DefaultTargetConfig struct {
	Name        string `mapstructure:"name" yaml:"-" json:"-"`
	BaseURL     string `mapstructure:"baseURL" yaml:"-" json:"-"`
	AdminAPIKey string `mapstructure:"adminAPIKey" yaml:"-" json:"-"`
}
...
//	DATABASE_URL         -> database.url (PostgreSQL; takes precedence over driver)
...
func load(path string, withEnv bool) (*Config, string, error) {
	...
	if withEnv {
		...
		v.AutomaticEnv()
		// PostgreSQL deployments preserve the legacy OPS_PANEL_ADMIN_* names;
		// ADMIN_* remain supported for SQLite/MySQL and older upstream config.
		_ = v.BindEnv("security.appSecret", "APP_SECRET")
		_ = v.BindEnv("auth.enabled", "AUTH_ENABLED")
		_ = v.BindEnv("auth.username", "OPS_PANEL_ADMIN_USERNAME", "ADMIN_USERNAME")
		_ = v.BindEnv("auth.password", "OPS_PANEL_ADMIN_PASSWORD", "ADMIN_PASSWORD")
		...
		_ = v.BindEnv("defaultTarget.name", "SUB2API_SITE_NAME")
		_ = v.BindEnv("defaultTarget.baseURL", "SUB2API_BASE_URL", "SUB2API_PUBLIC_URL")
		_ = v.BindEnv("defaultTarget.adminAPIKey", "SUB2API_ADMIN_API_KEY", "OPS_PANEL_ADMIN_API_KEY")
