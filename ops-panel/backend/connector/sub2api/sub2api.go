...
func (c *Client) GetCosts(ctx context.Context, ch *connector.Channel, session *connector.AuthSession) (*connector.CostResult, error) {
	...
	if stats.TodayActualCost == nil || stats.TotalActualCost == nil {
		return nil, fmt.Errorf("sub2api dashboard stats missing actual cost fields")
	}
	multiplier := c.rechargeMultiplier(ctx, ch, session)
...
// 渠道配置优先，未配置时跟随上游 balance_recharge_multiplier，与 GetBalance 口径一致。
func (c *Client) ResolveRechargeMultiplier(ctx context.Context, ch *connector.Channel, session *connector.AuthSession) (*float64, error) {
	return c.rechargeMultiplier(ctx, ch, session), nil
}
