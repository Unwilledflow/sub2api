...
func TestGetCosts(t *testing.T) {
	...
	mux.HandleFunc("/api/v1/usage/dashboard/stats", func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"code":0,"message":"","data":{"today_actual_cost":99.99,"total_actual_cost":88.88,"today_account_cost":1.23,"total_account_cost":45.67}}`))
	})
	srv := httptest.NewServer(mux)
	defer srv.Close()
	...
	if res.TodayCost != 99.99 {
		t.Fatalf("today cost = %v, want 99.99", res.TodayCost)
	}
	if res.TotalCost != 88.88 {
		t.Fatalf("total cost = %v, want 88.88", res.TotalCost)
	}
}
...
func TestGetCostsAppliesUpstreamRechargeMultiplier(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/api/v1/usage/dashboard/stats", func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"code":0,"message":"","data":{"today_actual_cost":14.4,"total_actual_cost":72,"today_account_cost":10.8,"total_account_cost":36}}`))
	})
...
func TestGetCostsRequiresActualCostFields(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/api/v1/usage/dashboard/stats", func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"code":0,"message":"","data":{"today_account_cost":1.23,"total_account_cost":45.67}}`))
	})
	srv := httptest.NewServer(mux)
	defer srv.Close()

	c := New()
	_, err := c.GetCosts(context.Background(), &connector.Channel{SiteURL: srv.URL}, &connector.AuthSession{AccessToken: "token"})
	if err == nil || !strings.Contains(err.Error(), "missing actual cost fields") {
		t.Fatalf("GetCosts error = %v, want missing actual cost fields", err)
	}
}
