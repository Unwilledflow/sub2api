...
func (a *AdminClient) CreateGroup(ctx context.Context, t AdminTarget, input AdminGroupCreateInput) (*AdminGroup, error) {
	...
	resp, err := a.client.http.R().
		...
		SetBody(body).
		Post(strings.TrimRight(t.BaseURL, "/") + "/api/v1/admin/groups")
	if err != nil {
		return nil, err
	}
	if resp.IsError() {
		return nil, fmt.Errorf("create admin group: %w", connector.HTTPStatusError(resp.StatusCode(), resp.Body()))
	}
...
func (a *AdminClient) ClearAccountError(ctx context.Context, t AdminTarget, id int64) (*AdminAccount, error) {
	return a.postAccountAction(ctx, t, id, "clear-error")
}

func (a *AdminClient) RefreshAccount(ctx context.Context, t AdminTarget, id int64) (*AdminAccount, error) {
	return a.postAccountAction(ctx, t, id, "refresh")
}

func (a *AdminClient) GetAccountUsage(ctx context.Context, t AdminTarget, id int64) (map[string]any, error) {
	body, err := a.getJSON(ctx, t, "/api/v1/admin/accounts/"+strconv.FormatInt(id, 10)+"/usage")
	if err != nil {
		return nil, err
	}
	out := map[string]any{}
	if err := json.Unmarshal(body, &out); err != nil {
		return nil, fmt.Errorf("decode admin account usage: %w", err)
	}
	return out, nil
}

func (a *AdminClient) postAccountAction(ctx context.Context, t AdminTarget, id int64, action string) (*AdminAccount, error) {
	resp, err := a.client.http.R().
		SetContext(ctx).
		SetHeader("Content-Type", "application/json").
		SetHeader("x-api-key", t.APIKey).
		SetBody([]byte(`{}`)).
		Post(strings.TrimRight(t.BaseURL, "/") + "/api/v1/admin/accounts/" + strconv.FormatInt(id, 10) + "/" + action)
	if err != nil {
		return nil, err
	}
	if resp.IsError() {
		return nil, fmt.Errorf("%s admin account: %w", action, connector.HTTPStatusError(resp.StatusCode(), resp.Body()))
	}
...
// DeleteChannelMonitor removes an operations-owned native monitor. A missing
// monitor is already in the desired state, so 404 is treated as success.
func (a *AdminClient) DeleteChannelMonitor(ctx context.Context, t AdminTarget, id int64) error {
	if id <= 0 {
		return nil
	}
	resp, err := a.client.http.R().
		SetContext(ctx).
		SetHeader("x-api-key", t.APIKey).
		Delete(strings.TrimRight(t.BaseURL, "/") + "/api/v1/admin/channel-monitors/" + strconv.FormatInt(id, 10))
	if err != nil {
		return err
	}
	...
	if resp.IsError() {
		return fmt.Errorf("delete admin channel monitor: %w", connector.HTTPStatusError(resp.StatusCode(), resp.Body()))
	}
	return nil
}
...
func decodeAdminGroup(body []byte) (*AdminGroup, error) {
	...
	if err := json.Unmarshal(body, &wrapped); err == nil && wrapped.Data != nil {
		...
		if out.Ratio == 0 && out.RateMultiplier != 0 {
			out.Ratio = out.RateMultiplier
		}
		return &out, nil
	}
	var out AdminGroup
