...
// ResolveRechargeMultiplier 实现 connector.RechargeMultiplierProvider：
// 渠道配置优先，未配置时从 /api/status 的 price 推导（1/price），与 GetBalance 口径一致。
func (c *Client) ResolveRechargeMultiplier(ctx context.Context, ch *connector.Channel, session *connector.AuthSession) (*float64, error) {
	site := strings.TrimRight(ch.SiteURL, "/")
	statusBody, err := c.getJSON(ctx, site+"/api/status", nil)
	if err != nil {
		return nil, err
	...
	return newAPIRechargeMultiplier(ch, status.Price), nil
}
...
func (c *Client) CreateAPIKey(ctx context.Context, ch *connector.Channel, session *connector.AuthSession, req connector.APIKeyCreateRequest) (*connector.APIKey, error) {
	...
	for i := range page.Items {
		if strings.TrimSpace(page.Items[i].Name) == name && page.Items[i].ID > 0 {
			return c.ensureFullAPIKey(ctx, ch, session, page.Items[i])
		}
	}
...
// ensureFullAPIKey 上游创建接口可能不返回完整密钥（如 NewAPI 对普通用户
// 仅返回脱敏值 sk-xxxx********xxxx），此时回退调用 reveal 接口补全，
// 避免把脱敏值当完整密钥透传给前端。
func (c *Client) ensureFullAPIKey(ctx context.Context, ch *connector.Channel, session *connector.AuthSession, item connector.APIKey) (*connector.APIKey, error) {
	if strings.TrimSpace(item.Key) == "" || strings.Contains(item.Key, "*") {
		key, err := c.RevealAPIKey(ctx, ch, session, item.ID)
		if err != nil {
			return nil, fmt.Errorf("newapi create api key reveal: %w", err)
		}
		item.Key = key
	}
	return &item, nil
