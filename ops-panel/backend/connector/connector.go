...
// RechargeMultiplierProvider 是能解析实际充值倍率的 connector 可选能力。
// 与 GetBalance / GetCosts 使用同一倍率口径（渠道配置优先，否则跟随上游），
// 供倍率采集落库换算使用，避免 rate_snapshots.ratio 与面板余额口径不一致。
type RechargeMultiplierProvider interface {
	ResolveRechargeMultiplier(ctx context.Context, channel *Channel, session *AuthSession) (*float64, error)
}
