...
// NormalizeUsageBucketsForProtocol converts only OpenAI-family usage into
// mutually exclusive billing buckets. Anthropic's input_tokens is already
// exclusive of cache_read/cache_creation and must not be split a second time.
func NormalizeUsageBucketsForProtocol(raw UsageTokens, upstreamProtocol string) UsageTokens {
	kind := protocol.Kind(strings.TrimSpace(upstreamProtocol))
	if protocol.IsOpenAIFamily(kind) {
