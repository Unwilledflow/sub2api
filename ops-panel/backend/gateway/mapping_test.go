...
func TestNormalizeUsageBucketsForProtocol(t *testing.T) {
	raw := UsageTokens{InputTokens: 100, CacheReadTokens: 30, CacheCreationTokens: 10}
	openAI := NormalizeUsageBucketsForProtocol(raw, "openai_responses")
	if openAI.InputTokens != 60 {
		t.Fatalf("OpenAI input should exclude cache buckets: %+v", openAI)
	}

	anthropic := NormalizeUsageBucketsForProtocol(raw, "anthropic")
	if anthropic != raw {
		t.Fatalf("Anthropic usage must remain mutually exclusive as received: got %+v want %+v", anthropic, raw)
	}
}
