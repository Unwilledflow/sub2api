...
- Go 版本必须是 **1.26.6**：三个 workflow 都用 `go-version-file: backend/go.mod` 取版本，随后硬断言 `go version | grep -q 'go1.26.6'`。升级 Go 时要同时改 `backend/go.mod`、`backend-ci.yml`（两处）、`release.yml`、`security-scan.yml` 里的这句断言，以及三个 Dockerfile 里的 Go 构建镜像（`Dockerfile` / `deploy/Dockerfile` 的 `ARG GOLANG_IMAGE`、`backend/Dockerfile` 的 `FROM golang:`）。
...
```bash
cd backend
go generate ./ent  # 重新生成 ent 代码（保持 json.RawMessage，生产构建不依赖 jsonv2 实验特性）
