...
type RefundRequest struct {
	...
	Reason  string
	// RefundSeq is the 1-based refund request sequence number for the order.
	// Providers use it to derive a deterministic upstream idempotency key
	// (e.g. Alipay out_request_no) so that retrying the same request reuses
	// the same key instead of submitting a brand-new refund.
	RefundSeq int64
}
