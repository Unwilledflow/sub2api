...
type ChannelLimits struct {
	...
	// Device 限定该实例服务的终端类型："mobile" 仅移动端、"desktop" 仅桌面端、
	// 空表示不限。用于把移动端流量路由到易支付、桌面端留在官方支付宝。
...
const deviceContextKey contextKey = "payment.device"
...
// WithDeviceContext 标记本次下单请求的终端类型，供实例选择按 device 过滤。
func WithDeviceContext(ctx context.Context, isMobile bool) context.Context {
	if isMobile {
		return context.WithValue(ctx, deviceContextKey, DeviceMobile)
	}
	return context.WithValue(ctx, deviceContextKey, DeviceDesktop)
}
...
func DeviceFromContext(ctx context.Context) string {
	...
	device, _ := ctx.Value(deviceContextKey).(string)
...
func (lb *DefaultLoadBalancer) queryEnabledInstances(
	...
	expectedDevice := DeviceFromContext(ctx)
	match := func(applyDeviceFilter bool) []*dbent.PaymentProviderInstance {
		var out []*dbent.PaymentProviderInstance
		for _, inst := range instances {
			if applyDeviceFilter && instanceExcludedByDevice(inst, paymentType, expectedDevice) {
				continue
			}
	...
	matched = match(expectedDevice != "")
	if len(matched) == 0 && expectedDevice != "" {
		slog.Warn("no instance matched device filter, falling back to all instances",
			"payment_type", paymentType, "device", expectedDevice)
		matched = match(false)
	}
	if len(matched) == 0 {
...
// InstanceDeviceForType 返回实例在指定支付方式下声明的 device 限定（mobile/desktop/空）。
func InstanceDeviceForType(inst *dbent.PaymentProviderInstance, paymentType PaymentType) string {
	return getInstanceChannelLimits(inst, paymentType).Device
}

// instanceExcludedByDevice 判断实例是否因 device 限定与请求终端不匹配而排除。
func instanceExcludedByDevice(inst *dbent.PaymentProviderInstance, paymentType PaymentType, expectedDevice string) bool {
	if strings.TrimSpace(expectedDevice) == "" {
		return false
	}
	device := getInstanceChannelLimits(inst, paymentType).Device
	return device != "" && !strings.EqualFold(device, expectedDevice)
}

// getInstanceChannelLimits returns the channel limits for a specific payment type.
func getInstanceChannelLimits(inst *dbent.PaymentProviderInstance, paymentType PaymentType) ChannelLimits {	if inst.Limits == "" {
...
// decryptConfig parses AES-256-GCM ciphertext and plaintext JSON records.
// Plaintext compatibility is retained for existing deployments and instances
// running without a configured encryption key. Unreadable values are treated
// as empty so the service keeps running while the admin re-enters the config.
