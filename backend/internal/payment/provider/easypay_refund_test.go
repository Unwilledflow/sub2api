...
func TestEasyPayRefundDoesNotSwitchToTradeNoWhenOutTradeNoNotFound(t *testing.T) {
	t.Parallel()
	...
	if err == nil {
		t.Fatal("Refund returned nil error, want order-not-found without channel switch")
	}
	if !strings.Contains(err.Error(), "订单编号不存在") {
		t.Fatalf("Refund error = %q, want 订单编号不存在", err.Error())
	}
	if len(gotForms) != 1 {
		t.Fatalf("refund attempts = %d, want 1 (no auto channel switch)", len(gotForms))
	}
	if got := gotForms[0].Get("out_trade_no"); got != "out-456" {
		t.Fatalf("form[out_trade_no] = %q, want out-456 (form=%v)", got, gotForms[0])
	}
	if got := gotForms[0].Get("trade_no"); got != "" {
		t.Fatalf("form[trade_no] = %q, want empty (form=%v)", got, gotForms[0])
	}
}

func TestEasyPayQueryRefundMapsOrderStatus(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "refunded", body: `{"code":1,"trade_status":"TRADE_REFUND","trade_no":"trade-123"}`, wantStatus: payment.ProviderStatusSuccess},
		{name: "still paid", body: `{"code":1,"trade_status":"TRADE_SUCCESS","trade_no":"trade-123"}`, wantStatus: payment.ProviderStatusFailed},
		{name: "pending", body: `{"code":1,"trade_status":"TRADE_PENDING","trade_no":"trade-123"}`, wantStatus: payment.ProviderStatusPending},
	}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if r.URL.Query().Get("act") != "order" {
					t.Errorf("query act = %q, want order", r.URL.Query().Get("act"))
				}
				w.Header().Set("Content-Type", "application/json")
			...
			if resp == nil || resp.Status != tt.wantStatus || resp.RefundID != "out-456" {
				t.Fatalf("QueryRefund response = %+v, want status %s refundID out-456", resp, tt.wantStatus)
			}
		})
	}
}
