...
//
// The out_request_no (Alipay's refund idempotency key) is derived
// deterministically from the order id and the refund request sequence:
// retrying the same refund request reuses the same out_request_no, so Alipay
// executes the refund only once even if the HTTP response is lost or the
// admin retries after a timeout. A brand-new request (e.g. a second partial
// refund) gets a different sequence number and therefore a different key.
func (a *Alipay) Refund(ctx context.Context, req payment.RefundRequest) (*payment.RefundResponse, error) {
	...
	outRequestNo := alipayRefundRequestNo(req.OrderID, req.RefundSeq, req.Amount)
	if outRequestNo == "" {
		return nil, fmt.Errorf("alipay refund missing order identifier")
	}
	...
	result, err := alipayTradeRefund(ctx, client, alipay.TradeRefund{
		...
		// Alipay deduplicates refunds by OutRequestNo. Keep it stable across
		// retries (per RefundSeq) so a gateway timeout after accepting the
		// request cannot issue a second refund when the admin retries.
	...
	return &payment.RefundResponse{
		// RefundID carries the deterministic out_request_no so the refund
		// query (and any later verification) can reuse the exact same key.
		RefundID: outRequestNo,
		Status:   refundStatus,
	}, nil
}

// alipayRefundRequestNo builds the deterministic out_request_no used both as
// the refund idempotency key and as the refund identifier for queries.
// Format: <orderID>-refund-<seq>. Falls back to a deterministic
// orderID + normalized amount when no sequence is provided, so identical
// submissions still share the same key.
func alipayRefundRequestNo(orderID string, seq int64, amount string) string {
	orderID = strings.TrimSpace(orderID)
	if orderID == "" {
		return ""
	}
	if seq > 0 {
		return orderID + "-refund-" + strconv.FormatInt(seq, 10)
	}
...
// QueryRefund queries the refund status via alipay.trade.fastpay.refund.query.
// RefundID is expected to carry the out_request_no previously used for the
// refund request (see alipayRefundRequestNo).
func (a *Alipay) QueryRefund(ctx context.Context, req payment.RefundQueryRequest) (*payment.RefundResponse, error) {
	...
	if outRequestNo == "" {
		outRequestNo = alipayRefundRequestNo(req.OrderID, 0, req.Amount)
	}
	if outRequestNo == "" {
	...
	if param.TradeNo == "" && param.OutTradeNo == "" {
		return nil, fmt.Errorf("alipay query refund: missing trade identifiers")
	}
	...
	status := payment.ProviderStatusFailed
	if strings.TrimSpace(result.RefundStatus) == alipayRefundSuccess {
		status = payment.ProviderStatusSuccess
	}
