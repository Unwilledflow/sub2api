...
func TestOpenAIStreamFailureShouldCooldown(t *testing.T) {
	require.True(t, openAIStreamFailureShouldCooldown("codex upstream stalled: no real data for 5m0s, connection recycled"))
	require.True(t, openAIStreamFailureShouldCooldown("upstream stalled: no real data for 5m0s"))
	require.False(t, openAIStreamFailureShouldCooldown("flagged for cyber policy"))
	require.False(t, openAIStreamFailureShouldCooldown("context_length_exceeded"))
	require.False(t, openAIStreamFailureShouldCooldown(""))
}

func TestOpenAIGatewayService_StreamFailureCooldownRequiresAlternateAccount(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	c.Set("api_key", &APIKey{ID: 701, GroupID: &groupID})
	...
	unsupported := primary
	unsupported.ID = 91003
	unsupported.Credentials = map[string]any{"model_mapping": map[string]any{"gpt-4o": "gpt-4o"}}
	backup := primary
	backup.ID = 91004

	svc := &OpenAIGatewayService{
		accountRepo: groupAwareStubOpenAIAccountRepo{stubOpenAIAccountRepo: stubOpenAIAccountRepo{accounts: []Account{primary, otherGroup, unsupported}}},
		cfg:         &config.Config{},
	}
	require.False(t, svc.openAIStreamFailureCooldownHasAlternate(ctx, c, &primary, "gpt-5.5"))

	svc.accountRepo = groupAwareStubOpenAIAccountRepo{stubOpenAIAccountRepo: stubOpenAIAccountRepo{accounts: []Account{primary, backup}}}
	require.True(t, svc.openAIStreamFailureCooldownHasAlternate(ctx, c, &primary, "gpt-5.5"))
}
