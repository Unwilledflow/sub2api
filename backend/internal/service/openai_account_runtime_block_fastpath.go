...
func (s *OpenAIGatewayService) handleOpenAIAccountUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, canonicalModel ...string) bool {
	...
	balanceCooldown := statusCode == http.StatusPaymentRequired ||
		((statusCode == http.StatusBadRequest || statusCode == http.StatusForbidden) &&
			isBalanceErrorResponse(extractUpstreamErrorMessage(responseBody), responseBody))
	if shouldDisable && !modelTempMatched && !balanceCooldown {
