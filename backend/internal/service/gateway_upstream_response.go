...
type streamingResult struct {
	usage            *ClaudeUsage
	firstTokenMs     *int
	semanticTTFTMs   *int // 首个可见内容 token 时间（排除思考 delta）
...
func semanticTTFTOrNil(r *streamingResult) *int {
	if r == nil {
		return nil
	}
	return r.semanticTTFTMs
}
...
func partialStreamUsageResult(c *gin.Context, resp *http.Response, streamResult *streamingResult, model, upstreamModel string, startTime time.Time, err error) *ForwardResult {
	...
	return &ForwardResult{
		...
		SemanticFirstTokenMs: semanticTTFTOrNil(streamResult),
	...
	for {
		...
		case ev, ok := <-events:
			...
			if trimmed == "" {
				...
				if err != nil {
					if clientDisconnected {
						return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, semanticTTFTMs: semanticTTFTMs, clientDisconnect: true}, nil
					}
					return nil, err
				}
				...
				for _, block := range outputBlocks {
					...
					if data != "" {
						...
						if semanticTTFTMs == nil && anthropicStreamDataStartsVisibleOutput(strings.TrimSpace(data)) {
							ms := int(time.Since(startTime).Milliseconds())
							semanticTTFTMs = &ms
						}
						if usagePatch != nil {
		...
		case <-firstOutputCh:
			...
			return nil, s.newAnthropicFirstOutputTimeoutError(ctx, c, account, startTime, originalModel, firstOutputTimeout, resp.Header)
		...
		case <-intervalCh:
			...
			// 尚无首字时优先按 first-output 超时走 failover，而不是普通 interval 错误。
