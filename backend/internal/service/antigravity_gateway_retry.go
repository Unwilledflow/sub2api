...
func (s *AntigravityGatewayService) handleSmartRetry(p antigravityRetryLoopParams, resp *http.Response, respBody []byte, baseURL string, urlIdx int, availableURLs []string) *smartRetryResult {
	...
	if shouldSmartRetry {
		...
		if isModelCapacityExhausted {
			...
			// 全局去重：覆盖「正在重试中」窗口 + 失败后 cooldown。
			// 进入重试循环前先写入 in-flight 标记，避免并发请求各自放大为 N×60 次上游调用。
			if modelName != "" {
				...
				// 预占重试窗口：maxAttempts×wait + 小缓冲，让并发请求快速失败
				inflightUntil := time.Now().Add(time.Duration(maxAttempts)*waitDuration + antigravityModelCapacityCooldown)
				modelCapacityExhaustedUntil[modelName] = inflightUntil
				modelCapacityExhaustedMu.Unlock()
			}
		}
		...
		for attempt := 1; attempt <= antigravityMaxRetries; attempt++ {
			...
			if resp.StatusCode >= 400 {
				...
				if resp.StatusCode == http.StatusTooManyRequests || resp.StatusCode == http.StatusServiceUnavailable {
					...
					// 429/503 不是 INTERNAL 500，必须清除标记，避免 [500,429,500] 被误判为「全部 INTERNAL 500」
