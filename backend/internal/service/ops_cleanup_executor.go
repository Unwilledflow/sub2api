...
//   - days == 0 → 拒绝静默全清：跳过（ok=false）。TRUNCATE 不得由 retention=0 触发。
//   - days > 0  → 批量 DELETE 早于 now-N天 的行，cutoff = now - N 天
func opsCleanupPlan(now time.Time, days int) (cutoff time.Time, truncate, ok bool) {
	if days <= 0 {
		// retention=0 previously meant TRUNCATE; that is too destructive for a
		// silent cron path (including wiping cleanup audit tables). Skip instead.
