...
// parseSchedulingResetAt 解析上游重置时间（ISO8601 / 时间戳）为 time.Time 指针。
// 内联自官方 account_scheduling_threshold_eval.go；定制版已移除该文件，此处按需保留。
func parseSchedulingResetAt(raw any) *time.Time {
	...
	case string:
		...
		ts, err := cnParseSchedulingTime(trimmed)
		if err != nil {
			return nil
		}
		return &ts
