...
func (s *BackupService) RestoreBackup(ctx context.Context, backupID string) error {
	...
	return s.restoreWithSecurityGuard(ctx, gzReader)
}
...
func (s *BackupService) executeRestore(record *BackupRecord, objectStore BackupObjectStore) {
	...
	if err := s.restoreWithSecurityGuard(ctx, gzReader); err != nil {
		record.RestoreStatus = "failed"
		record.RestoreError = fmt.Sprintf("pg restore: %v", err)
		_ = s.saveRecord(context.Background(), record)
		return
...
// restoreWithSecurityGuard runs a full DB restore while protecting live security state
// (JWT/TOTP encryption keys, step-up/TOTP switches, admin password hashes).
// Without this, a restore rolls the control plane back to the backup epoch and can
// disable 2FA or revive leaked JWT secrets.
func (s *BackupService) restoreWithSecurityGuard(ctx context.Context, data io.Reader) error {
	snapshot, snapErr := s.snapshotSecurityState(ctx)
	if snapErr != nil {
		logger.LegacyPrintf("service.backup", "[Backup] security snapshot before restore failed (continuing): %v", snapErr)
	}
	if err := s.dumper.Restore(ctx, data); err != nil {
		return fmt.Errorf("pg restore: %w", err)
	}
	if snapErr == nil && snapshot != nil {
		if err := s.applySecurityState(ctx, snapshot); err != nil {
			logger.LegacyPrintf("service.backup", "[Backup] CRITICAL: restore completed but failed to re-apply live security state: %v — rotate JWT/TOTP keys and admin passwords immediately", err)
			return fmt.Errorf("restore succeeded but security state re-apply failed: %w", err)
		}
		logger.LegacyPrintf("service.backup", "[Backup] restore completed; live security_secrets and security settings preserved; admin passwords preserved from pre-restore snapshot")
	} else {
		logger.LegacyPrintf("service.backup", "[Backup] restore completed without security snapshot — manually verify step-up/2FA/JWT and rotate secrets")
	}
	return nil
}

type backupSecuritySnapshot struct {
	Secrets        map[string]string
	Settings       map[string]string
	AdminPasswords map[int64]string
}

var backupProtectedSettingKeys = []string{
	SettingKeyTotpEnabled,
	SettingKeyStepUpEnabled,
	SettingKeyAdminAPIKey,
}

func (s *BackupService) snapshotSecurityState(ctx context.Context) (*backupSecuritySnapshot, error) {
	if s == nil || s.db == nil {
		return nil, errors.New("database handle unavailable for security snapshot")
	}
	snap := &backupSecuritySnapshot{
		Secrets:        map[string]string{},
		Settings:       map[string]string{},
		AdminPasswords: map[int64]string{},
	}
	rows, err := s.db.QueryContext(ctx, `SELECT key, value FROM security_secrets`)
	if err != nil {
		return nil, fmt.Errorf("query security_secrets: %w", err)
	}
	...
	if s.settingRepo != nil {
		for _, key := range backupProtectedSettingKeys {
	...
	adminRows, err := s.db.QueryContext(ctx, `SELECT id, password_hash FROM users WHERE role = 'admin' AND password_hash IS NOT NULL AND password_hash <> ''`)
	if err == nil {
		defer func() { _ = adminRows.Close() }()
		for adminRows.Next() {
			var id int64
			var hash string
			if err := adminRows.Scan(&id, &hash); err != nil {
				return nil, err
			}
			snap.AdminPasswords[id] = hash
		}
...
func (s *BackupService) applySecurityState(ctx context.Context, snap *backupSecuritySnapshot) error {
	if s == nil || s.db == nil || snap == nil {
		return errors.New("invalid security re-apply state")
	}
	...
	for key, value := range snap.Secrets {
		...
		`, key, value); err != nil {
			return fmt.Errorf("restore security_secrets %s: %w", key, err)
		}
	}
	for key, value := range snap.Settings {
		if _, err := tx.ExecContext(ctx, `
			...
			ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW()
		`, key, value); err != nil {
			// settings schema may use different PK name in some installs; best-effort via repo after commit.
			logger.LegacyPrintf("service.backup", "[Backup] settings upsert via SQL failed for %s: %v", key, err)
		}
	}
	for id, hash := range snap.AdminPasswords {
		if _, err := tx.ExecContext(ctx, `UPDATE users SET password_hash = $1 WHERE id = $2 AND role = 'admin'`, hash, id); err != nil {
			return fmt.Errorf("restore admin password id=%d: %w", id, err)
		}
	}
	// Force step-up on after restore so a backup taken before 2FA cannot leave the panel unprotected.
	if _, err := tx.ExecContext(ctx, `
		INSERT INTO settings (key, value, updated_at) VALUES ($1, 'true', NOW())
		ON CONFLICT (key) DO UPDATE SET value = 'true', updated_at = NOW()
	`, SettingKeyStepUpEnabled); err != nil {
		logger.LegacyPrintf("service.backup", "[Backup] force-enable step_up_enabled failed: %v", err)
	}
	...
	if s.settingRepo != nil {
		for key, value := range snap.Settings {
			_ = s.settingRepo.Set(ctx, key, value)
		}
		_ = s.settingRepo.Set(ctx, SettingKeyStepUpEnabled, "true")
	}
	return nil
}
