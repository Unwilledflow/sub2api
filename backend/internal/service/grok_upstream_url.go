...
// buildGrokVoiceURL resolves a Voice endpoint while preserving configured
// upstream security policy. CLI relay URLs fall back to the official Voice API.
func buildGrokVoiceURL(account *Account, cfg *config.Config, endpoint string) (string, error) {
	...
	if strings.TrimSpace(baseURL) == "" || isGrokCLIProxyBaseURL(baseURL) {
	...
	return strings.TrimRight(validated, "/") + "/" + strings.Join(escaped, "/"), nil
}
