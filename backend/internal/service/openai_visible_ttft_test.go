...
// 无 .delta 流只在终止事件携带全量 output（缓存命中/压缩输出）时，
// 语义 TTFT 必须保持 nil，否则 first_token_ms == duration_ms（H1 回归）。
func TestOpenAIResponsesNoDeltaTTFTStaysNil(t *testing.T) {
	for _, passthrough := range []bool{false, true} {
		name := "native"
		if passthrough {
			name = "passthrough"
		...
		t.Run(name, func(t *testing.T) {
			result := runSyntheticVisibleTTFTStream(t, passthrough, 120*time.Millisecond, 0,
				`{"type":"response.completed","response":{"id":"resp_test","output":[{"type":"message","content":[{"type":"output_text","text":"cached output"}]}]}}`)
