...
// observeAntigravityGeminiSSELine 观察一条 Gemini SSE data 行的上游响应模型。
// ObserveGemini 同时支持 v1internal 包装与原生 Gemini 形态；主流循环会再对同一行
// 做业务解包，这里不做重复 unwrap。
	...
	for {
		...
		case ev, ok := <-events:
			...
			if strings.HasPrefix(trimmed, "data:") {
				...
				if geminiObserver := upstreamResponseModelObserverFromContext(c); geminiObserver != nil {
					geminiObserver.ObserveGemini([]byte(payload))
				}
