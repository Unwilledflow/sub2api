...
func classifyOpenAITransportError(err error) openAITransportErrorClass {
	...
	// Endpoint, routing and DNS failures are shared infrastructure signals, not
	// proof that this account is bad. Fail over for the request without changing
	// account availability; only explicit proxy credential rejection is durable.
	if errors.Is(err, syscall.ECONNREFUSED) ||
		errors.Is(err, syscall.EHOSTUNREACH) ||
		errors.Is(err, syscall.ENETUNREACH) {
		return openAITransportErrorClass{}
	}
	var transientDNSErr *net.DNSError
	if errors.As(err, &transientDNSErr) && transientDNSErr.IsNotFound {
		return openAITransportErrorClass{}
	}
	transientMessage := strings.ToLower(err.Error())
	for _, marker := range []string{"connection refused", "no route to host", "network is unreachable", "no such host"} {
		if strings.Contains(transientMessage, marker) {
			return openAITransportErrorClass{}
		}
	}
