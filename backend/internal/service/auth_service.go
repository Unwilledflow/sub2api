...
var (
	...
	// ErrOAuthAccountBindRequired is returned when OAuth email matches an existing
	// account that has not yet bound this identity. Callers must require password
	// login or an explicit bind confirmation instead of auto-linking.
	ErrOAuthAccountBindRequired = infraerrors.Conflict("OAUTH_ACCOUNT_BIND_REQUIRED", "email already registered; sign in with password and bind this login method in settings")
...
type AuthService struct {
	...
	// 登录失败账户级锁定（进程级内存，防分布式撞库）。
	loginFailMu     sync.Mutex
	loginFailCount  map[string]int
	loginFailLocked map[string]time.Time
}
...
func (s *AuthService) Login(ctx context.Context, email, password string) (string, *User, error) {
	// 查找用户
	loginID := strings.TrimSpace(email)
	if loginID == "" {
		return "", nil, ErrInvalidCredentials
	}

	// 账户级登录失败锁定：冷却期内直接拒绝，防分布式撞库。
	if s.isLoginLocked(loginID) {
		return "", nil, ErrInvalidCredentials
	}

	// New API imports keep the original username while Sub2API may use a
	// placeholder email, so accept either email or username for password login.
	user, err := s.userRepo.GetByEmail(ctx, loginID)
	if errors.Is(err, ErrUserNotFound) {
		if usernameRepo, ok := s.userRepo.(interface {
			GetByUsername(context.Context, string) (*User, error)
		}); ok {
			user, err = usernameRepo.GetByUsername(ctx, loginID)
		}
	}
	if err != nil {
		if errors.Is(err, ErrUserNotFound) {
...
const (
	loginFailLockThreshold = 10               // 连续失败次数阈值
	loginFailLockCooldown  = 15 * time.Minute // 锁定冷却时长
)

// isLoginLocked 判断账户是否处于登录失败锁定期。
func (s *AuthService) isLoginLocked(loginID string) bool {
	s.loginFailMu.Lock()
	defer s.loginFailMu.Unlock()
	if s.loginFailLocked == nil {
		return false
	}
	if lockedAt, ok := s.loginFailLocked[loginID]; ok {
		if time.Since(lockedAt) < loginFailLockCooldown {
			return true
		}
		// 冷却到期：清除锁定与计数。
		delete(s.loginFailLocked, loginID)
		delete(s.loginFailCount, loginID)
	}
	return false
}

// recordLoginFailure 累计失败次数，达阈值则锁定。
func (s *AuthService) recordLoginFailure(loginID string) {
	s.loginFailMu.Lock()
	defer s.loginFailMu.Unlock()
	if s.loginFailCount == nil {
		s.loginFailCount = map[string]int{}
	}
	if s.loginFailLocked == nil {
		s.loginFailLocked = map[string]time.Time{}
	}
	s.loginFailCount[loginID]++
	if s.loginFailCount[loginID] >= loginFailLockThreshold {
		s.loginFailLocked[loginID] = time.Now()
		logger.LegacyPrintf("service.auth", "[Auth] login locked for %s after %d failures", loginID, s.loginFailCount[loginID])
	}
}

// clearLoginFailure 登录成功后清零失败计数与锁定。
func (s *AuthService) clearLoginFailure(loginID string) {
	s.loginFailMu.Lock()
	defer s.loginFailMu.Unlock()
	delete(s.loginFailCount, loginID)
	delete(s.loginFailLocked, loginID)
...
// canBypassRegistrationDisabledForOAuth allows explicitly enabled identity providers
// to create users while password registration remains disabled.
func (s *AuthService) canBypassRegistrationDisabledForOAuth(ctx context.Context, signupSource string) bool {
	...
	case "oidc":
		cfg, err := s.settingService.GetOIDCConnectOAuthConfig(ctx)
...
func (s *AuthService) RefreshTokenPair(ctx context.Context, refreshToken string) (*TokenPairWithUser, error) {
	...
	// 原子取并删：防止并发刷新在 GET→DEL 窗口双活签发
	data, err := s.refreshTokenCache.TakeRefreshToken(ctx, tokenHash)
	if err != nil {
		if errors.Is(err, ErrRefreshTokenReused) {
			logger.LegacyPrintf("service.auth", "[Auth] Refresh token reuse detected")
		...
		logger.LegacyPrintf("service.auth", "[Auth] Error taking refresh token: %v", err)
		return nil, ErrServiceUnavailable
	}

	// 检查Token是否过期（已原子删除，无需再 DEL）
	...
	// 生成新的Token对，保持同一个家族ID（旧 token 已在 Take 时原子失效）
