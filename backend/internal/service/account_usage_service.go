...
type UsageLogRepository interface {
	// Create persists a usage log and reports whether this call claimed the idempotency key.
	// A positive-cost retry may claim an existing zero-cost row; settled duplicates return false.
	...
	if account.CanGetUsage() {
		...
		if apiResp == nil {
			...
			result, flightErr, _ := s.cache.apiFlight.Do(flightKey, func() (any, error) {
				...
				if fetchErr != nil {
					// 客户端取消类错误（调用方断连）不写入负缓存：它不代表上游
					// 状态且不可重试，缓存会把单次断连放大成整个账号 1 分钟的
					// 用量查询失败。仅限流/超时等上游状态类错误进入负缓存。
					if !isCachableUsageFetchError(fetchErr) {
						return nil, fetchErr
					}
...
// GetUsageBatch 批量获取账号使用量：按账号 best-effort——单账号失败不影响
// 其他账号，错误写入 errorsByAccount；并发上限 6，避免账号列表页瞬时打爆上游。
...
func (s *AccountUsageService) getUsageForAccount(ctx context.Context, account *Account, forceProbe bool) (*UsageInfo, error) {
	return s.GetUsage(ctx, account.ID, forceProbe)
}
...
// isCachableUsageFetchError 判断用量抓取错误是否可写入负缓存。
// context.Canceled（客户端断连/刷新）不可重试、不代表上游状态，不应缓存；
// 限流（429）、超时等上游状态类错误可以缓存以抑制重试风暴。
func isCachableUsageFetchError(err error) bool {
	if err == nil {
		return false
	}
