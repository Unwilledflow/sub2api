...
func TestOpenAIGatewayServiceForward_DisablesResponsesLiteParallelToolCallsForAPIKey(t *testing.T) {
	...
	for _, passthrough := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			body := []byte("{\"model\":\"gpt-5.6-terra\",\"stream\":true,\"parallel_tool_calls\":true,\"tools\":[{\"type\":\"function\",\"name\":\"lookup\",\"parameters\":{\"type\":\"object\"}}],\"input\":[{\"type\":\"message\",\"role\":\"user\",\"content\":\"hello\"}]}")
