...
func TestParseSSEUsage_DeltaOmittedTTLFieldPreservesPreviousBucket(t *testing.T) {
	svc := newMinimalGatewayService()
	usage := &ClaudeUsage{CacheCreation5mTokens: 2, CacheCreation1hTokens: 6}

	svc.parseSSEUsage(`{"type":"message_delta","usage":{"cache_creation":{"ephemeral_5m_input_tokens":1}}}`, usage)

	require.Equal(t, 1, usage.CacheCreation5mTokens)
	require.Equal(t, 6, usage.CacheCreation1hTokens, "an omitted TTL field must not be treated as an explicit zero")
}
