...
type BatchImageRepository interface {
	...
	// ListBatchImageJobsDueForOutputCleanup 返回待清理输出的 job。
	// now：completed 的 output_expires_at 截止时间；
	// terminalCutoff：failed/cancelled 按 finished_at 等时间戳的保留期截止。
	ListBatchImageJobsDueForOutputCleanup(ctx context.Context, now time.Time, terminalCutoff time.Time, limit int) ([]*BatchImageJob, error)
