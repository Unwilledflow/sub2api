...
func (s *refreshTokenCacheStub) TakeRefreshToken(context.Context, string) (*RefreshTokenData, error) {
	return nil, ErrRefreshTokenNotFound
}
...
func TestAuthService_LoginOrRegisterOAuthWithTokenPair_AllowsOIDCSignupWhenPasswordRegistrationDisabled(t *testing.T) {
	repo := &userRepoStub{nextID: 89}
	service := newAuthService(repo, map[string]string{
		SettingKeyRegistrationEnabled: "false",
		SettingKeyOIDCConnectEnabled:  "true",
	}, nil, nil)
	service.cfg.OIDC = minOIDCConfig(false)
	service.refreshTokenCache = &refreshTokenCacheStub{}
	...
	require.NotNil(t, tokenPair)
	require.NotNil(t, user)
	require.Equal(t, int64(89), user.ID)
	require.Len(t, repo.created, 1)
}

func TestAuthService_LoginOrRegisterOAuthWithTokenPair_RejectsOIDCSignupWhenOIDCDisabled(t *testing.T) {
	repo := &userRepoStub{nextID: 90}
	service := newAuthService(repo, map[string]string{
		SettingKeyRegistrationEnabled: "false",
	...
	_, _, err := service.LoginOrRegisterOAuthWithTokenPair(
		context.Background(),
		"disabled-oidc-user@example.com",
		"disabled-oidc-user",
		"",
		"",
		"oidc",
...
// newAuthServiceWithOAuthCfg 构建一个含 OAuth config 的 AuthService，
// 用于测试 canBypassRegistrationDisabledForOAuth。
func newAuthServiceWithOAuthCfg(
	settings map[string]string,
	dtCfg config.DingTalkConnectConfig,
	oidcCfg config.OIDCConnectConfig,
) *AuthService {
	cfg := &config.Config{
...
func minOIDCConfig(enabled bool) config.OIDCConnectConfig {
	return config.OIDCConnectConfig{
		Enabled:             enabled,
		...
		FrontendRedirectURL: "https://app.example.com/auth/oidc/callback",
...
func TestCanBypassRegistrationDisabledForOAuth(t *testing.T) {
	cases := []struct {
		...
		{
			name:         "oidc enabled by setting -> true",
			signupSource: "oidc",
			settings: map[string]string{
				SettingKeyOIDCConnectEnabled: "true",
			},
			oidcCfg: minOIDCConfig(false),
			want:    true,
		},
		{
			name:         "oidc enabled by config fallback -> true",
			signupSource: "oidc",
			settings:     map[string]string{},
			oidcCfg:      minOIDCConfig(true),
			want:         true,
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			svc := newAuthServiceWithOAuthCfg(tc.settings, tc.dtCfg, tc.oidcCfg)
