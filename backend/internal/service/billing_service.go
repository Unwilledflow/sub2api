...
func (s *BillingService) initFallbackPricing() {
	...
	// Claude Haiku 4.5（官方 $1 / $5 per MTok，与 3.5 Haiku 同档）
	// 缺此条目时 getFallbackPricing 会落到 claude-3-haiku（$0.25/$1.25），4 倍少收。
	...
	s.fallbackPrices["claude-haiku-4-5"] = s.fallbackPrices["claude-haiku-4.5"]
	...
	s.fallbackPrices["kimi-k3"] = &ModelPricing{
		InputPricePerToken:     20e-6,  // ¥20 per MTok（用户确认官方价 2026-08-24）
		OutputPricePerToken:    100e-6, // ¥100 per MTok
		CacheReadPricePerToken: 2e-6,   // 输入价 1/10
		SupportsCacheBreakdown: false,
	}
...
func (s *BillingService) getFallbackPricing(model string) *ModelPricing {
	...
	if strings.Contains(modelLower, "haiku") {
		// 4.5 代必须先于 3.5/3 判定，避免 claude-haiku-4-5 落到 haiku-3。
		if strings.Contains(modelLower, "4-5") || strings.Contains(modelLower, "4.5") {
...
// Explicit zero values remain meaningful; only negative detail values are clamped.
...
// CalculateCostWithFallback 定价解析失败时的 fail-closed 兜底：
// 强制用模型系列回退价（未知 claude→sonnet 等）计费，避免 $0 免费放行。
// 无任何回退价时返回 nil（调用方自行降级）。
func (s *BillingService) CalculateCostWithFallback(model string, tokens UsageTokens, rateMultiplier float64) *CostBreakdown {
	pricing := s.getFallbackPricing(model)
	if pricing == nil {
		// 完全未知模型：用最保守的 sonnet 价兜底，宁可多收不可免费。
		pricing = s.fallbackPrices["claude-sonnet-4"]
	}
	if pricing == nil {
		return nil
	}
	return s.computeTokenBreakdown(pricing, tokens, rateMultiplier, "", true)
}
...
type VideoPriceConfig struct {
	...
	// ModelPrices is an optional model-family -> resolution -> USD/s override.
...
// CalculateSearchCost bills server-side search invocations per 1k calls.
func (s *BillingService) CalculateSearchCost(numCalls int, groupPricePer1k *float64, rateMultiplier float64) *CostBreakdown {
	...
	totalCost := pricePer1k * float64(numCalls) / 1000.0
	return &CostBreakdown{
		TotalCost:   totalCost,
		ActualCost:  totalCost * rateMultiplier,
		BillingMode: string(BillingModePerRequest),
...
// CalculateAudioCost bills realtime minutes, TTS million characters, or STT hours.
func (s *BillingService) CalculateAudioCost(mode string, durationOrUnits float64, groupConfig *audioPriceConfig, rateMultiplier float64) *CostBreakdown {
	...
	totalCost := unitPrice * durationOrUnits
	return &CostBreakdown{
		TotalCost:   totalCost,
		ActualCost:  totalCost * rateMultiplier,
		BillingMode: string(BillingModePerRequest),
