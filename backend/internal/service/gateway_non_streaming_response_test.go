...
func TestGatewayFailoverOn400IsOptInAtStatusLevel(t *testing.T) {
	svc := &GatewayService{}

	require.False(t, svc.shouldFailoverUpstreamError(http.StatusBadRequest))
	require.True(t, svc.shouldFailoverOn400([]byte(`{"error":{"message":"anthropic-beta header is required"}}`)))
	require.False(t, svc.shouldFailoverOn400([]byte(`{"error":{"message":"invalid user request"}}`)))
}
