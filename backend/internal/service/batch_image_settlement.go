...
func (s *BatchImageSettlementService) Settle(ctx context.Context, batchID string) (*BatchImageSettlementResult, error) {
	...
	if err := s.Repo.MarkBatchImageJobSettled(ctx, MarkBatchImageJobSettledParams{
		...
		// capture 已成功时 MarkSettled 失败也必须计入重试，否则 settling 无限 requeue。
		// 重试耗尽走 failExhaustedSettlement；release 在已 capture 场景下可能失败，
		// 该路径另有终态出口（见 failExhaustedSettlement）。
		if errors.Is(err, ErrBatchImageAlreadySettled) {
			return nil, err
		}
		msg := truncateBatchImageMessage(err.Error(), batchImageMaxErrorMessageLength)
		if failErr := s.recordSettlementFailure(ctx, job, "SETTLEMENT_MARK_FAILED", msg); failErr != nil {
			return nil, failErr
		}
		return nil, err
	}
...
func (s *BatchImageSettlementService) failExhaustedSettlement(ctx context.Context, job *BatchImageJob, message string) error {
	...
	if err := releaseBatchImageBalanceHold(ctx, s.BillingRepo, job, batchImageDerefString(job.RequestHash)); err != nil {
		...
		// 仍推进终态，禁止无限 requeue；释放失败记事件告警，留给人工/补偿队列。
		logger.L().Error("batch_image.settlement_release_failed_forcing_terminal",
			zap.String("batch_id", job.BatchID),
	...
	if releaseFailed {
		msg = truncateBatchImageMessage(msg+"; hold release failed (needs manual compensation)", batchImageMaxErrorMessageLength)
	}
	errorCode := "SETTLEMENT_BILLING_RETRY_EXHAUSTED"
	if releaseFailed {
		errorCode = "SETTLEMENT_RELEASE_FAILED_TERMINAL"
	}
	if err := s.Repo.TransitionBatchImageJobStatus(ctx, job.BatchID, BatchImageJobStatusFailed, BatchImageTransitionOptions{
		ErrorCode:    batchImageStringPtr(errorCode),
...
func (p *BatchImagePipelineProcessor) Process(ctx context.Context, batchID string) (BatchImageProcessResult, error) {
	...
	if job.Status == BatchImageJobStatusSettling {
		...
		if err != nil {
			// 任意结算错误：若 job 已终态则 Ack；否则延迟 requeue。
			// 避免 MarkSettled 等非 BillingFailed 错误走 worker 无限 60s requeue。
			...
			if errors.Is(err, ErrBatchImageSettlementBillingFailed) ||
				errors.Is(err, ErrBatchImageSettlementCostExceedsHold) ||
				errors.Is(err, ErrBatchImageSettlementInvalidCounts) ||
				errors.Is(err, ErrBatchImageSettlementManifestConflict) ||
				errors.Is(err, ErrBatchImageSettlementPricingMissing) {
				delay := p.RetryDelay
				if delay <= 0 {
			...
			// 未知/瞬时错误也 requeue，由 settlement retry 计数与 failExhausted 兜底
			delay := p.RetryDelay
			if delay <= 0 {
				delay = batchImageSettlementRetryDelay
			}
