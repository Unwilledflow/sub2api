...
// TestIsCachableUsageFetchError 客户端取消类错误（context.Canceled）不得写入负缓存，
// 避免单次断连被放大为整个账号 1 分钟的用量查询失败；限流/超时等上游状态类
// 错误仍应缓存以抑制重试风暴。
func TestIsCachableUsageFetchError(t *testing.T) {
	if isCachableUsageFetchError(context.Canceled) {
		t.Fatal("context.Canceled must not be negative-cached")
	}
	if isCachableUsageFetchError(fmt.Errorf("wrapped: %w", context.Canceled)) {
		t.Fatal("wrapped context.Canceled must not be negative-cached")
	}
	if !isCachableUsageFetchError(context.DeadlineExceeded) {
		t.Fatal("context.DeadlineExceeded should be negative-cached")
	}
	if !isCachableUsageFetchError(errors.New("API returned status 429: rate limited")) {
		t.Fatal("upstream status errors should be negative-cached")
	}
	if isCachableUsageFetchError(nil) {
		t.Fatal("nil error must not be negative-cached")
	}
}
