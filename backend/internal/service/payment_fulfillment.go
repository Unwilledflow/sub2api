...
func (s *PaymentService) HandlePaymentNotification(ctx context.Context, n *payment.PaymentNotification, pk string) error {
	if n == nil {
		return errors.New("payment notification is missing")
	}
	if n.Status != payment.NotificationStatusSuccess {
		return nil
	}
	if strings.TrimSpace(n.OrderID) == "" {
		return errors.New("payment notification order id is missing")
	}
...
func (s *PaymentService) confirmPayment(ctx context.Context, oid int64, tradeNo string, paid float64, pk string, metadata map[string]string) error {
	...
	if normalizedTradeNo := strings.TrimSpace(tradeNo); normalizedTradeNo != "" {
		duplicate, err := s.entClient.PaymentOrder.Query().Where(
			paymentorder.PaymentTradeNoEQ(normalizedTradeNo),
			paymentorder.IDNEQ(o.ID),
		).Exist(ctx)
		if err != nil {
			return fmt.Errorf("check payment trade number uniqueness: %w", err)
		}
		if duplicate {
			s.writeAuditLog(ctx, o.ID, "PAYMENT_TRADE_NO_REUSED", pk, map[string]any{
				"tradeNo": normalizedTradeNo,
			})
			return fmt.Errorf("payment trade number already belongs to another order")
		}
	}
...
func (s *PaymentService) ExecuteBalanceFulfillment(ctx context.Context, oid int64) error {
	...
	if o.PaidAt == nil {
		return infraerrors.BadRequest("INVALID_STATUS", "order has no verified payment")
	}
...
func (s *PaymentService) doBalance(ctx context.Context, o *dbent.PaymentOrder, lease *paymentFulfillmentLease) error {
	...
	if strings.TrimSpace(o.RechargeCode) == "" {
		return errors.New("payment order recharge code is missing")
	}
	// Idempotency: check if redeem code already exists (from a previous partial run)
	existing, lookupErr := s.redeemService.GetByCode(ctx, o.RechargeCode)
	if lookupErr != nil && !errors.Is(lookupErr, ErrRedeemCodeNotFound) {
		return fmt.Errorf("lookup payment recharge code: %w", lookupErr)
	}
	if existing != nil {
		if err := validatePaymentRechargeCode(o, existing); err != nil {
			s.writeAuditLog(ctx, o.ID, "PAYMENT_REDEEM_CODE_MISMATCH", "system", map[string]any{
...
func validatePaymentRechargeCode(order *dbent.PaymentOrder, code *RedeemCode) error {
	if order == nil || code == nil {
		return errors.New("payment recharge code context is missing")
	}
	if order.OrderType != payment.OrderTypeBalance {
		return fmt.Errorf("order type %q is not a balance recharge", order.OrderType)
	}
	if strings.TrimSpace(order.RechargeCode) == "" || code.Code != order.RechargeCode {
		return errors.New("recharge code does not belong to the payment order")
	}
	if code.Type != RedeemTypeBalance {
		return fmt.Errorf("recharge code type %q is not balance", code.Type)
	}
	if !isValidProviderAmount(order.Amount) || math.Abs(code.Value-order.Amount) > paymentAmountToleranceForCurrency(PaymentOrderCurrency(order)) {
		return fmt.Errorf("recharge code amount %.8f does not match order amount %.8f", code.Value, order.Amount)
	}
	if code.IsUsed() {
		if code.UsedBy == nil || *code.UsedBy != order.UserID {
			return errors.New("recharge code was already used by another user")
		}
	} else if code.UsedBy != nil {
		return errors.New("unused recharge code has an owner")
	}
	return nil
}
