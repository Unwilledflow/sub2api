...
type OpenAIForwardResult struct {
	...
	// Runtime scheduling must use FirstTokenMsForScheduling so display metrics
	// never make a slow account look fast.
	...
	WebSearchCalls int
	// AudioUsage and SearchCount are populated by media/search adapters when
	// applicable and keep response-model billing from treating those requests
	// as ordinary token-only traffic.
...
// construction. It is set during process wiring before the gateway serves
// traffic and remains optional for isolated tests.
func (s *OpenAIGatewayService) SetUpstreamRouteHealth(health *UpstreamRouteHealth) {
	if s != nil {
...
// when RecordUsage receives an AdaptiveBillingContext. Optional for unit tests
// that only exercise non-Adaptive billing.
