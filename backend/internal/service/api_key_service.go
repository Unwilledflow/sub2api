...
var (
	...
	ErrInvalidGroupID                   = infraerrors.BadRequest("INVALID_GROUP_ID", "group_id must be non-negative")
	ErrInvalidIPPattern                 = infraerrors.BadRequest("INVALID_IP_PATTERN", "invalid IP or CIDR pattern")
	ErrInvalidFallbackGroup             = infraerrors.BadRequest("INVALID_FALLBACK_GROUP", "invalid fallback group")
	ErrFallbackGroupNotAllowed          = infraerrors.Forbidden("FALLBACK_GROUP_NOT_ALLOWED", "user is not allowed to use this fallback group")
...
type APIKeyUpdateFields struct {
	...
	GroupID          bool
	FallbackGroups   bool
	AdaptiveSettings bool
	BlockOpenAIFast  bool
	ExpiresAt        bool
...
// Accounting resets are kept outside APIKeyRepository.Update so configuration
// writes cannot replay counters from a stale APIKey snapshot.
type apiKeyRateLimitUsageResetter interface {
	ResetRateLimitUsage(ctx context.Context, id int64) error
}

type apiKeyConfigUsageResetUpdater interface {
	UpdateWithUsageReset(ctx context.Context, key *APIKey, resetQuota, resetRateLimit bool) error
}
...
type CreateAPIKeyRequest struct {
	...
	AdaptiveRoutingPreference      string   `json:"adaptive_routing_preference"`  // intelligence | price
	AdaptiveMaxRateMultiplier      *float64 `json:"adaptive_max_rate_multiplier"` // budget ceiling; nil = unlimited
	AntiStallTier                  string   `json:"anti_stall_tier"`              // off|basic|pro|ultra
	AdaptivePassiveFailoverEnabled bool     `json:"adaptive_passive_failover_enabled"`
	BlockOpenAIFast                bool     `json:"block_openai_fast"`
	// Quota fields
...
type UpdateAPIKeyRequest struct {
	...
	FallbackGroupIDs          *[]int64 `json:"fallback_group_ids"`
	AdaptiveRoutingPreference *string  `json:"adaptive_routing_preference"` // intelligence | price
	// AdaptiveMaxRateMultiplier: nil = no change; pointer to nil float via ClearAdaptiveMaxRate
	// is not used — send 0 or negative to clear budget (treated as unlimited).
	AdaptiveMaxRateMultiplier      *float64 `json:"adaptive_max_rate_multiplier"`
	ClearAdaptiveMaxRate           bool     `json:"clear_adaptive_max_rate"` // when true, clear budget limit
	AntiStallTier                  *string  `json:"anti_stall_tier"`         // off|basic|pro|ultra
	AdaptivePassiveFailoverEnabled *bool    `json:"adaptive_passive_failover_enabled"`
	BlockOpenAIFast                *bool    `json:"block_openai_fast"`
	// Quota fields
...
func normalizeAPIKeyGroupIDInput(groupID *int64) (*int64, error) {
	if groupID == nil {
		return nil, nil
	}
	if *groupID < 0 {
...
func normalizeAPIKeyFallbackGroupIDs(ids []int64, primaryGroupID *int64) []int64 {
	if len(ids) == 0 {
		return []int64{}
	}
	...
	for _, id := range ids {
		...
		if primaryGroupID != nil && id == *primaryGroupID {
			continue
		}
		if _, ok := seen[id]; ok {
			continue
...
func (s *APIKeyService) validateFallbackGroups(ctx context.Context, user *User, primaryGroupID *int64, fallbackGroupIDs []int64) ([]int64, error) {
	ids := normalizeAPIKeyFallbackGroupIDs(fallbackGroupIDs, primaryGroupID)
	if len(ids) == 0 {
		return []int64{}, nil
	}
	if user == nil {
		return nil, ErrInvalidFallbackGroup
	}
	for _, groupID := range ids {
		...
		if group == nil || !group.IsActive() || !s.canUserBindGroup(ctx, user, group) {
			return nil, ErrFallbackGroupNotAllowed
		}
	}
	return ids, nil
}
...
func (s *APIKeyService) Create(ctx context.Context, userID int64, req CreateAPIKeyRequest) (*APIKey, error) {
	...
	normalizedGroupID, err := normalizeAPIKeyGroupIDInput(req.GroupID)
	if err != nil {
		return nil, err
	}

	// 验证分组权限（如果指定了分组）
	if normalizedGroupID != nil {
		group, err := s.groupRepo.GetByID(ctx, *normalizedGroupID)
		if err != nil {
	...
	fallbackGroupIDs, err := s.validateFallbackGroups(ctx, user, normalizedGroupID, req.FallbackGroupIDs)
	if err != nil {
		return nil, err
	}
	adaptivePref, err := NormalizeAdaptiveRoutingPreference(req.AdaptiveRoutingPreference)
	if err != nil {
		return nil, err
	}
	adaptiveMaxRate := NormalizeAdaptiveMaxRateMultiplier(req.AdaptiveMaxRateMultiplier)
	antiStallTier := NormalizeAntiStallTier(req.AntiStallTier)
	passiveFailoverEnabled := req.AdaptivePassiveFailoverEnabled
	// Adaptive parent keys do not use key-level fallback chains.
	if normalizedGroupID != nil && s.isAdaptiveParentGroup(ctx, *normalizedGroupID) {
		fallbackGroupIDs = nil
	} else {
		// Adaptive recovery options only apply to Adaptive parent keys.
		antiStallTier = AntiStallTierOff
		passiveFailoverEnabled = false
	}
	...
	apiKey := &APIKey{
		...
		AdaptiveRoutingPreference:      adaptivePref,
		AdaptiveMaxRateMultiplier:      adaptiveMaxRate,
		AntiStallTier:                  antiStallTier,
		AdaptivePassiveFailoverEnabled: passiveFailoverEnabled,
		BlockOpenAIFast:                req.BlockOpenAIFast,
		Quota:                          req.Quota,
		QuotaUsed:                      0,
...
func (s *APIKeyService) isAdaptiveParentGroup(ctx context.Context, groupID int64) bool {
	if s == nil || groupID <= 0 {
		return false
	}
	if checker, ok := s.apiKeyRepo.(interface {
		IsAdaptiveParentGroup(context.Context, int64) bool
	}); ok {
		return checker.IsAdaptiveParentGroup(ctx, groupID)
	}
	return false
}
...
func (s *APIKeyService) Update(ctx context.Context, id int64, userID int64, req UpdateAPIKeyRequest) (*APIKey, error) {
	...
	if req.GroupID != nil {
		...
		if normalizedGroupID == nil {
			...
			apiKey.GroupID = normalizedGroupID
			apiKey.Group = group
		}
		fields.GroupID = true
	}
	...
	if req.FallbackGroupIDs != nil {
		...
		fallbackGroupIDs, err := s.validateFallbackGroups(ctx, user, apiKey.GroupID, *req.FallbackGroupIDs)
		if err != nil {
			return nil, err
		}
		apiKey.FallbackGroupIDs = fallbackGroupIDs
		fields.FallbackGroups = true
	} else if req.GroupID != nil {
		apiKey.FallbackGroupIDs = normalizeAPIKeyFallbackGroupIDs(apiKey.FallbackGroupIDs, apiKey.GroupID)
		fields.FallbackGroups = true
	}
	if req.AdaptiveRoutingPreference != nil {
		pref, err := NormalizeAdaptiveRoutingPreference(*req.AdaptiveRoutingPreference)
		if err != nil {
			return nil, err
		}
		apiKey.AdaptiveRoutingPreference = pref
		fields.AdaptiveSettings = true
	}
	if req.ClearAdaptiveMaxRate {
		apiKey.AdaptiveMaxRateMultiplier = nil
		fields.AdaptiveSettings = true
	} else if req.AdaptiveMaxRateMultiplier != nil {
		apiKey.AdaptiveMaxRateMultiplier = NormalizeAdaptiveMaxRateMultiplier(req.AdaptiveMaxRateMultiplier)
		fields.AdaptiveSettings = true
	}
	if req.AntiStallTier != nil {
		apiKey.AntiStallTier = NormalizeAntiStallTier(*req.AntiStallTier)
		fields.AdaptiveSettings = true
	}
	if req.AdaptivePassiveFailoverEnabled != nil {
		apiKey.AdaptivePassiveFailoverEnabled = *req.AdaptivePassiveFailoverEnabled
		fields.AdaptiveSettings = true
	}
	if req.GroupID != nil {
		fields.AdaptiveSettings = true
		if apiKey.GroupID != nil && s.isAdaptiveParentGroup(ctx, *apiKey.GroupID) {
			apiKey.FallbackGroupIDs = nil
			fields.FallbackGroups = true
			if apiKey.AdaptiveRoutingPreference == "" {
				apiKey.AdaptiveRoutingPreference = AdaptiveRoutingPreferenceIntelligence
			}
		} else {
			apiKey.AntiStallTier = AntiStallTierOff
			apiKey.AdaptivePassiveFailoverEnabled = false
		}
	}
	...
	if req.BlockOpenAIFast != nil {
		apiKey.BlockOpenAIFast = *req.BlockOpenAIFast
		fields.BlockOpenAIFast = true
	}
...
func (s *APIKeyService) GetAvailableGroups(ctx context.Context, userID int64) ([]Group, error) {
	...
	// Mark Adaptive parent groups for key UI (hide fallback, show preference).
	parentIDs := make([]int64, 0)
	for i := range availableGroups {
		availableGroups[i].IsAdaptiveParent = s.isAdaptiveParentGroup(ctx, availableGroups[i].ID)
		if availableGroups[i].IsAdaptiveParent {
			parentIDs = append(parentIDs, availableGroups[i].ID)
		}
	}
	// Adaptive parents: expose leaf rate range (min~max) for frontend display.
	if len(parentIDs) > 0 {
		ranges := s.loadAdaptiveLeafRateRanges(ctx, parentIDs)
		for i := range availableGroups {
			if rng, ok := ranges[availableGroups[i].ID]; ok {
				minR, maxR := rng[0], rng[1]
				availableGroups[i].RateMultiplierMin = &minR
				availableGroups[i].RateMultiplierMax = &maxR
				// Display base rate as the cheaper end so "price first" is clear.
				availableGroups[i].RateMultiplier = minR
			}
		}
	}
...
// loadAdaptiveLeafRateRanges returns parentID -> [min, max] rate among enabled leaves.
func (s *APIKeyService) loadAdaptiveLeafRateRanges(ctx context.Context, parentIDs []int64) map[int64][2]float64 {
	out := make(map[int64][2]float64)
	if s == nil || len(parentIDs) == 0 {
		return out
	}
	if loader, ok := s.apiKeyRepo.(interface {
		LoadAdaptiveLeafRateRanges(ctx context.Context, parentIDs []int64) (map[int64][2]float64, error)
	}); ok {
		ranges, err := loader.LoadAdaptiveLeafRateRanges(ctx, parentIDs)
