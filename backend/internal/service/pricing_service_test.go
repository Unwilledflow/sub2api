...
// TestGetModelPricing_FamilyMatchDeterministicPicksShortestKey 同一 family pattern
// 命中多个定价 key 时（claude-opus-4-7 与各日期版），必须确定性选择基础版
// （最短 key），消除 map 无序迭代导致的随机定价。
func TestGetModelPricing_FamilyMatchDeterministicPicksShortestKey(t *testing.T) {
	pricingSvc := &PricingService{pricingData: map[string]*LiteLLMModelPricing{
		"claude-opus-4-7":              {InputCostPerToken: 10e-6, OutputCostPerToken: 50e-6},
		"claude-opus-4-7-20251231":     {InputCostPerToken: 20e-6, OutputCostPerToken: 60e-6},
		"claude-opus-4-7-20251231-beta": {InputCostPerToken: 20e-6, OutputCostPerToken: 60e-6},
	}}
	svc := NewBillingService(&config.Config{}, pricingSvc)

	// claude-opus-4-7-v1 不在目录中，经家族兜底匹配：每次都必须命中基础版价格。
	for i := 0; i < 20; i++ {
		pricing, err := svc.GetModelPricing("claude-opus-4-7-v1")
...
// TestGetModelPricing_BaseNameFallbackDeterministicForUnknownDateVariant 请求模型带
// 目录中不存在的日期后缀时，baseName 兜底必须确定性选择基础版而非随机日期版。
func TestGetModelPricing_BaseNameFallbackDeterministicForUnknownDateVariant(t *testing.T) {
	pricingSvc := &PricingService{pricingData: map[string]*LiteLLMModelPricing{
		"claude-opus-4-7":          {InputCostPerToken: 10e-6, OutputCostPerToken: 50e-6},
		"claude-opus-4-7-20250501": {InputCostPerToken: 20e-6, OutputCostPerToken: 60e-6},
	}}
	svc := NewBillingService(&config.Config{}, pricingSvc)

	for i := 0; i < 20; i++ {
		pricing, err := svc.GetModelPricing("claude-opus-4-7-20251101")
		require.NoError(t, err)
		require.InDelta(t, 10e-6, pricing.InputPricePerToken, 1e-12)
		require.InDelta(t, 50e-6, pricing.OutputPricePerToken, 1e-12)
	}
