...
// ErrChannelMonitorNotExternallyManaged 外部托管（ops-panel syncer）模式下拒绝本地修改。
var ErrChannelMonitorNotExternallyManaged = infraerrors.Conflict(
	"CHANNEL_MONITOR_NOT_EXTERNALLY_MANAGED",
	"channel monitor is not externally managed",
)

var (
	ErrChannelMonitorMissingExternalResults = infraerrors.BadRequest(
		"CHANNEL_MONITOR_MISSING_EXTERNAL_RESULTS",
		"external-managed channel monitor requires external results",
	)
	ErrChannelMonitorInvalidExternalResult = infraerrors.BadRequest(
		"CHANNEL_MONITOR_INVALID_EXTERNAL_RESULT",
		"external result payload is invalid",
	)
	ErrChannelMonitorInvalidManagementMode = infraerrors.Forbidden(
		"CHANNEL_MONITOR_INVALID_MANAGEMENT_MODE",
		"management mode does not allow this operation",
	)
)
