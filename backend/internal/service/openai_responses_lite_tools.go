...
func appendOpenAIResponsesLiteAdditionalTools(input any, namespaceTools []any) ([]any, error) {
	...
	// 上游要求 compaction_trigger 必须是 input 的最后一个 item（远程压缩回合）。
	// 合成的 additional_tools 载体不得追加到它之后，须插入到第一个
	// compaction_trigger 之前；无 trigger 时保持追加到末尾的原行为。
	insertIdx := len(items)
	for i, rawItem := range items {
		if item, ok := rawItem.(map[string]any); ok &&
			strings.TrimSpace(firstNonEmptyString(item["type"])) == "compaction_trigger" {
			insertIdx = i
			break
		}
	}
	...
	items[insertIdx] = map[string]any{
