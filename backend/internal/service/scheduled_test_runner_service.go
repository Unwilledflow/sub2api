...
func (s *ScheduledTestRunnerService) runScheduled() {
	...
	// Multi-instance guard: only the leader runs due plans each tick, avoiding
	// N× real upstream test calls and duplicate auto-recover side effects.
	lockCtx, lockCancel := context.WithTimeout(context.Background(), 2*time.Second)
	release, ok := tryAcquireSingletonLeaderLock(
		lockCtx,
		s.lockCache,
		s.db,
		scheduledTestRunnerLeaderLockKey,
		s.instanceID,
		scheduledTestRunnerLeaderLockTTL,
	)
	lockCancel()
	if !ok {
		return
