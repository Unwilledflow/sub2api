...
// minimal=true 时把 max_tokens 压到 16（分组监控健康探测用），避免话痨模型烧输出 Token。
func createTestPayload(modelID string, minimal bool) (map[string]any, error) {
...
func (s *AccountTestService) testClaudeAccountConnection(c *gin.Context, account *Account, modelID string, mode string) error {
	...
	payload, err := createTestPayload(testModelID, normalizeAccountTestMode(mode) == AccountTestModeHealthProbe)
	if err != nil {
...
func (s *AccountTestService) testClaudeVertexServiceAccountConnection(c *gin.Context, ctx context.Context, account *Account, testModelID string) error {
	...
	payload, err := createTestPayload(testModelID, false)
	if err != nil {
		return s.sendErrorAndEnd(c, "Failed to create test payload")
	}
	payloadBytes, _ := json.Marshal(payload)
...
func (s *AccountTestService) routeAntigravityTest(c *gin.Context, account *Account, modelID string, prompt string) error {
	if account.Type == AccountTypeAPIKey {
		...
		return s.testClaudeAccountConnection(c, account, modelID, AccountTestModeDefault)
	}
