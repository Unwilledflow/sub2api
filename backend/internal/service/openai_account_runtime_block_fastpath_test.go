...
func TestOpenAIBalanceFastPathKeepsTemporaryRuntimeCooldown(t *testing.T) {
	repo := &rateLimitAccountRepoStub{}
	rateLimitService := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
	gateway := &OpenAIGatewayService{rateLimitService: rateLimitService}
	rateLimitService.SetAccountRuntimeBlocker(gateway)
	account := &Account{ID: 451, Platform: PlatformOpenAI, Type: AccountTypeAPIKey}
	startedAt := time.Now()
	...
	require.True(t, ok)
	require.GreaterOrEqual(t, until, startedAt.Add(balanceErrorCooldown-time.Second))
	require.LessOrEqual(t, until, startedAt.Add(balanceErrorCooldown+time.Second))
}

func TestOpenAINonBalance400StillBlocksRuntime(t *testing.T) {
	repo := &rateLimitAccountRepoStub{}
	rateLimitService := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
	gateway := &OpenAIGatewayService{rateLimitService: rateLimitService}
	rateLimitService.SetAccountRuntimeBlocker(gateway)
	account := &Account{ID: 452, Platform: PlatformOpenAI, Type: AccountTypeAPIKey}
	...
	shouldDisable := gateway.handleOpenAIAccountUpstreamError(
		...
		http.Header{},
		[]byte(`{"error":{"message":"organization has been disabled"}}`),
	)
