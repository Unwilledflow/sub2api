...
func TestWSResponseCreate_APIKeyBlockStripsPriorityAndPreventsForcePriority(t *testing.T) {
	settings := &OpenAIFastPolicySettings{
		Rules: []OpenAIFastPolicyRule{{
			ServiceTier: OpenAIFastTierAny,
			Action:      OpenAIFastPolicyActionForcePriority,
	...
	priorityFrame := []byte(`{"type":"response.create","model":"gpt-5.6-sol","service_tier":"priority"}`)
	updated, blocked, err := svc.applyOpenAIFastPolicyToWSResponseCreate(blockedCtx, account, "gpt-5.6-sol", priorityFrame)
	require.NoError(t, err)
	require.Nil(t, blocked)
	require.False(t, gjson.GetBytes(updated, "service_tier").Exists())

	flexFrame := []byte(`{"type":"response.create","model":"gpt-5.6-sol","service_tier":"flex"}`)
	updated, blocked, err = svc.applyOpenAIFastPolicyToWSResponseCreate(blockedCtx, account, "gpt-5.6-sol", flexFrame)
	require.NoError(t, err)
	require.Nil(t, blocked)
