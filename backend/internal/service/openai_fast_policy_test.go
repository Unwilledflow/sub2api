...
func TestApplyOpenAIFastPolicyToBody_ForcePriorityInjectsMissingTierForWhitelistedOAuth(t *testing.T) {
	settings := &OpenAIFastPolicySettings{
		Rules: []OpenAIFastPolicyRule{{
			ServiceTier:    OpenAIFastTierAny,
			Action:         OpenAIFastPolicyActionForcePriority,
	...
	updated, err := svc.applyOpenAIFastPolicyToBody(context.Background(), oauthAccount, "gpt-5.6-sol", body)
	require.NoError(t, err)
	require.Equal(t, OpenAIFastTierPriority, gjson.GetBytes(updated, "service_tier").String())

	updated, err = svc.applyOpenAIFastPolicyToBody(context.Background(), apiKeyAccount, "gpt-5.6-sol", body)
	require.NoError(t, err)
...
func TestApplyOpenAIFastPolicyToBody_APIKeyBlockOverridesPriorityAndForcePriority(t *testing.T) {
	settings := &OpenAIFastPolicySettings{
		Rules: []OpenAIFastPolicyRule{{
			ServiceTier:    OpenAIFastTierAny,
			Action:         OpenAIFastPolicyActionForcePriority,
	...
	blockedCtx := context.WithValue(context.Background(), ctxkey.BlockOpenAIFast, true)

	for _, rawTier := range []string{"priority", "fast"} {
		body := []byte(`{"model":"gpt-5.6-sol","service_tier":"` + rawTier + `"}`)
		updated, err := svc.applyOpenAIFastPolicyToBody(blockedCtx, account, "gpt-5.6-sol", body)
		require.NoError(t, err)
		require.False(t, gjson.GetBytes(updated, "service_tier").Exists(), "tier %q should be stripped", rawTier)
	}

	missingTierBody := []byte(`{"model":"gpt-5.6-sol","input":[]}`)
	updated, err := svc.applyOpenAIFastPolicyToBody(blockedCtx, account, "gpt-5.6-sol", missingTierBody)
	require.NoError(t, err)
	require.False(t, gjson.GetBytes(updated, "service_tier").Exists(), "force_priority must not inject fast mode")

	flexBody := []byte(`{"model":"gpt-5.6-sol","service_tier":"flex"}`)
	updated, err = svc.applyOpenAIFastPolicyToBody(blockedCtx, account, "gpt-5.6-sol", flexBody)
	require.NoError(t, err)
	require.Equal(t, OpenAIFastTierFlex, gjson.GetBytes(updated, "service_tier").String())

	updated, err = svc.applyOpenAIFastPolicyToBody(context.Background(), account, "gpt-5.6-sol", missingTierBody)
	require.NoError(t, err)
	require.Equal(t, OpenAIFastTierPriority, gjson.GetBytes(updated, "service_tier").String(), "default-off behavior must stay unchanged")
}
