...
func (s *PricingService) lookupIdentifiedModelPricingLocked(lookupCandidates []string) *LiteLLMModelPricing {
	...
	// 多个 key 命中同一 baseName（如 claude-opus-4-7 与 claude-opus-4-7-20251231）时
	// 必须确定性选择，否则 map 无序迭代会随机定价。
	...
	if key := pickDeterministicPricingKey(matchingKeys); key != "" {
...
// pickDeterministicPricingKey 从命中同一模式/家族的多个定价 key 中确定性选择：
// 最短 key（基础版本条目，如 claude-opus-4-7）优先，等长时按字典序取小者。
// 消除 Go map 无序迭代导致的同模型随机定价，保证金额可审计。
func pickDeterministicPricingKey(keys []string) string {
	if len(keys) == 0 {
		return ""
	}
...
func (s *PricingService) matchByModelFamily(model string) *LiteLLMModelPricing {
	...
	// 同一 pattern 可能命中多个 key（claude-opus-4-7 / claude-opus-4-7-20251231 /
	// -20251231-beta），必须确定性选择，否则同一模型每次请求随机命中不同价格。
