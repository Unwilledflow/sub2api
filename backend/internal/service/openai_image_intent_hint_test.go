...
func TestClearOpenAIImageIntentHintForcesMappedBodyReclassification(t *testing.T) {
	gin.SetMode(gin.TestMode)
	c := newOpenAIImageIntentHintTestContext(OpenAIClientTransportHTTP)
	SetOpenAIImageIntentHint(c, false)
	ClearOpenAIImageIntentHint(c)
	var calls atomic.Int64

	got := resolveOpenAIImageIntentHint(
		c,
		"gpt-image-2",
		[]byte(`{"model":"gpt-image-2","input":"draw"}`),
		countingOpenAIImageIntentClassifier(&calls),
	)
