	...
	if len(result) > 0 {
		if a.Platform == domain.PlatformAntigravity {
			ensureAntigravityDefaultPassthroughs(result, []string{
				...
				"gemini-3.6-flash-lite-thinking",
...
// IsPoolMode reports whether this account explicitly opts into provider-agnostic
// pool-mode retries. The flag is deliberately independent of account type so
// OpenAI, Gemini, Anthropic, and other upstream credentials share one policy.
...
// 未配置或配置非法时回退为默认值 5；小于 0 按 0 处理；过大则截断到 10。
...
// defaultPoolModeRetryableStatusCodes are transient upstream statuses that
// are worth retrying through an upstream account pool. Administrators can
// replace this list per account with pool_mode_retry_status_codes.
var defaultPoolModeRetryableStatusCodes = []int{
	401, 408, 425, 429,
	500, 502, 503, 504,
