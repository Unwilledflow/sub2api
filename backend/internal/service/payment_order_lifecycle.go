...
const (
	...
	pendingAllReconcileLimit = 30
	completedAuditLimit      = 50
	completedAuditLookback   = 48 * time.Hour
	reconcileAmountEpsilon   = 0.011
)
...
// ReconcilePendingAllOrders actively checks recent pending orders across all
// payment providers (easypay/alipay/stripe/...) so missed provider
// notifications do not wait until order expiry to fulfill. wxpay is covered by
// ReconcilePendingWxpayOrders; this widens the sweep to every provider that
// implements QueryOrder.
func (s *PaymentService) ReconcilePendingAllOrders(ctx context.Context) (int, error) {
	now := time.Now()
	orders, err := s.entClient.PaymentOrder.Query().
		Where(
			paymentorder.StatusEQ(OrderStatusPending),
			paymentorder.ExpiresAtGT(now),
			paymentorder.CreatedAtGTE(now.Add(-24*time.Hour)),
		).
		Order(dbent.Asc(paymentorder.FieldCreatedAt)).
		Limit(pendingAllReconcileLimit).
		All(ctx)
	if err != nil {
...
// AuditCompletedOrders re-verifies recently completed orders against the
// upstream provider and cross-checks the paid amount. Discrepancies are
// written to the payment audit log only (no state mutation) so operators can
// review without risking double-fulfillment.
func (s *PaymentService) AuditCompletedOrders(ctx context.Context) (int, error) {
	now := time.Now()
	orders, err := s.entClient.PaymentOrder.Query().
		Where(
			paymentorder.StatusEQ(OrderStatusCompleted),
			paymentorder.PaidAtGTE(now.Add(-completedAuditLookback)),
		).
		Order(dbent.Desc(paymentorder.FieldID)).
		Limit(completedAuditLimit).
		All(ctx)
	if err != nil {
		return 0, fmt.Errorf("query completed orders: %w", err)
	}
	...
	for _, o := range orders {
		...
		switch {
		case resp.Status != payment.ProviderStatusPaid:
			flagged++
			s.writeAuditLog(ctx, o.ID, "PAYMENT_RECONCILE_STATUS_MISMATCH", prov.ProviderKey(), map[string]any{
				"local_status":  string(o.Status),
				"upstream":      resp.Status,
				"trade_no":      resp.TradeNo,
				"out_trade_no":  o.OutTradeNo,
			})
			slog.Warn("reconcile: completed order not paid upstream", "orderID", o.ID, "upstream", resp.Status)
		case math.Abs(resp.Amount-o.PayAmount) > reconcileAmountEpsilon:
			flagged++
			s.writeAuditLog(ctx, o.ID, "PAYMENT_RECONCILE_AMOUNT_MISMATCH", prov.ProviderKey(), map[string]any{
				"expected": o.PayAmount,
				"upstream": resp.Amount,
				"trade_no": resp.TradeNo,
			})
			slog.Warn("reconcile: paid amount mismatch", "orderID", o.ID, "expected", o.PayAmount, "upstream", resp.Amount)
		}
	}
	return flagged, nil
}
