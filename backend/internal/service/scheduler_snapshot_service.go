...
// SetUpstreamRouteHealth installs a local monitor-driven overlay before the
// scheduler starts. It filters returned candidates without rewriting durable
// account state or cached scheduler snapshots.
func (s *SchedulerSnapshotService) SetUpstreamRouteHealth(health *UpstreamRouteHealth) {
	if s == nil {
		return
	}
...
func (s *SchedulerSnapshotService) ListSchedulableAccounts(ctx context.Context, groupID *int64, platform string, hasForcePlatform bool) ([]Account, bool, error) {
	...
	if s.cache != nil {
		...
		if err != nil {
			...
			return s.filterAccountsByUpstreamRouteHealth(derefAccounts(cached)), useMixed, nil
		}
...
// GetSchedulableAccount is the sticky-session counterpart to
// ListSchedulableAccounts. It preserves GetAccount for non-scheduling callers
// while ensuring an alert cannot keep selecting a stale sticky account.
func (s *SchedulerSnapshotService) GetSchedulableAccount(ctx context.Context, accountID int64) (*Account, error) {
...
func (s *SchedulerSnapshotService) filterAccountsByUpstreamRouteHealth(accounts []Account) []Account {
...
func (s *SchedulerSnapshotService) isAccountAllowedByUpstreamRouteHealth(account *Account) bool {
...
func (s *SchedulerSnapshotService) GetGroupByID(ctx context.Context, groupID int64) (*Group, error) {
	if group, ok := ctx.Value(ctxkey.Group).(*Group); ok && group != nil && group.ID == groupID && IsGroupContextValid(group) {
		return group, nil
	}
	if s.groupRepo == nil {
		return nil, nil
