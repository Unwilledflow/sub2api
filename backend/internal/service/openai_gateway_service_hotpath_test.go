...
func TestOpenAISchedulerPerformanceClassSeparatesLatencyDrivers(t *testing.T) {
	standalone := OpenAISchedulerPerformanceClass(
		[]byte(`{"model":"gpt-5.6-sol","reasoning":{"effort":"max"},"service_tier":"priority","input":"hello"}`),
		"gpt-5.6-sol",
	)
	require.Equal(t, "effort=max;tier=priority;context=small;chain=standalone", standalone)

	continuationBody := []byte(`{"model":"gpt-5.6-sol","reasoning":{"effort":"x-high"},"previous_response_id":"resp_123","input":"` + strings.Repeat("x", openAISchedulerMediumRequestMinBytes) + `"}`)
	continuation := OpenAISchedulerPerformanceClass(continuationBody, "gpt-5.6-sol")
	require.Equal(t, "effort=xhigh;tier=default;context=medium;chain=continuation", continuation)

	largeBody := []byte(`{"model":"gpt-5.6-sol","input":"` + strings.Repeat("x", openAISchedulerLargeRequestMinBytes) + `"}`)
	large := OpenAISchedulerPerformanceClass(largeBody, "gpt-5.6-sol")
	require.Equal(t, "effort=default;tier=default;context=large;chain=standalone", large)
}
