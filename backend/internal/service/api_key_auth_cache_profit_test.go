...
type confirmedNilRPMOverrideRepo struct {
	UserGroupRateRepository
}

func (confirmedNilRPMOverrideRepo) GetRPMOverrideByUserAndGroup(context.Context, int64, int64) (*int, error) {
	return nil, nil
}
...
func TestAPIKeyAuthSnapshotProfitControlRoundtrip(t *testing.T) {
	...
	require.Equal(t, 21, snapshot.Version, "v21 起认证快照区分已确认的 nil RPM override")
...
func TestAPIKeyAuthSnapshotCarriesConfirmedNilRPMOverride(t *testing.T) {
	groupID := int64(50)
	apiKey := profitAuthTestAPIKey()
	apiKey.GroupID = &groupID

	svc := &APIKeyService{userGroupRateRepo: confirmedNilRPMOverrideRepo{}}
	snapshot := svc.snapshotFromAPIKey(context.Background(), apiKey)
	require.NotNil(t, snapshot)
	require.Nil(t, snapshot.User.UserGroupRPMOverride)
	require.True(t, snapshot.User.UserGroupRPMOverrideLoaded)

	materialized, used, err := svc.applyAuthCacheEntry(apiKey.Key, &APIKeyAuthCacheEntry{Snapshot: snapshot})
	require.NoError(t, err)
	require.True(t, used)
