...
func TestCreateEasyPayProviderEncryptsStoredConfig(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
	key := []byte("0123456789abcdef0123456789abcdef")
	secret := "test-easypay-pkey-not-plaintext"
	...
	instance, err := svc.CreateProviderInstance(ctx, CreateProviderInstanceRequest{
		...
		Config: map[string]string{
			...
			"notifyUrl": "https://merchant.example.com/api/v1/payment/callback/easypay",
			"returnUrl": "https://merchant.example.com/payment/result",
		},
		Enabled: false,
	})
	require.NoError(t, err)
	require.NotContains(t, instance.Config, secret)
	require.NotContains(t, instance.Config, `"pkey"`)

	plaintext, err := payment.Decrypt(instance.Config, key)
	require.NoError(t, err)
	var stored map[string]string
	require.NoError(t, json.Unmarshal([]byte(plaintext), &stored))
	require.Equal(t, secret, stored["pkey"])

	decrypted, err := svc.decryptConfig(instance.Config)
	require.NoError(t, err)
	require.Equal(t, stored, decrypted)
}
