...
func TestIsOpenAIModelCapacityErrorResponseShapes(t *testing.T) {
	message := "Selected model is at capacity. Please try a different model."
	require.True(t, isOpenAIModelCapacityError(message, nil))
	require.True(t, isOpenAIModelCapacityError("", []byte(`{"error":{"message":"Selected model is at capacity. Please try a different model."}}`)))
	require.True(t, isOpenAIModelCapacityError("", []byte(`{"type":"response.failed","response":{"error":{"message":"Selected model is at capacity. Please try a different model."}}}`)))
	require.False(t, isOpenAIModelCapacityError("invalid model", []byte(`{"error":{"message":"model not found"}}`)))
}
