...
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, sub2apiUserID int64) (*AccountSelectionResult, error) {
	...
	if s.concurrencyService == nil || !cfg.LoadBatchEnabled {
		...
		for {
			...
			if err == nil && result.Acquired {
				...
				return s.registeredSelectionResult(ctx, account, sessionHash, true, result.ReleaseFunc, nil)
			}
			...
			if stickyAccountID > 0 && stickyAccountID == account.ID && s.concurrencyService != nil {
				...
				if waitingCount < cfg.StickySessionMaxWaiting {
					return s.registeredSelectionResult(ctx, account, sessionHash, false, nil, &AccountWaitPlan{
	...
	if len(routingAccountIDs) > 0 && s.concurrencyService != nil {
		...
		if len(routingCandidates) > 0 {
						...
						if rpmPass { // 粘性会话窗口费用+RPM 检查
							...
							if err == nil && result.Acquired {
								...
								if !s.checkAndRegisterSession(ctx, stickyAccount, sessionHash) {
									...
									return s.registeredSelectionResult(ctx, stickyAccount, sessionHash, true, result.ReleaseFunc, nil)
								}
							}
							...
							if stickyCacheMissReason == "" {
								...
								if waitingCount < cfg.StickySessionMaxWaiting {
									...
									if !s.checkAndRegisterSession(ctx, stickyAccount, sessionHash) {
										...
										return s.registeredSelectionResult(ctx, stickyAccount, sessionHash, false, nil, &AccountWaitPlan{
			...
			if len(routingAvailable) > 0 {
				// 排序：有界的优先级负载分数 > 负载率 > 最后使用时间。
				// 小幅优先级差不能压过明显的实时负载差，避免单账号形成热点。
				sort.SliceStable(routingAvailable, func(i, j int) bool {
					a, b := routingAvailable[i], routingAvailable[j]
					aScore, bScore := accountRoutingScore(a), accountRoutingScore(b)
					if aScore != bScore {
						return aScore < bScore
					}
					if a.account.Priority != b.account.Priority {
						return boundedSchedulingPriority(a.account.Priority) < boundedSchedulingPriority(b.account.Priority)
					}
					aLoadRate, bLoadRate := routingLoadRate(a.loadInfo), routingLoadRate(b.loadInfo)
				...
				for _, item := range routingAvailable {
					...
					if err == nil && result.Acquired {
						...
						return s.registeredSelectionResult(ctx, item.account, sessionHash, true, result.ReleaseFunc, nil)
					}
				}
				...
				for _, item := range routingAvailable {
					...
					return s.registeredSelectionResult(ctx, item.account, sessionHash, false, nil, &AccountWaitPlan{
	...
	if err != nil {
		...
		for len(available) > 0 {
			// 1. 取优先级/实时负载综合分数最小的集合
			candidates := filterByMinRoutingScore(available)
			...
			if err == nil && result.Acquired {
				...
				if !s.checkAndRegisterSession(ctx, selected.account, sessionHash) {
					...
					return s.registeredSelectionResult(ctx, selected.account, sessionHash, true, result.ReleaseFunc, nil)
				}
			}
	...
	for _, acc := range candidates {
		...
		return s.registeredSelectionResult(ctx, acc, sessionHash, false, nil, &AccountWaitPlan{
			AccountID:      acc.ID,
...
if sessionHash != "" && s.cache != nil {
		...
		selection, err := s.registeredSelectionResult(ctx, acc, sessionHash, true, result.ReleaseFunc, nil)
		if err != nil {
...
func (s *GatewayService) listSchedulableAccounts(ctx context.Context, groupID *int64, platform string, hasForcePlatform bool) ([]Account, bool, error) {
	if s.schedulerSnapshot != nil {
		...
		accounts = s.filterAccountsByUpstreamRouteHealth(accounts)
		if err == nil {
	...
	if useMixed {
		...
		return s.filterAccountsByUpstreamRouteHealth(filtered), useMixed, nil
	}
	...
	return s.filterAccountsByUpstreamRouteHealth(accounts), useMixed, nil
}
...
func (s *GatewayService) isAccountSchedulableForSelection(account *Account) bool {
	...
	return account.IsSchedulable() && s.isAccountAllowedByUpstreamRouteHealth(account)
}
...
func (s *GatewayService) isAccountSchedulableForModelSelection(ctx context.Context, account *Account, requestedModel string) bool {
	...
	return s.isAccountAllowedByUpstreamRouteHealth(account) && account.IsSchedulableForModelWithContext(ctx, requestedModel)
}
...
// unregisterSession 注销 checkAndRegisterSession 注册的会话配额。
// 仅用于等待/选择路径失败时的显式回收，避免会话空占到空闲超时（默认 5 分钟）。
func (s *GatewayService) unregisterSession(ctx context.Context, account *Account, sessionID string) {
	if account == nil || sessionID == "" || s.sessionLimitCache == nil {
		return
	}
	if !account.IsAnthropicOAuthOrSetupToken() {
		return
...
// UnregisterGatewaySession 供 handler 在等待计划超时/失败等路径显式注销会话配额。
func (s *GatewayService) UnregisterGatewaySession(ctx context.Context, accountID int64, sessionID string) {
	if accountID <= 0 || sessionID == "" || s.sessionLimitCache == nil {
		return
	}
	_ = s.sessionLimitCache.UnregisterSession(ctx, accountID, sessionID)
}

func (s *GatewayService) getSchedulableAccount(ctx context.Context, accountID int64) (*Account, error) {
	if s.schedulerSnapshot != nil {
		return s.schedulerSnapshot.GetSchedulableAccount(ctx, accountID)
	}
	account, err := s.accountRepo.GetByID(ctx, accountID)
	if err != nil || !s.isAccountAllowedByUpstreamRouteHealth(account) {
		return nil, err
	}
	return account, nil
}
...
func (s *GatewayService) hydrateSelectedAccount(ctx context.Context, account *Account) (*Account, error) {
	if account == nil {
		return nil, fmt.Errorf("selected gateway account is nil during hydration")
	}
	if s.schedulerSnapshot == nil {
		if !s.isAccountAllowedByUpstreamRouteHealth(account) {
			return nil, fmt.Errorf("selected gateway account %d is no longer schedulable by upstream route health", account.ID)
		}
		// Re-check full schedulability: the account may have been paused,
		// rate-limited or quota-exhausted between listing and hydration.
		if !s.isAccountSchedulableForSelection(account) {
			return nil, fmt.Errorf("selected gateway account %d is no longer schedulable during hydration", account.ID)
		}
		return account, nil
	}
	hydrated, err := s.schedulerSnapshot.GetSchedulableAccount(ctx, account.ID)
	if err != nil {
		return nil, err
	}
	if hydrated == nil {
	...
	// Snapshot accounts can be stale inside the outbox rebuild window; re-check
	// full schedulability so a paused/rate-limited/disabled account is not
	// selected and forwarded only to fail upstream.
...
func (s *GatewayService) filterAccountsByUpstreamRouteHealth(accounts []Account) []Account {
	if s == nil || s.upstreamRouteHealth == nil {
		return accounts
	}
	return s.upstreamRouteHealth.FilterAccounts(accounts)
}

func (s *GatewayService) isAccountAllowedByUpstreamRouteHealth(account *Account) bool {
	return s == nil || s.upstreamRouteHealth == nil || s.upstreamRouteHealth.AllowsAccount(account)
}
...
func (s *GatewayService) newSelectionResult(ctx context.Context, account *Account, acquired bool, release func(), waitPlan *AccountWaitPlan) (*AccountSelectionResult, error) {
	...
	if hydrated == nil {
		return nil, fmt.Errorf("selected gateway account is no longer schedulable")
	}
...
// registeredSelectionResult 在 hydrate/构建结果失败时回收已注册的会话配额与并发槽位，
// 避免 wait-plan / acquired 路径在 selection 构建失败后遗留空占（会话配额正反馈来源之一）。
func (s *GatewayService) registeredSelectionResult(ctx context.Context, account *Account, sessionHash string, acquired bool, release func(), waitPlan *AccountWaitPlan) (*AccountSelectionResult, error) {
	selection, err := s.newSelectionResult(ctx, account, acquired, release, waitPlan)
	if err != nil {
		s.unregisterSession(ctx, account, sessionHash)
		if release != nil {
			release()
		}
		return nil, err
...
func (s *GatewayService) newAcquiredSelectionResult(ctx context.Context, account *Account, release func()) (*AccountSelectionResult, error) {
	...
	if err == nil && (selection == nil || selection.Account == nil) {
		selection = nil
		err = fmt.Errorf("selected gateway account is no longer schedulable")
	}
...
const (
	maxSchedulingPriority        = AccountPriorityMax
	schedulingPriorityLoadWeight = 3
)

func boundedSchedulingPriority(priority int) int {
	if priority < AccountPriorityMin {
		return AccountPriorityMin
	}
	if priority > maxSchedulingPriority {
		return maxSchedulingPriority
	}
	return priority
}

func routingLoadRate(loadInfo *AccountLoadInfo) int {
	if loadInfo == nil {
		return 0
	}
	if loadInfo.LoadRate < 0 {
		return 0
	}
	if loadInfo.LoadRate > 100 {
		return 100
...
func accountRoutingScore(account accountWithLoad) int {
	return routingLoadRate(account.loadInfo) + boundedSchedulingPriority(account.account.Priority)*schedulingPriorityLoadWeight
}

func filterByMinRoutingScore(accounts []accountWithLoad) []accountWithLoad {
	if len(accounts) == 0 {
		return accounts
	}
	minScore := accountRoutingScore(accounts[0])
	for _, account := range accounts[1:] {
		if score := accountRoutingScore(account); score < minScore {
			minScore = score
		}
	}
	result := make([]accountWithLoad, 0, len(accounts))
	for _, account := range accounts {
		if accountRoutingScore(account) == minScore {
...
func filterByMinLoadRate(accounts []accountWithLoad) []accountWithLoad {
	...
	minLoadRate := routingLoadRate(accounts[0].loadInfo)
	for _, acc := range accounts[1:] {
		if loadRate := routingLoadRate(acc.loadInfo); loadRate < minLoadRate {
			minLoadRate = loadRate
		}
	}
	result := make([]accountWithLoad, 0, len(accounts))
	for _, acc := range accounts {
		if routingLoadRate(acc.loadInfo) == minLoadRate {
			result = append(result, acc)
		}
	}
	return result
...
func sameAccountWithLoadGroup(a, b accountWithLoad) bool {
	...
	if routingLoadRate(a.loadInfo) != routingLoadRate(b.loadInfo) {
		return false
	}
