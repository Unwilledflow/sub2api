...
func TestOpenAIWSPayloadTransientStatus_ModelCapacity(t *testing.T) {
	for _, payload := range [][]byte{
		[]byte(`{"type":"error","error":{"type":"invalid_request_error","message":"Selected model is at capacity. Please try a different model."}}`),
		[]byte(`{"type":"response.failed","response":{"error":{"type":"invalid_request_error","message":"Selected model is at capacity. Please try a different model."}}}`),
	} {
		require.Equal(t, http.StatusServiceUnavailable, openAIWSPayloadTransientStatus(payload))
	}
}
