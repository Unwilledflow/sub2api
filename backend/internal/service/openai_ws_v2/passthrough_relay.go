...
// wsClientCloseStatusCoder matches gateway-owned close errors (e.g. service
// OpenAIWSClientCloseError) that expose a WebSocket close status without
// implementing coder/websocket.CloseError.
type wsClientCloseStatusCoder interface {
	StatusCode() coderws.StatusCode
}

func isGracefulWSCloseStatus(code coderws.StatusCode) bool {
	switch code {
...
func isDisconnectError(err error) bool {
	...
	// Inter-turn idle timeout wraps context.DeadlineExceeded inside a gateway
	// close error with StatusNormalClosure — treat as graceful so drain runs.
	var statusCoder wsClientCloseStatusCoder
	if errors.As(err, &statusCoder) && isGracefulWSCloseStatus(statusCoder.StatusCode()) {
		return true
	}
	if isGracefulWSCloseStatus(coderws.CloseStatus(err)) {
		return true
	}
	...
	return strings.Contains(message, "failed to read frame header: eof") ||
		...
		strings.Contains(message, "websocket idle timeout") ||
		strings.Contains(message, "openai ws client close")
}
