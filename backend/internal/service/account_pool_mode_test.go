...
func TestIsPoolModeIsProviderAndAccountTypeAgnostic(t *testing.T) {
	tests := []struct {
		name     string
		platform string
		typ      string
	}{
		{name: "openai_oauth", platform: PlatformOpenAI, typ: AccountTypeOAuth},
		{name: "gemini_oauth", platform: PlatformGemini, typ: AccountTypeOAuth},
		{name: "anthropic_oauth", platform: PlatformAnthropic, typ: AccountTypeOAuth},
		{name: "anthropic_setup_token", platform: PlatformAnthropic, typ: AccountTypeSetupToken},
		{name: "grok_oauth", platform: PlatformGrok, typ: AccountTypeOAuth},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			account := &Account{
			...
			require.Equal(t, defaultPoolModeRetryCount, account.GetPoolModeRetryCount())
		})
	}

	require.False(t, (*Account)(nil).IsPoolMode())
}
