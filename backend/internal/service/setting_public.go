...
func (s *SettingService) GetPublicSettings(ctx context.Context) (*PublicSettings, error) {
	...
	return &PublicSettings{
		...
		// Custom endpoints are returned only by the authenticated, recharge-gated endpoint.
		CustomEndpoints:             "[]",
...
// normalizeTencentCaptchaRegion 非法值一律视为中国站（"cn"|"intl"）。
...
func (s *SettingService) GetChannelMonitorRuntime(ctx context.Context) ChannelMonitorRuntime {
	...
	if err != nil {
		return ChannelMonitorRuntime{Enabled: true, DefaultIntervalSeconds: channelMonitorIntervalFallback, ShowQuota: true}
	}
