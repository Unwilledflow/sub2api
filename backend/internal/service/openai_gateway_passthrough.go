...
func needsOpenAIResponsesClientToolAdaptation(body []byte) bool {
	...
	visit = func(value gjson.Result) bool {
		if value.IsObject() {
			...
			// namespace-adaptation-gate: pure namespace declarations also need
			// lowering because mixed-dialect API-key upstreams reject them.
			...
			// defer-loading-gate: tools carrying the defer_loading marker imply
			// tool_search semantics that strict upstreams reject when the
			// declaration is absent; route them through the lowering pass which
			// strips the marker (and keeps a tool_search proxy when declared).
			if value.Get("defer_loading").Type == gjson.True {
				needsAdaptation = true
				return false
			}
		}
...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	var responseHeaderMs int64
	var forwardToResponseHeaderMs int64
	var upstreamRequestStart time.Time
	...
	upstreamRequestStart = upstreamStart
	resp, err = s.doOpenAIUpstream(upstreamReq, proxyURL, account)
	responseHeaderMs = time.Since(upstreamStart).Milliseconds()
	forwardToResponseHeaderMs = time.Since(startTime).Milliseconds()
	SetOpsLatencyMs(c, OpsUpstreamLatencyMsKey, responseHeaderMs)
		if err != nil {
		...
		if reqStream {
			...
			if result != nil {
				result.timing.forwardToResponseHeaderMs = int(forwardToResponseHeaderMs)
			}
			usage = result.usage
			semanticFirstTokenMs = result.firstTokenMs
			firstTokenMs = result.firstTokenMs
			// 语义 TTFT 定制：真实首字为首个可见输出 token，基准为上游请求发出时刻，
			// 扣除本地调度/上传等前置耗时（v13 定制，见 openai_gateway_ttft_diagnostics.go）。
			if result.firstTokenMs != nil && !upstreamRequestStart.IsZero() && upstreamRequestStart.After(startTime) {
				pre := int(upstreamRequestStart.Sub(startTime).Milliseconds())
				adj := *result.firstTokenMs - pre
				if adj < 0 {
					adj = 0
				}
				firstTokenMs = &adj
			...
			if result != nil {
				logOpenAISlowTTFTDiagnostic(ctx, c, account, body, reqModel, responseHeaderMs, result.timing)
			}
			responseID = strings.TrimSpace(result.responseID)
			imageCount = result.imageCount
			imageOutputSizes = result.imageOutputSizes
...
func openAIStreamFrameStartsVisibleOutput(frame openAISSEDataFrame) bool {
	...
	if strings.HasSuffix(eventType, ".delta") {
		// 仅可见文本 delta 计为首个可见输出；reasoning_* delta 是思考内容，
		// 计入会把思考时间算进 TTFT，导致 token/s 失真。
	...
	case "response.completed", "response.done":
		// response.completed / response.done 携带的 output 数组不算"首个可见输出"：
		// 无 .delta 流会在流末尾才设置 TTFT，导致 first_token_ms == duration_ms。
		// 终止事件仍保留为 failover/空流判定的结构性信号（openAIStreamDataStartsClientOutput）。
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	firstUpstreamEventMs := -1
	firstUpstreamEventType := ""
	firstSemanticEventMs := -1
	firstSemanticEventType := ""
	...
	resultWithUsage := func() *openaiStreamingResultPassthrough {
		return &openaiStreamingResultPassthrough{
			...
			timing: openAIStreamTiming{
				firstUpstreamEventMs:   firstUpstreamEventMs,
				firstUpstreamEventType: firstUpstreamEventType,
				firstSemanticEventMs:   firstSemanticEventMs,
				firstSemanticEventType: firstSemanticEventType,
	...
	for documentScanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			eventType := frame.eventType
			// [DONE] 是空流的唯一事件，计入会让 TTFT ≈ 总耗时；空流应保持 -1，
			// 由语义判定（nil）兜底显示。
			if firstUpstreamEventMs < 0 && trimmedData != "" && trimmedData != "[DONE]" {
				firstUpstreamEventMs = int(time.Since(startTime).Milliseconds())
				firstUpstreamEventType = openAIStreamDiagnosticEventType(eventType, trimmedData)
			}
			...
			if firstTokenMs == nil && openAIStreamFrameStartsVisibleOutput(frame) {
				...
				firstTokenMs = &ms
				firstSemanticEventMs = ms
				firstSemanticEventType = openAIStreamDiagnosticEventType(eventType, trimmedData)
			}
			s.parseSSEUsageBytesWithType(dataBytes, eventType, usage)
		}
		if line == "" {
