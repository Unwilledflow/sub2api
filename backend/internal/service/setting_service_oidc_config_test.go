...
func TestGetOIDCConnectOAuthConfig_ResolvesIssuerFromExplicitDiscoveryURL(t *testing.T) {
	var discoveryHits int
	var baseURL string
	...
	require.NoError(t, err)
	require.Equal(t, 1, discoveryHits)
	require.Equal(t, srv.URL+"/issuer", got.IssuerURL)
	require.Equal(t, srv.URL+"/authorize", got.AuthorizeURL)
	require.Equal(t, srv.URL+"/token", got.TokenURL)
	require.Equal(t, srv.URL+"/userinfo", got.UserInfoURL)
	require.Equal(t, srv.URL+"/jwks", got.JWKSURL)
}
...
func TestGetOIDCConnectOAuthConfig_PublicClientRequiresPKCE(t *testing.T) {
	cfg := &config.Config{
		OIDC: config.OIDCConnectConfig{
			Enabled:             true,
			ProviderName:        "OIDC",
...
func TestGetOIDCConnectOAuthConfig_PreservesUserInfoDisplayNamePath(t *testing.T) {
	cfg := &config.Config{
		OIDC: config.OIDCConnectConfig{
			Enabled:             true,
			ProviderName:        "OIDC",
			...
			TokenAuthMethod:     "client_secret_post",
			AllowedSigningAlgs:  "RS256",
			UserInfoDisplayPath: "profile.display_name",
		},
	}

	repo := &settingOIDCRepoStub{values: map[string]string{
		SettingKeyOIDCConnectEnabled:             "true",
		SettingKeyOIDCConnectUserInfoDisplayPath: "user.display_name",
	}}
	svc := NewSettingService(repo, cfg)

	got, err := svc.GetOIDCConnectOAuthConfig(context.Background())
	require.NoError(t, err)
	require.Equal(t, "user.display_name", got.UserInfoDisplayPath)
}
