...
// Responses Lite rejects omitted or true parallel_tool_calls. Normalize the
// field before account routing so managed, passthrough, OAuth, and API-key
// accounts all satisfy the same upstream contract.
func normalizeOpenAIResponsesLiteParallelToolCalls(body []byte) ([]byte, bool, error) {
	parallel := parseRawJSONView(body).Get("parallel_tool_calls")
	if parallel.Exists() && parallel.Type != gjson.False && parallel.Type != gjson.True {
		return body, false, newOpenAIResponsesLiteValidationError(
			"parallel_tool_calls",
...
// It is the []byte counterpart of openAIResponsesLiteHasTools: besides the
// top-level "tools" array it also recognizes the Responses Lite carrier.
...
const (
	openAISchedulerMediumRequestMinBytes = 128 * 1024
	openAISchedulerLargeRequestMinBytes  = 512 * 1024
)

// OpenAISchedulerPerformanceClass groups requests with comparable upstream
// work so latency feedback does not mix effort, context, tier, or chain shape.
func OpenAISchedulerPerformanceClass(body []byte, modelCandidates ...string) string {
	effort := "default"
	if value := extractOpenAIReasoningEffortFromBody(body, modelCandidates...); value != nil {
		effort = *value
	} else if raw := strings.TrimSpace(gjson.GetBytes(body, "output_config.effort").String()); raw != "" {
		if normalized := normalizeOpenAIReasoningEffortForModel(raw, firstNonEmpty(modelCandidates...)); normalized != "" {
			effort = normalized
		}
	}
	...
	contextClass := "small"
	switch {
	case len(body) >= openAISchedulerLargeRequestMinBytes:
		contextClass = "large"
	case len(body) >= openAISchedulerMediumRequestMinBytes:
		contextClass = "medium"
	}
	...
	return fmt.Sprintf("effort=%s;tier=%s;context=%s;chain=%s", effort, tier, contextClass, chain)
}
...
func (s *OpenAIGatewayService) evaluateOpenAIFastPolicy(ctx context.Context, account *Account, model, serviceTier string) (action, errMsg string) {
	...
	blockForAPIKey := blockOpenAIFastFromContext(ctx)
	if blockForAPIKey {
		switch tier {
	...
	if blockForAPIKey && action == OpenAIFastPolicyActionForcePriority {
		return BetaPolicyActionPass, ""
	}
	return action, errMsg
}

func blockOpenAIFastFromContext(ctx context.Context) bool {
	if ctx == nil {
		return false
	}
	blocked, _ := ctx.Value(ctxkey.BlockOpenAIFast).(bool)
	return blocked
}
...
func (s *OpenAIGatewayService) applyOpenAIFastPolicyToBody(ctx context.Context, account *Account, model string, body []byte) ([]byte, error) {
	...
	if rawTier == "" {
		// Missing tiers remain a no-op unless an explicit "all" rule uses
		// force_priority. This keeps the default behavior unchanged while
		// allowing administrators to opt selected models into priority service.
		action, _ := s.evaluateOpenAIFastPolicy(ctx, account, model, "missing")
