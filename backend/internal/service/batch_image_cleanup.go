...
func (s *BatchImageCleanupService) RunOnce(ctx context.Context, now time.Time) (BatchImageCleanupRunResult, error) {
	...
	// failed/cancelled 无 output_expires_at 时，按与 completed 相同的输出保留期从 finished_at 起算。
	terminalCutoff := now.Add(-s.outputRetentionAfterTerminal())
	outputJobs, err := s.Repo.ListBatchImageJobsDueForOutputCleanup(ctx, now, terminalCutoff, limit)
	if err != nil {
		return result, err
	}
...
func (s *BatchImageCleanupService) outputRetentionAfterTerminal() time.Duration {
