...
func (s *PaymentOrderExpiryService) runOnce() {
	...
	allCtx, allCancel := context.WithTimeout(context.Background(), expiryCheckTimeout)
	recoveredAll, err := s.paymentSvc.ReconcilePendingAllOrders(allCtx)
	allCancel()
	if err != nil {
		slog.Warn("[PaymentOrderExpiry] failed to reconcile pending orders", "error", err)
	} else if recoveredAll > 0 {
		slog.Info("[PaymentOrderExpiry] reconciled paid orders (all providers)", "count", recoveredAll)
	}

	auditCtx, auditCancel := context.WithTimeout(context.Background(), expiryCheckTimeout)
	flagged, err := s.paymentSvc.AuditCompletedOrders(auditCtx)
	auditCancel()
	if err != nil {
		slog.Warn("[PaymentOrderExpiry] failed to audit completed orders", "error", err)
	} else if flagged > 0 {
		slog.Warn("[PaymentOrderExpiry] completed orders failed upstream audit", "count", flagged)
	}
