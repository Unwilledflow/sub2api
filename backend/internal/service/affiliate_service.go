...
type AffiliateRepository interface {
	...
	// AccrueQuota credits inviter quota and writes an accrue ledger row.
	// perInviteeCap > 0 enforces the per-invitee lifetime cap inside the same
	// transaction (FOR UPDATE + SUM) to close TOCTOU races.
	AccrueQuota(ctx context.Context, inviterID, inviteeUserID int64, amount float64, freezeHours int, sourceOrderID *int64, perInviteeCap float64) (applied bool, credited float64, err error)
	GetAccruedRebateFromInvitee(ctx context.Context, inviterID, inviteeUserID int64) (float64, error)
	// ClawbackQuotaForOrder reverses accrue rows for sourceOrderID and deducts
	// inviter quota (available first, then frozen), never below zero.
	ClawbackQuotaForOrder(ctx context.Context, sourceOrderID int64) (clawed float64, err error)
...
func (s *AffiliateService) AccrueInviteRebateForOrder(ctx context.Context, inviteeUserID int64, baseRechargeAmount float64, sourceOrderID *int64) (float64, error) {
	...
	// per-invitee cap is enforced atomically inside AccrueQuota (same tx as
	// ledger insert) to close TOCTOU races under concurrent orders.
	...
	applied, credited, err := s.repo.AccrueQuota(ctx, *inviteeSummary.InviterID, inviteeUserID, rebate, freezeHours, sourceOrderID, perInviteeCap)
	if err != nil {
		return 0, err
	}
	if !applied || credited <= 0 {
		return 0, nil
	}
	return credited, nil
}

// ClawbackInviteRebateForOrder reverses rebates previously accrued for the
// given payment order. Idempotent: a second call finds no remaining accrue
// net and returns 0.
func (s *AffiliateService) ClawbackInviteRebateForOrder(ctx context.Context, sourceOrderID int64) (float64, error) {
	if s == nil || s.repo == nil || sourceOrderID <= 0 {
		return 0, nil
	}
	return s.repo.ClawbackQuotaForOrder(ctx, sourceOrderID)
}
