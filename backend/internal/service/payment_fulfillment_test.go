...
func (r *paymentFulfillmentAffiliateRepoStub) AccrueQuota(_ context.Context, inviterID, inviteeUserID int64, amount float64, freezeHours int, sourceOrderID *int64, _ float64) (bool, float64, error) {
...
func (r *paymentFulfillmentAffiliateRepoStub) ClawbackQuotaForOrder(context.Context, int64) (float64, error) {
	return 0, nil
}
...
func TestExecuteBalanceFulfillmentRecoversAfterRedeemWithoutCreditingAgain(t *testing.T) {
	...
	redeemRepo := &redeemCodeRepoStub{codesByCode: map[string]*RedeemCode{
		order.RechargeCode: {
			...
			Value:  order.Amount,
			Status: StatusUsed,
			UsedBy: func() *int64 { v := order.UserID; return &v }(),
		},
	}}
	svc := &PaymentService{
		entClient:     client,
...
func TestExecuteBalanceFulfillmentRejectsOrderWithoutVerifiedPayment(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
	user, err := client.User.Create().
		SetEmail("unpaid-fulfillment@example.com").
		SetPasswordHash("hash").
		SetUsername("unpaid-fulfillment-user").
		Save(ctx)
	require.NoError(t, err)
	...
	order, err := client.PaymentOrder.Create().
		...
		SetPayAmount(25).
		SetFeeRate(0).
		SetRechargeCode("PAY-UNPAID-FULFILLMENT").
		SetOutTradeNo("sub2_unpaid_fulfillment").
		SetPaymentType(payment.TypeAlipay).
		SetPaymentTradeNo("").
		SetOrderType(payment.OrderTypeBalance).
		SetStatus(OrderStatusPaid).
	...
	userRepo.updateBalanceFn = func(context.Context, int64, float64) error {
		t.Fatal("unpaid order attempted to update user balance")
		return nil
	}
	redeemRepo := &redeemCodeRepoStub{codesByCode: map[string]*RedeemCode{
		order.RechargeCode: {
...
func TestExecuteBalanceFulfillmentRejectsMismatchedRechargeCode(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
	user, err := client.User.Create().
		SetEmail("mismatched-recharge@example.com").
		SetPasswordHash("hash").
		SetUsername("mismatched-recharge-user").
		Save(ctx)
	require.NoError(t, err)
	...
	order, err := client.PaymentOrder.Create().
		...
		SetPayAmount(25).
		SetFeeRate(0).
		SetRechargeCode("PAY-MISMATCHED-RECHARGE").
		SetOutTradeNo("sub2_mismatched_recharge").
		SetPaymentType(payment.TypeAlipay).
		SetPaymentTradeNo("trade-mismatched-recharge").
		SetOrderType(payment.OrderTypeBalance).
		SetStatus(OrderStatusPaid).
		SetPaidAt(time.Now()).
		SetExpiresAt(time.Now().Add(time.Hour)).
	...
	userRepo := &mockUserRepo{getByIDUser: &User{ID: user.ID, Email: user.Email, Username: user.Username}}
	userRepo.updateBalanceFn = func(context.Context, int64, float64) error {
		t.Fatal("mismatched recharge code attempted to update user balance")
		return nil
	}
	redeemRepo := &redeemCodeRepoStub{codesByCode: map[string]*RedeemCode{
		order.RechargeCode: {
...
func TestHandlePaymentNotificationRejectsReusedTradeNumber(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
	firstUser, err := client.User.Create().
		SetEmail("first-trade-user@example.com").
		SetPasswordHash("hash").
		SetUsername("first-trade-user").
		Save(ctx)
	require.NoError(t, err)
	secondUser, err := client.User.Create().
		SetEmail("second-trade-user@example.com").
		SetPasswordHash("hash").
		SetUsername("second-trade-user").
		Save(ctx)
	require.NoError(t, err)
	...
	_, err = client.PaymentOrder.Create().
		...
		SetPayAmount(10).
		SetFeeRate(0).
		SetRechargeCode("PAY-FIRST-TRADE").
		SetOutTradeNo("sub2_first_trade").
		SetPaymentType(payment.TypeAlipay).
		SetPaymentTradeNo("trade-reused").
		SetOrderType(payment.OrderTypeBalance).
	...
	secondOrder, err := client.PaymentOrder.Create().
		...
		SetPayAmount(10).
		SetFeeRate(0).
		SetRechargeCode("PAY-SECOND-TRADE").
		SetOutTradeNo("sub2_second_trade").
		SetPaymentType(payment.TypeAlipay).
		SetPaymentTradeNo("").
		SetOrderType(payment.OrderTypeBalance).
	...
	err = svc.HandlePaymentNotification(ctx, &payment.PaymentNotification{
		OrderID: secondOrder.OutTradeNo,
		TradeNo: "trade-reused",
		Amount:  10,
		Status:  payment.NotificationStatusSuccess,
	}, payment.TypeAlipay)
	require.ErrorContains(t, err, "already belongs to another order")
	reloaded, err := client.PaymentOrder.Get(ctx, secondOrder.ID)
	require.NoError(t, err)
	require.Equal(t, OrderStatusPending, reloaded.Status)
}

func TestHandlePaymentNotificationRejectsNilNotification(t *testing.T) {
	svc := &PaymentService{}
	require.ErrorContains(t, svc.HandlePaymentNotification(context.Background(), nil, payment.TypeAlipay), "notification is missing")
}
