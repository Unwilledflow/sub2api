...
// IsExplicitImageGenerationIntent identifies requests that cannot be safely
// downgraded to text. Passive image_gen namespace declarations are excluded
// so Chat Completions-only accounts remain eligible for ordinary Codex calls.
func IsExplicitImageGenerationIntent(endpoint string, requestedModel string, body []byte) bool {
	...
	parseRawJSONView(body).ForEach(func(key, value gjson.Result) bool {
		...
		case "tools":
			if !toolsSeen {
				toolsSeen = true
				imageIntent = openAIJSONToolsContainExplicitNativeImageGeneration(value)
			}
		case "tool_choice":
			if !toolChoiceSeen {
				toolChoiceSeen = true
...
// IsActiveImageGenerationIntent identifies requests that need image routing or
// concurrency, while ignoring passive image_gen namespace catalogs.
func IsActiveImageGenerationIntent(endpoint string, requestedModel string, body []byte) bool {
	if IsImageGenerationEndpoint(endpoint) || isOpenAIImageGenerationModel(requestedModel) {
		return true
	}
	if len(body) == 0 || !gjson.ValidBytes(body) {
...
// StripOptionalImageGenerationTools removes image tool declarations while
// preserving text tools, input content, and non-image tool choices.
func StripOptionalImageGenerationTools(body []byte) ([]byte, bool, error) {
...
func openAIJSONToolsContainExplicitNativeImageGeneration(tools gjson.Result) bool {
	if !tools.IsArray() {
		return false
	}
	found := false
	tools.ForEach(func(_, item gjson.Result) bool {
		found = isOpenAIImageGenerationType(openAIJSONString(item.Get("type"))) &&
			openAIJSONString(item.Get("model")) != ""
		return !found
	})
	return found
}
