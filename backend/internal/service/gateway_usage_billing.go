...
type competitiveUpstreamCancellationKey struct{}

// WithCompetitiveUpstreamCancellation keeps race-loser cancellation attached
// to streaming upstream requests. Normal streams retain their detached drain.
func WithCompetitiveUpstreamCancellation(ctx context.Context) context.Context {
	if ctx == nil {
		ctx = context.Background()
	}
	return context.WithValue(ctx, competitiveUpstreamCancellationKey{}, true)
}

func hasCompetitiveUpstreamCancellation(ctx context.Context) bool {
	if ctx == nil {
		return false
	}
	enabled, _ := ctx.Value(competitiveUpstreamCancellationKey{}).(bool)
	return enabled
}
...
func detachStreamUpstreamContext(ctx context.Context, stream bool) (context.Context, context.CancelFunc) {
	...
	if !stream || hasCompetitiveUpstreamCancellation(ctx) {
		return ctx, func() {}
	}
...
func detachUpstreamContext(ctx context.Context) (context.Context, context.CancelFunc) {
	...
	if hasCompetitiveUpstreamCancellation(ctx) {
		return ctx, func() {}
	}
	return context.WithoutCancel(ctx), func() {}
}
...
func writeUsageLogBestEffort(ctx context.Context, repo UsageLogRepository, usageLog *UsageLog, logKey string) {
	...
	if writer, ok := repo.(usageLogBestEffortWriter); ok {
		if err := writer.CreateBestEffort(usageCtx, usageLog); err != nil {
			...
			// 重复写入由 usage_logs 的请求键冲突处理防护；已结算重试可升级此前的零成本行。
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	competitiveWasteLogs := s.buildCompetitiveWasteLogs(ctx, result, usageLog, apiKey, account)
	if len(competitiveWasteLogs) > 0 {
		defer s.recordCompetitiveWaste(ctx, competitiveWasteLogs)
	}
	...
	// Adaptive parent groups settle through authorize/capture (15% fee). Capture
	// inserts the usage evidence row; do not also applyUsageBilling.
...
func (s *GatewayService) calculateTokenCost(
	...
	if err != nil {
		// 定价缺失 fail-closed：不再静默 $0 免费放行。
		// 用保守兜底（sonnet 价）计费并告警，避免未知模型被免费服务。
		logger.LegacyPrintf("service.gateway", "Calculate cost failed (fail-closed to sonnet fallback): %v", err)
		fallback := s.billingService.CalculateCostWithFallback(billingModel, tokens, multiplier)
		if fallback != nil {
			return fallback
		}
		return &CostBreakdown{ActualCost: 0}
...
func (s *GatewayService) buildRecordUsageLog(
	...
	usageLog := &UsageLog{
		...
		FirstTokenMs:          semanticFirstTokenMs(result),
