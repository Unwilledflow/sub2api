...
func ExtractGrokVideoBillingFromStatusBody(statusBody []byte, pending *GrokVideoPendingBilling, requestID string) *OpenAIForwardResult {
	...
	if pending != nil {
		if model == "" {
			model = grokVideoFirstNonEmpty(pending.BillingModel, pending.Model, pending.OriginalModel)
		}
		if billingModel == "" {
			billingModel = grokVideoFirstNonEmpty(pending.BillingModel, pending.Model)
		}
		if upstreamModel == "" {
...
func grokVideoFirstNonEmpty(values ...string) string {
