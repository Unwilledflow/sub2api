...
func validateAccountSchedulingInput(priority, loadFactor *int) error {
	if priority != nil {
	...
	if loadFactor != nil {
		if err := ValidateAccountLoadFactor(*loadFactor, true); err != nil {
			return err
		}
	}
	return nil
...
func buildAccountForCreate(input *CreateAccountInput, accountExtra map[string]any) (*Account, error) {
	// Create 载荷的 Priority 为非指针 int：0 表示未配置（建号时落到 1..30 频段
	// 默认值），不得触发越界校验；仅显式提供的非零值需要范围检查。
	if input.Priority != 0 {
	...
	if input.LoadFactor != nil {
		if err := ValidateAccountLoadFactor(*input.LoadFactor, true); err != nil {
			return nil, err
		}
	}
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if err := validateAccountSchedulingInput(input.Priority, input.LoadFactor); err != nil {
		return nil, err
	}
