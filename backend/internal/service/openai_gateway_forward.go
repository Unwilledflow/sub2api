...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	// reasoning 归一化可能改写 body，OAuth-like 场景需在改写后二次执行 Lite tools 归一化。
	...
	// 出站前兜底：上游要求 compaction_trigger 必须是 input 末位。任何上游归一化
	// （如 responses Lite 追加 additional_tools）都可能把它挤出末位，这里幂等修复。
	if compactPath {
		if fixed, changedFix, fixErr := EnsureOpenAICompactionTriggerLast(body); fixErr == nil && changedFix {
			body = fixed
		}
	}
	...
	if rawTier := requestView.ServiceTier; rawTier != "" {
		...
		action, _ := s.evaluateOpenAIFastPolicy(ctx, account, upstreamModel, "missing")
	...
	// transform-path-client-tool-lowering: API-key /v1/responses upstreams speak
	// mixed dialects; some reject Codex custom/tool_search/namespace tools outright
	// (e.g. 400 missing format.syntax). Lower them to plain function tools (same
	// adaptation the passthrough path applies) and register the restore mapping so
	// the shared response handlers replay custom_tool_call / namespace payloads.
	if account.Platform == PlatformOpenAI && account.Type == AccountTypeAPIKey &&
		!compactPath && needsOpenAIResponsesClientToolAdaptation(body) {
		adaptedBody, mapping, adaptErr := adaptOpenAIResponsesClientTools(body)
		if adaptErr != nil {
			return nil, adaptErr
		}
		if adaptedBody != nil {
			...
			logger.LegacyPrintf("service.openai_gateway", "[OpenAI] Lowered /responses client tools for API-key upstream (account: %s)", account.Name)
		}
	}
	...
	wsRetryLoop:
		for attempt := 1; attempt <= maxAttempts; attempt++ {
			wsAttempts = attempt
			// 每次重连尝试单独计时：首 token 相对本次尝试开始，避免把前次失败
			// 尝试与 backoff 的耗时累积进 TTFT（成功尝试值优先）。
		...
		if wsErr == nil {
			...
			// Duration 保持 Forward 级总耗时口径（firstTokenMs 已按尝试内计时）。
			wsResult.Duration = time.Since(startTime)
	...
	for {
		// 每次尝试单独计时：首 token 相对本次尝试开始，避免把前次失败尝试
		// （agent 任务恢复/重试/backoff）的耗时累积进 TTFT。
		attemptStartTime := time.Now()
		...
		if reqStream {
			streamResult, err := s.handleStreamingResponseWithReasoning(ctx, resp, c, account, attemptStartTime, originalModel, upstreamModel, reasoningEffortValue)
			if streamResult != nil {
			...
			// 非流式上游回显观察：与 kedaya 的 handleNonStreamingResponse 保持一致，
			// 确保 service_tier 回显优先于出站请求体 tier，避免上游降级后仍按 fast 计费。
			if bodyBytes, readErr := io.ReadAll(resp.Body); readErr == nil {
				...
				if bodyHasSSEFraming(bodyBytes) {
					observeOpenAISSEBody(observer, string(bodyBytes))
				} else {
					observer.ObserveOpenAI(bodyBytes, strings.TrimSpace(gjson.GetBytes(bodyBytes, "type").String()))
				}
...
func (s *OpenAIGatewayService) codexIdentityOverrideUA(account *Account) string {
	...
	if account != nil && account.IsOpenAIPersonalAccessToken() {
		// PAT 关联的 TUI 身份与 CLI 身份分流：上游对 TUI 的容量策略更宽松，
		// 与 kedaya 的 buildCodexCLIUserAgent 使用 CodexDefaultOriginator (codex-tui) 保持一致
		return openai.CodexDefaultOriginator + "/" + codexCLIVersion + codexCLIUserAgentSuffix
	}
