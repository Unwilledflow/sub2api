...
func (s *GatewayService) forwardAnthropicAPIKeyPassthroughWithInput(
	...
	if input.RequestStream {
		...
		semanticTTFTMs = streamResult.semanticTTFTMs
	...
	return &ForwardResult{
		...
		SemanticFirstTokenMs: semanticTTFTMs,
...
func (s *GatewayService) handleStreamingResponseAnthropicAPIKeyPassthrough(
	...
	firstOutputTimeout := s.anthropicFirstOutputTimeout()
	...
	for {
		...
		case ev, ok := <-events:
				...
				if !sawTerminalEvent {
					if clientDisconnected && streamInterval > 0 {
						...
						if time.Since(lastRead) >= streamInterval {
							return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, semanticTTFTMs: semanticTTFTMs, clientDisconnect: true}, fmt.Errorf("stream usage incomplete after timeout")
						}
					}
					return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, semanticTTFTMs: semanticTTFTMs, clientDisconnect: clientDisconnected}, fmt.Errorf("stream usage incomplete: missing terminal event")
				}
				return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, semanticTTFTMs: semanticTTFTMs, clientDisconnect: clientDisconnected}, nil
			}
			if ev.err != nil {
				if sawTerminalEvent {
					return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, semanticTTFTMs: semanticTTFTMs, clientDisconnect: clientDisconnected}, nil
				...
				if clientDisconnected {
					return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, semanticTTFTMs: semanticTTFTMs, clientDisconnect: true}, fmt.Errorf("stream usage incomplete after disconnect: %w", ev.err)
				}
				if errors.Is(ev.err, context.Canceled) || errors.Is(ev.err, context.DeadlineExceeded) {
					return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, semanticTTFTMs: semanticTTFTMs, clientDisconnect: true}, fmt.Errorf("stream usage incomplete: %w", ev.err)
				}
				if errors.Is(ev.err, bufio.ErrTooLong) {
					...
					return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, semanticTTFTMs: semanticTTFTMs}, ev.err
				}
				return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, semanticTTFTMs: semanticTTFTMs}, fmt.Errorf("stream read error: %w", ev.err)
			}
			...
			if data, ok := extractAnthropicSSEDataLine(line); ok {
				...
				if semanticTTFTMs == nil && anthropicStreamDataStartsVisibleOutput(trimmed) {
					ms := int(time.Since(startTime).Milliseconds())
					semanticTTFTMs = &ms
				}
		...
		case <-firstOutputCh:
			...
			if clientDisconnected {
				return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, semanticTTFTMs: semanticTTFTMs, clientDisconnect: true}, fmt.Errorf("stream usage incomplete after first-output timeout")
			}
			return nil, s.newAnthropicFirstOutputTimeoutError(ctx, c, account, startTime, model, firstOutputTimeout, resp.Header)
		...
		case <-intervalCh:
			...
			return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, semanticTTFTMs: semanticTTFTMs}, fmt.Errorf("stream data interval timeout")
...
// anthropicStreamDataStartsVisibleOutput 判定 SSE data 是否携带首个可见内容：
// 仅 content_block_delta 的 text_delta 计为可见输出；thinking_delta /
// signature_delta / input_json_delta 等推理/工具事件不计，避免思考时间虚高 TTFT。
func anthropicStreamDataStartsVisibleOutput(trimmed string) bool {
	if trimmed == "" || trimmed == "[DONE]" || !gjson.Valid(trimmed) {
		return false
	}
	if gjson.Get(trimmed, "type").String() != "content_block_delta" {
		return false
	}
