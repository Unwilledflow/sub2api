...
type CompetitiveWasteReason string

const (
	CompetitiveWasteReasonCanceledLoser CompetitiveWasteReason = "canceled_loser"
	CompetitiveWasteReasonFailedBatch   CompetitiveWasteReason = "failed_batch"
)

// CompetitiveWasteAttempt is internal settlement evidence for a non-winning
// upstream attempt. It must never be merged into the customer's Usage fields.
type CompetitiveWasteAttempt struct {
	AttemptNo             int
	AccountID             int64
...
type ForwardResult struct {
	...
	// response body and reflects the model the upstream actually used.
	...
	// SemanticFirstTokenMs 是首个「可见内容」token 的时间，排除思考/推理 delta。
	// 用于落库 first_token_ms 与 token/s 计算，避免思考时间虚高 TTFT。
	...
	// AudioUsage and SearchCount reserve the media/search billing contract used
	// by response-model pricing gates. Nil/zero preserve the legacy path.
	AudioUsage  *AudioUsage
	SearchCount int
}

// AudioUsage carries the provider-neutral billing unit for voice requests.
// DurationOrUnits is minutes for realtime, million characters for TTS, and
// hours for STT.
...
// semanticFirstTokenMs 返回落库/统计用的首字时间：优先语义值（排除思考），
// 无则回退协议首事件时间。
func semanticFirstTokenMs(r *ForwardResult) *int {
	if r == nil {
		return nil
	}
...
type UpstreamFailoverError struct {
	...
	SuppressScheduleFailure  bool // race timeout health is reported only after the rolling threshold
}
...
type GatewayService struct {
	...
	upstreamRouteHealth   *UpstreamRouteHealth
...
// SetUpstreamRouteHealth attaches the monitor-driven route overlay after
// construction. Keeping it as a setter preserves the many narrow test
// constructors while production wiring installs it before request serving.
func (s *GatewayService) SetUpstreamRouteHealth(health *UpstreamRouteHealth) {
	if s != nil {
		s.upstreamRouteHealth = health
	}
}
...
func (s *GatewayService) GenerateSessionHash(parsed *ParsedRequest) string {
	...
	// 3. 最后 fallback: 稳定会话键（上下文 + system + 首条 user/会话锚点），
	// 不把全量对话写入 key，避免每轮 hash 变化导致粘性永不命中与 Redis 键膨胀。
	...
	if firstUser := extractFirstUserTextFromMessagesRaw(parsed.MessagesRaw()); firstUser != "" {
		_, _ = combined.WriteString(firstUser)
	} else {
	...
	// 仅有上下文、无任何会话锚点时仍可区分客户端，但不把消息正文写入 Redis key 材料。
	if combined.Len() == contentStart && parsed.SessionContext == nil {
		return ""
	}
	hash := s.hashContent(combined.String())
	slog.Info("sticky.hash_source",
		"source", "stable_session_fallback",
		"hash", hash,
...
// extractFirstUserTextFromMessagesRaw 取 messages 中第一条 user 文本作为会话锚点。
func extractFirstUserTextFromMessagesRaw(raw []byte) string {
	if len(raw) == 0 {
		return ""
	}
...
// ClearStickySession removes a session binding without changing account state.
