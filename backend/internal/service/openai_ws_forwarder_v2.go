...
func (s *OpenAIGatewayService) forwardOpenAIWSV2(
	...
	for {
		...
		if eventType == "response.failed" {
			...
			if !wroteDownstream && isOpenAIModelCapacityError(failedMessage, message) {
				lease.MarkBroken()
				s.handleOpenAIAccountUpstreamError(ctx, account, http.StatusServiceUnavailable, lease.HandshakeHeaders(), message, mappedModel)
				return nil, newOpenAIWSModelCapacityFailoverError(lease.HandshakeHeaders(), message, failedMessage)
			}
		}
		...
		if eventType == "error" {
			...
			if !wroteDownstream && isOpenAIModelCapacityError(errMsgRaw, message) {
				lease.MarkBroken()
				return nil, newOpenAIWSModelCapacityFailoverError(lease.HandshakeHeaders(), message, errMsgRaw)
			}
