...
func (s *OpenAIGatewayService) performOpenAIWSGeneratePrewarm(
	...
	for {
		...
		if eventType == "error" {
			...
			lease.MarkBroken()
			if isOpenAIModelCapacityError(errMsgRaw, message) {
				s.handleOpenAIAccountUpstreamError(ctx, account, http.StatusServiceUnavailable, lease.HandshakeHeaders(), message)
				return newOpenAIWSModelCapacityFailoverError(lease.HandshakeHeaders(), message, errMsgRaw)
			}
			if canFallback {
		...
		if eventType == "response.failed" {
			...
			if isOpenAIModelCapacityError(failedMessage, message) {
				lease.MarkBroken()
				s.handleOpenAIAccountUpstreamError(ctx, account, http.StatusServiceUnavailable, lease.HandshakeHeaders(), message)
				return newOpenAIWSModelCapacityFailoverError(lease.HandshakeHeaders(), message, failedMessage)
			}
		}
...
func openAIWSPayloadTransientStatus(payload []byte) int {
	...
	if isOpenAIModelCapacityError("", payload) {
...
func classifyOpenAIWSErrorEventFromRaw(codeRaw, errTypeRaw, msgRaw string) (string, bool) {
	...
	if isOpenAIModelCapacityError(msgRaw, nil) {
...
func newOpenAIWSModelCapacityFailoverError(headers http.Header, payload []byte, message string) *UpstreamFailoverError {
