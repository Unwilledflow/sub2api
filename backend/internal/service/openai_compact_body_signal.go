...
// EnsureOpenAICompactionTriggerLast 保证 input 中的 compaction_trigger item
// 位于数组末尾。上游 OpenAI 对远程压缩回合的硬性要求是
// "The 'compaction_trigger' item must be the final input item."；网关链路上
// 任何往 input 末尾追加合成 item 的归一化（如 responses Lite 的
// additional_tools 载体）都会把 trigger 挤出末位导致上游 400。该函数在出站
// 前做幂等修复：把第一个 compaction_trigger 移动到末尾（其余 trigger 若存在
// 也一并移除，上游只接受单个且必须末位）。无 trigger 时原样返回。
func EnsureOpenAICompactionTriggerLast(body []byte) ([]byte, bool, error) {
	if len(body) == 0 {
		return body, false, nil
	}
	...
	if triggerIdx == len(items)-1 && extraTriggers == 0 {
		return body, false, nil
	}

	kept := make([]string, 0, len(items))
	triggerRaw := items[triggerIdx].Raw
	for i, item := range items {
		if item.Get("type").String() == "compaction_trigger" {
			continue
		}
...
// EnsureOpenAICompactionTriggerLastForTest 暴露给测试/调用方的别名。
func EnsureOpenAICompactionTriggerLastForTest(body []byte) ([]byte, bool, error) {
	return EnsureOpenAICompactionTriggerLast(body)
}
