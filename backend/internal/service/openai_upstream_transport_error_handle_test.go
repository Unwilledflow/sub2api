...
func TestHandleOpenAIUpstreamTransportError_ConnectionRefusedFailsOverWithoutEviction(t *testing.T) {
	repo := &openaiTransportAccountRepoStub{}
	svc := &OpenAIGatewayService{accountRepo: repo}
	account := &Account{ID: 100, Name: "temporarily-down", Platform: PlatformOpenAI}
	c, rec := newOpenAITransportErrTestContext()

	err := svc.handleOpenAIUpstreamTransportError(context.Background(), c, account,
		fmt.Errorf(`Post "https://upstream.example/v1/responses": dial tcp: %w`, syscall.ECONNREFUSED), false)

	var fo *UpstreamFailoverError
	require.True(t, errors.As(err, &fo), "connection refusal must still fail over")
	require.Equal(t, http.StatusBadGateway, fo.StatusCode)
	require.Empty(t, repo.tempUnschedCalls, "connection refusal must not change account availability")
	require.False(t, svc.isOpenAIAccountRuntimeBlocked(account))
	require.Equal(t, 0, rec.Body.Len())
}
