...
func (s *apiKeyRepoStubForGroupUpdate) ResetRateLimitUsage(_ context.Context, id int64) error {
	if s.updateErr != nil {
		return s.updateErr
	}
	s.rateLimitResetCalled = true
	if s.key != nil && s.key.ID == id {
		s.key.Usage5h = 0
		s.key.Usage1d = 0
		s.key.Usage7d = 0
		s.key.Window5hStart = nil
		s.key.Window1dStart = nil
		s.key.Window7dStart = nil
	}
	return nil
}
...
func TestAdminService_AdminUpdateAPIKeyGroupID_GroupNilNotFound(t *testing.T) {
	existing := &APIKey{ID: 1, Key: "sk-test"}
	apiKeyRepo := &apiKeyRepoStubForGroupUpdate{key: existing}
...
func TestAdminService_AdminResetAPIKeyRateLimitUsage_UsesExplicitAccountingReset(t *testing.T) {
	now := time.Now()
	existing := &APIKey{
		ID:            1,
		UserID:        42,
	...
	require.NoError(t, err)
	require.True(t, apiKeyRepo.rateLimitResetCalled)
	require.Nil(t, apiKeyRepo.updated, "reset must not replay a stale config snapshot")
...
func TestAPIKeyService_Update_GroupIDNilPreservesExistingGroup(t *testing.T) {
	existing := &APIKey{ID: 1, UserID: 42, Key: "sk-test", GroupID: int64Ptr(10), Group: &Group{ID: 10, Name: "Existing"}}
	apiKeyRepo := &apiKeyRepoStubForGroupUpdate{key: existing}
	cache := &apiKeyCacheStub{}
	svc := &APIKeyService{apiKeyRepo: apiKeyRepo, cache: cache}

	got, err := svc.Update(context.Background(), 1, 42, UpdateAPIKeyRequest{})
	require.NoError(t, err)
	...
	require.Equal(t, int64(10), *apiKeyRepo.updated.GroupID)
	require.Equal(t, []string{svc.authCacheKey("sk-test")}, cache.deleteAuthKeys)
}

func TestAPIKeyService_Update_GroupIDZeroClearsGroup(t *testing.T) {
	existing := &APIKey{ID: 1, UserID: 42, Key: "sk-test", GroupID: int64Ptr(10), Group: &Group{ID: 10, Name: "Existing"}}
	apiKeyRepo := &apiKeyRepoStubForGroupUpdate{key: existing}
	cache := &apiKeyCacheStub{}
	zero := int64(0)
	...
	got, err := svc.Update(context.Background(), 1, 42, UpdateAPIKeyRequest{GroupID: &zero})
	require.NoError(t, err)
...
func TestAPIKeyService_Update_GroupIDPositiveSwitchesGroup(t *testing.T) {
	existing := &APIKey{ID: 1, UserID: 42, Key: "sk-test", GroupID: int64Ptr(10), Group: &Group{ID: 10, Name: "Old"}}
	apiKeyRepo := &apiKeyRepoStubForGroupUpdate{key: existing}
	groupRepo := &groupRepoStubForGroupUpdate{group: &Group{ID: 20, Name: "New", Status: StatusActive}}
	userRepo := &userRepoStubForGroupUpdate{user: &User{ID: 42, Status: StatusActive}}
	cache := &apiKeyCacheStub{}
	target := int64(20)
	svc := &APIKeyService{apiKeyRepo: apiKeyRepo, groupRepo: groupRepo, userRepo: userRepo, cache: cache}

	got, err := svc.Update(context.Background(), 1, 42, UpdateAPIKeyRequest{GroupID: &target})
	require.NoError(t, err)
	require.NotNil(t, got.GroupID)
	require.Equal(t, int64(20), *got.GroupID)
	require.NotNil(t, got.Group)
	require.Equal(t, "New", got.Group.Name)
	require.NotNil(t, apiKeyRepo.updated)
	require.NotNil(t, apiKeyRepo.updated.GroupID)
	require.Equal(t, int64(20), *apiKeyRepo.updated.GroupID)
	require.Equal(t, int64(20), groupRepo.lastGetByIDArg)
	require.Equal(t, []string{svc.authCacheKey("sk-test")}, cache.deleteAuthKeys)
}
