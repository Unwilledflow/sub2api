...
type ChannelMonitorRepository interface {
	...
	ListPublic(ctx context.Context) ([]*ChannelMonitor, error)
...
// ChannelMonitorResultObserver receives a fully completed monitor cycle. The
// monitor remains the source of truth for persistence; observers only build
// local, disposable control-plane projections such as routing health.
type ChannelMonitorResultObserver interface {
	ConfigureChannelMonitor(monitor *ChannelMonitor)
	RemoveChannelMonitor(monitorID int64)
	ObserveChannelMonitorResults(ctx context.Context, monitor *ChannelMonitor, results []*CheckResult)
}
...
type ChannelMonitorService struct {
	...
	resultObserver ChannelMonitorResultObserver
}
...
const (
	ChannelMonitorExternalRefMetadataKey    = "sub2api:external_ref"
	ChannelMonitorPublicVisibleMetadataKey  = "sub2api:public_visible"
	ChannelMonitorManagementModeMetadataKey = "sub2api:management_mode"
	ChannelMonitorManagementModeExternal    = "external"
)
...
func (s *ChannelMonitorService) Create(ctx context.Context, p ChannelMonitorCreateParams) (*ChannelMonitor, error) {
	...
	m := &ChannelMonitor{
		...
		ManagementMode:   strings.TrimSpace(p.ManagementMode),
	}
...
func (s *ChannelMonitorService) Update(ctx context.Context, id int64, p ChannelMonitorUpdateParams) (*ChannelMonitor, error) {
	...
	s.syncResultObserverMonitor(existing)
	if s.scheduler != nil {
...
func (s *ChannelMonitorService) Delete(ctx context.Context, id int64) error {
	...
	s.removeResultObserverMonitor(id)
	if s.scheduler != nil {
...
// RecordExternalResults persists results produced by an external control plane.
// Externally managed monitors remain disabled in the native scheduler.
func (s *ChannelMonitorService) RecordExternalResults(ctx context.Context, id int64, results []*CheckResult) error {
	m, err := s.repo.GetByID(ctx, id)
	if err != nil {
		return err
	}
	if m.ManagementMode != ChannelMonitorManagementModeExternal {
		return ErrChannelMonitorNotExternallyManaged
	}
	if len(results) == 0 {
		return ErrChannelMonitorMissingExternalResults
	}
	for _, result := range results {
		if result == nil || strings.TrimSpace(result.Model) == "" || result.CheckedAt.IsZero() {
			return ErrChannelMonitorInvalidExternalResult
		}
		...
		result.Message = truncateMessage(sanitizeErrorMessage(result.Message))
	}
	if promoteExternalResultModel(m, results) {
		if err := s.repo.Update(ctx, m); err != nil {
			return fmt.Errorf("update external channel monitor model: %w", err)
		}
	}
	s.persistCheckResults(ctx, m, results)
	return nil
...
func promoteExternalResultModel(m *ChannelMonitor, results []*CheckResult) bool {
	...
	if latest == nil || latest.Model == m.PrimaryModel {
		return false
	}

	seen := map[string]struct{}{latest.Model: {}}
	extraModels := make([]string, 0, len(m.ExtraModels)+1)
	...
	m.PrimaryModel = latest.Model
	m.ExtraModels = extraModels
	return true
}
...
func (s *ChannelMonitorService) persistCheckResults(ctx context.Context, m *ChannelMonitor, results []*CheckResult) {
	...
	for _, result := range results {
		if result != nil && result.CheckedAt.After(checkedAt) {
	...
	if err := s.repo.MarkChecked(ctx, m.ID, checkedAt); err != nil {
		slog.Error("channel_monitor: mark checked failed",
			"monitor_id", m.ID, "error", err)
	}
	s.notifyResultObserver(ctx, m, results)
}
...
// SetResultObserver installs the process-local consumer for completed monitor
// cycles. It is intentionally separate from MonitorScheduler: monitor CRUD
// controls scheduling, while result observation controls routing overlays.
func (s *ChannelMonitorService) SetResultObserver(observer ChannelMonitorResultObserver) {
	if s == nil {
		return
	}
	s.observerMu.Lock()
	s.resultObserver = observer
	s.observerMu.Unlock()
}

func (s *ChannelMonitorService) notifyResultObserver(ctx context.Context, monitor *ChannelMonitor, results []*CheckResult) {
	if s == nil || monitor == nil {
		return
	}
	s.observerMu.RLock()
	observer := s.resultObserver
	s.observerMu.RUnlock()
	if observer != nil {
		observer.ObserveChannelMonitorResults(ctx, monitor, results)
	}
}

func (s *ChannelMonitorService) syncResultObserverMonitor(monitor *ChannelMonitor) {
	if s == nil || monitor == nil {
		return
	}
	s.observerMu.RLock()
	...
	s.observerMu.RUnlock()
	if observer != nil {
		observer.ConfigureChannelMonitor(monitor)
	}
}

func (s *ChannelMonitorService) removeResultObserverMonitor(monitorID int64) {
	if s == nil {
		return
	}
	s.observerMu.RLock()
	...
	s.observerMu.RUnlock()
	if observer != nil {
		observer.RemoveChannelMonitor(monitorID)
	}
}
...
func applyMonitorUpdate(existing *ChannelMonitor, p ChannelMonitorUpdateParams) error {
	...
	if p.ExternalRef != nil {
		existing.ExternalRef = strings.TrimSpace(*p.ExternalRef)
	}
	if p.PublicVisible != nil {
		existing.PublicVisible = *p.PublicVisible
	}
	if p.ManagementMode != nil {
		mode := strings.TrimSpace(*p.ManagementMode)
		if mode != "" && mode != ChannelMonitorManagementModeExternal {
			return ErrChannelMonitorInvalidManagementMode
		}
		existing.ManagementMode = mode
	}
	if p.Enabled != nil {
