...
const openAIWSResponseAccountBindTimeout = 3 * time.Second

func bindOpenAIWSResponseAccount(
	ctx context.Context,
	...
	if store == nil {
		return errors.New("OpenAI WS state store is nil")
	}
	base := context.Background()
	if ctx != nil {
		base = context.WithoutCancel(ctx)
	}
	bindCtx, cancel := context.WithTimeout(base, openAIWSResponseAccountBindTimeout)
	defer cancel()
	return store.BindResponseAccount(bindCtx, groupID, responseID, accountID, ttl)
}
