...
func (p *BatchImageProviderProcessor) Process(ctx context.Context, batchID string) (BatchImageProcessResult, error) {
	...
	if !ok || provider == nil {
		return p.failPermanent(ctx, job, "UNSUPPORTED_PROVIDER", ErrBatchImageUnsupportedProvider)
	}
	if job.AccountID == nil || *job.AccountID <= 0 {
		return p.failPermanent(ctx, job, "MISSING_ACCOUNT_ID", ErrBatchImageMissingAccountID)
	}
	...
	if !provider.SupportsAccount(account) {
		return p.failPermanent(ctx, job, "UNSUPPORTED_ACCOUNT", ErrBatchImageProviderUnsupportedAccount)
	}
	if strings.TrimSpace(batchImageDerefString(job.ProviderJobName)) == "" {
		return p.failPermanent(ctx, job, "MISSING_PROVIDER_JOB_NAME", ErrBatchImageMissingProviderJobName)
	}
...
// failPermanent 将不可恢复错误转为 failed 终态并释放 hold，避免 worker 无限 requeue。
func (p *BatchImageProviderProcessor) failPermanent(ctx context.Context, job *BatchImageJob, code string, cause error) (BatchImageProcessResult, error) {
	if job == nil {
		return BatchImageProcessResult{}, cause
	}
	msg := ""
	if cause != nil {
		msg = truncateBatchImageMessage(cause.Error(), batchImageMaxErrorMessageLength)
	}
	if err := p.Repo.TransitionBatchImageJobStatus(ctx, job.BatchID, BatchImageJobStatusFailed, BatchImageTransitionOptions{
		EventType:    "job_failed",
		EventPayload: map[string]any{"error_code": code, "permanent": true},
		ErrorCode:    batchImageStringPtr(code),
		ErrorMessage: batchImageOptionalStringPtr(msg),
	}); err != nil {
		return BatchImageProcessResult{}, err
