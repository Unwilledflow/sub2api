...
func TestPluginPackageInstallerInstallUnsignedDevelopmentPackage(t *testing.T) {
	...
	// Windows does not expose POSIX executable bits through FileMode; the
	// installer still opens the runtime with 0700 on Unix, so only assert the
	// bit where the platform can represent it.
