...
// isHTMLResponse recognizes the endpoint-level HTML error pages emitted by
// proxies/CDNs without misclassifying JSON, XML, or plain-text errors.
...
type RateLimitService struct {
	...
	openAIPool5xxCounterCache OpenAIPool5xxCounterCache
	settingService            *SettingService
	tokenCacheInvalidator     TokenCacheInvalidator
	runtimeBlocker            AccountRuntimeBlocker
	accountFailureGuard       *AccountFailureGuardService
...
const (
	...
	balanceErrorCooldown            = 10 * time.Minute
)
...
func (s *RateLimitService) SetOpenAIPool5xxCounterCache(cache OpenAIPool5xxCounterCache) {
	s.openAIPool5xxCounterCache = cache
}
...
// SetAccountFailureGuard 注入连续失败守卫（可选依赖）。
func (s *RateLimitService) SetAccountFailureGuard(guard *AccountFailureGuardService) {
	s.accountFailureGuard = guard
}
...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	...
	// 连续失败守卫计数必须在所有早退分支之前执行，保证池模式/自定义错误码
	// 账号同样纳入「连续失败 → 自动禁用」统计。
	if s.accountFailureGuard != nil {
		s.accountFailureGuard.RecordFailure(ctx, account, statusCode)
	}
	ctx = withTempUnschedulableModel(ctx, requestedModel)
	...
	if account.IsPoolMode() && !customErrorCodesEnabled {
		...
		if s.tryOpenAIAPIKey5xxCooldown(ctx, account, statusCode, responseBody, requestedModel...) {
			return true
		}
	...
	if !customErrorCodesEnabled && s.tryOpenAIAPIKey5xxCooldown(ctx, account, statusCode, responseBody, requestedModel...) {
		return true
	}
	...
	case 400:
		...
		if strings.Contains(strings.ToLower(upstreamMsg), "organization has been disabled") {
			...
			shouldDisable = true
		} else if account.Platform == PlatformAnthropic && isBalanceErrorResponse(upstreamMsg, responseBody) {
			// Anthropic API key 余额不足：进入可恢复冷却，不写永久 error。
			msg := "Credit balance exhausted (400): " + upstreamMsg
			s.handleBalanceError(ctx, account, msg)
			shouldDisable = true
	...
	case 403:
		...
		if isBalanceErrorResponse(upstreamMsg, responseBody) {
			msg := buildForbiddenErrorMessage("Balance unavailable (403):", upstreamMsg, responseBody, "upstream balance is insufficient")
			s.handleBalanceError(ctx, account, msg)
			shouldDisable = true
			break
		}
...
func isBalanceErrorMessage(message string) bool {
...
func isBalanceErrorResponse(message string, responseBody []byte) bool {
	return isBalanceErrorMessage(message) || isBalanceErrorMessage(string(responseBody))
}

// handleBalanceError keeps the account active and uses the shared temporary
// unschedulable state so a later probe can retry after the balance is restored.
func (s *RateLimitService) handleBalanceError(ctx context.Context, account *Account, errorMsg string) {
	if account == nil {
		return
	}
	until := time.Now().Add(balanceErrorCooldown)
	s.notifyAccountSchedulingBlocked(account, until, "balance_error")
	reason := "balance_error: " + errorMsg
	if err := s.accountRepo.SetTempUnschedulable(ctx, account.ID, until, reason); err != nil {
		slog.Warn("account_set_balance_cooldown_failed", "account_id", account.ID, "error", err)
		return
	}
	slog.Warn("account_balance_temporary_cooldown", "account_id", account.ID, "until", until, "error", errorMsg)
}
...
// RecordOpenAIUpstreamSuccess makes the 5xx circuit breaker consecutive-failure
// aware. A healthy response breaks an older sampled failure streak so a few
// intermittent upstream errors do not quarantine an otherwise busy account.
func (s *RateLimitService) RecordOpenAIUpstreamSuccess(ctx context.Context, account *Account) {
	if s == nil || s.openAIPool5xxCounterCache == nil || account == nil || account.ID <= 0 ||
		account.Platform != PlatformOpenAI || account.Type != AccountTypeAPIKey {
		return
	}
	if err := s.openAIPool5xxCounterCache.ResetOpenAIPool5xxCount(ctx, account.ID); err != nil {
		slog.Warn("openai_pool_5xx_success_reset_failed", "account_id", account.ID, "error", err)
	}
}
...
func isOpenAIAPIKeyCooldownStatus(statusCode int) bool {
	switch statusCode {
...
func (s *RateLimitService) tryOpenAIAPIKey5xxCooldown(ctx context.Context, account *Account, statusCode int, responseBody []byte, requestedModel ...string) bool {
	if s == nil || account == nil || account.Platform != PlatformOpenAI || account.Type != AccountTypeAPIKey || !isOpenAIAPIKeyCooldownStatus(statusCode) {
		return false
	}
	if s.cfg == nil || s.openAIPool5xxCounterCache == nil {
		return false
	}
	threshold := s.cfg.Gateway.OpenAIPool5xxFailureThreshold
	windowSeconds := s.cfg.Gateway.OpenAIPool5xxFailureWindowSeconds
	sampleIntervalSeconds := s.cfg.Gateway.OpenAIPool5xxSampleIntervalSeconds
	cooldownSeconds := s.cfg.Gateway.OpenAIPool5xxCooldownSeconds
	if threshold < 2 || windowSeconds <= 0 || sampleIntervalSeconds <= 0 || cooldownSeconds <= 0 {
		return false
	}

	count, sampled, err := s.openAIPool5xxCounterCache.ObserveOpenAIPool5xxFailure(ctx, account.ID, windowSeconds, sampleIntervalSeconds)
	if err != nil {
		slog.Warn("openai_pool_5xx_observe_failed", "account_id", account.ID, "status_code", statusCode, "error", err)
		return false
	}
	if !sampled {
		slog.Debug("openai_pool_5xx_coalesced", "account_id", account.ID, "status_code", statusCode, "count", count, "sample_interval_seconds", sampleIntervalSeconds)
		return false
	}
	slog.Info("openai_pool_5xx_observed", "account_id", account.ID, "status_code", statusCode, "count", count, "threshold", threshold, "window_seconds", windowSeconds, "sample_interval_seconds", sampleIntervalSeconds)
	...
	message = truncateForLog([]byte(sanitizeUpstreamErrorMessage(message)), 512)
	state := &TempUnschedState{
		UntilUnix:       until.Unix(),
		TriggeredAtUnix: now.Unix(),
		StatusCode:      statusCode,
		MatchedKeyword:  "openai_pool_5xx",
		RuleIndex:       -1,
		ErrorMessage:    fmt.Sprintf("OpenAI API-key transient upstream error (%d): %s", statusCode, message),
	}
	if len(requestedModel) > 0 {
		state.Model = strings.TrimSpace(requestedModel[0])
	}
	reason := state.ErrorMessage
	if reasonBytes, err := json.Marshal(state); err == nil {
	...
	if err := s.accountRepo.SetTempUnschedulable(ctx, account.ID, until, reason); err != nil {
		slog.Warn("openai_pool_5xx_set_temp_unschedulable_failed", "account_id", account.ID, "status_code", statusCode, "error", err)
		return false
	}

	s.notifyAccountSchedulingBlocked(account, until, "openai_pool_5xx")
	if s.tempUnschedCache != nil {
		if err := s.tempUnschedCache.SetTempUnsched(ctx, account.ID, state); err != nil {
			slog.Warn("openai_pool_5xx_set_cache_failed", "account_id", account.ID, "status_code", statusCode, "error", err)
		}
	}
	if err := s.openAIPool5xxCounterCache.ResetOpenAIPool5xxCount(ctx, account.ID); err != nil {
		slog.Warn("openai_pool_5xx_reset_failed", "account_id", account.ID, "error", err)
	}
	slog.Warn("openai_pool_5xx_temp_unschedulable", "account_id", account.ID, "status_code", statusCode, "failure_count", count, "until", until, "cooldown_seconds", cooldownSeconds)
	return true
}
