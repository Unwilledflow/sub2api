...
type canceledContextProbeStateStore struct {
	OpenAIWSStateStore
	sawCanceled bool
}

func (s *canceledContextProbeStateStore) BindResponseAccount(ctx context.Context, _ int64, _ string, _ int64, _ time.Duration) error {
	select {
	case <-ctx.Done():
		s.sawCanceled = true
		return ctx.Err()
...
func TestBindOpenAIWSResponseAccount_DetachesCanceledRequestContext(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	store := &canceledContextProbeStateStore{}

	err := bindOpenAIWSResponseAccount(ctx, store, 85, "resp_detached", 625, time.Hour)

	require.NoError(t, err)
	require.False(t, store.sawCanceled)
}
