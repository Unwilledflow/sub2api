...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	if apiKey := getAPIKeyFromContext(c); apiKey != nil && s.settingService != nil {
		antiCfg := ResolveAntiStallForKey(s.settingService.GetAntiStallAdminConfig(ctx), apiKey.AntiStallTier)
		if antiCfg.Enabled {
			stopKeepalive := StartAntiStallWebSocketKeepalive(ctx, clientConn)
			defer stopKeepalive()
		}
	}
	...
	parseClientPayload := func(turn int, raw []byte) (openAIWSClientPayload, error) {
		...
		// 出站兜底：compaction_trigger 必须保持 input 末位（Lite 归一化可能挤位）。
		if HasCompactionTriggerInInput(normalized) {
			if fixed, changedFix, fixErr := EnsureOpenAICompactionTriggerLast(normalized); fixErr == nil && changedFix {
				normalized = fixed
			}
		}
	...
	sendAndRelay := func(turn int, lease *openAIWSConnLease, payload []byte, payloadBytes int, originalModel string, imageBillingModel string, imageSizeTier string, imageInputSize string) (*OpenAIForwardResult, error) {
		...
		for {
			...
			if eventType == "error" {
				...
				if !wroteDownstream && isOpenAIModelCapacityError(errMsgRaw, upstreamMessage) {
					lease.MarkBroken()
					return nil, newOpenAIWSModelCapacityFailoverError(lease.HandshakeHeaders(), upstreamMessage, errMsgRaw)
				}
			}
			...
			if eventType == "response.failed" {
				...
				if !wroteDownstream && isOpenAIModelCapacityError(failedMessage, upstreamMessage) {
					lease.MarkBroken()
					canonicalModel := canonicalOpenAIAccountSchedulingModel(account, originalModel)
					s.handleOpenAIAccountUpstreamError(ctx, account, http.StatusServiceUnavailable, lease.HandshakeHeaders(), upstreamMessage, canonicalModel)
					return nil, newOpenAIWSModelCapacityFailoverError(lease.HandshakeHeaders(), upstreamMessage, failedMessage)
				}
			}
