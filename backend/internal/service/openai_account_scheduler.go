...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	var err error
	req, err = s.resolveRequestGroup(ctx, req)
	if err != nil {
...
func (s *defaultOpenAIAccountScheduler) resolveRequestGroup(
	ctx context.Context,
	req OpenAIAccountScheduleRequest,
) (OpenAIAccountScheduleRequest, error) {
	if req.GroupID == nil || *req.GroupID <= 0 || s == nil || s.service == nil || s.service.schedulerSnapshot == nil {
		return req, nil
	}
	group, err := s.service.schedulerSnapshot.GetGroupByID(ctx, *req.GroupID)
	if err != nil {
		return req, fmt.Errorf("resolve OpenAI scheduler group %d: %w", *req.GroupID, err)
	}
	req.resolvedGroup = group
	req.routingResolved = true
	if group != nil && req.RequestedModel != "" {
		req.routingAccountIDs = append([]int64(nil), group.GetRoutingAccountIDs(req.RequestedModel)...)
	}
	return req, nil
}
...
func isOpenAIAccountCandidateBetter(left openAIAccountCandidateScore, right openAIAccountCandidateScore) bool {
	...
	// 全等时随机 tie-break，避免固定偏向小 ID 账号造成热点/饿死。
...
func (s *defaultOpenAIAccountScheduler) buildOpenAIAccountLoadPlan(
	...
	if strings.TrimSpace(req.PerformanceClass) != "" && hasTTFTSample && minTTFT > 0 {
		outliers := 0
		for i := range candidates {
			candidate := &candidates[i]
...
func (s *defaultOpenAIAccountScheduler) tryFallbackToWeightedSticky(
	...
	for _, accountID := range []int64{req.StickyPreviousAccountID, req.StickyAccountID} {
		...
		if req.resolvedGroup != nil && req.resolvedGroup.RequirePrivacySet && !account.IsPrivacySet() {
			continue
		}
		...
		if s.service.concurrencyService != nil && waitSelection == nil {
			cfg := s.service.schedulingConfig()
			waitSelection = attachSelectionProfitGate(ctx, &AccountSelectionResult{
				Account: account,
				WaitPlan: &AccountWaitPlan{
...
func (s *defaultOpenAIAccountScheduler) trySelectByLoadBalancePool(
	...
	if openAISelectionOverflowExpanded(plan) {
		budget.enableLimit()
	}
	...
	if s.service.concurrencyService != nil && !budget.acquireExhausted() {
		...
		if freshLoadMap, loadErr := s.service.concurrencyService.GetAccountsLoadBatchFresh(ctx, loadReq); loadErr == nil {
			...
			if openAISelectionOverflowExpanded(freshPlan) {
				budget.enableLimit()
			}
...
func openAISelectionOverflowExpanded(plan openAIAccountLoadPlan) bool {
	return plan.topK > 0 && len(plan.selectionOrder) > plan.topK
}
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatibleReason(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) (bool, string) {
	...
	if req.routingResolved && !openAIAccountAllowedByRoutingIDs(account.ID, req.routingAccountIDs) {
		return false, "model_routing_excluded"
	}
...
func openAIAccountAllowedByRoutingIDs(accountID int64, routedIDs []int64) bool {
	if len(routedIDs) == 0 {
		return true
	}
	for _, routedID := range routedIDs {
		if routedID == accountID {
			return true
		}
	}
	return false
...
func (s *OpenAIGatewayService) getOpenAIAccountScheduler(ctx context.Context) OpenAIAccountScheduler {
	...
	// 运行时统计（EWMA 错误率/TTFT）常驻采集，不依赖高级调度器开关，
	// 为默认路径的可观测与未来的软降权提供数据。
	...
	if !s.isOpenAIAdvancedSchedulerEnabled(ctx) {
		// 高级调度器关闭：不返回评分调度器（默认路径），但 stats 已常驻采集。
		return nil
	}
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForCapability(
	...
	return s.SelectAccountWithSchedulerForCapabilityClass(
		...
		useUpstreamTokenCost, "", platformOverride...,
	)
}
