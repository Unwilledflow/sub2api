...
const (
	...
	// geminiPoolMode429FallbackCooldown 池模式账号 429 且上游未带 reset 信息时的兜底冷却。
	// 短冷却只为把限流账号暂时摘出调度，避免粘性会话反复撞限流。
	geminiPoolMode429FallbackCooldown = 2 * time.Minute
)
...
func (s *GeminiMessagesCompatService) SetAccountFailureGuard(guard *AccountFailureGuardService) {
...
func (s *GeminiMessagesCompatService) skippedErrorPolicyFailoverError(c *gin.Context, account *Account, statusCode int, respBody []byte, upstreamRequestID string) *UpstreamFailoverError {
	...
	// 连续失败守卫：池模式/自定义错误码跳过的错误同样计入统计，避免这些账号
	// 反复撞限流/5xx 却永远不被自动禁用。
	if s.accountFailureGuard != nil {
		s.accountFailureGuard.RecordFailure(c.Request.Context(), account, statusCode)
	}
	// 429：池模式（未启用自定义错误码）跳过了默认账号状态处理，若不写冷却，
	// 被限流账号会持续中选（粘性会话把新请求继续绑到同一账号），
	// 同账号重试白白烧掉 failover 预算。按上游 reset 信息写限流冷却
	// （调度器经 IsSchedulable 排除并清理粘性绑定），解析不到则短冷却；
	// 且不再同账号重试，立即换号。自定义错误码账号保持管理员显式语义，不写冷却。
	retryable := account.IsPoolMode() && account.IsPoolModeRetryableStatus(statusCode)
	if statusCode == http.StatusTooManyRequests && account.IsPoolMode() && !account.IsCustomErrorCodesEnabled() {
		var resetAt time.Time
		if ts := ParseGeminiRateLimitResetTime(respBody); ts != nil {
			resetAt = time.Unix(*ts, 0)
		} else {
			resetAt = time.Now().Add(geminiPoolMode429FallbackCooldown)
		}
		if s.accountRepo != nil {
			if err := s.accountRepo.SetRateLimited(c.Request.Context(), account.ID, resetAt); err != nil {
				logger.LegacyPrintf("service.gemini_messages_compat", "[Gemini 429] pool account %d set rate limited until %v failed: %v", account.ID, resetAt, err)
			}
		}
		retryable = false
	}
...
func extractGeminiUsage(data []byte) *ClaudeUsage {
	...
	// 钳制 >=0：上游异常返回 cached>prompt 时避免负 input 导致负计费。
...
func (s *GeminiMessagesCompatService) handleGeminiUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, body []byte) {
	...
	// 连续失败守卫：非池模式的 gemini 错误路径在此计数（池模式 failover 错误
	// 走 skippedErrorPolicyFailoverError，已在那边挂点）。
	if s.accountFailureGuard != nil {
