...
func TestGenerateSessionHash_ContinuousConversation_StableAcrossRounds(t *testing.T) {
	svc := &GatewayService{}
	...
	require.Equal(t, h1, h2, "stable fallback must keep sticky key across conversation rounds")
	require.Equal(t, h2, h3, "stable fallback must keep sticky key across conversation rounds")
}
...
func TestGenerateSessionHash_MessageRollbackKeepsStableKey(t *testing.T) {
	svc := &GatewayService{}
	ctx := &SessionContext{ClientIP: "1.2.3.4", UserAgent: "test", APIKeyID: 1}
	...
	require.Equal(t, hOrig, hRollback, "stable fallback anchors on first user message; later rollback must not change sticky key")
}
...
func TestGenerateSessionHash_SameUserGrowingConversation(t *testing.T) {
	...
	for round := 1; round <= len(messages); round += 2 {
		...
		if sticky == "" {
			...
			require.Equal(t, sticky, h, "round %d hash should stay sticky with first-user anchor", round)
		}
...
func TestGenerateSessionHash_MultipleUserMessages(t *testing.T) {
	...
	// Later user turns must not perturb sticky key when first user text is unchanged.
	...
	h2 := svc.GenerateSessionHash(parsed2)
	require.Equal(t, h, h2, "changing non-first user messages must not change stable sticky key")

	parsed3 := mustParseSessionHashRequest(t, anthropicSessionBody(nil, []any{msg("user", "CHANGED"), msg("user", "second"), msg("user", "third"), msg("user", "fourth"), msg("user", "fifth")}, ""), ctx)
	h3 := svc.GenerateSessionHash(parsed3)
	require.NotEqual(t, h, h3, "changing first user message should change sticky key")
}

func TestGenerateSessionHash_FirstUserMessageOrderMatters(t *testing.T) {
	svc := &GatewayService{}
	ctx := &SessionContext{ClientIP: "1.2.3.4", UserAgent: "test", APIKeyID: 1}
	...
	h1 := svc.GenerateSessionHash(parsed1)
	h2 := svc.GenerateSessionHash(parsed2)
	require.NotEqual(t, h1, h2, "different first user messages should produce different sticky keys")
}
...
func TestGenerateSessionHash_LongConversation(t *testing.T) {
	...
	h2 := svc.GenerateSessionHash(parsed2)
	require.Equal(t, h, h2, "appending turns must keep stable sticky key")
}
...
func TestGenerateSessionHash_GeminiMultiTurnStable(t *testing.T) {
	...
	require.NotEmpty(t, h1)
	require.NotEmpty(t, h2)
	require.Equal(t, h1, h2, "Gemini multi-turn should keep stable sticky key")
}
...
func TestGenerateSessionHash_GeminiMultiTurnHashSticky(t *testing.T) {
	...
	require.Equal(t, hashes[0], hashes[1], "round 1 vs 2 hash should stay sticky")
	require.Equal(t, hashes[1], hashes[2], "round 2 vs 3 hash should stay sticky")
