...
type APIKeyAuthSnapshot struct {
	...
	AntiStallTier                  string                   `json:"anti_stall_tier,omitempty"`
	AdaptivePassiveFailoverEnabled bool                     `json:"adaptive_passive_failover_enabled,omitempty"`
...
type APIKeyAuthGroupSnapshot struct {
	...
	// 长上下文阶梯定价开关与分组自定义模型价：计费热路径（calculateTokenCost /
	// matchGroupModelPricing）读的就是这份快照物化出的 Group。漏掉这两个字段，
	// 缓存路径会拿到零值 LongContextPricingEnabled=false 而静默关闭官方阶梯，
	// 分组自定义价也会整体失效（2026-08-24 生产事故根因）。
