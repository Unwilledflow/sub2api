...
type balanceUserRepoStub struct {
	*userRepoStub
	...
	balanceDeltas []float64
	setBalances   []float64
}

func (s *balanceUserRepoStub) AdjustBalance(ctx context.Context, id int64, delta float64) (BalanceChange, error) {
	change, err := s.apply(func(current float64) float64 { return current + delta })
	if err == nil {
		s.balanceDeltas = append(s.balanceDeltas, delta)
	}
	return change, err
}

func (s *balanceUserRepoStub) SetBalance(ctx context.Context, id int64, value float64) (BalanceChange, error) {
	change, err := s.apply(func(float64) float64 { return value })
	if err == nil {
		s.setBalances = append(s.setBalances, value)
	}
	return change, err
}
...
func TestAdminService_UpdateUserBalance_InvalidatesAuthCache(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []float64{5}, repo.balanceDeltas)
...
func TestAdminService_UpdateUserBalance_NoChangeNoInvalidate(t *testing.T) {
	...
	require.NoError(t, err)
	require.Empty(t, repo.balanceDeltas)
	require.Equal(t, []float64{10}, repo.setBalances)
	require.Empty(t, invalidator.userIDs)
	require.Empty(t, redeemRepo.created)
}

func TestAdminService_UpdateUserBalance_UsesExplicitAtomicMutation(t *testing.T) {
	tests := []struct {
		name      string
		operation string
		amount    float64
		...
		{name: "set", operation: "set", amount: 7, wantSet: []float64{7}},
		{name: "subtract", operation: "subtract", amount: 4, wantDelta: -4},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			user, err := svc.UpdateUserBalance(context.Background(), 7, tt.amount, tt.operation, "")
			require.NoError(t, err)
			require.Equal(t, tt.wantSet, repo.setBalances)
			if tt.wantSet != nil {
				require.Empty(t, repo.balanceDeltas)
				require.Equal(t, tt.amount, user.Balance)
			} else {
				require.Equal(t, []float64{tt.wantDelta}, repo.balanceDeltas)
				require.Equal(t, 10+tt.wantDelta, user.Balance)
			}
		})
	}
}
