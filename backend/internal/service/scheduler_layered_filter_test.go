...
func TestFilterByMinRoutingScoreLetsLoadBeatSmallPriorityGap(t *testing.T) {
	accounts := []accountWithLoad{
		{account: &Account{ID: 1, Priority: 0}, loadInfo: &AccountLoadInfo{LoadRate: 80}},
		{account: &Account{ID: 2, Priority: 1}, loadInfo: &AccountLoadInfo{LoadRate: 10}},
		{account: &Account{ID: 3, Priority: 5}, loadInfo: &AccountLoadInfo{LoadRate: 0}},
	}
	result := filterByMinRoutingScore(accounts)
	require.Len(t, result, 1)
	require.Equal(t, int64(2), result[0].account.ID)
}

func TestRoutingHelpersTreatMissingLoadAsZero(t *testing.T) {
	t.Run("priority and load bounds", func(t *testing.T) {
		require.Equal(t, 0, boundedSchedulingPriority(-1))
		require.Equal(t, maxSchedulingPriority, boundedSchedulingPriority(1_000_000))
		require.Equal(t, 0, routingLoadRate(&AccountLoadInfo{LoadRate: -1}))
		require.Equal(t, 100, routingLoadRate(&AccountLoadInfo{LoadRate: 1_000}))
	})

	t.Run("routing score", func(t *testing.T) {
		account := accountWithLoad{account: &Account{ID: 1, Priority: 1}}
		require.Equal(t, 3, accountRoutingScore(account))
	})

	t.Run("routing score filter", func(t *testing.T) {
		accounts := []accountWithLoad{
			{account: &Account{ID: 1, Priority: 1}},
			{account: &Account{ID: 2, Priority: 0}, loadInfo: &AccountLoadInfo{LoadRate: 4}},
		}
		result := filterByMinRoutingScore(accounts)
		require.Len(t, result, 1)
		require.Equal(t, int64(1), result[0].account.ID)
	...
	t.Run("load filter", func(t *testing.T) {
		accounts := []accountWithLoad{
			...
			{account: &Account{ID: 2}, loadInfo: &AccountLoadInfo{LoadRate: 4}},
		}
