...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	if err != nil {
		...
		// Simple 模式不落账扣费（下方直接写 usage_log 返回），维持与
		// SimpleMode 既有语义一致的零成本落日志，不触发 fail-closed。
		if s.cfg != nil && s.cfg.RunMode == config.RunModeSimple {
			...
			).Warn("openai_usage.pricing_missing_simple_mode_zero_cost", zap.Error(err))
			cost = &CostBreakdown{BillingMode: string(BillingModeToken)}
		} else {
			// Fail closed: never silently bill $0 when model pricing is missing.
			logger.L().With(
				zap.String("component", "service.openai_gateway"),
				zap.Strings("billing_models", billingModels),
				...
				zap.Int64("api_key_id", apiKey.ID),
				zap.Int64("account_id", account.ID),
			).Error("openai_usage.pricing_missing_refuse_settlement", zap.Error(err))
			return err
		}
	}
	...
	if result.UpstreamResponseModelConflict {
		logger.LegacyPrintf("service.openai_gateway", "upstream_response_model_conflict platform=%s account_id=%d request_id=%s sent_model=%s selected_response_model=%s",
			account.Platform, account.ID, requestID, sentModel, strings.TrimSpace(result.UpstreamResponseModel),
		)
	}
	...
	usageLog.FirstTokenMs = result.FirstTokenMsForScheduling()
	...
	// Adaptive parent groups settle through authorize/capture so the 15% fee and
	// hold lifecycle stay consistent. Capture itself inserts the usage row.
	if input.AdaptiveBilling != nil {
		baseActual := 0.0
		if cost != nil {
			baseActual = cost.ActualCost
		}
		if err := settleAdaptiveCustomerUsage(ctx, s.adaptiveBilling, input.AdaptiveBilling, usageLog, baseActual); err != nil {
			return err
		}
...
// RecordOpenAIUpstreamSuccess updates scheduling health in request completion order.
// Billing runs asynchronously and must not reset a newer failure observation.
func (s *OpenAIGatewayService) RecordOpenAIUpstreamSuccess(
	ctx context.Context,
	account *Account,
	result *OpenAIForwardResult,
	skip bool,
) {
	if s == nil || s.rateLimitService == nil || skip || result == nil || !result.SucceededForScheduling() {
		return
	}
	...
	resetCtx, cancel := context.WithTimeout(detached, 3*time.Second)
	defer cancel()
	s.rateLimitService.RecordOpenAIUpstreamSuccess(resetCtx, account)
}
...
// hasIdentifiedOpenAIResponsePricing mirrors the gateway response-model gate
// while keeping OpenAI channel pricing resolution local to this service.
