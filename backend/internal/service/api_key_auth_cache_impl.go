...
const apiKeyAuthSnapshotVersion = 21 // v21: include group long-context pricing toggle + group model pricing
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		AntiStallTier:                  apiKey.AntiStallTier,
		AdaptivePassiveFailoverEnabled: apiKey.AdaptivePassiveFailoverEnabled,
		BlockOpenAIFast:                apiKey.BlockOpenAIFast,
		Quota:                          apiKey.Quota,
	...
	if apiKey.Group != nil {
		snapshot.Group = &APIKeyAuthGroupSnapshot{
			...
			CompetitiveConcurrency:          apiKey.Group.CompetitiveConcurrency,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		AntiStallTier:                  NormalizeAntiStallTier(snapshot.AntiStallTier),
		AdaptivePassiveFailoverEnabled: snapshot.AdaptivePassiveFailoverEnabled,
		BlockOpenAIFast:                snapshot.BlockOpenAIFast,
	...
	if snapshot.Group != nil {
		apiKey.Group = &Group{
			...
			CompetitiveConcurrency:          snapshot.Group.CompetitiveConcurrency,
