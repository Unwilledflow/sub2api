...
func (s *PaymentConfigService) resolveEnabledVisibleMethodInstance(
	...
	if device := payment.DeviceFromContext(ctx); device != "" {
		filtered := make([]*dbent.PaymentProviderInstance, 0, len(matching))
		for _, inst := range matching {
			instDevice := payment.InstanceDeviceForType(inst, method)
			if instDevice == "" || strings.EqualFold(instDevice, device) {
				filtered = append(filtered, inst)
			}
		}
		if len(filtered) > 0 {
