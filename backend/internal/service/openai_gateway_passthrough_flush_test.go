...
func runPassthroughFlushTest(
	...
	return runPassthroughFlushTestAt(t, body, failAfterWrites, time.Now(), setups...)
}

func runPassthroughFlushTestAt(
	t *testing.T,
	body io.ReadCloser,
	failAfterWrites int,
	startTime time.Time,
...
func TestOpenAIStreamingPassthroughCapturesUpstreamAndSemanticEventTimings(t *testing.T) {
	upstream := "event: response.created\n" +
		`data: {"type":"response.created","response":{"id":"resp_timing"}}` + "\n\n" +
		`data: {"type":"response.output_text.delta","delta":"ready"}` + "\n\n" +
		`data: {"type":"response.completed","response":{"id":"resp_timing","usage":{"input_tokens":4,"output_tokens":1,"total_tokens":5}}}` + "\n\n"

	startTime := time.Now().Add(-9 * time.Second)
	result, _, _, err := runPassthroughFlushTestAt(
		t,
	...
	require.NoError(t, err)
	require.NotNil(t, result)
	require.InDelta(t, 9000, result.timing.firstUpstreamEventMs, 500)
	require.Equal(t, "response.created", result.timing.firstUpstreamEventType)
	require.InDelta(t, 9000, result.timing.firstSemanticEventMs, 500)
	require.Equal(t, "response.output_text.delta", result.timing.firstSemanticEventType)
}
