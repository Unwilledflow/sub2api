...
func shouldAutoPauseOpenAIAccountByQuota(ctx context.Context, account *Account) (bool, openAIQuotaAutoPauseDecision) {
	...
	// 透支调度（合并自 guardian/overdraft 思路，仅 OAuth 账号、默认关）：
	// 开启后不因配额阈值预暂停，让账号继续接单直到真实 429；
	// sub2api 的 429 排除+到点自动恢复机制兜底，避免高并发时凭空少容量。
	if account.IsOpenAIOAuth() && resolveAccountExtraBool(account.Extra, "overdraft_enabled") {
		return false, openAIQuotaAutoPauseDecision{}
	}
	// 自动用卡有独立阈值：达到消费阈值时必须先退出调度；仅达到普通暂停阈值时，
	// 只有新鲜状态明确存在可用卡才继续放行到消费阈值。
	if creditConfig := ResolveOpenAIAutoResetCreditConfig(account); creditConfig.Enabled {
		now := time.Now()
		...
		if has5h && utilization5h >= creditConfig.Threshold5h {
			notifyOpenAIAutoReset(account.ID)
			return true, openAIQuotaAutoPauseDecision{window: "5h", threshold: creditConfig.Threshold5h, utilization: utilization5h, reason: "quota_auto_reset_pending_5h"}
		}
		if has7d && utilization7d >= creditConfig.Threshold7d {
			notifyOpenAIAutoReset(account.ID)
			return true, openAIQuotaAutoPauseDecision{window: "7d", threshold: creditConfig.Threshold7d, utilization: utilization7d, reason: "quota_auto_reset_pending_7d"}
		}
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, useUpstreamTokenCost bool) (*AccountSelectionResult, error) {
	...
	parentLookupL2 := func(id int64) *Account {
		...
		if !s.isAccountAllowedByUpstreamRouteHealth(a) {
			a = nil
		}
		parentCacheL2[id] = a
		return a
...
func (s *OpenAIGatewayService) listSchedulableAccounts(ctx context.Context, groupID *int64, platform string) ([]Account, error) {
	...
	if s.schedulerSnapshot != nil {
		...
		return s.filterAccountsByUpstreamRouteHealth(accounts), nil
	}
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDBBeforeProfit(ctx context.Context, account *Account, groupID *int64, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	...
	if !s.isAccountAllowedByUpstreamRouteHealth(latest) {
		return nil
	}
...
func (s *OpenAIGatewayService) getSchedulableAccount(ctx context.Context, accountID int64) (*Account, error) {
	...
	if s.schedulerSnapshot != nil {
		account, err = s.schedulerSnapshot.GetSchedulableAccount(ctx, accountID)
	} else {
	...
	if err == nil && hydrated != nil {
		...
		if codexQuotaOverdraftCandidateKnown(ctx, account.ID) {
			if normalized, ok := normalizeCodexQuotaOverdraftHydratedAccount(ctx, hydrated, time.Now().UTC()); ok {
				if !s.isAccountAllowedByUpstreamRouteHealth(normalized) {
					return nil, nil
				}
		...
		if !codexQuotaOverdraftCandidateKnown(ctx, account.ID) {
			if !s.isAccountAllowedByUpstreamRouteHealth(hydrated) {
				return nil, nil
			}
			return hydrated, nil
		}
...
func (s *OpenAIGatewayService) filterAccountsByUpstreamRouteHealth(accounts []Account) []Account {
...
func (s *OpenAIGatewayService) isAccountAllowedByUpstreamRouteHealth(account *Account) bool {
...
func (s *OpenAIGatewayService) newAcquiredSelectionResult(ctx context.Context, account *Account, release func()) (*AccountSelectionResult, error) {
	...
	if err == nil && (selection == nil || selection.Account == nil) {
		selection = nil
		err = fmt.Errorf("selected openai account is no longer schedulable")
	}
