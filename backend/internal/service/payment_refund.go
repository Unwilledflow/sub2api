...
func (s *PaymentService) handleGwFail(ctx context.Context, p *RefundPlan, gErr error) (*RefundResult, error) {
	// Never roll back balance/subscription immediately on gateway errors.
	// A lost/timeout response may mean the upstream refund already succeeded;
	// rolling back would double-pay the user (balance + gateway refund).
	// Park the order in REFUND_PENDING and require QueryAndFinalizeRefund
	// (which queries the channel) before completing or rolling back.
	if isDefinitiveRefundGatewayFailure(gErr) {
		return s.confirmGatewayRefundAbsentThenFail(ctx, p, gErr)
	}
	s.writeAuditLog(ctx, p.OrderID, "REFUND_GATEWAY_UNKNOWN", "admin", map[string]any{
		"detail": psErrMsg(gErr),
		"action": "hold_pending_verify",
	})
	return s.markRefundPending(ctx, p, &payment.RefundResponse{
		Status: payment.ProviderStatusPending,
	})
}

// confirmGatewayRefundAbsentThenFail queries the channel when possible. Only
// after the channel confirms the refund is absent (or the provider cannot
// query) do we roll back local deductions and mark REFUND_FAILED.
func (s *PaymentService) confirmGatewayRefundAbsentThenFail(ctx context.Context, p *RefundPlan, gErr error) (*RefundResult, error) {
	if p != nil && p.Order != nil && strings.TrimSpace(p.Order.PaymentTradeNo) != "" {
		if status, refundID, qErr := s.queryGatewayRefundStatus(ctx, p); qErr == nil {
			switch strings.TrimSpace(status) {
			case payment.ProviderStatusSuccess, payment.ProviderStatusRefunded:
				s.writeAuditLog(ctx, p.OrderID, "REFUND_GATEWAY_CONFIRMED_AFTER_ERROR", "admin", map[string]any{
					"detail":   psErrMsg(gErr),
					"refundID": refundID,
					"status":   status,
				})
				return s.markRefundOk(ctx, p)
			case payment.ProviderStatusPending:
				s.writeAuditLog(ctx, p.OrderID, "REFUND_GATEWAY_PENDING_AFTER_ERROR", "admin", map[string]any{
					"detail":   psErrMsg(gErr),
					"refundID": refundID,
				})
				return s.markRefundPending(ctx, p, &payment.RefundResponse{
			...
			case payment.ProviderStatusFailed:
				// Channel confirms no successful refund — safe to roll back.
				return s.failRefundAfterVerifiedAbsent(ctx, p, gErr, refundID)
			}
		} else {
			// Query itself failed: keep pending for manual/scheduled verify.
			s.writeAuditLog(ctx, p.OrderID, "REFUND_GATEWAY_VERIFY_FAILED", "admin", map[string]any{
				"detail":     psErrMsg(gErr),
				"verifyError": psErrMsg(qErr),
			})
			return s.markRefundPending(ctx, p, &payment.RefundResponse{
				Status: payment.ProviderStatusPending,
			})
	...
	return s.failRefundAfterVerifiedAbsent(ctx, p, gErr, "")
}

func (s *PaymentService) failRefundAfterVerifiedAbsent(ctx context.Context, p *RefundPlan, gErr error, refundID string) (*RefundResult, error) {
	s.writeAuditLog(ctx, p.OrderID, "REFUND_ROLLBACK_BEFORE_VERIFY", "admin", map[string]any{
		"detail":   psErrMsg(gErr),
		"refundID": refundID,
		"verified": true,
	})
...
func (s *PaymentService) queryGatewayRefundStatus(ctx context.Context, p *RefundPlan) (status, refundID string, err error) {
	if p == nil || p.Order == nil {
		return "", "", fmt.Errorf("refund plan missing order")
	}
	prov, err := s.getRefundProvider(ctx, p.Order)
	if err != nil {
		return "", "", err
	...
	queryProvider, ok := prov.(payment.RefundQueryProvider)
	if !ok {
		return "", "", fmt.Errorf("provider does not support refund query")
	}
	finishProviderCall := servertiming.ObserveDependency(ctx, "payment")
	resp, err := queryProvider.QueryRefund(ctx, payment.RefundQueryRequest{
		TradeNo: p.Order.PaymentTradeNo,
	...
	if resp == nil {
		return "", "", fmt.Errorf("empty refund query response")
	}
	return strings.TrimSpace(resp.Status), strings.TrimSpace(resp.RefundID), nil
}

// isDefinitiveRefundGatewayFailure reports errors that clearly indicate the
// refund was rejected before acceptance (validation / business reject), as
// opposed to transport/timeouts where the outcome is unknown.
func isDefinitiveRefundGatewayFailure(err error) bool {
	if err == nil {
		return false
	}
	...
	unknownMarkers := []string{
		...
		"eof", "temporary failure", "tls handshake", "no such host",
		"network is unreachable", "http 502", "http 503", "http 504",
	...
	// Explicit failed status from provider validation is definitive.
	if strings.Contains(msg, "payment refund failed") ||
		strings.Contains(msg, "status failed") ||
		strings.Contains(msg, "refund failed") {
		return true
	}
	// Default: treat as unknown (safer — hold pending).
	return false
}
...
func (s *PaymentService) markRefundOk(ctx context.Context, p *RefundPlan) (*RefundResult, error) {
	...
	clawed, clawErr := s.clawbackAffiliateRebateForOrder(ctx, p.Order)
	if clawErr != nil {
		slog.Error("affiliate rebate clawback failed after refund success", "orderID", p.OrderID, "error", clawErr)
		s.writeAuditLog(ctx, p.OrderID, "AFFILIATE_REBATE_CLAWBACK_FAILED", "admin", map[string]any{
			"error": psErrMsg(clawErr),
		})
	} else if clawed > 0 {
		s.writeAuditLog(ctx, p.OrderID, "AFFILIATE_REBATE_CLAWBACK", "admin", map[string]any{
			"clawbackAmount": clawed,
		})
	}
	s.writeAuditLog(ctx, p.OrderID, "REFUND_SUCCESS", "admin", map[string]any{"refundAmount": p.RefundAmount, "reason": p.Reason, "balanceDeducted": p.BalanceToDeduct, "force": p.Force, "affiliateClawback": clawed})
...
func (s *PaymentService) markRefundOkTx(ctx context.Context, client *dbent.Client, p *RefundPlan) (*RefundResult, error) {
	...
	clawed, clawErr := s.clawbackAffiliateRebateForOrder(ctx, p.Order)
	if clawErr != nil {
		return nil, fmt.Errorf("clawback affiliate rebate: %w", clawErr)
	}
	detail, err := json.Marshal(map[string]any{"refundAmount": p.RefundAmount, "reason": p.Reason, "balanceDeducted": p.BalanceToDeduct, "force": p.Force, "affiliateClawback": clawed})
	if err != nil {
	...
	if clawed > 0 {
		clawDetail, err := json.Marshal(map[string]any{"clawbackAmount": clawed})
		if err != nil {
			return nil, fmt.Errorf("marshal clawback audit: %w", err)
		}
		if _, err := client.PaymentAuditLog.Create().
			SetOrderID(strconv.FormatInt(p.OrderID, 10)).
			SetAction("AFFILIATE_REBATE_CLAWBACK").
			SetDetail(string(clawDetail)).
			SetOperator("admin").
			Save(ctx); err != nil {
			return nil, fmt.Errorf("write clawback audit: %w", err)
		}
	}
	return &RefundResult{Success: true, BalanceDeducted: p.BalanceToDeduct, SubDaysDeducted: p.SubDaysToDeduct}, nil
}

// clawbackAffiliateRebateForOrder reverses affiliate rebates issued for the
// payment order (18-H1). Safe no-op when affiliate service is unset or no
// accrue rows exist for the order.
func (s *PaymentService) clawbackAffiliateRebateForOrder(ctx context.Context, o *dbent.PaymentOrder) (float64, error) {
	if s == nil || s.affiliateService == nil || o == nil || o.ID <= 0 {
		return 0, nil
	}
	return s.affiliateService.ClawbackInviteRebateForOrder(ctx, o.ID)
}
