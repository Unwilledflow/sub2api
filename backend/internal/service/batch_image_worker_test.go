...
func TestBatchImageWorker_DeadLettersAfterMaxProcessFailures(t *testing.T) {
	queue := newFakeBatchImageQueue("imgbatch_worker_dead")
	processor := &fakeBatchImageProcessor{err: errors.New("permanent poison")}
	worker := NewBatchImageWorker(queue, processor, BatchImageWorkerOptions{
		ErrorRetryDelay:    time.Second,
	...
	require.Len(t, queue.requeued, 2)
	require.Empty(t, queue.acked)

	require.NoError(t, worker.RunOnce(context.Background()))
	require.Equal(t, []string{"imgbatch_worker_dead"}, queue.acked)
	require.Len(t, queue.requeued, 2)
}
