...
// sessionLimitCheckStub implements only RegisterSession; the embedded
// interface keeps this test focused on the gateway decision and avoids a
// dependency on the Redis-backed implementation.
...
func TestCheckAndRegisterSessionForAPIKeyAccounts(t *testing.T) {
	...
	t.Run("api key account with max_sessions enforces the cap", func(t *testing.T) {
		...
		require.True(t, svc.checkAndRegisterSession(ctx, account, "session-a")) // existing session remains allowed
		require.True(t, svc.checkAndRegisterSession(ctx, account, "session-b"))
		require.False(t, svc.checkAndRegisterSession(ctx, account, "session-c")) // third new session is rejected
	})
