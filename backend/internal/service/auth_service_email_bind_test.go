...
func (s *emailBindRefreshTokenCacheStub) TakeRefreshToken(ctx context.Context, tokenHash string) (*service.RefreshTokenData, error) {
	data, err := s.GetRefreshToken(ctx, tokenHash)
	if err != nil {
		return nil, err
	}
	if err := s.DeleteRefreshToken(ctx, tokenHash); err != nil {
		return nil, err
	}
	return data, nil
}
