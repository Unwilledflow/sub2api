...
func (s *AuthService) loginOrRegisterVerifiedEmailOAuth(
	...
	if user == nil {
		...
		if err != nil {
			...
			// Existing local account with matching verified email but no prior OAuth identity.
			// Do not auto-bind: require password login + manual bind (or bind-login pending flow)
			// to prevent account takeover via email-matching OAuth providers.
			return nil, nil, ErrOAuthAccountBindRequired
		}
	}
...
func (s *AuthService) createEmailOAuthUser(ctx context.Context, email, username, providerType, invitationCode, affiliateCode string) (*User, error) {
	...
	if err := s.userRepo.Create(ctx, user); err != nil {
		if errors.Is(err, ErrEmailExists) {
			// Race with concurrent registration: never fall through to silent bind.
			return nil, ErrOAuthAccountBindRequired
		}
		return nil, ErrServiceUnavailable
	}
