...
func cnNormalizeResetTime(raw any) string {
	...
	case string:
		...
		if ts, err := cnParseSchedulingTime(s); err == nil {
...
// cnParseSchedulingTime 解析上游返回的 ISO8601 重置时间字符串（对齐官方
// account_scheduling_threshold_eval.go 的 parseSchedulingTime 语义；定制版已移除该文件，此处内联）。
func cnParseSchedulingTime(raw string) (time.Time, error) {
	formats := []string{
		time.RFC3339,
		time.RFC3339Nano,
