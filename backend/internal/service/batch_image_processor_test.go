...
func (r *fakeBatchImageRepository) ListBatchImageJobsDueForOutputCleanup(_ context.Context, now time.Time, terminalCutoff time.Time, limit int) ([]*BatchImageJob, error) {
	if limit <= 0 {
		limit = 100
	}
	var jobs []*BatchImageJob
	for _, job := range r.jobs {
		...
		case BatchImageJobStatusFailed, BatchImageJobStatusCancelled:
			if job.OutputExpiresAt != nil {
				if job.OutputExpiresAt.After(now) {
					continue
				}
			} else {
				at := job.FinishedAt
