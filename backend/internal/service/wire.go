...
func ProvideSchedulerSnapshotService(
	...
	svc.SetUpstreamRouteHealth(upstreamRouteHealth)
	svc.Start()
	return svc
}
...
// signal store for shared global health state across all nodes. Falls back
// to process-local storage if Redis is unavailable.
func ProvideAdaptiveRoutePlanner(
	...
	signals := NewAdaptiveRouteSignalStore(redisClient)
	return NewAdaptiveRoutePlanner(pool, accounts, groups, channels, signals)
}

// ProvideGatewayService installs the process-local upstream route-health
// overlay on every gateway path, including direct account reads that do not
// pass through SchedulerSnapshotService.
func ProvideGatewayService(
	...
	svc.SetUpstreamRouteHealth(upstreamRouteHealth)
	svc.SetAdaptiveBillingCoordinator(adaptiveBilling)
	return svc
}

// ProvideOpenAIGatewayService is the OpenAI-compatible counterpart of
// ProvideGatewayService. Both gateways must observe the same route breaker so
// a monitor alert cannot be bypassed by protocol choice.
...
// ProvideUpstreamRouteHealth creates the disposable, monitor-fed scheduling
// overlay. PostgreSQL remains the source of account configuration; this
// projection is intentionally rebuilt from future monitor observations.
func ProvideUpstreamRouteHealth(cfg *config.Config) *UpstreamRouteHealth {
	enabled := cfg != nil && cfg.Gateway.Scheduling.UpstreamRouteHealthEnabled
	return NewUpstreamRouteHealth(enabled)
}
...
func ProvideRateLimitService(
	...
	svc.SetOpenAIPool5xxCounterCache(openAIPool5xxCounterCache)
...
var ProviderSet = wire.NewSet(
	...
	ProvideOpenAIPool5xxRecoveryProbeService,
	ProvideAccountFailureGuardService,
	...
	ProvideGroupMonitorRunnerService,
...
func ProvideChannelMonitorRunner(svc *ChannelMonitorService, settingService *SettingService, upstreamRouteHealth *UpstreamRouteHealth) *ChannelMonitorRunner {
	svc.SetResultObserver(upstreamRouteHealth)
...
// ProvideGroupMonitorService 创建分组监控服务。
func ProvideGroupMonitorService(
	repo GroupMonitorRepository,
	accountTestSvc *AccountTestService,
	rateLimitSvc *RateLimitService,
) *GroupMonitorService {
	return NewGroupMonitorService(repo, accountTestSvc, rateLimitSvc)
}

// ProvideGroupMonitorRunnerService 创建并启动分组监控调度器。
func ProvideGroupMonitorRunnerService(
