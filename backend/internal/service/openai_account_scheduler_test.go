...
func (r schedulerTestOpenAIAccountRepo) SetError(context.Context, int64, string) error {
	return nil
}
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_StickyWeightedSessionInTopKUsesStickyFirst(t *testing.T) {
	...
	cfg.Gateway.OpenAIScheduler.StickyEscapeTTFTMs = 7000
	cfg.Gateway.OpenAIScheduler.StickyEscapeErrorRate = 0.25
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{
		"openai:session_hash_weighted_topk": 37101,
	}}
	stats := newOpenAIAccountRuntimeStats()
	fastTTFT := 3000
	stats.report(37101, true, &fastTTFT)
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accounts},
		cache:              cache,
		cfg:                cfg,
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_StickyWeightedSessionEscapesSlowAccount(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	...
	cfg.Gateway.OpenAIWS.SchedulerScoreWeights.Queue = 0.7
	cfg.Gateway.OpenAIWS.SchedulerScoreWeights.ErrorRate = 0.8
	cfg.Gateway.OpenAIWS.SchedulerScoreWeights.TTFT = 3
	cfg.Gateway.OpenAIWS.SchedulerScoreWeights.SessionSticky = 3
	cfg.Gateway.OpenAIScheduler.StickyEscapeEnabled = true
	cfg.Gateway.OpenAIScheduler.StickyEscapeTTFTMs = 7000
	cfg.Gateway.OpenAIScheduler.StickyEscapeErrorRate = 0.25
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{
		"openai:session_hash_weighted_escape": 37131,
	}}
	stats := newOpenAIAccountRuntimeStats()
	slowTTFT := 20_000
	stats.report(37131, true, &slowTTFT)
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accounts},
		cache:              cache,
		cfg:                cfg,
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(37132), selection.Account.ID)
	require.Equal(t, openAIAccountScheduleLayerLoadBalance, decision.Layer)
	require.False(t, decision.StickySessionHit)
	require.Equal(t, int64(37131), cache.sessionBindings["openai:session_hash_weighted_escape"])
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
	}
}

func TestOpenAIGatewayService_SelectAccountWithScheduler_StickyWeightedEscapeFallsBackWithoutAlternative(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	...
	cfg.Gateway.OpenAIScheduler.StickyEscapeErrorRate = 0.25
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{
		"openai:session_hash_weighted_escape_only": account.ID,
	}}
	stats := newOpenAIAccountRuntimeStats()
	slowTTFT := 20_000
	stats.report(account.ID, true, &slowTTFT)
	svc := &OpenAIGatewayService{
	...
	selection, decision, err := svc.SelectAccountWithScheduler(
		...
		&groupID,
		"",
		"session_hash_weighted_escape_only",
		"gpt-5.1",
		nil,
		OpenAIUpstreamTransportAny,
		false,
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_StickyWeightedSessionEscapesHighErrorRate(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()

	ctx := context.Background()
	groupID := int64(101077)
	accounts := []Account{
		{ID: 37161, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 100, GroupIDs: []int64{groupID}},
		{ID: 37162, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupID}},
	}
	cfg := &config.Config{}
	cfg.Gateway.OpenAIWS.LBTopK = 2
	cfg.Gateway.OpenAIScheduler.StickyEscapeEnabled = true
	...
	cfg.Gateway.OpenAIScheduler.StickyEscapeErrorRate = 0.25
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{
		"openai:session_hash_weighted_error_escape": 37161,
	}}
	stats := newOpenAIAccountRuntimeStats()
	stats.report(37161, false, nil)
	stats.report(37161, false, nil)
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accounts},
		cache:              cache,
	...
	selection, decision, err := svc.SelectAccountWithScheduler(
		...
		&groupID,
		"",
		"session_hash_weighted_error_escape",
		"gpt-5.1",
		nil,
		OpenAIUpstreamTransportAny,
		false,
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(37162), selection.Account.ID)
	require.Equal(t, openAIAccountScheduleLayerLoadBalance, decision.Layer)
	require.False(t, decision.StickySessionHit)
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_PreviousEscapePreservesDifferentSessionBinding(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()

	ctx := context.Background()
	groupID := int64(101078)
	wsExtra := map[string]any{"openai_apikey_responses_websockets_v2_enabled": true}
	accounts := []Account{
		{ID: 37171, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 50, GroupIDs: []int64{groupID}, Extra: wsExtra},
		{ID: 37172, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 100, GroupIDs: []int64{groupID}, Extra: wsExtra},
		{ID: 37173, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupID}, Extra: wsExtra},
	}
	cfg := newSchedulerTestOpenAIWSV2Config()
	...
	cfg.Gateway.OpenAIScheduler.StickyEscapeErrorRate = 0.25
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{
		"openai:session_hash_previous_escape": 37171,
	}}
	stats := newOpenAIAccountRuntimeStats()
	slowTTFT := 20_000
	stats.report(37172, true, &slowTTFT)
	svc := &OpenAIGatewayService{
		accountRepo:      schedulerTestOpenAIAccountRepo{accounts: accounts},
		cache:            cache,
		cfg:              cfg,
	...
	store := svc.getOpenAIWSStateStore()
	require.NoError(t, store.BindResponseAccount(ctx, groupID, "resp_previous_escape", 37172, time.Hour))

	selection, decision, err := svc.SelectAccountWithSchedulerForCapability(
		ctx,
		&groupID,
		"resp_previous_escape",
		"session_hash_previous_escape",
		"gpt-5.1",
		nil,
		OpenAIUpstreamTransportAny,
		OpenAIEndpointCapabilityChatCompletions,
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(37173), selection.Account.ID)
	require.Equal(t, int64(37171), cache.sessionBindings["openai:session_hash_previous_escape"])
	require.False(t, decision.StickyPreviousHit)
	require.False(t, decision.StickySessionHit)
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
...
func TestDefaultOpenAIAccountScheduler_WeightedEscapeDoesNotBypassPrivacy(t *testing.T) {
	ctx := context.Background()
...
func TestDefaultOpenAIAccountScheduler_WeightedEscapeDoesNotRestorePreExcludedAccount(t *testing.T) {
	ctx := context.Background()
	groupID := int64(101080)
	account := Account{ID: 37182, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, GroupIDs: []int64{groupID}}
	cfg := &config.Config{}
	cfg.Gateway.OpenAIScheduler.StickyEscapeEnabled = true
	cfg.Gateway.OpenAIScheduler.StickyEscapeTTFTMs = 7000
	cfg.Gateway.OpenAIScheduler.StickyEscapeErrorRate = 0.25
	...
	selection, _, err := scheduler.Select(ctx, OpenAIAccountScheduleRequest{
		...
		StickyWeighted:  true,
		RequestedModel:  "gpt-5.1",
		ExcludedIDs:     map[int64]struct{}{account.ID: {}},
	})
	require.Error(t, err)
	require.Nil(t, selection)
}

func TestOpenAIGatewayService_SelectAccountWithScheduler_WeightedEscapeCompactFallsBackToSlowSupportedAccount(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	...
	accounts := []Account{
		{
			ID: 37183, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 100, GroupIDs: []int64{groupID},
			Extra: map[string]any{"openai_compact_mode": OpenAICompactModeForceOn},
		},
		{
			ID: 37184, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupID},
	...
	cfg.Gateway.OpenAIScheduler.StickyEscapeErrorRate = 0.25
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{
		"openai:session_hash_weighted_compact_escape": 37183,
	}}
	stats := newOpenAIAccountRuntimeStats()
	slowTTFT := 20_000
	stats.report(37183, true, &slowTTFT)
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accounts},
		cache:              cache,
		cfg:                cfg,
	...
	selection, decision, err := svc.SelectAccountWithSchedulerForCapability(
		...
		&groupID,
		"",
		"session_hash_weighted_compact_escape",
		"gpt-5.1",
		nil,
		OpenAIUpstreamTransportAny,
		OpenAIEndpointCapabilityChatCompletions,
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(37183), selection.Account.ID)
	require.True(t, decision.StickySessionHit)
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
	}
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_WeightedEscapePrefersHealthyCompactUnknown(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	...
	accounts := []Account{
		{
			ID: 37187, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupID},
			Extra: map[string]any{"openai_compact_mode": OpenAICompactModeForceOn},
		},
		{ID: 37188, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 100, GroupIDs: []int64{groupID}},
	}
	cfg := &config.Config{}
	cfg.Gateway.OpenAIWS.LBTopK = 2
	cfg.Gateway.OpenAIScheduler.StickyEscapeEnabled = true
	...
	cfg.Gateway.OpenAIScheduler.StickyEscapeErrorRate = 0.25
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{
		"openai:session_hash_weighted_compact_unknown": 37187,
	}}
	stats := newOpenAIAccountRuntimeStats()
	slowTTFT := 20_000
	stats.report(37187, true, &slowTTFT)
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accounts},
		cache:              cache,
		cfg:                cfg,
	...
	selection, decision, err := svc.SelectAccountWithSchedulerForCapability(
		...
		&groupID,
		"",
		"session_hash_weighted_compact_unknown",
		"gpt-5.1",
		nil,
		OpenAIUpstreamTransportAny,
		OpenAIEndpointCapabilityChatCompletions,
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(37188), selection.Account.ID)
	require.False(t, decision.StickySessionHit)
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
	}
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_WeightedEscapeOverridesSubscriptionPriority(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()

	ctx := context.Background()
	groupID := int64(101084)
	accounts := []Account{
		{
			ID: 37189, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupID},
			Credentials: map[string]any{"plan_type": "pro"},
		},
		{ID: 37190, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 100, GroupIDs: []int64{groupID}},
	}
	...
	cfg.Gateway.OpenAIScheduler.StickyEscapeErrorRate = 0.25
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{
		"openai:session_hash_weighted_subscription_escape": 37189,
	}}
	stats := newOpenAIAccountRuntimeStats()
	slowTTFT := 20_000
	stats.report(37189, true, &slowTTFT)
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accounts},
		cache:              cache,
		cfg:                cfg,
	...
	selection, decision, err := svc.SelectAccountWithScheduler(
		...
		&groupID,
		"",
		"session_hash_weighted_subscription_escape",
		"gpt-5.1",
		nil,
		OpenAIUpstreamTransportAny,
		false,
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(37190), selection.Account.ID)
	require.False(t, decision.StickySessionHit)
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
	}
...
func TestDefaultOpenAIAccountScheduler_WeightedStickyFallbackTriesAllAccountsBeforeWaiting(t *testing.T) {
	ctx := context.Background()
	groupID := int64(101082)
	accounts := []Account{
		{ID: 37185, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, GroupIDs: []int64{groupID}},
		{ID: 37186, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, GroupIDs: []int64{groupID}},
	}
	svc := &OpenAIGatewayService{
		accountRepo: schedulerTestOpenAIAccountRepo{accounts: accounts},
		cfg:         &config.Config{},
	...
	selection, err := scheduler.tryFallbackToWeightedSticky(ctx, OpenAIAccountScheduleRequest{
		...
		Platform:                PlatformOpenAI,
		StickyWeighted:          true,
		StickyPreviousAccountID: 37185,
		StickyAccountID:         37186,
		RequestedModel:          "gpt-5.1",
	})
	require.NoError(t, err)
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(37186), selection.Account.ID)
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_StickyWeightedPreviousRequiresMovableContext(t *testing.T) {
	...
	stats := newOpenAIAccountRuntimeStats()
	slowTTFT := 20_000
	stats.report(37111, true, &slowTTFT)
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accounts},
		cache:              &schedulerTestGatewayCache{},
		cfg:                cfg,
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_StickyWeightedPreviousEscapesSlowAccount(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	...
	stats := newOpenAIAccountRuntimeStats()
	slowTTFT := 20_000
	stats.report(37141, true, &slowTTFT)
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accounts},
		cache:              &schedulerTestGatewayCache{},
		cfg:                cfg,
	...
	store := svc.getOpenAIWSStateStore()
	require.NoError(t, store.BindResponseAccount(ctx, groupID, "resp_weighted_escape", 37141, time.Hour))

	selection, decision, err := svc.SelectAccountWithSchedulerForCapability(
		ctx,
		&groupID,
		"resp_weighted_escape",
		"",
		"gpt-5.1",
		nil,
		OpenAIUpstreamTransportAny,
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(37142), selection.Account.ID)
	require.Equal(t, openAIAccountScheduleLayerLoadBalance, decision.Layer)
	require.False(t, decision.StickyPreviousHit)
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyBusyOverflowsWhenEscapeDisabled(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10100)
	accounts := []Account{
		{
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyEscapeByTTFT(t *testing.T) {
	...
	cfg.Gateway.OpenAIWS.LBTopK = 1
	cfg.Gateway.OpenAIWS.SchedulerScoreWeights.Priority = 1
	concurrencyCache := schedulerTestConcurrencyCache{acquireResults: map[int64]bool{21101: true, 21102: true}}
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accounts},
		cache:              cache,
		cfg:                cfg,
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyEscapeDisabledStillOverflowsFullAccount(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10104)
	accounts := []Account{
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_LoadBalanceOverflowsPastFullTopK(t *testing.T) {
	ctx := context.Background()
	groupID := int64(111)
	accounts := []Account{
		...
		{ID: 3013, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0},
	}
	cfg := &config.Config{}
	cfg.Gateway.OpenAIWS.LBTopK = 2
	cfg.Gateway.OpenAIWS.SchedulerScoreWeights.Load = 1
	...
	concurrencyCache := schedulerTestConcurrencyCache{
		loadMap: map[int64]*AccountLoadInfo{
			3011: {AccountID: 3011, LoadRate: 10, CurrentConcurrency: 1},
			3012: {AccountID: 3012, LoadRate: 20, CurrentConcurrency: 1},
			3013: {AccountID: 3013, LoadRate: 90, CurrentConcurrency: 0},
		},
		acquireResults: map[int64]bool{3013: true},
		acquiredIDs:    &acquiredIDs,
	}
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accounts},
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(3013), selection.Account.ID)
	require.Equal(t, 3, decision.CandidateCount)
	require.Equal(t, 2, decision.TopK)
	require.Equal(t, []int64{3013}, acquiredIDs)
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
	}
}
...
func TestOpenAIAccountRuntimeStats_ModelSnapshotsAreIsolated(t *testing.T) {
	stats := newOpenAIAccountRuntimeStats()
	fast := 900
	slow := 12_000
	stats.reportForModel(1001, "gpt-5.6-terra", true, &fast)
	stats.reportForModel(1001, "gpt-5.4", false, &slow)

	errorRate, ttft, hasTTFT := stats.snapshotForModel(1001, "gpt-5.6-terra")
	require.True(t, hasTTFT)
	require.InDelta(t, 0.0, errorRate, 1e-9)
	require.InDelta(t, 900.0, ttft, 1e-9)

	errorRate, ttft, hasTTFT = stats.snapshotForModel(1001, "gpt-5.4")
	require.True(t, hasTTFT)
	require.InDelta(t, 0.2, errorRate, 1e-9)
	require.InDelta(t, 12_000.0, ttft, 1e-9)
}
...
func TestOpenAIAccountRuntimeStats_StalePerformanceClassProbeExpires(t *testing.T) {
	stats := newOpenAIAccountRuntimeStats()
	...
	stat, ok := value.(*openAIAccountRuntimeStat)
	require.True(t, ok)
	stat.probeStartedAtUnixNano.Store(time.Now().Add(-openAIAccountRuntimeStatsProbeTTL - time.Second).UnixNano())
...
func TestTryAcquireOpenAISelectionOrderDefersPendingClassProbeWithoutLosingFallback(t *testing.T) {
	class := "effort=medium;tier=default;context=large;chain=standalone"
	accounts := []Account{
		{ID: 1001, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 10},
		{ID: 1002, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 10},
		{ID: 1003, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 10},
	}
	selectionOrder := make([]openAIAccountCandidateScore, 0, len(accounts))
	for i := range accounts {
		selectionOrder = append(selectionOrder, openAIAccountCandidateScore{
			account:  &accounts[i],
			loadInfo: &AccountLoadInfo{AccountID: accounts[i].ID},
		})
	}
	...
	for _, test := range []struct {
		...
		{
			name:           "healthy alternative wins",
			acquireResults: map[int64]bool{1002: true},
			wantAccountID:  1002,
			wantAcquired:   []int64{1002},
		},
		{
			name:           "pending probe remains final fallback",
			acquireResults: map[int64]bool{1002: false, 1003: false, 1001: true},
			wantAccountID:  1001,
			wantAcquired:   []int64{1002, 1003, 1001},
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			svc := &OpenAIGatewayService{
				...
				cfg:         &config.Config{},
				concurrencyService: NewConcurrencyService(schedulerTestConcurrencyCache{
					acquireResults: test.acquireResults,
					acquiredIDs:    &acquiredIDs,
				}),
			}
			scheduler := &defaultOpenAIAccountScheduler{service: svc, stats: stats}
			...
			require.NoError(t, err)
			require.NotNil(t, selection)
			require.Equal(t, test.wantAccountID, selection.Account.ID)
			require.Equal(t, test.wantAcquired, acquiredIDs)
			if selection.ReleaseFunc != nil {
				selection.ReleaseFunc()
			}
		})
...
func TestOpenAIAccountRuntimeStats_ModelSnapshotFallsBackToAccount(t *testing.T) {
	stats := newOpenAIAccountRuntimeStats()
	ttftSample := 2400
	stats.report(1001, true, &ttftSample)

	errorRate, ttft, hasTTFT := stats.snapshotForModel(1001, "gpt-5.6-sol")
	require.True(t, hasTTFT)
	require.InDelta(t, 0.0, errorRate, 1e-9)
	require.InDelta(t, 2400.0, ttft, 1e-9)
}

func TestOpenAIAccountRuntimeStats_ModelEntriesAreBounded(t *testing.T) {
	stats := newOpenAIAccountRuntimeStats()
	ttft := 900

	for i := 0; i < openAIAccountModelRuntimeStatsMaxEntries+128; i++ {
		stats.reportForModel(1001, fmt.Sprintf("custom-model-%d", i), true, &ttft)
	}

	modelCount := 0
	stats.models.Range(func(_, _ any) bool {
		modelCount++
		return true
	})
	require.Equal(t, openAIAccountModelRuntimeStatsMaxEntries, modelCount)
	_, ok := stats.models.Load(openAIAccountModelRuntimeStatKey{
		accountID: 1001,
		model:     fmt.Sprintf("custom-model-%d", openAIAccountModelRuntimeStatsMaxEntries+127),
	})
	require.True(t, ok)
}

func TestOpenAIAccountRuntimeStats_OversizedModelUsesAccountStatsOnly(t *testing.T) {
	stats := newOpenAIAccountRuntimeStats()
	ttft := 900
	model := "gpt-5.6-sol" + strings.Repeat(" ", openAIAccountModelRuntimeStatsMaxModelBytes)

	stats.reportForModel(1001, model, true, &ttft)
	...
	errorRate, gotTTFT, hasTTFT := stats.snapshot(1001)
	require.True(t, hasTTFT)
	require.InDelta(t, 0.0, errorRate, 1e-9)
	require.InDelta(t, 900.0, gotTTFT, 1e-9)
}

func TestOpenAIAccountScheduler_ResolveRequestGroupLoadsRoutingOnce(t *testing.T) {
	groupID := int64(14)
	...
	scheduler := &defaultOpenAIAccountScheduler{service: &OpenAIGatewayService{
		schedulerSnapshot: NewSchedulerSnapshotService(nil, nil, nil, groupRepo, &config.Config{}),
	}}

	req, err := scheduler.resolveRequestGroup(context.Background(), OpenAIAccountScheduleRequest{
		GroupID:        &groupID,
		RequestedModel: "gpt-5.6-sol",
	})
	require.NoError(t, err)
	require.True(t, req.routingResolved)
	require.Equal(t, []int64{618, 3}, req.routingAccountIDs)
	require.True(t, openAIAccountAllowedByRoutingIDs(618, req.routingAccountIDs))
	require.False(t, openAIAccountAllowedByRoutingIDs(619, req.routingAccountIDs))
	require.Equal(t, 1, groupRepo.getByIDCalls)
}

func TestOpenAIAccountScheduler_GroupModelRoutingFiltersAccount(t *testing.T) {
	routedIDs := []int64{618, 3}
	require.True(t, openAIAccountAllowedByRoutingIDs(618, routedIDs))
	require.True(t, openAIAccountAllowedByRoutingIDs(3, routedIDs))
	require.False(t, openAIAccountAllowedByRoutingIDs(619, routedIDs))
	require.True(t, openAIAccountAllowedByRoutingIDs(619, nil))

	scheduler := &defaultOpenAIAccountScheduler{}
	compatible, reason := scheduler.isAccountRequestCompatibleReason(context.Background(), &Account{ID: 619}, OpenAIAccountScheduleRequest{
		routingResolved:   true,
		routingAccountIDs: routedIDs,
	})
	require.False(t, compatible)
	require.Equal(t, "model_routing_excluded", reason)
}
