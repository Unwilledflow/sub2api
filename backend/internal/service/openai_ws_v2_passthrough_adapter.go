...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	policyClientConn := &openAIWSPolicyEnforcingFrameConn{
		...
		filter: func(msgType coderws.MessageType, payload []byte) (out []byte, blocked *OpenAIFastBlockedError, filterErr error) {
			...
			if isResponseCreate {
				...
				// 出站兜底：compaction_trigger 必须保持 input 末位。
				if HasCompactionTriggerInInput(payload) {
					if fixed, changedFix, fixErr := EnsureOpenAICompactionTriggerLast(payload); fixErr == nil && changedFix {
						payload = fixed
					}
				}
				if hooks != nil && (hooks.MaxReasoningEffort != "" || len(hooks.ReasoningEffortMappings) > 0) {
	...
	// Session-scoped image counter for passthrough billing parity with ingress.
		...
		Options: openaiwsv2.RelayOptions{
			...
			OnTurnComplete: func(turn openaiwsv2.RelayTurnResult) {
				...
				if imageCount := imageCounter.Count(); imageCount > 0 {
					...
					turnResult.ImageOutputSizes = imageCounter.Sizes()
					turnResult.BillingModel = turnRequestModel
				}
				// Reset so the next turn only bills its own image outputs.
				imageCounter = newOpenAIImageOutputCounter()
				logOpenAIWSV2Passthrough(
					"relay_turn_completed account_id=%d turn=%d request_id=%s terminal_event=%s turn_requested_model=%s turn_upstream_model=%s duration_ms=%d first_token_ms=%d input_tokens=%d output_tokens=%d cache_read_tokens=%d image_count=%d",
					account.ID,
					turnNo,
			...
			BeforeWriteClient: func(msgType coderws.MessageType, payload []byte, wroteDownstream bool) error {
				...
				if eventType == "response.failed" {
					...
					if isOpenAIModelCapacityError(failedMessage, payload) {
						return newOpenAIWSModelCapacityFailoverError(handshakeHeaders, payload, failedMessage)
					}
					return nil
				}
				if eventType != "error" {
				...
				if isOpenAIModelCapacityError(errMsgRaw, payload) {
					return newOpenAIWSModelCapacityFailoverError(handshakeHeaders, payload, errMsgRaw)
				}
	...
	if imageCount := imageCounter.Count(); imageCount > 0 {
		...
		result.BillingModel = resultRequestModel
	}
