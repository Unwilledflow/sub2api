...
type RefreshTokenCache interface {
	...
	// TakeRefreshToken 原子获取并删除 Refresh Token（轮转专用）。
	// 成功返回 (data, nil) 且 token 已失效；不存在返回 (nil, ErrRefreshTokenNotFound)；
	// 已轮转后的重放返回 (nil, ErrRefreshTokenReused)（实现侧应尽量撤销 token 家族）。
	TakeRefreshToken(ctx context.Context, tokenHash string) (*RefreshTokenData, error)
