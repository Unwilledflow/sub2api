...
func TestClassifyOpenAIWSErrorEvent(t *testing.T) {
	...
	reason, recoverable = classifyOpenAIWSErrorEvent([]byte(`{"type":"error","error":{"type":"invalid_request_error","message":"Selected model is at capacity. Please try a different model."}}`))
	require.Equal(t, "model_capacity", reason)
	require.True(t, recoverable)
}

func TestNewOpenAIWSModelCapacityFailoverError(t *testing.T) {
	payload := []byte(`{"type":"response.failed","response":{"error":{"message":"Selected model is at capacity. Please try a different model."}}}`)
	err := newOpenAIWSModelCapacityFailoverError(http.Header{"X-Request-Id": []string{"req_capacity"}}, payload, "Selected model is at capacity. Please try a different model.")

	require.Equal(t, http.StatusServiceUnavailable, err.StatusCode)
	require.True(t, err.ShouldRetryNextAccount())
	require.False(t, err.RetryableOnSameAccount)
	require.Equal(t, "req_capacity", err.ResponseHeaders.Get("X-Request-Id"))
	require.Equal(t, payload, err.ResponseBody)
}
