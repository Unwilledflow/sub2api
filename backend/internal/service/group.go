...
type Group struct {
	...
	Description string
	Platform    string
	// IsAdaptiveParent is true when this group is an enabled Adaptive pool parent.
	// Used by user key UI to hide fallback and show intelligence/price preference.
	IsAdaptiveParent bool
	// UserVisible controls whether this group appears in user-facing available
	// group lists (API key binding, channel marketplace, etc.). Adaptive leaf
	// groups can be hidden so end users only see the Adaptive parent pool.
	// Default true. Admin UI always sees all groups.
	UserVisible            bool
	RateMultiplier         float64
	CompetitiveConcurrency bool
	// RateMultiplierMin/Max are set for Adaptive parents when exposing the
	// pool to users: display as "min~max" floating range across enabled leaves.
	// Nil means not an Adaptive range (use RateMultiplier alone).
	...
	// VideoModelPrices overrides flat video prices for a matching model family
	// and resolution. Values are USD per second.
...
// GetVideoPriceForModel prefers a model-family price before the legacy flat tier.
...
// VideoPriceConfig builds the billing configuration for this group.
