...
// DeleteExpired removes completed/failed jobs and their events older than cutoff.
// Events are deleted first to satisfy FKs; orphan terminal jobs are then purged.
// Returns deleted event and job counts for observability.
func (r *PostgreSQLRepository) DeleteExpired(ctx context.Context, cutoff time.Time, batchSize int) (deletedEvents, deletedJobs int64, err error) {
	if r == nil || r.db == nil {
		return 0, 0, errors.New("prompt audit repository unavailable")
	}
	...
	for {
		...
		if txErr != nil {
			return deletedEvents, deletedJobs, txErr
		}
		res, execErr := tx.ExecContext(ctx, `
			DELETE FROM prompt_audit_events
			WHERE id IN (
				SELECT e.id FROM prompt_audit_events e
		...
		if execErr != nil {
			_ = tx.Rollback()
			return deletedEvents, deletedJobs, execErr
		}
		...
		res, execErr = tx.ExecContext(ctx, `
			...
			WHERE id IN (
				...
				WHERE j.status IN ('done','failed')
				  AND j.updated_at < $1
				  AND NOT EXISTS (SELECT 1 FROM prompt_audit_events e WHERE e.job_id = j.id)
				ORDER BY j.id
				LIMIT $2
			)`, cutoff.UTC(), batchSize)
		if execErr != nil {
		...
		nJobs, _ := res.RowsAffected()
		if err := tx.Commit(); err != nil {
			return deletedEvents, deletedJobs, err
		}
		deletedEvents += nEvents
		deletedJobs += nJobs
		if nEvents == 0 && nJobs == 0 {
			return deletedEvents, deletedJobs, nil
		}
	}
}
