...
type fakePromptDeleteTotp struct{}

func (fakePromptDeleteTotp) VerifyCode(context.Context, int64, string) error { return nil }
...
func promptAdminRouter(service PromptAdminService) *gin.Engine {
	...
	handler := NewPromptAdminHandler(service, &fakePromptDeleteTotp{})
...
func TestPromptAdminDeleteConfirmationErrorsStayGeneric(t *testing.T) {
	...
	response := promptAdminRequest(t, promptAdminRouter(service), http.MethodPost, "/admin/prompt-audit/events/delete-by-filter", DeleteByFilterRequest{
		SnapshotMaxID: 3, FilterHash: strings.Repeat("a", 64), ConfirmationToken: "secret-confirmation", Confirm: true, TotpCode: "123456",
	})
	require.Equal(t, http.StatusBadRequest, response.Code)
