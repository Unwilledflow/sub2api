...
func (c *Coordinator) Check(ctx context.Context, req Request) Decision {
	...
	switch mode {
	case ModeAsync:
		// Enqueue is deliberately best-effort for the request path, but failures
		// must be observable (metrics + warn) so audit gaps are not silent.
		if err := c.prompt.Enqueue(ctx, req.Clone()); err != nil {
