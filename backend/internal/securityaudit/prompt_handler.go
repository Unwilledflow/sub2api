...
// PromptDeleteTotpVerifier requires live TOTP for prompt-audit evidence deletion.
type PromptDeleteTotpVerifier interface {
	VerifyCode(ctx context.Context, userID int64, code string) error
}

type PromptAdminHandler struct {
	service PromptAdminService
	totp    PromptDeleteTotpVerifier
}

func NewPromptAdminHandler(service PromptAdminService, totp PromptDeleteTotpVerifier) *PromptAdminHandler {
	return &PromptAdminHandler{service: service, totp: totp}
}
...
func (h *PromptAdminHandler) DeleteEvent(c *gin.Context) {
	...
	if code := strings.TrimSpace(c.GetHeader("X-Totp-Code")); code != "" && strings.TrimSpace(req.TotpCode) == "" {
		req.TotpCode = code
	}
	if !h.requirePromptDeleteStepUp(c, req.TotpCode) {
		return
	}
...
func (h *PromptAdminHandler) BatchDelete(c *gin.Context) {
	...
	if !h.requirePromptDeleteStepUp(c, request.TotpCode) {
		return
	}
...
// requirePromptDeleteStepUp enforces the same bar as audit_logs clear:
// no Admin API Key, live TOTP required, admin session only.
func (h *PromptAdminHandler) requirePromptDeleteStepUp(c *gin.Context, totpCode string) bool {
	if c.GetString("auth_method") == service.AuditAuthMethodAdminAPIKey {
		setPromptAdminAudit(c, "failed", "STEP_UP_ADMIN_API_KEY_FORBIDDEN", nil)
		response.ErrorWithDetails(c, http.StatusForbidden,
			"Admin API key cannot delete prompt audit evidence; a two-factor verified admin session is required",
			"STEP_UP_ADMIN_API_KEY_FORBIDDEN", nil)
		return false
	}
	subject, ok := middleware.GetAuthSubjectFromContext(c)
	if !ok || subject.UserID <= 0 {
		setPromptAdminAudit(c, "failed", "UNAUTHORIZED", nil)
		response.Unauthorized(c, "Unauthorized")
		return false
	}
	code := strings.TrimSpace(totpCode)
	if code == "" {
		setPromptAdminAudit(c, "failed", "TOTP_CODE_REQUIRED", nil)
		response.ErrorWithDetails(c, http.StatusBadRequest,
			"TOTP code is required to delete prompt audit evidence", "TOTP_CODE_REQUIRED", nil)
		return false
	}
	if h.totp == nil {
		setPromptAdminAudit(c, "failed", "TOTP_UNAVAILABLE", nil)
		response.ErrorWithDetails(c, http.StatusServiceUnavailable,
			"TOTP verification is unavailable", "TOTP_UNAVAILABLE", nil)
		return false
	}
	if err := h.totp.VerifyCode(c.Request.Context(), subject.UserID, code); err != nil {
		setPromptAdminAudit(c, "failed", "TOTP_INVALID", nil)
		response.ErrorFrom(c, err)
		return false
	}
	return true
