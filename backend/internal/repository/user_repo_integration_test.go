...
func (s *UserRepoSuite) TestUpdatePreservesAccountingAppliedAfterSnapshotLoad() {
	user := s.mustCreateUser(&service.User{
		Email:    "stale-accounting-update@test.com",
		Username: "before",
		Balance:  10,
	})
	...
	// Reproduce the lost-update ordering: a request changes accounting after
	// the profile snapshot was loaded but before that snapshot is saved.
	...
	updated, err := s.repo.GetByID(s.ctx, user.ID)
	s.Require().NoError(err)
	s.Equal("after", updated.Username)
	s.InDelta(12.5, updated.Balance, 1e-6)
	s.InDelta(2.5, updated.TotalRecharged, 1e-6)
}
...
func (s *UserRepoSuite) TestAdminBalanceMutationsDoNotChangeRechargeAccounting() {
	user := s.mustCreateUser(&service.User{Email: "admin-balance-only@test.com", Balance: 10})
	s.Require().NoError(s.repo.UpdateBalance(s.ctx, user.ID, 5))

	balance, err := s.repo.SetBalanceByID(s.ctx, user.ID, 7)
	s.Require().NoError(err)
	s.InDelta(7, balance, 1e-6)

	balance, err = s.repo.AdjustBalanceByID(s.ctx, user.ID, 2)
	s.Require().NoError(err)
	s.InDelta(9, balance, 1e-6)

	updated, err := s.repo.GetByID(s.ctx, user.ID)
	s.Require().NoError(err)
	s.InDelta(9, updated.Balance, 1e-6)
	s.InDelta(5, updated.TotalRecharged, 1e-6,
		"admin balance-only mutations must preserve recharge statistics")
}

func (s *UserRepoSuite) TestAdjustBalanceByIDRejectsNegativeResult() {
	user := s.mustCreateUser(&service.User{Email: "admin-balance-guard@test.com", Balance: 5})

	_, err := s.repo.AdjustBalanceByID(s.ctx, user.ID, -7)
	s.Require().ErrorIs(err, service.ErrInsufficientBalance)

	updated, err := s.repo.GetByID(s.ctx, user.ID)
	s.Require().NoError(err)
	s.InDelta(5, updated.Balance, 1e-6)
}
