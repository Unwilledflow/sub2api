...
func TestMarshalSchedulerCacheAccountFiltersSecretsAndKeepsHotPath(t *testing.T) {
	account := service.Account{
		ID: 803,
		Credentials: map[string]any{
			"model_mapping": map[string]any{"z": "<last>", "a": "&first"},
			"access_token":  "at-secret",
			"refresh_token": "rt-secret",
			"cookie":        "session=drop-me",
			"private_key":   "pk-drop-me",
			"api_key":       "sk-keep",
		},
	...
	var fullAcc, metaAcc service.Account
	require.NoError(t, json.Unmarshal(full, &fullAcc))
	require.NoError(t, json.Unmarshal(meta, &metaAcc))

	require.Equal(t, "at-secret", fullAcc.Credentials["access_token"])
	require.Equal(t, "rt-secret", fullAcc.Credentials["refresh_token"])
	require.Equal(t, "sk-keep", fullAcc.Credentials["api_key"])
	require.NotContains(t, fullAcc.Credentials, "cookie")
	require.NotContains(t, fullAcc.Credentials, "private_key")
	require.NotContains(t, fullAcc.Extra, "unused_large_field")
	require.Equal(t, true, fullAcc.Extra["mixed_scheduling"])

	require.NotContains(t, metaAcc.Credentials, "access_token")
	require.NotContains(t, metaAcc.Credentials, "refresh_token")
	require.Equal(t, "sk-keep", metaAcc.Credentials["api_key"])
}
