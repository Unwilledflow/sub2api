...
func (s *httpUpstreamService) Do(req *http.Request, proxyURL string, accountID int64, accountConcurrency int) (*http.Response, error) {
	...
	hasResponseHeaderTimeoutOverride := false
	if req != nil {
		profile = service.HTTPUpstreamProfileFromContext(req.Context())
		responseHeaderTimeoutOverride, hasResponseHeaderTimeoutOverride = service.HTTPUpstreamResponseHeaderTimeoutFromContext(req.Context())
	}

	// 获取或创建对应的客户端，并标记请求占用
	entry, err := s.acquireClientWithProfile(proxyURL, accountID, accountConcurrency, profile, responseHeaderTimeoutOverride, hasResponseHeaderTimeoutOverride)
	if err != nil {
		return nil, err
	}
...
func (s *httpUpstreamService) DoWithTLS(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, profile *tlsfingerprint.Profile) (*http.Response, error) {
	...
	entry, err := s.acquireClientWithTLS(proxyURL, accountID, accountConcurrency, profile, upstreamProfile, responseHeaderTimeoutOverride, hasResponseHeaderTimeoutOverride)
	if err != nil {
...
func (s *httpUpstreamService) acquireClientWithTLS(proxyURL string, accountID int64, accountConcurrency int, profile *tlsfingerprint.Profile, upstreamProfile service.HTTPUpstreamProfile, responseHeaderTimeoutOverride time.Duration, hasResponseHeaderTimeoutOverride bool) (*upstreamClientEntry, error) {
	return s.getClientEntryWithTLS(proxyURL, accountID, accountConcurrency, profile, upstreamProfile, responseHeaderTimeoutOverride, hasResponseHeaderTimeoutOverride, true, true)
}

// getClientEntryWithTLS 获取或创建带 TLS 指纹的客户端条目
// TLS 指纹客户端使用独立的缓存键，与普通客户端隔离
func (s *httpUpstreamService) getClientEntryWithTLS(proxyURL string, accountID int64, accountConcurrency int, profile *tlsfingerprint.Profile, upstreamProfile service.HTTPUpstreamProfile, responseHeaderTimeoutOverride time.Duration, hasResponseHeaderTimeoutOverride bool, markInFlight bool, enforceLimit bool) (*upstreamClientEntry, error) {
	...
	if hasResponseHeaderTimeoutOverride {
		settings.responseHeaderTimeout = responseHeaderTimeoutOverride
	}
	// TLS 指纹客户端使用独立的缓存键，加 "tls:" 前缀
	cacheKey := "tls:" + buildCacheKey(isolation, proxyKey, accountID, upstreamProtocolModeDefault)
	if hasResponseHeaderTimeoutOverride {
		cacheKey = buildResponseHeaderTimeoutOverrideCacheKey(cacheKey, responseHeaderTimeoutOverride)
	}
...
func (s *httpUpstreamService) acquireClient(proxyURL string, accountID int64, accountConcurrency int) (*upstreamClientEntry, error) {
	return s.acquireClientWithProfile(proxyURL, accountID, accountConcurrency, service.HTTPUpstreamProfileDefault, 0, false)
}

// acquireClientWithProfile 获取或创建客户端，并按请求 profile 选择协议策略。
func (s *httpUpstreamService) acquireClientWithProfile(proxyURL string, accountID int64, accountConcurrency int, profile service.HTTPUpstreamProfile, responseHeaderTimeoutOverride time.Duration, hasResponseHeaderTimeoutOverride bool) (*upstreamClientEntry, error) {
	return s.getClientEntryWithResponseHeaderTimeout(proxyURL, accountID, accountConcurrency, profile, responseHeaderTimeoutOverride, hasResponseHeaderTimeoutOverride, true, true)
}
...
func (s *httpUpstreamService) getClientEntry(proxyURL string, accountID int64, accountConcurrency int, profile service.HTTPUpstreamProfile, markInFlight bool, enforceLimit bool) (*upstreamClientEntry, error) {
	return s.getClientEntryWithResponseHeaderTimeout(proxyURL, accountID, accountConcurrency, profile, 0, false, markInFlight, enforceLimit)
}

func (s *httpUpstreamService) getClientEntryWithResponseHeaderTimeout(proxyURL string, accountID int64, accountConcurrency int, profile service.HTTPUpstreamProfile, responseHeaderTimeoutOverride time.Duration, hasResponseHeaderTimeoutOverride bool, markInFlight bool, enforceLimit bool) (*upstreamClientEntry, error) {
	// 获取隔离模式
	isolation := s.getIsolationMode()
	// 标准化代理 URL 并解析
	proxyKey, parsedProxy, err := normalizeProxyURL(proxyURL)
...
func buildResponseHeaderTimeoutOverrideCacheKey(cacheKey string, timeout time.Duration) string {
	return cacheKey + "|header_timeout_override:" + timeout.String()
}
