...
var _ service.AdminUserBalanceRepository = (*userRepository)(nil)
...
// SetBalanceByID performs an explicit absolute balance update without changing
// recharge statistics. Generic user metadata updates never write balance.
func (r *userRepository) SetBalanceByID(ctx context.Context, id int64, balance float64) (float64, error) {
...
// AdjustBalanceByID applies a balance-only delta and rejects an update that
// would make the current database balance negative. The guard and mutation are
// one statement, so a concurrent hold cannot slip between validation and write.
func (r *userRepository) AdjustBalanceByID(ctx context.Context, id int64, delta float64) (float64, error) {
	exec := txAwareSQLExecutor(ctx, r.sql, r.client)
	if exec == nil {
		return 0, fmt.Errorf("sql executor is not configured")
	}
	...
	return currentBalance, service.ErrInsufficientBalance
}
