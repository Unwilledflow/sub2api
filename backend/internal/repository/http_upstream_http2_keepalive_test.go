...
func requireHTTP2Configured(t *testing.T, tr *http.Transport, msg string) {
	t.Helper()
	// Go 1.27 exposes the configured protocol through Transport.Protocols,
	// while Go 1.26/x/net/http2 registers the equivalent round tripper in
	// TLSNextProto. Accept either representation so the production invariant
	// is tested across the supported toolchains.
	if tr.Protocols != nil {
		require.True(t, tr.Protocols.HTTP2(), msg)
		return
	}
	require.NotNil(t, tr.TLSNextProto["h2"], msg)
}
