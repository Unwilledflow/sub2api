...
// 单独合并，以保留所有绝不会发往上游的 sub2api 内部元数据。
...
func (r *channelMonitorRequestTemplateRepository) applyToMonitorsWithClient(
	...
	result, err := client.ExecContext(ctx, `
		...
		SET extra_headers = $1::jsonb || COALESCE((
			SELECT jsonb_object_agg(entry.key, entry.value)
			FROM jsonb_each(COALESCE(channel_monitors.extra_headers, '{}'::jsonb)) AS entry
			WHERE entry.key LIKE 'sub2api:%'
		), '{}'::jsonb)
		WHERE template_id = $2
		  AND id = ANY($3)
		  AND provider = $4
		  AND api_mode = $5
	`, string(templateHeadersJSON), id, pq.Array(monitorIDs), string(tpl.Provider), defaultAPIModeRepo(tpl.APIMode))
	if err != nil {
