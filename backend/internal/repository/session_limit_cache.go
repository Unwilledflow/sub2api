...
var (
	...
	// unregisterSessionScript 主动注销一个会话
	// KEYS[1] = session_limit:account:{accountID}
	// ARGV[1] = sessionUUID
	unregisterSessionScript = redis.NewScript(`
		local key = KEYS[1]
		local sessionUUID = ARGV[1]
		redis.call('ZREM', key, sessionUUID)
		return 1
	`)
)
...
// UnregisterSession 主动注销一个会话并立即释放其配额。
func (c *sessionLimitCache) UnregisterSession(ctx context.Context, accountID int64, sessionUUID string) error {
	...
	key := sessionLimitKey(accountID)
	_, err := unregisterSessionScript.Run(ctx, c.rdb, []string{key}, sessionUUID).Result()
	return err
}
