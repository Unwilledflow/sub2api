...
const (
	...
	// refreshTokenReuseMarkerTTL 标记已轮转 token 的短暂窗口，用于检测重放。
	refreshTokenReuseMarkerTTL = 7 * 24 * time.Hour
)

// takeRefreshTokenScript atomically GET+DEL the token key.
// Returns:
//   - JSON payload on success
//   - "REUSED:<family_id>" (or "REUSED") if reuse marker present
//   - false if missing
var takeRefreshTokenScript = redis.NewScript(`
local key = KEYS[1]
local reuseKey = KEYS[2]
local reuseTTL = tonumber(ARGV[1])
local val = redis.call('GET', key)
if val then
  redis.call('DEL', key)
  local family = ''
  local ok, decoded = pcall(cjson.decode, val)
  if ok and type(decoded) == 'table' and decoded['family_id'] ~= nil then
    family = tostring(decoded['family_id'])
  end
  if reuseTTL and reuseTTL > 0 then
    redis.call('SET', reuseKey, family, 'EX', reuseTTL)
  end
  return val
end
local marker = redis.call('GET', reuseKey)
if marker ~= false then
  if marker == nil or marker == '' then
    return 'REUSED'
  end
  return 'REUSED:' .. marker
end
return false
`)
...
func refreshTokenReuseKey(tokenHash string) string {
	return refreshTokenKeyPrefix + "used:" + tokenHash
}

func parseRefreshTokenReuseResult(v string) (familyID string, reused bool) {
	if v == "REUSED" {
		return "", true
	}
	if strings.HasPrefix(v, "REUSED:") {
		return strings.TrimPrefix(v, "REUSED:"), true
	}
	return "", false
}

func (c *refreshTokenCache) TakeRefreshToken(ctx context.Context, tokenHash string) (*service.RefreshTokenData, error) {
	key := refreshTokenKey(tokenHash)
	reuseKey := refreshTokenReuseKey(tokenHash)
	ttlSeconds := int64(refreshTokenReuseMarkerTTL / time.Second)
	result, err := takeRefreshTokenScript.Run(ctx, c.rdb, []string{key, reuseKey}, ttlSeconds).Result()
	if err != nil {
		if err == redis.Nil {
			return nil, service.ErrRefreshTokenNotFound
		}
	...
	case string:
		if familyID, reused := parseRefreshTokenReuseResult(v); reused {
			if familyID != "" {
				_ = c.DeleteTokenFamily(ctx, familyID)
			}
			return nil, service.ErrRefreshTokenReused
		}
		var data service.RefreshTokenData
		...
		return &data, nil
	case []byte:
		if familyID, reused := parseRefreshTokenReuseResult(string(v)); reused {
			if familyID != "" {
				_ = c.DeleteTokenFamily(ctx, familyID)
			}
			return nil, service.ErrRefreshTokenReused
	...
	case bool:
		...
		return nil, fmt.Errorf("unexpected take refresh token result: %v", result)
	default:
		return nil, fmt.Errorf("unexpected take refresh token type: %T", result)
	}
}

func (c *refreshTokenCache) DeleteRefreshToken(ctx context.Context, tokenHash string) error {
	key := refreshTokenKey(tokenHash)
	return c.rdb.Del(ctx, key, refreshTokenReuseKey(tokenHash)).Err()
}
