...
func TestApplyChannelMonitorTemplatePreservesInternalMetadataAtomically(t *testing.T) {
	...
	mock.ExpectExec(`(?s)UPDATE channel_monitors\s+SET extra_headers = \$1::jsonb \|\| COALESCE\(\(\s+SELECT jsonb_object_agg\(entry\.key, entry\.value\)\s+FROM jsonb_each\(COALESCE\(channel_monitors\.extra_headers, '\{\}'::jsonb\)\) AS entry\s+WHERE entry\.key LIKE 'sub2api:%'\s+\), '\{\}'::jsonb\)\s+WHERE template_id = \$2\s+AND id = ANY\(\$3\)\s+AND provider = \$4\s+AND api_mode = \$5`).
		WithArgs(
...
func TestApplyChannelMonitorTemplateRollsBackWhenHeaderRowCountDiffers(t *testing.T) {
	...
	mock.ExpectExec(`(?s)UPDATE "channel_monitors" SET .*WHERE `).
		WillReturnResult(sqlmock.NewResult(0, 2))
	mock.ExpectExec(`(?s)UPDATE channel_monitors\s+SET extra_headers = \$1::jsonb \|\| COALESCE\(\(.*WHERE entry\.key LIKE 'sub2api:%'.*WHERE template_id = \$2.*AND id = ANY\(\$3\)`).
		WithArgs(
			`{"User-Agent":"template-client"}`,
			templateID,
			`{41,42}`,
