...
func (s *HTTPUpstreamSuite) TestOpenAIProfileResponseHeaderTimeoutOverrideUsesDistinctClient() {
	s.cfg.Gateway = config.GatewayConfig{
		OpenAIHTTP2: config.GatewayOpenAIHTTP2Config{Enabled: true},
	}
	svc := s.newService()

	normalEntry, err := svc.getClientEntry("", 1, 1, service.HTTPUpstreamProfileOpenAI, false, false)
	require.NoError(s.T(), err)
	overrideEntry, err := svc.getClientEntryWithResponseHeaderTimeout("", 1, 1, service.HTTPUpstreamProfileOpenAI, 12*time.Second, true, false, false)
	require.NoError(s.T(), err)
	overrideEntryAgain, err := svc.getClientEntryWithResponseHeaderTimeout("", 1, 1, service.HTTPUpstreamProfileOpenAI, 12*time.Second, true, false, false)
	require.NoError(s.T(), err)
	normalEntryAgain, err := svc.getClientEntry("", 1, 1, service.HTTPUpstreamProfileOpenAI, false, false)
	require.NoError(s.T(), err)

	require.NotSame(s.T(), normalEntry, overrideEntry, "fallback header timeout override must not mutate the normal OpenAI client")
	require.Same(s.T(), overrideEntry, overrideEntryAgain, "same override should reuse its own pool entry")
	require.Same(s.T(), normalEntry, normalEntryAgain, "normal OpenAI client should remain reusable")

	normalTransport, ok := normalEntry.client.Transport.(*http.Transport)
	require.True(s.T(), ok, "expected *http.Transport")
	overrideTransport, ok := overrideEntry.client.Transport.(*http.Transport)
	require.True(s.T(), ok, "expected *http.Transport")
	require.Equal(s.T(), time.Duration(0), normalTransport.ResponseHeaderTimeout)
	require.Equal(s.T(), 12*time.Second, overrideTransport.ResponseHeaderTimeout)
}
...
func (s *HTTPUpstreamSuite) TestOpenAIProfileTLSFingerprintDoesNotInheritGenericHeaderTimeout() {
	...
	svc := s.newService()
	entry, err := svc.getClientEntryWithTLS("", 1, 1, &tlsfingerprint.Profile{Name: "test"}, service.HTTPUpstreamProfileOpenAI, 0, false, false, false)
	require.NoError(s.T(), err)
	transport, ok := entry.client.Transport.(*http.Transport)
	require.True(s.T(), ok, "expected *http.Transport")
...
func (s *HTTPUpstreamSuite) TestOpenAIProfileTLSFingerprintResponseHeaderTimeoutOverrideUsesDistinctClient() {
	s.cfg.Gateway = config.GatewayConfig{
		OpenAIHTTP2: config.GatewayOpenAIHTTP2Config{Enabled: true},
	}
	svc := s.newService()
	...
	normalEntry, err := svc.getClientEntryWithTLS("", 1, 1, profile, service.HTTPUpstreamProfileOpenAI, 0, false, false, false)
	require.NoError(s.T(), err)
	overrideEntry, err := svc.getClientEntryWithTLS("", 1, 1, profile, service.HTTPUpstreamProfileOpenAI, 12*time.Second, true, false, false)
	require.NoError(s.T(), err)
	overrideEntryAgain, err := svc.getClientEntryWithTLS("", 1, 1, profile, service.HTTPUpstreamProfileOpenAI, 12*time.Second, true, false, false)
	require.NoError(s.T(), err)
	normalEntryAgain, err := svc.getClientEntryWithTLS("", 1, 1, profile, service.HTTPUpstreamProfileOpenAI, 0, false, false, false)
	require.NoError(s.T(), err)

	require.NotSame(s.T(), normalEntry, overrideEntry, "fallback header timeout override must not mutate the normal OpenAI TLS client")
	require.Same(s.T(), overrideEntry, overrideEntryAgain, "same TLS override should reuse its own pool entry")
	require.Same(s.T(), normalEntry, normalEntryAgain, "normal OpenAI TLS client should remain reusable")
