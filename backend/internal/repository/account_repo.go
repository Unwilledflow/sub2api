...
func (r *accountRepository) ListOpenAIPool5xxRecoveryCandidates(ctx context.Context, limit int) ([]service.Account, error) {
	if r.sql == nil {
		return nil, errors.New("account repository SQL executor not configured")
	}
	if limit <= 0 || limit > 1000 {
		return nil, errors.New("openai pool 5xx recovery candidate limit must be between 1 and 1000")
	}
	rows, err := r.sql.QueryContext(ctx, `
		SELECT id
		FROM accounts
		WHERE deleted_at IS NULL
			...
			AND temp_unschedulable_reason LIKE '%"matched_keyword":"openai_pool_5xx"%'
		ORDER BY id ASC
		LIMIT $1
	`, limit)
	if err != nil {
...
// ListCooledDownAccounts lists accounts currently in the consecutive-failure
// cooldown, identified by the fixed temp_unschedulable_reason prefix written
// at cooldown time.
func (r *accountRepository) ListCooledDownAccounts(ctx context.Context, limit int) ([]service.Account, error) {
	if r.sql == nil {
		return nil, errors.New("account repository SQL executor not configured")
	}
	if limit <= 0 || limit > 1000 {
		return nil, errors.New("cooled-down account limit must be between 1 and 1000")
	}
	rows, err := r.sql.QueryContext(ctx, `
		SELECT id
		FROM accounts
		WHERE deleted_at IS NULL
			AND status = 'active'
			AND temp_unschedulable_until > NOW()
			AND temp_unschedulable_reason LIKE 'auto-disabled: %'
		ORDER BY id ASC
		LIMIT $1
	`, limit)
	if err != nil {
...
func (r *accountRepository) ClearOpenAIPool5xxTempUnschedulableIfMatch(
	ctx context.Context,
	id int64,
	...
	result, err := r.sql.ExecContext(ctx, `
		WITH updated AS (
			...
			WHERE a.id = $1
				...
				AND a.temp_unschedulable_reason = $3
				AND a.updated_at = $4
				AND a.temp_unschedulable_until > NOW()
				AND a.temp_unschedulable_reason LIKE '%"matched_keyword":"openai_pool_5xx"%'
			RETURNING a.id
		)
		INSERT INTO scheduler_outbox (event_type, account_id, group_id, payload)
		SELECT $5, updated.id, NULL, NULL FROM updated
	`, id, expectedUntil, expectedReason, expectedUpdatedAt, service.SchedulerOutboxEventAccountChanged)
	if err != nil {
		return false, err
	}
	affected, err := result.RowsAffected()
...
func (r *accountRepository) loadAccountGroups(ctx context.Context, accountIDs []int64) (map[int64][]*service.Group, map[int64][]int64, map[int64][]service.AccountGroup, error) {
	...
	for start := 0; start < len(accountIDs); start += postgresParameterBatchSize {
		...
		for _, ag := range entries {
			...
			// Account-group rows can outlive a soft-deleted group when they were
			// created by legacy cleanup paths. Do not expose those stale bindings
			// to callers or the admin editor.
			if groupSvc == nil {
				continue
			}
...
func (r *accountRepository) loadAccountGroupIDs(ctx context.Context, accountID int64) ([]int64, error) {
	...
	groups, err := r.loadGroups(ctx, groupIDs)
	if err != nil {
		return nil, err
	}
	for _, entry := range entries {
		if groups[entry.GroupID] != nil {
