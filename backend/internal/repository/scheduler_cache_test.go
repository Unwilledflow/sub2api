...
func TestSchedulerMetadataAccountProjectsUpstreamBillingProbe(t *testing.T) {
	...
	// Full payload keeps hot-path fields only; oversized probe diagnostics stay out of Redis.
	require.NotContains(t, string(fullPayload), lastError)
	require.NotContains(t, string(metaPayload), "last_error")
	require.Contains(t, string(fullPayload), "upstream_billing_probe")
	require.Contains(t, string(metaPayload), "upstream_billing_probe")
}
