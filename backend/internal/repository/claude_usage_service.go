...
func (s *claudeUsageService) FetchUsageWithOptions(ctx context.Context, opts *service.ClaudeUsageFetchOptions) (*service.ClaudeUsageResponse, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		// 429/401/403 是上游账号状态（限流/授权失效/封禁），必须映射为可区分的
		// HTTP 状态与 reason，避免一律包装成网关 500 导致 UI 无法区分
		// "配额耗尽/限流"与"网关故障"，且 429 需透传 Retry-After 语义。
		var appErr *infraerrors.ApplicationError
		switch resp.StatusCode {
		case http.StatusTooManyRequests:
			appErr = infraerrors.TooManyRequests("RATE_LIMITED", msg)
		case http.StatusUnauthorized:
			appErr = infraerrors.Unauthorized("UNAUTHENTICATED", msg)
		case http.StatusForbidden:
			appErr = infraerrors.Forbidden("FORBIDDEN", msg)
		default:
			appErr = infraerrors.New(http.StatusInternalServerError, "UPSTREAM_ERROR", msg)
		}
		if retryAfter := strings.TrimSpace(resp.Header.Get("Retry-After")); retryAfter != "" {
			appErr = appErr.WithMetadata(map[string]string{"retry_after": retryAfter})
		}
		return nil, appErr
	}
