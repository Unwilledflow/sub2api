...
func (s *APIKeyRepoSuite) TestUpdatePreservesAccountingAppliedAfterSnapshotLoad() {
	user := s.mustCreateUser("stale-api-key-accounting@test.com")
	key := &service.APIKey{
		UserID:      user.ID,
		Key:         "sk-stale-accounting",
		Name:        "Before",
		Status:      service.StatusActive,
		Quota:       100,
		RateLimit5h: 100,
	...
	// Reproduce the lost-update ordering: usage is recorded after loading a
	// configuration snapshot and before saving that stale snapshot.
	_, err = s.repo.IncrementQuotaUsed(s.ctx, key.ID, 2.5)
	s.Require().NoError(err)
	s.Require().NoError(s.repo.IncrementRateLimitUsage(s.ctx, key.ID, 2.5))
	stale.Name = "After"
	s.Require().NoError(s.repo.Update(s.ctx, stale, service.APIKeyUpdateFields{Name: true}))

	updated, err := s.repo.GetByID(s.ctx, key.ID)
	s.Require().NoError(err)
	s.Equal("After", updated.Name)
	s.InDelta(2.5, updated.QuotaUsed, 1e-6)
	s.InDelta(2.5, updated.Usage5h, 1e-6)
	s.InDelta(2.5, updated.Usage1d, 1e-6)
	s.InDelta(2.5, updated.Usage7d, 1e-6)
	s.NotNil(updated.Window5hStart)
	s.NotNil(updated.Window1dStart)
	s.NotNil(updated.Window7dStart)
}

func (s *APIKeyRepoSuite) TestUpdateWithUsageResetCommitsConfigAndAccountingTogether() {
	user := s.mustCreateUser("transactional-api-key-reset@test.com")
	key := &service.APIKey{
		UserID:      user.ID,
		Key:         "sk-transactional-reset",
		Name:        "Before",
		Status:      service.StatusActive,
		Quota:       100,
		RateLimit5h: 100,
	...
	s.Require().NoError(s.repo.Create(s.ctx, key))
	_, err := s.repo.IncrementQuotaUsed(s.ctx, key.ID, 4)
	s.Require().NoError(err)
	s.Require().NoError(s.repo.IncrementRateLimitUsage(s.ctx, key.ID, 4))

	key.Name = "After"
	s.Require().NoError(s.repo.UpdateWithUsageReset(s.ctx, key, true, true))
	...
	s.Zero(updated.Usage1d)
	s.Zero(updated.Usage7d)
	s.Nil(updated.Window5hStart)
	s.Nil(updated.Window1dStart)
	s.Nil(updated.Window7dStart)
}
...
func (s *APIKeyRepoSuite) TestIncrementQuotaUsedAndGetState() {
	...
	state, err := s.repo.IncrementQuotaUsedAndGetState(s.ctx, key.ID, 2)
	s.Require().NoError(err, "IncrementQuotaUsedAndGetState")
	s.Require().NotNil(state)
	s.Require().Equal(3.0, state.QuotaUsed)
	...
	got, err := s.repo.GetByID(s.ctx, key.ID)
	s.Require().NoError(err, "GetByID")
	s.Require().Equal(3.0, got.QuotaUsed)
	s.Require().Equal(service.StatusAPIKeyQuotaExhausted, got.Status)
}

func (s *APIKeyRepoSuite) TestIncrementQuotaUsed_RecordsDeliveredUsageBesideAdaptiveReservation() {
	user := s.mustCreateUser("quota-reserved@test.com")
	key := s.mustCreateApiKey(user.ID, "sk-quota-reserved", "QuotaReserved", nil)
	_, err := s.repo.sql.ExecContext(s.ctx, `
		UPDATE api_keys
		SET quota = 10, quota_used = 4, reserved_quota_usd = 5
		WHERE id = $1
	`, key.ID)
	s.Require().NoError(err)
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.repo.sql, `
		SELECT quota_used, reserved_quota_usd FROM api_keys WHERE id = $1
	`, []any{key.ID}, &used, &reserved))
	s.Require().InDelta(6, used, 1e-9)
	s.Require().InDelta(5, reserved, 1e-9)
}

func (s *APIKeyRepoSuite) TestIncrementRateLimitUsage_RecordsDeliveredUsageBesideAdaptiveReservation() {
	user := s.mustCreateUser("rate-reserved@test.com")
	key := s.mustCreateApiKey(user.ID, "sk-rate-reserved", "RateReserved", nil)
	_, err := s.repo.sql.ExecContext(s.ctx, `
		UPDATE api_keys
		SET rate_limit_5h = 10, usage_5h = 4, reserved_usage_5h_usd = 5,
			window_5h_start = NOW()
		WHERE id = $1
	`, key.ID)
	s.Require().NoError(err)

	err = s.repo.IncrementRateLimitUsage(s.ctx, key.ID, 2)
	s.Require().NoError(err)

	var used, reserved float64
	s.Require().NoError(scanSingleRow(s.ctx, s.repo.sql, `
		SELECT usage_5h, reserved_usage_5h_usd FROM api_keys WHERE id = $1
	`, []any{key.ID}, &used, &reserved))
	s.Require().InDelta(6, used, 1e-9)
	s.Require().InDelta(5, reserved, 1e-9)
}
...
func TestIncrementQuotaUsed_ConcurrentDeliveredUsageIsNeverDropped(t *testing.T) {
	client := testEntClient(t)
	repo := NewAPIKeyRepository(client, integrationDB).(*apiKeyRepository)
	ctx := context.Background()

	u := mustCreateUser(t, client, &service.User{
		Email:        "quota-reservation-race-" + time.Now().Format(time.RFC3339Nano) + "@test.com",
		PasswordHash: "hash",
	})
	k := mustCreateApiKey(t, client, &service.APIKey{
		UserID: u.ID,
		Key:    "sk-quota-reservation-race-" + time.Now().Format(time.RFC3339Nano),
		Name:   "Quota reservation race",
		Quota:  10,
	})
	require.NoError(t, integrationDB.QueryRowContext(ctx, `
		UPDATE api_keys SET reserved_quota_usd = 4 WHERE id = $1
		RETURNING id
	`, k.ID).Scan(&k.ID))
	t.Cleanup(func() {
		_, _ = integrationDB.ExecContext(ctx, "DELETE FROM api_keys WHERE id = $1", k.ID)
	...
	for i := 0; i < 2; i++ {
		go func() {
			<-start
			_, err := repo.IncrementQuotaUsed(ctx, k.ID, 4)
			errs <- err
		}()
	}
	close(start)
	...
	require.NoError(t, integrationDB.QueryRowContext(ctx, `
		SELECT quota_used, reserved_quota_usd FROM api_keys WHERE id = $1
	`, k.ID).Scan(&used, &reserved))
	require.InDelta(t, 8, used, 1e-9)
	require.InDelta(t, 4, reserved, 1e-9)
	require.InDelta(t, 12, used+reserved, 1e-9)
}
