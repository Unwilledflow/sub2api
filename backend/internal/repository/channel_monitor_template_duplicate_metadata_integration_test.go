...
func TestApplyChannelMonitorTemplatePreservesInternalMetadata(t *testing.T) {
	tx := testEntTx(t)
	...
	require.Equal(t, "ops:1:account:17", stored.ExtraHeaders[service.ChannelMonitorExternalRefMetadataKey])
	require.Equal(t, "true", stored.ExtraHeaders[service.ChannelMonitorPublicVisibleMetadataKey])
	require.Equal(t, service.ChannelMonitorManagementModeExternal, stored.ExtraHeaders[service.ChannelMonitorManagementModeMetadataKey])

	runtimeMonitor := entToServiceMonitor(stored)
	require.Equal(t, "operation-digest", runtimeMonitor.DuplicateOperationID)
	require.Equal(t, "ops:1:account:17", runtimeMonitor.ExternalRef)
	require.True(t, runtimeMonitor.PublicVisible)
	require.Equal(t, service.ChannelMonitorManagementModeExternal, runtimeMonitor.ManagementMode)
	require.NotContains(t, runtimeMonitor.ExtraHeaders, service.ChannelMonitorDuplicateOperationIDMetadataKey)
	require.NotContains(t, runtimeMonitor.ExtraHeaders, service.ChannelMonitorExternalRefMetadataKey)
}
