...
func TestUserRepositoryGetByUsernameNormalizesSpacingAndCase(t *testing.T) {
	repo, _ := newUserEntRepo(t)
	ctx := context.Background()

	err := repo.Create(ctx, &service.User{
		Email:        "username@example.com",
		Username:     " AdminNewAPI ",
		PasswordHash: "hash",
		Role:         service.RoleUser,
		Status:       service.StatusActive,
	})
	...
	got, err := repo.GetByUsername(ctx, "adminnewapi")
	require.NoError(t, err)
	require.Equal(t, " AdminNewAPI ", got.Username)
}
