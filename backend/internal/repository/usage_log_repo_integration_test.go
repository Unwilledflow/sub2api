...
func TestUsageLogRepositoryCreate_SettlesZeroCostRowOnPositiveRetry(t *testing.T) {
	ctx := context.Background()
	client := testEntClient(t)
	repo := newUsageLogRepositoryWithSQL(client, integrationDB)

	user := mustCreateUser(t, client, &service.User{Email: fmt.Sprintf("usage-settle-%d@example.com", time.Now().UnixNano())})
	apiKey := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: "sk-usage-settle-" + uuid.NewString(), Name: "k"})
	account := mustCreateAccount(t, client, &service.Account{Name: "acc-usage-settle-" + uuid.NewString()})
	requestID := uuid.NewString()

	failed := &service.UsageLog{
		UserID: user.ID, APIKeyID: apiKey.ID, AccountID: account.ID,
		RequestID: requestID, Model: "claude-3", InputTokens: 10,
		OutputTokens: 20, TotalCost: 0.5, ActualCost: 0,
		CreatedAt: time.Now().UTC(),
	}
	inserted, err := repo.Create(ctx, failed)
	require.NoError(t, err)
	require.True(t, inserted)

	success := *failed
	success.ID = 0
	success.ActualCost = 0.5
	success.CreatedAt = time.Now().UTC()
	settled, err := repo.Create(ctx, &success)
	require.NoError(t, err)
	require.True(t, settled, "the positive retry claims the previously unsettled row")
	require.Equal(t, failed.ID, success.ID)

	var actualCost float64
	require.NoError(t, integrationDB.QueryRowContext(ctx,
		"SELECT actual_cost FROM usage_logs WHERE request_id = $1 AND api_key_id = $2",
		requestID, apiKey.ID,
	).Scan(&actualCost))
	...
	require.InDelta(t, 0.5, actualCost, 1e-9, "a settled row must not be repriced by a duplicate")

	lateFailure := success
	lateFailure.ID = 0
	lateFailure.ActualCost = 0
	inserted, err = repo.Create(ctx, &lateFailure)
	require.NoError(t, err)
	require.False(t, inserted)
	require.NoError(t, integrationDB.QueryRowContext(ctx,
		"SELECT actual_cost FROM usage_logs WHERE request_id = $1 AND api_key_id = $2",
	...
	require.InDelta(t, 0.5, actualCost, 1e-9, "a failed retry must not clear a settled row")
}

func TestUsageLogRepositoryCreateBestEffort_SettlesZeroCostRowOnPositiveRetry(t *testing.T) {
	ctx := context.Background()
	client := testEntClient(t)
	repo := newUsageLogRepositoryWithSQL(client, integrationDB)

	user := mustCreateUser(t, client, &service.User{Email: fmt.Sprintf("usage-best-effort-settle-%d@example.com", time.Now().UnixNano())})
	apiKey := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: "sk-usage-best-effort-settle-" + uuid.NewString(), Name: "k"})
	account := mustCreateAccount(t, client, &service.Account{Name: "acc-usage-best-effort-settle-" + uuid.NewString()})
	requestID := uuid.NewString()
	newLog := func(actualCost float64) *service.UsageLog {
		return &service.UsageLog{
			UserID: user.ID, APIKeyID: apiKey.ID, AccountID: account.ID,
			RequestID: requestID, Model: "claude-3", InputTokens: 10,
			OutputTokens: 20, TotalCost: 0.5, ActualCost: actualCost,
			CreatedAt: time.Now().UTC(),
		}
	}

	require.NoError(t, repo.CreateBestEffort(ctx, newLog(0)))
	require.NoError(t, repo.CreateBestEffort(ctx, newLog(0.5)))
	require.Eventually(t, func() bool {
		var actualCost float64
		...
		return err == nil && actualCost > 0.49
	}, 3*time.Second, 20*time.Millisecond)
}
