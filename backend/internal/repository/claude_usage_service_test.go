...
func (s *ClaudeUsageServiceSuite) TestFetchUsage_NonOK() {
	...
	require.True(s.T(), infraerrors.IsUnauthorized(err), "expected 401 error, got %v", err)
	require.Equal(s.T(), "UNAUTHENTICATED", infraerrors.Reason(err))
}

// TestFetchUsage_RateLimited 上游 429（配额耗尽/限流高频场景）必须映射为可区分
// 的 429 + RATE_LIMITED，而不是网关 500，并透传 Retry-After。
func (s *ClaudeUsageServiceSuite) TestFetchUsage_RateLimited() {
	s.srv = newLocalTestServer(s.T(), http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Retry-After", "60")
		w.WriteHeader(http.StatusTooManyRequests)
		_, _ = io.WriteString(w, "Rate limited. Please try again later.")
	}))
	...
	_, err := s.fetcher.FetchUsage(context.Background(), "at", "")
	require.Error(s.T(), err)
	require.True(s.T(), infraerrors.IsTooManyRequests(err), "expected 429 error, got %v", err)
	require.Equal(s.T(), "RATE_LIMITED", infraerrors.Reason(err))
	require.False(s.T(), infraerrors.IsInternalServer(err), "429 must not be wrapped as gateway 500")
	require.Equal(s.T(), "60", infraerrors.FromError(err).Metadata["retry_after"])
}

func (s *ClaudeUsageServiceSuite) TestFetchUsage_ForbiddenMappedAs403() {
	s.srv = newLocalTestServer(s.T(), http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusForbidden)
		_, _ = io.WriteString(w, "account banned")
	}))
	...
	_, err := s.fetcher.FetchUsage(context.Background(), "at", "")
	require.Error(s.T(), err)
	require.True(s.T(), infraerrors.IsForbidden(err), "expected 403 error, got %v", err)
	require.Equal(s.T(), "FORBIDDEN", infraerrors.Reason(err))
}
