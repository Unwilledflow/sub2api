...
func (r *batchImageRepository) ListBatchImageJobsDueForOutputCleanup(ctx context.Context, now time.Time, terminalCutoff time.Time, limit int) ([]*service.BatchImageJob, error) {
	if limit <= 0 || limit > 1000 {
		limit = 100
	}
	// completed：按 output_expires_at（结算时写入）清理。
	// failed/cancelled：Submit 时可能已写入 provider_output_ref 但从未设置
	// output_expires_at，按 finished_at（及回退时间戳）相对 terminalCutoff 清理，避免 GCS 永久泄漏。
	rows, err := r.sql.QueryContext(ctx, batchImageJobSelectSQL+`
 WHERE output_deleted_at IS NULL
   ...
   AND (
     ...
     (status IN ('failed', 'cancelled')
       AND (
         (output_expires_at IS NOT NULL AND output_expires_at <= $1)
         OR
         (output_expires_at IS NULL
           AND COALESCE(finished_at, settled_at, updated_at, created_at) <= $2)
       ))
   )
 ORDER BY COALESCE(output_expires_at, finished_at, settled_at, updated_at, created_at) ASC, id ASC
 LIMIT $3`, now, terminalCutoff, limit)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
