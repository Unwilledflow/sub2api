...
func (s *AccountRepoSuite) TestGetByID_SkipsSoftDeletedGroupBindings() {
	group := mustCreateGroup(s.T(), s.client, &service.Group{Name: "deleted-group"})
	account := mustCreateAccount(s.T(), s.client, &service.Account{Name: "account-with-deleted-group"})
	mustBindAccountToGroup(s.T(), s.client, account.ID, group.ID, 1)

	groupRepo := newGroupRepositoryWithSQL(s.client, s.repo.sql)
	s.Require().NoError(groupRepo.Delete(s.ctx, group.ID))
...
func (s *AccountRepoSuite) TestClearError_PreservesSchedulableAccount() {
	account := mustCreateAccount(s.T(), s.client, &service.Account{
		Name:         "acc-clear-err-schedulable",
		Status:       service.StatusError,
		ErrorMessage: "temporary error",
		Schedulable:  true,
	})
