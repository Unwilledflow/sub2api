...
func deductUsageBillingBalance(ctx context.Context, tx *sql.Tx, userID int64, amount float64) (float64, bool, error) {
	...
	err = tx.QueryRowContext(ctx, `
		...
		WHERE id = $2 AND deleted_at IS NULL
		  AND adaptive_reserved_balance = 0
		RETURNING balance
	`, amount, userID).Scan(&newBalance)
	if err == nil {
	...
	// The legacy overdraft policy must not consume money already promised to an
	// Adaptive request. Lock the row before classifying the failed deduction so a
	// concurrent release cannot turn this into a stale decision.
	var adaptiveReserved float64
	err = tx.QueryRowContext(ctx, `
		SELECT adaptive_reserved_balance
		FROM users
		WHERE id = $1 AND deleted_at IS NULL
		FOR UPDATE
	`, userID).Scan(&adaptiveReserved)
	if errors.Is(err, sql.ErrNoRows) {
