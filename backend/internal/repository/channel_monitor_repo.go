...
func (r *channelMonitorRepository) ListPublic(ctx context.Context) ([]*service.ChannelMonitor, error) {
	rows, err := r.client.ChannelMonitor.Query().
		Where(channelmonitor.Or(
			channelmonitor.EnabledEQ(true),
			func(selector *entsql.Selector) {
				selector.Where(sqljson.ValueEQ(
					channelmonitor.FieldExtraHeaders,
					"true",
					sqljson.Path(service.ChannelMonitorPublicVisibleMetadataKey),
				))
			},
		)).
		All(ctx)
...
func entToServiceMonitor(row *dbent.ChannelMonitor) *service.ChannelMonitor {
	...
	externalRef := strings.TrimSpace(headers[service.ChannelMonitorExternalRefMetadataKey])
	publicVisible := headers[service.ChannelMonitorPublicVisibleMetadataKey] == "true"
	managementMode := strings.TrimSpace(headers[service.ChannelMonitorManagementModeMetadataKey])
	for _, key := range []string{
		service.ChannelMonitorDuplicateOperationIDMetadataKey,
		service.ChannelMonitorExternalRefMetadataKey,
		service.ChannelMonitorPublicVisibleMetadataKey,
		service.ChannelMonitorManagementModeMetadataKey,
	} {
		delete(headers, key)
	}
...
func channelMonitorHeadersForPersistence(m *service.ChannelMonitor) map[string]string {
	...
	headers := make(map[string]string, len(m.ExtraHeaders)+4)
	for key, value := range m.ExtraHeaders {
		if strings.HasPrefix(key, "sub2api:") {
			continue
		}
		headers[key] = value
	}
	...
	if externalRef := strings.TrimSpace(m.ExternalRef); externalRef != "" {
		headers[service.ChannelMonitorExternalRefMetadataKey] = externalRef
	}
	if m.PublicVisible {
		headers[service.ChannelMonitorPublicVisibleMetadataKey] = "true"
	}
	if mode := strings.TrimSpace(m.ManagementMode); mode != "" {
		headers[service.ChannelMonitorManagementModeMetadataKey] = mode
	}
	return headers
}
