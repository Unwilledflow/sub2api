...
func (r *affiliateRepository) AccrueQuota(ctx context.Context, inviterID, inviteeUserID int64, amount float64, freezeHours int, sourceOrderID *int64, perInviteeCap float64) (bool, float64, error) {
	if amount <= 0 {
		return false, 0, nil
	}
	...
	err := r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		// Lock inviter row so concurrent accruals from the same invitee serialize
		// before the per-invitee cap check (closes 18-H2 TOCTOU).
		lockRows, err := txClient.QueryContext(txCtx,
			`SELECT user_id FROM user_affiliates WHERE user_id = $1 FOR UPDATE`, inviterID)
		if err != nil {
			return fmt.Errorf("lock inviter affiliate row: %w", err)
		}
		hasRow := lockRows.Next()
		_ = lockRows.Close()
		if !hasRow {
			applied = false
			credited = 0
		...
		creditAmount := amount
		if perInviteeCap > 0 {
			existing, err := sumInviteeRebateNetTx(txCtx, txClient, inviterID, inviteeUserID)
			if err != nil {
				return err
			}
			...
			if remaining := perInviteeCap - existing; creditAmount > remaining {
		...
		res, err := txClient.ExecContext(txCtx, updateSQL, creditAmount, inviterID)
		if err != nil {
			return err
		}
		affected, _ := res.RowsAffected()
...
VALUES ($1, 'accrue', $2, $3, $4, NOW() + make_interval(hours => $5), NOW(), NOW())`,
				inviterID, creditAmount, inviteeUserID, nullableInt64Arg(sourceOrderID), freezeHours); err != nil {
				return fmt.Errorf("insert affiliate accrue ledger: %w", err)
			}
		} else {
			if _, err = txClient.ExecContext(txCtx, `
INSERT INTO user_affiliate_ledger (user_id, action, amount, source_user_id, source_order_id, created_at, updated_at)
VALUES ($1, 'accrue', $2, $3, $4, NOW(), NOW())`, inviterID, creditAmount, inviteeUserID, nullableInt64Arg(sourceOrderID)); err != nil {
				return fmt.Errorf("insert affiliate accrue ledger: %w", err)
			}
		}
	...
	return applied, credited, nil
}

func sumInviteeRebateNetTx(ctx context.Context, client affiliateQueryExecer, inviterID, inviteeUserID int64) (float64, error) {
	// Net = accrue - clawback so prior refund clawbacks free cap again.
	rows, err := client.QueryContext(ctx, `
SELECT COALESCE(SUM(
    CASE
        WHEN action = 'accrue' THEN amount
        WHEN action = 'clawback' THEN -amount
        ELSE 0
    END
), 0)::double precision
FROM user_affiliate_ledger
WHERE user_id = $1 AND source_user_id = $2 AND action IN ('accrue', 'clawback')`,
...
func (r *affiliateRepository) GetAccruedRebateFromInvitee(ctx context.Context, inviterID, inviteeUserID int64) (float64, error) {
	client := clientFromContext(ctx, r.client)
	return sumInviteeRebateNetTx(ctx, client, inviterID, inviteeUserID)
}

func (r *affiliateRepository) ClawbackQuotaForOrder(ctx context.Context, sourceOrderID int64) (float64, error) {
	if sourceOrderID <= 0 {
		return 0, nil
	}
	var clawed float64
	err := r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		// Lock ledger rows for the order, then aggregate net in Go
		// (PostgreSQL forbids FOR UPDATE with GROUP BY).
		rows, err := txClient.QueryContext(txCtx, `
SELECT user_id, source_user_id, action, amount::double precision
FROM user_affiliate_ledger
WHERE source_order_id = $1
  AND action IN ('accrue', 'clawback')
FOR UPDATE`, sourceOrderID)
		if err != nil {
			return fmt.Errorf("query affiliate accrue for clawback: %w", err)
		}
		type clawKey struct {
			userID       int64
			sourceUserID int64
		...
		netByKey := map[clawKey]float64{}
		for rows.Next() {
			var userID int64
			var sourceUserID sql.NullInt64
			var action string
			var amount float64
			if err := rows.Scan(&userID, &sourceUserID, &action, &amount); err != nil {
				_ = rows.Close()
				return err
			}
			key := clawKey{userID: userID}
			if sourceUserID.Valid {
				key.sourceUserID = sourceUserID.Int64
				key.hasSource = true
			}
		...
		var total float64
		for key, amount := range netByKey {
			if amount <= 0 {
				continue
			}
			// Lock inviter and deduct available then frozen, floor at 0.
			// PG evaluates SET expressions against original row values.
			if _, err := txClient.ExecContext(txCtx, `
UPDATE user_affiliates
SET aff_quota = GREATEST(0, aff_quota - LEAST(aff_quota, $1)),
    aff_frozen_quota = GREATEST(0, aff_frozen_quota - GREATEST(0, $1 - LEAST(aff_quota, $1))),
    aff_history_quota = GREATEST(0, aff_history_quota - $1),
    updated_at = NOW()
WHERE user_id = $2`, amount, key.userID); err != nil {
				return fmt.Errorf("deduct affiliate quota on clawback: %w", err)
			}

			var sourceUser any
			if key.hasSource {
				sourceUser = key.sourceUserID
			}
			if _, err = txClient.ExecContext(txCtx, `
INSERT INTO user_affiliate_ledger (user_id, action, amount, source_user_id, source_order_id, created_at, updated_at)
VALUES ($1, 'clawback', $2, $3, $4, NOW(), NOW())`,
				key.userID, amount, sourceUser, sourceOrderID); err != nil {
