...
func (r *groupRepository) persistUserVisible(ctx context.Context, id int64, visible bool) error {
...
// attachUserVisible overlays groups.user_visible (raw column; not yet in ent schema).
func (r *groupRepository) attachUserVisible(ctx context.Context, groups []service.Group) {
	...
	for i := range groups {
		// Default true before overlay.
		groups[i].UserVisible = true
		ids = append(ids, groups[i].ID)
		index[groups[i].ID] = append(index[groups[i].ID], i)
	}
	rows, err := r.sql.QueryContext(ctx, `
		SELECT id, COALESCE(user_visible, TRUE)
		  FROM groups
		 WHERE id = ANY($1)
	`, pq.Array(ids))
	if err != nil {
	...
	for rows.Next() {
		...
		for _, i := range index[id] {
			groups[i].UserVisible = visible
		}
	}
}

func (r *groupRepository) attachUserVisibleOne(ctx context.Context, g *service.Group) {
	if g == nil {
		return
	}
	slice := []service.Group{*g}
	r.attachUserVisible(ctx, slice)
	*g = slice[0]
}
...
func (r *groupRepository) Create(ctx context.Context, groupIn *service.Group) error {
	...
	// user_visible is outside ent. Admin Create always sets UserVisible.
	// Other create paths should set true (DB default) when leaving zero-value.
	visible := true
	if groupIn != nil {
		visible = groupIn.UserVisible
		// Zero-value false from callers that never set the field would hide the
		// group; only force-default true when the field was never populated.
		// Service CreateGroup always assigns UserVisible explicitly.
	}
	if err := r.persistUserVisible(ctx, groupIn.ID, visible); err != nil {
		logger.LegacyPrintf("repository.group", "[Group] persist user_visible on create failed: group=%d err=%v", groupIn.ID, err)
	}
...
func createGroupRecord(ctx context.Context, client *dbent.Client, groupIn *service.Group) error {
	...
	builder := client.Group.Create().
		...
		SetCompetitiveConcurrency(groupIn.CompetitiveConcurrency).
...
func (r *groupRepository) GetByIDLite(ctx context.Context, id int64) (*service.Group, error) {
	...
	r.attachUserVisibleOne(ctx, out)
	return out, nil
}
...
func (r *groupRepository) Update(ctx context.Context, groupIn *service.Group) error {
	...
	if err := r.persistUserVisible(ctx, groupIn.ID, groupIn.UserVisible); err != nil {
		logger.LegacyPrintf("repository.group", "[Group] persist user_visible on update failed: group=%d err=%v", groupIn.ID, err)
	}
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventGroupChanged, nil, &groupIn.ID, nil); err != nil {
...
func (r *groupRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, status, search string, isExclusive *bool) ([]service.Group, *pagination.PaginationResult, error) {
	...
	r.attachUserVisible(ctx, outGroups)
