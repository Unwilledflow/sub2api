...
func TestAccountRepository_ListOpenAIPool5xxRecoveryCandidates_SQLFilter(t *testing.T) {
	...
	accounts, err := repo.ListOpenAIPool5xxRecoveryCandidates(context.Background(), 25)
	require.NoError(t, err)
	...
	require.Contains(t, normalized, `credentials @> '{"pool_mode":true}'::jsonb`)
	require.Contains(t, normalized, "temp_unschedulable_until > NOW()")
	require.Contains(t, normalized, `temp_unschedulable_reason LIKE '%"matched_keyword":"openai_pool_5xx"%'`)
	require.Contains(t, normalized, "ORDER BY id ASC")
	require.Contains(t, normalized, "LIMIT $1")
	require.Equal(t, []any{25}, capturedArgs)
	require.NoError(t, mock.ExpectationsWereMet())
}

func TestAccountRepository_ClearOpenAIPool5xxTempUnschedulableIfMatch_IsAtomicAndConditional(t *testing.T) {
	exec := &recordingSQLExecutor{result: rowsAffectedResult(1)}
	repo := newAccountRepositoryWithSQL(nil, exec, nil)
	until := time.Date(2026, 7, 17, 4, 5, 6, 123000000, time.UTC)
	updatedAt := until.Add(-time.Minute)
	reason := `{"matched_keyword":"openai_pool_5xx","model":"gpt-5.5"}`

	cleared, err := repo.ClearOpenAIPool5xxTempUnschedulableIfMatch(
		context.Background(), 42, until, reason, updatedAt,
	)
	...
	require.Contains(t, normalized, "temp_unschedulable_until = $2")
	require.Contains(t, normalized, "temp_unschedulable_reason = $3")
	require.Contains(t, normalized, "updated_at = $4")
	require.Contains(t, normalized, "INSERT INTO scheduler_outbox")
	require.Contains(t, normalized, "SELECT $5, updated.id, NULL, NULL FROM updated")
	require.Equal(t, []any{int64(42), until, reason, updatedAt, service.SchedulerOutboxEventAccountChanged}, exec.execArgs[0])
}

func TestAccountRepository_ClearOpenAIPool5xxTempUnschedulableIfMatch_StaleSnapshotDoesNotWriteOutbox(t *testing.T) {
	exec := &recordingSQLExecutor{result: rowsAffectedResult(0)}
	repo := newAccountRepositoryWithSQL(nil, exec, nil)

	cleared, err := repo.ClearOpenAIPool5xxTempUnschedulableIfMatch(
		context.Background(), 42, time.Now().Add(time.Minute), "old", time.Now(),
	)
