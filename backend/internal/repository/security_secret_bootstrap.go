...
const (
	...
	securitySecretKeyTotpEncryptionKey = "totp_encryption_key"
...
// ensureTotpEncryptionKey loads or persists a durable AES-256 key (64 hex chars).
// Explicit config is preferred and written once; otherwise a key is generated once
// into security_secrets so restarts do not invalidate encrypted TOTP/backup data.
func ensureTotpEncryptionKey(ctx context.Context, client *ent.Client, cfg *config.Config) error {
	cfg.Totp.EncryptionKey = strings.TrimSpace(cfg.Totp.EncryptionKey)
	if cfg.Totp.EncryptionKey != "" {
		if err := validateTotpEncryptionKeyHex(cfg.Totp.EncryptionKey); err != nil {
			return fmt.Errorf("configured totp encryption key: %w", err)
		}
		stored, err := createSecuritySecretIfAbsent(ctx, client, securitySecretKeyTotpEncryptionKey, cfg.Totp.EncryptionKey)
		if err != nil {
			return fmt.Errorf("persist totp encryption key: %w", err)
		}
		if stored != cfg.Totp.EncryptionKey {
	...
	secret, created, err := getOrCreateGeneratedSecuritySecret(ctx, client, securitySecretKeyTotpEncryptionKey, 32)
	if err != nil {
		return fmt.Errorf("ensure totp encryption key: %w", err)
	}
	if err := validateTotpEncryptionKeyHex(secret); err != nil {
		return fmt.Errorf("stored totp encryption key: %w", err)
	}
	cfg.Totp.EncryptionKey = secret
	// Persisted (config or DB) keys are durable across restarts.
	cfg.Totp.EncryptionKeyConfigured = true
	if created {
		log.Println("Warning: TOTP encryption key auto-generated and persisted to database. Set totp.encryption_key explicitly for production key management.")
	}
	return nil
}

func validateTotpEncryptionKeyHex(key string) error {
