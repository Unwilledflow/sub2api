...
const (
	...
	overdraftBalanceDeductSQL   = `(?s)UPDATE users\s+SET balance = balance - \$1,\s+updated_at = NOW\(\)\s+WHERE id = \$2 AND deleted_at IS NULL\s+AND adaptive_reserved_balance = 0\s+RETURNING balance`
	classifyAdaptiveBalanceSQL  = `(?s)SELECT adaptive_reserved_balance\s+FROM users\s+WHERE id = \$1 AND deleted_at IS NULL\s+FOR UPDATE`
...
func TestDeductUsageBillingBalance_AdaptiveHoldDisablesOverdraft(t *testing.T) {
	ctx := context.Background()
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer func() { _ = db.Close() }()
	...
	mock.ExpectQuery(overdraftBalanceDeductSQL).
		WithArgs(10.0, int64(42)).
		WillReturnError(sql.ErrNoRows)
	mock.ExpectQuery(classifyAdaptiveBalanceSQL).
		WithArgs(int64(42)).
		WillReturnRows(sqlmock.NewRows([]string{"adaptive_reserved_balance"}).AddRow(8.0))
	mock.ExpectRollback()
