...
func (s *UserSubscriptionRepoSuite) TestIncrementUsage_RecordsDeliveredUsageBesideAdaptiveReservation() {
	user := s.mustCreateUser("increment-reserved@test.com", service.RoleUser)
	group := s.mustCreateGroup("g-increment-reserved")
	sub := s.mustCreateSubscription(user.ID, group.ID, nil)

	_, err := s.client.ExecContext(s.ctx, `
		UPDATE groups SET daily_limit_usd = 10 WHERE id = $1
	`, group.ID)
	s.Require().NoError(err)
	_, err = s.client.ExecContext(s.ctx, `
		UPDATE user_subscriptions
		SET daily_usage_usd = 4, reserved_usage_usd = 5, daily_window_start = NOW()
		WHERE id = $1
	`, sub.ID)
	s.Require().NoError(err)

	err = s.repo.IncrementUsage(s.ctx, sub.ID, 2)
	s.Require().NoError(err)

	var used, reserved float64
	s.Require().NoError(scanSingleRow(s.ctx, s.client, `
		SELECT daily_usage_usd, reserved_usage_usd
		FROM user_subscriptions WHERE id = $1
	`, []any{sub.ID}, &used, &reserved))
...
func TestIncrementSubscriptionUsage_ConcurrentDeliveredUsageIsNeverDropped(t *testing.T) {
	client := testEntClient(t)
	repo := NewUserSubscriptionRepository(client)
	ctx := context.Background()

	u := mustCreateUser(t, client, &service.User{
		Email:        "subscription-reservation-race-" + time.Now().Format(time.RFC3339Nano) + "@test.com",
		PasswordHash: "hash",
	})
	dailyLimit := 10.0
	g := mustCreateGroup(t, client, &service.Group{
		Name:             "subscription-reservation-race-" + time.Now().Format(time.RFC3339Nano),
	...
	require.NoError(t, integrationDB.QueryRowContext(ctx, `
		UPDATE user_subscriptions
		SET reserved_usage_usd = 4, daily_window_start = NOW()
		WHERE id = $1
		RETURNING id
	`, sub.ID).Scan(&sub.ID))
	t.Cleanup(func() {
	...
	for i := 0; i < 2; i++ {
		go func() {
			<-start
			errs <- repo.IncrementUsage(ctx, sub.ID, 4)
		}()
	}
	close(start)
