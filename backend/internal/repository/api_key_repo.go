...
func (r *apiKeyRepository) Create(ctx context.Context, key *service.APIKey) error {
	builder := r.client.APIKey.Create().
		...
		SetBlockOpenaiFast(key.BlockOpenAIFast).
	...
	if err == nil {
		...
		_ = r.persistAdaptiveRoutingPreference(ctx, key.ID, key.AdaptiveRoutingPreference)
		_ = r.persistAdaptiveMaxRateMultiplier(ctx, key.ID, key.AdaptiveMaxRateMultiplier)
		_ = r.persistAntiStallTier(ctx, key.ID, key.AntiStallTier)
		_ = r.persistAdaptivePassiveFailover(ctx, key.ID, key.AdaptivePassiveFailoverEnabled)
	}
...
func (r *apiKeyRepository) GetByID(ctx context.Context, id int64) (*service.APIKey, error) {
	...
	r.loadAdaptiveRoutingPreferences(ctx, []*service.APIKey{out})
	return out, nil
}
	...
	m, err := r.activeQuery().
		...
		WithGroup(func(q *dbent.GroupQuery) {
			q.Select(
				...
				group.FieldCompetitiveConcurrency,
...
func (r *apiKeyRepository) Update(ctx context.Context, key *service.APIKey, fields service.APIKeyUpdateFields) error {
	...
	if fields.FallbackGroups {
		if len(key.FallbackGroupIDs) > 0 {
			builder.SetFallbackGroupIds(key.FallbackGroupIDs)
		} else {
			builder.ClearFallbackGroupIds()
		}
	}
	if fields.BlockOpenAIFast {
		builder.SetBlockOpenaiFast(key.BlockOpenAIFast)
	}
...
// UpdateWithUsageReset keeps configuration changes and explicitly requested
// accounting resets in one transaction. Update itself remains metadata-only.
func (r *apiKeyRepository) UpdateWithUsageReset(
	ctx context.Context,
	key *service.APIKey,
	resetQuota bool,
	resetRateLimit bool,
	...
	if dbent.TxFromContext(ctx) != nil {
		return r.updateWithUsageReset(ctx, key, resetQuota, resetRateLimit)
	}
	...
	if err := r.updateWithUsageReset(txCtx, key, resetQuota, resetRateLimit); err != nil {
		return err
	}
	return tx.Commit()
}
...
func (r *apiKeyRepository) updateWithUsageReset(
	...
	metadataFields := service.APIKeyUpdateFields{
		Name:             true,
		Status:           true,
		Quota:            true,
		GroupID:          true,
	...
	if err := r.Update(ctx, key, metadataFields); err != nil {
		return err
	}
	if resetQuota {
		if err := r.ResetQuotaUsed(ctx, key.ID); err != nil {
			return err
		}
	}
	if resetRateLimit {
		if err := r.ResetRateLimitUsage(ctx, key.ID); err != nil {
			return err
		}
	}
	return nil
...
func (r *apiKeyRepository) ListByUserID(ctx context.Context, userID int64, params pagination.PaginationParams, filters service.APIKeyListFilters) ([]service.APIKey, *pagination.PaginationResult, error) {
	...
	r.attachAdaptiveRoutingPreferencesSlice(ctx, outKeys)
...
func (r *apiKeyRepository) attachAdaptiveRoutingPreferencesSlice(ctx context.Context, keys []service.APIKey) {
	if len(keys) == 0 {
		return
	}
	...
	r.loadAdaptiveRoutingPreferences(ctx, ptrs)
}
...
// This is post-delivery accounting: admission limits must never roll it back.
...
// ResetQuotaUsed is the explicit accounting operation for clearing consumed quota.
// Configuration updates deliberately do not write quota_used, so a stale API key
// snapshot cannot erase usage recorded by a concurrent request.
...
// IncrementRateLimitUsage atomically records delivered usage in every window.
// Limits are admission checks and do not reject already delivered usage.
...
// ResetRateLimitUsage is the explicit accounting operation for clearing all
// rolling-window counters and their anchors.
...
func (r *apiKeyRepository) persistAdaptiveRoutingPreference(ctx context.Context, id int64, pref string) error {
	if r == nil || r.sql == nil || id <= 0 {
		return nil
	}
	normalized, err := service.NormalizeAdaptiveRoutingPreference(pref)
	if err != nil {
		normalized = service.AdaptiveRoutingPreferenceIntelligence
	}
	_, err = r.sql.ExecContext(ctx, `
		UPDATE api_keys
		   SET adaptive_routing_preference = $1
		 WHERE id = $2 AND deleted_at IS NULL`,
		normalized, id)
	return err
}

func (r *apiKeyRepository) persistAdaptiveMaxRateMultiplier(ctx context.Context, id int64, maxRate *float64) error {
	if r == nil || r.sql == nil || id <= 0 {
		return nil
	}
	normalized := service.NormalizeAdaptiveMaxRateMultiplier(maxRate)
	_, err := r.sql.ExecContext(ctx, `
		UPDATE api_keys
		   SET adaptive_max_rate_multiplier = $1
		 WHERE id = $2 AND deleted_at IS NULL`,
		nullFloat64(normalized), id)
	return err
}

func (r *apiKeyRepository) persistAntiStallTier(ctx context.Context, id int64, tier string) error {
	if r == nil || r.sql == nil || id <= 0 {
		return nil
	}
	normalized := service.NormalizeAntiStallTier(tier)
	_, err := r.sql.ExecContext(ctx, `
		UPDATE api_keys
		   SET anti_stall_tier = $1
		 WHERE id = $2 AND deleted_at IS NULL`,
		normalized, id)
	return err
}

func (r *apiKeyRepository) persistAdaptivePassiveFailover(ctx context.Context, id int64, enabled bool) error {
	if r == nil || r.sql == nil || id <= 0 {
		return nil
	}
	_, err := r.sql.ExecContext(ctx, `
		UPDATE api_keys
		   SET adaptive_passive_failover_enabled = $1
		 WHERE id = $2 AND deleted_at IS NULL`,
		enabled, id)
	return err
}
...
func (r *apiKeyRepository) loadAdaptiveRoutingPreferences(ctx context.Context, keys []*service.APIKey) {
	...
	for _, k := range keys {
		...
		ids = append(ids, k.ID)
		index[k.ID] = k
		if k.AdaptiveRoutingPreference == "" {
			k.AdaptiveRoutingPreference = service.AdaptiveRoutingPreferenceIntelligence
		}
		if k.AntiStallTier == "" {
			k.AntiStallTier = service.AntiStallTierOff
		}
	}
	if len(ids) == 0 {
		return
	...
	rows, err := r.sql.QueryContext(ctx, `
		SELECT id,
		       COALESCE(adaptive_routing_preference, 'intelligence'),
		       adaptive_max_rate_multiplier,
		       COALESCE(anti_stall_tier, 'off'),
		       COALESCE(adaptive_passive_failover_enabled, FALSE)
		  FROM api_keys
	...
	for rows.Next() {
		...
		var antiTier string
		var passiveFailover bool
		if err := rows.Scan(&id, &pref, &maxRate, &antiTier, &passiveFailover); err != nil {
			return
		}
		if k := index[id]; k != nil {
			if normalized, nerr := service.NormalizeAdaptiveRoutingPreference(pref); nerr == nil {
				k.AdaptiveRoutingPreference = normalized
			}
			if maxRate.Valid {
				v := maxRate.Float64
				k.AdaptiveMaxRateMultiplier = service.NormalizeAdaptiveMaxRateMultiplier(&v)
			} else {
				k.AdaptiveMaxRateMultiplier = nil
			}
			k.AntiStallTier = service.NormalizeAntiStallTier(antiTier)
			k.AdaptivePassiveFailoverEnabled = passiveFailover
		}
	}
}

// IsAdaptiveParentGroup reports whether groupID is configured as an Adaptive parent.
func (r *apiKeyRepository) IsAdaptiveParentGroup(ctx context.Context, groupID int64) bool {
	if r == nil || r.sql == nil || groupID <= 0 {
		return false
	}
	rows, err := r.sql.QueryContext(ctx, `
		SELECT EXISTS(
			SELECT 1 FROM adaptive_group_configs c
			 WHERE c.parent_group_id = $1 AND c.enabled = TRUE
		)`, groupID)
	if err != nil {
		return false
	}
...
// LoadAdaptiveLeafRateRanges returns parentID -> [minRate, maxRate] across enabled
// active leaf memberships for Adaptive parent pools.
func (r *apiKeyRepository) LoadAdaptiveLeafRateRanges(ctx context.Context, parentIDs []int64) (map[int64][2]float64, error) {
	out := make(map[int64][2]float64)
	if r == nil || r.sql == nil || len(parentIDs) == 0 {
		return out, nil
	}
	rows, err := r.sql.QueryContext(ctx, `
		SELECT c.parent_group_id,
		       MIN(g.rate_multiplier)::float8,
		       MAX(g.rate_multiplier)::float8
		  FROM adaptive_group_configs c
		  JOIN adaptive_group_memberships m ON m.config_id = c.id AND m.enabled = TRUE
		  JOIN groups g ON g.id = m.leaf_group_id
		               AND g.deleted_at IS NULL
		               AND g.status = 'active'
		 WHERE c.enabled = TRUE
		   AND c.parent_group_id = ANY($1)
		 GROUP BY c.parent_group_id
	`, pq.Array(parentIDs))
	if err != nil {
		return out, err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&parentID, &minR, &maxR); err != nil {
			return out, err
		}
		out[parentID] = [2]float64{minR, maxR}
	}
	return out, rows.Err()
}
...
func apiKeyEntityToService(m *dbent.APIKey) *service.APIKey {
	...
	out := &service.APIKey{
		...
		AdaptiveRoutingPreference: service.AdaptiveRoutingPreferenceIntelligence,
		BlockOpenAIFast:           m.BlockOpenaiFast,
...
func groupEntityToService(g *dbent.Group) *service.Group {
	...
	return &service.Group{
		...
		CompetitiveConcurrency: g.CompetitiveConcurrency,
		IsExclusive:            g.IsExclusive,
		// Default true until attachUserVisible overlays the DB column.
		UserVisible:                     true,
		Status:                          g.Status,
		Hydrated:                        true,
