...
type OIDCConnectConfig struct {
	...
	UserInfoIDPath       string `mapstructure:"userinfo_id_path"`
	UserInfoUsernamePath string `mapstructure:"userinfo_username_path"`
	UserInfoDisplayPath  string `mapstructure:"userinfo_display_name_path"`
}
...
func hasExplicitConfigOrAnyEnv(configKey string, envKeys ...string) bool {
	if viper.InConfig(configKey) {
		return true
	}
...
func shouldApplyLegacyOIDCEnv(configKey, envKey string) bool {
	if viper.InConfig(configKey) {
		return false
	}
	_, hasNewEnv := os.LookupEnv(envKey)
...
func firstOIDCLegacyEnvValue(envKeys ...string) (string, bool) {
	for _, envKey := range envKeys {
...
func normalizeOIDCTokenAuthMethod(raw string) string {
	switch strings.ToLower(strings.TrimSpace(raw)) {
	case "0", "auto":
		return ""
	case "1", "params", "param", "post", "client_secret_post":
		return "client_secret_post"
	case "2", "header", "headers", "basic", "client_secret_basic":
...
func applyLegacyOIDCString(target *string, configKey, envKey string, aliases ...string) {
	if target == nil || !shouldApplyLegacyOIDCEnv(configKey, envKey) {
		return
	}
	if value, ok := firstOIDCLegacyEnvValue(aliases...); ok {
		*target = value
	}
}

func applyLegacyOIDCBool(target *bool, configKey, envKey string, aliases ...string) {
	if target == nil || !shouldApplyLegacyOIDCEnv(configKey, envKey) {
		return
	}
	raw, ok := firstOIDCLegacyEnvValue(aliases...)
	if !ok {
		return
	}
...
func applyLegacyOIDCInt(target *int, configKey, envKey string, aliases ...string) {
	if target == nil || !shouldApplyLegacyOIDCEnv(configKey, envKey) {
		return
	}
	raw, ok := firstOIDCLegacyEnvValue(aliases...)
...
func applyLegacyOIDCConnectEnvCompatibility(cfg *OIDCConnectConfig) {
	if cfg == nil {
		return
	}

	applyLegacyOIDCBool(&cfg.Enabled, "oidc_connect.enabled", "OIDC_CONNECT_ENABLED",
		"OIDC_ENABLED", "OIDC_OAUTH_ENABLED", "OAUTH_OIDC_ENABLED", "OPENID_CONNECT_ENABLED")
	applyLegacyOIDCString(&cfg.ProviderName, "oidc_connect.provider_name", "OIDC_CONNECT_PROVIDER_NAME",
		"OIDC_PROVIDER_NAME", "OIDC_NAME", "OIDC_DISPLAY_NAME", "OIDC_LABEL", "OIDC_TITLE", "OIDC_LOGIN_NAME",
		"OAUTH_OIDC_PROVIDER_NAME", "OAUTH_OIDC_DISPLAY_NAME", "OPENID_CONNECT_PROVIDER_NAME", "OPENID_CONNECT_DISPLAY_NAME")
	applyLegacyOIDCString(&cfg.ClientID, "oidc_connect.client_id", "OIDC_CONNECT_CLIENT_ID",
		"OIDC_CLIENT_ID", "OIDC_CLIENTID", "OIDC_APP_ID", "OIDC_CLIENT_KEY",
		"OAUTH_OIDC_CLIENT_ID", "OPENID_CONNECT_CLIENT_ID")
	applyLegacyOIDCString(&cfg.ClientSecret, "oidc_connect.client_secret", "OIDC_CONNECT_CLIENT_SECRET",
		"OIDC_CLIENT_SECRET", "OIDC_CLIENTSECRET", "OIDC_APP_SECRET", "OIDC_SECRET",
		"OAUTH_OIDC_CLIENT_SECRET", "OPENID_CONNECT_CLIENT_SECRET")
	applyLegacyOIDCString(&cfg.IssuerURL, "oidc_connect.issuer_url", "OIDC_CONNECT_ISSUER_URL",
		"OIDC_ISSUER_URL", "OIDC_ISSUER", "OAUTH_OIDC_ISSUER_URL", "OPENID_CONNECT_ISSUER_URL")
	applyLegacyOIDCString(&cfg.DiscoveryURL, "oidc_connect.discovery_url", "OIDC_CONNECT_DISCOVERY_URL",
		"OIDC_DISCOVERY", "OIDC_DISCOVERY_URL", "OIDC_WELL_KNOWN", "OIDC_WELL_KNOWN_URL",
		"OIDC_WELLKNOWN", "OIDC_WELLKNOWN_URL", "OIDC_OPENID_CONFIGURATION_URL", "OIDC_OPENID_CONFIG_URL",
		"OAUTH_OIDC_DISCOVERY_URL", "OAUTH_OIDC_WELL_KNOWN_URL", "OPENID_CONNECT_DISCOVERY_URL")
	applyLegacyOIDCString(&cfg.AuthorizeURL, "oidc_connect.authorize_url", "OIDC_CONNECT_AUTHORIZE_URL",
		"OIDC_AUTHORIZE_URL", "OIDC_AUTH_URL", "OIDC_AUTHORIZATION_URL", "OIDC_AUTHORIZATION_ENDPOINT",
		"OAUTH_OIDC_AUTHORIZE_URL", "OPENID_CONNECT_AUTHORIZE_URL")
	applyLegacyOIDCString(&cfg.TokenURL, "oidc_connect.token_url", "OIDC_CONNECT_TOKEN_URL",
		"OIDC_TOKEN_URL", "OIDC_TOKEN_ENDPOINT", "OAUTH_OIDC_TOKEN_URL", "OPENID_CONNECT_TOKEN_URL")
	applyLegacyOIDCString(&cfg.UserInfoURL, "oidc_connect.userinfo_url", "OIDC_CONNECT_USERINFO_URL",
		"OIDC_USERINFO_URL", "OIDC_USER_INFO_URL", "OIDC_USERINFO_ENDPOINT", "OIDC_USER_INFO_ENDPOINT", "OAUTH_OIDC_USERINFO_URL", "OPENID_CONNECT_USERINFO_URL")
	applyLegacyOIDCString(&cfg.JWKSURL, "oidc_connect.jwks_url", "OIDC_CONNECT_JWKS_URL",
		"OIDC_JWKS_URL", "OIDC_JWKS_URI", "OIDC_JWKS_ENDPOINT", "OAUTH_OIDC_JWKS_URL", "OPENID_CONNECT_JWKS_URL")
	applyLegacyOIDCString(&cfg.Scopes, "oidc_connect.scopes", "OIDC_CONNECT_SCOPES",
		"OIDC_SCOPES", "OIDC_SCOPE", "OAUTH_OIDC_SCOPES", "OPENID_CONNECT_SCOPES")
	applyLegacyOIDCString(&cfg.RedirectURL, "oidc_connect.redirect_url", "OIDC_CONNECT_REDIRECT_URL",
		"OIDC_REDIRECT_URL", "OIDC_REDIRECT_URI", "OIDC_CALLBACK_URL", "OIDC_CALLBACK_URI", "OIDC_CALLBACK",
		"OAUTH_OIDC_REDIRECT_URL", "OPENID_CONNECT_REDIRECT_URL")
	applyLegacyOIDCString(&cfg.FrontendRedirectURL, "oidc_connect.frontend_redirect_url", "OIDC_CONNECT_FRONTEND_REDIRECT_URL",
		"OIDC_FRONTEND_REDIRECT_URL", "OAUTH_OIDC_FRONTEND_REDIRECT_URL", "OPENID_CONNECT_FRONTEND_REDIRECT_URL")
	applyLegacyOIDCString(&cfg.TokenAuthMethod, "oidc_connect.token_auth_method", "OIDC_CONNECT_TOKEN_AUTH_METHOD",
		"OIDC_TOKEN_AUTH_METHOD", "OIDC_AUTH_STYLE", "OIDC_CONNECT_AUTH_STYLE",
		"OAUTH_OIDC_TOKEN_AUTH_METHOD", "OAUTH_OIDC_AUTH_STYLE",
		"OPENID_CONNECT_TOKEN_AUTH_METHOD", "OPENID_CONNECT_AUTH_STYLE")
	applyLegacyOIDCBool(&cfg.UsePKCE, "oidc_connect.use_pkce", "OIDC_CONNECT_USE_PKCE",
		"OIDC_USE_PKCE", "OAUTH_OIDC_USE_PKCE", "OPENID_CONNECT_USE_PKCE")
	applyLegacyOIDCBool(&cfg.ValidateIDToken, "oidc_connect.validate_id_token", "OIDC_CONNECT_VALIDATE_ID_TOKEN",
		"OIDC_VALIDATE_ID_TOKEN", "OIDC_VALIDATE_IDTOKEN", "OAUTH_OIDC_VALIDATE_ID_TOKEN", "OPENID_CONNECT_VALIDATE_ID_TOKEN")
	applyLegacyOIDCString(&cfg.AllowedSigningAlgs, "oidc_connect.allowed_signing_algs", "OIDC_CONNECT_ALLOWED_SIGNING_ALGS",
		"OIDC_ALLOWED_SIGNING_ALGS", "OIDC_ALLOWED_ALGS", "OAUTH_OIDC_ALLOWED_SIGNING_ALGS", "OPENID_CONNECT_ALLOWED_SIGNING_ALGS")
	applyLegacyOIDCInt(&cfg.ClockSkewSeconds, "oidc_connect.clock_skew_seconds", "OIDC_CONNECT_CLOCK_SKEW_SECONDS",
		"OIDC_CLOCK_SKEW_SECONDS", "OIDC_CLOCK_SKEW", "OAUTH_OIDC_CLOCK_SKEW_SECONDS", "OPENID_CONNECT_CLOCK_SKEW_SECONDS")
	applyLegacyOIDCBool(&cfg.RequireEmailVerified, "oidc_connect.require_email_verified", "OIDC_CONNECT_REQUIRE_EMAIL_VERIFIED",
		"OIDC_REQUIRE_EMAIL_VERIFIED", "OAUTH_OIDC_REQUIRE_EMAIL_VERIFIED", "OPENID_CONNECT_REQUIRE_EMAIL_VERIFIED")
	applyLegacyOIDCString(&cfg.UserInfoEmailPath, "oidc_connect.userinfo_email_path", "OIDC_CONNECT_USERINFO_EMAIL_PATH",
		"OIDC_USERINFO_EMAIL_PATH", "OIDC_USER_INFO_EMAIL_PATH", "OIDC_USERINFO_EMAIL_FIELD", "OIDC_USER_INFO_EMAIL_FIELD",
		"OIDC_EMAIL_FIELD", "OIDC_USER_EMAIL_FIELD",
		"OAUTH_OIDC_USERINFO_EMAIL_PATH", "OAUTH_OIDC_USERINFO_EMAIL_FIELD", "OPENID_CONNECT_USERINFO_EMAIL_PATH", "OPENID_CONNECT_USERINFO_EMAIL_FIELD")
	applyLegacyOIDCString(&cfg.UserInfoIDPath, "oidc_connect.userinfo_id_path", "OIDC_CONNECT_USERINFO_ID_PATH",
		"OIDC_USERINFO_ID_PATH", "OIDC_USER_INFO_ID_PATH", "OIDC_USERINFO_ID_FIELD", "OIDC_USER_INFO_ID_FIELD",
		"OIDC_USER_ID_FIELD", "OIDC_USERID_FIELD", "OIDC_ID_FIELD", "OIDC_SUBJECT_FIELD", "OIDC_SUB_FIELD",
		"OAUTH_OIDC_USERINFO_ID_PATH", "OAUTH_OIDC_USERINFO_ID_FIELD", "OPENID_CONNECT_USERINFO_ID_PATH", "OPENID_CONNECT_USERINFO_ID_FIELD")
	applyLegacyOIDCString(&cfg.UserInfoUsernamePath, "oidc_connect.userinfo_username_path", "OIDC_CONNECT_USERINFO_USERNAME_PATH",
		"OIDC_USERINFO_USERNAME_PATH", "OIDC_USER_INFO_USERNAME_PATH", "OIDC_USERINFO_USERNAME_FIELD", "OIDC_USER_INFO_USERNAME_FIELD",
		"OIDC_USERNAME_FIELD", "OIDC_USER_NAME_FIELD", "OIDC_PREFERRED_USERNAME_FIELD",
		"OAUTH_OIDC_USERINFO_USERNAME_PATH", "OAUTH_OIDC_USERINFO_USERNAME_FIELD", "OPENID_CONNECT_USERINFO_USERNAME_PATH", "OPENID_CONNECT_USERINFO_USERNAME_FIELD")
	applyLegacyOIDCString(&cfg.UserInfoDisplayPath, "oidc_connect.userinfo_display_name_path", "OIDC_CONNECT_USERINFO_DISPLAY_NAME_PATH",
		"OIDC_USERINFO_DISPLAY_NAME_PATH", "OIDC_USER_INFO_DISPLAY_NAME_PATH", "OIDC_USERINFO_DISPLAY_NAME_FIELD", "OIDC_USER_INFO_DISPLAY_NAME_FIELD",
		"OIDC_DISPLAY_NAME_FIELD", "OIDC_NAME_FIELD", "OIDC_USER_DISPLAY_NAME_FIELD",
		"OAUTH_OIDC_USERINFO_DISPLAY_NAME_PATH", "OAUTH_OIDC_USERINFO_DISPLAY_NAME_FIELD", "OPENID_CONNECT_USERINFO_DISPLAY_NAME_PATH", "OPENID_CONNECT_USERINFO_DISPLAY_NAME_FIELD")
}
...
type ServerConfig struct {
	...
	ShutdownTimeoutSeconds   int       `mapstructure:"shutdown_timeout"`      // graceful shutdown 超时（秒），给在途流排空
}
...
type GatewayConfig struct {
	...
	// OpenAIFallbackResponseHeaderTimeout: API key 配置了备用分组时，非最后一次分组尝试等待
	// OpenAI/Codex 上游响应头的超时时间（秒）。0 表示不额外收紧，避免首字过慢时迟迟不切用户备用分组。
	...
	// Grok holds Grok-specific gateway scheduling knobs (free-tier soft gate,
	// password-to-SSO OAuth flow).
	...
	// AnthropicFirstOutputTimeoutSeconds: Anthropic /v1/messages 首个语义输出超时（秒），0表示禁用。
	// 超时按 UpstreamFailoverError 处理以触发换账号重试。
	AnthropicFirstOutputTimeoutSeconds int `mapstructure:"anthropic_first_output_timeout_seconds"`
	...
	// keys whose primary group is an Adaptive pool. Default false until pools
	// are migrated and the 15% settlement path is verified in production.
	...
	// 账户切换最大次数（遇到上游错误时切换到其他账户的次数上限）。0 表示关闭系统自动切换。
	OpenAIPool5xxFailureThreshold       int  `mapstructure:"openai_pool_5xx_failure_threshold"`
	OpenAIPool5xxFailureWindowSeconds   int  `mapstructure:"openai_pool_5xx_failure_window_seconds"`
	OpenAIPool5xxSampleIntervalSeconds  int  `mapstructure:"openai_pool_5xx_sample_interval_seconds"`
	OpenAIPool5xxCooldownSeconds        int  `mapstructure:"openai_pool_5xx_cooldown_seconds"`
	OpenAIPool5xxRecoveryProbeEnabled   bool `mapstructure:"openai_pool_5xx_recovery_probe_enabled"`
	OpenAIPool5xxRecoveryScanSeconds    int  `mapstructure:"openai_pool_5xx_recovery_scan_seconds"`
	OpenAIPool5xxRecoveryProbeSeconds   int  `mapstructure:"openai_pool_5xx_recovery_probe_seconds"`
	OpenAIPool5xxRecoveryTimeoutSeconds int  `mapstructure:"openai_pool_5xx_recovery_timeout_seconds"`
	OpenAIPool5xxRecoveryConcurrency    int  `mapstructure:"openai_pool_5xx_recovery_concurrency"`
	OpenAIPool5xxRecoveryBatchSize      int  `mapstructure:"openai_pool_5xx_recovery_batch_size"`
	MaxAccountSwitches                  int  `mapstructure:"max_account_switches"`
	// Gemini 账户切换最大次数（Gemini 平台单独配置）。0 表示关闭系统自动切换。
	MaxAccountSwitchesGemini int `mapstructure:"max_account_switches_gemini"`

	// AccountFailureGuard 账号失败守护（连续上游失败→临时冷却→定时探测恢复）。
	AccountFailureGuardEnabled                 bool `mapstructure:"account_failure_guard_enabled"`
	AccountFailureGuardThreshold               int  `mapstructure:"account_failure_guard_threshold"`
	AccountFailureGuardWindowSeconds           int  `mapstructure:"account_failure_guard_window_seconds"`
	AccountFailureGuardSampleIntervalSeconds   int  `mapstructure:"account_failure_guard_sample_interval_seconds"`
	AccountFailureGuardCooldownSeconds         int  `mapstructure:"account_failure_guard_cooldown_seconds"`
	AccountFailureGuardRecoveryIntervalSeconds int  `mapstructure:"account_failure_guard_recovery_interval_seconds"`
	AccountFailureGuardDebounceSeconds         int  `mapstructure:"account_failure_guard_debounce_seconds"`
	AccountFailureGuardMaxMultiplier           int  `mapstructure:"account_failure_guard_max_multiplier"`
	AccountFailureGuardRecoveryBatchSize       int  `mapstructure:"account_failure_guard_recovery_batch_size"`
	AccountFailureGuardProbeTimeoutSeconds     int  `mapstructure:"account_failure_guard_probe_timeout_seconds"`
...
//
//   - password_auth_enabled: optional password-to-SSO OAuth flow (default false).
//   - free_quota_soft_gate_enabled: local rolling-window scheduling guard for
//     explicitly free Grok OAuth accounts only.
//   - free_quota_token_limit: nominal rolling-window allowance.
...
type GatewaySchedulingConfig struct {
	// UpstreamRouteHealthEnabled enables monitor-driven account suppression.
	// It is opt-in until the route-health snapshot is shared across instances.
	UpstreamRouteHealthEnabled bool `mapstructure:"upstream_route_health_enabled"`
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	applyLegacyOIDCConnectEnvCompatibility(&cfg.OIDC)
	...
	cfg.OIDC.TokenAuthMethod = normalizeOIDCTokenAuthMethod(cfg.OIDC.TokenAuthMethod)
	...
	cfg.OIDC.UserInfoDisplayPath = strings.TrimSpace(cfg.OIDC.UserInfoDisplayPath)
	cfg.OIDC.UsePKCEExplicit = hasExplicitConfigOrAnyEnv("oidc_connect.use_pkce",
		"OIDC_CONNECT_USE_PKCE", "OIDC_USE_PKCE", "OAUTH_OIDC_USE_PKCE", "OPENID_CONNECT_USE_PKCE")
	cfg.OIDC.ValidateIDTokenExplicit = hasExplicitConfigOrAnyEnv("oidc_connect.validate_id_token",
		"OIDC_CONNECT_VALIDATE_ID_TOKEN", "OIDC_VALIDATE_ID_TOKEN", "OIDC_VALIDATE_IDTOKEN", "OAUTH_OIDC_VALIDATE_ID_TOKEN", "OPENID_CONNECT_VALIDATE_ID_TOKEN")
	...
	// Without an explicit non-empty trusted proxy list, never honor client-controlled
	// forwarded headers for security-sensitive IP resolution.
	...
	// TOTP encryption key: prefer explicit config. Empty key is left empty here and
	// resolved/persisted later via security_secrets (see ensureBootstrapSecrets).
	// Never generate an ephemeral in-memory key that dies on restart.
	...
	if cfg.Totp.EncryptionKey != "" {
		...
		if !allowMissingJWTSecret {
			// Full load path without bootstrap: require a durable key (config or prior persistence).
			// Bootstrap/server path allows empty until DB persistence fills it.
			slog.Warn("totp.encryption_key is empty; will require durable secret from security_secrets or explicit config")
		}
	}
...
func setDefaults() {
	...
	viper.SetDefault("server.shutdown_timeout", 30) // graceful shutdown 30s，给在途流排空
	// H2C 默认配置
	...
	// Fail-closed defaults: private/metadata hosts and plain HTTP are denied unless explicitly enabled.
	...
	// Do not trust client-controlled forwarded headers unless operators opt in and configure trusted_proxies.
	...
	viper.SetDefault("oidc_connect.userinfo_display_name_path", "")
	...
	viper.SetDefault("redis.pool_size", 256)
	viper.SetDefault("redis.min_idle_conns", 32)
	...
	viper.SetDefault("gateway.openai_fallback_response_header_timeout", 12)
	...
	viper.SetDefault("gateway.anthropic_first_output_timeout_seconds", 60)
	// Default off: upstream error bodies often echo API keys/tokens; enable explicitly when needed.
	viper.SetDefault("gateway.log_upstream_error_body", false)
	viper.SetDefault("gateway.log_upstream_error_body_max_bytes", 2048)
	viper.SetDefault("gateway.inject_beta_for_apikey", false)
	viper.SetDefault("gateway.failover_on_400", false)
	viper.SetDefault("gateway.max_account_switches", 3)
	viper.SetDefault("gateway.max_account_switches_gemini", 3)
	viper.SetDefault("gateway.account_failure_guard_enabled", true)
	viper.SetDefault("gateway.account_failure_guard_threshold", 5)
	viper.SetDefault("gateway.account_failure_guard_window_seconds", 600)
	viper.SetDefault("gateway.account_failure_guard_sample_interval_seconds", 20)
	viper.SetDefault("gateway.account_failure_guard_cooldown_seconds", 600)
	viper.SetDefault("gateway.account_failure_guard_recovery_interval_seconds", 300)
	viper.SetDefault("gateway.account_failure_guard_debounce_seconds", 1800)
	viper.SetDefault("gateway.account_failure_guard_max_multiplier", 8)
	viper.SetDefault("gateway.account_failure_guard_recovery_batch_size", 20)
	viper.SetDefault("gateway.account_failure_guard_probe_timeout_seconds", 15)
	...
	viper.SetDefault("gateway.openai_pool_5xx_failure_threshold", 3)
	viper.SetDefault("gateway.openai_pool_5xx_failure_window_seconds", 60)
	viper.SetDefault("gateway.openai_pool_5xx_sample_interval_seconds", 10)
	viper.SetDefault("gateway.openai_pool_5xx_cooldown_seconds", 300)
	viper.SetDefault("gateway.openai_pool_5xx_recovery_probe_enabled", true)
	viper.SetDefault("gateway.openai_pool_5xx_recovery_scan_seconds", 10)
	viper.SetDefault("gateway.openai_pool_5xx_recovery_probe_seconds", 15)
	viper.SetDefault("gateway.openai_pool_5xx_recovery_timeout_seconds", 8)
	viper.SetDefault("gateway.openai_pool_5xx_recovery_concurrency", 4)
	viper.SetDefault("gateway.openai_pool_5xx_recovery_batch_size", 32)
	...
	viper.SetDefault("gateway.max_idle_conns", 8192)          // 最大空闲连接总数（高并发场景默认）
	viper.SetDefault("gateway.max_idle_conns_per_host", 2048) // 每主机最大空闲连接（<= max_conns_per_host，修正语义矛盾）
	viper.SetDefault("gateway.max_conns_per_host", 2048)      // 每主机最大连接数（含活跃；流式/HTTP1.1 场景可调大）
	viper.SetDefault("gateway.idle_conn_timeout_seconds", 90) // 空闲连接超时（秒）
	viper.SetDefault("gateway.max_upstream_clients", 5000)
	viper.SetDefault("gateway.client_idle_ttl_seconds", 900)
	viper.SetDefault("gateway.concurrency_slot_ttl_minutes", 60) // 并发槽位过期时间（支持超长请求，减少长请求超卖）
	viper.SetDefault("gateway.stream_data_interval_timeout", 180)
	viper.SetDefault("gateway.stream_keepalive_interval", 10)
	viper.SetDefault("gateway.image_stream_data_interval_timeout", 900)
	viper.SetDefault("gateway.image_stream_keepalive_interval", 10)
	viper.SetDefault("gateway.image_nonstream_keepalive_interval", 0)
	viper.SetDefault("gateway.max_line_size", 16*1024*1024)
	viper.SetDefault("gateway.scheduling.sticky_session_max_waiting", 3)
	viper.SetDefault("gateway.scheduling.upstream_route_health_enabled", false)
...
func (c *Config) Validate() error {
	...
	if !c.Server.TrustedProxiesConfigured || len(c.Server.TrustedProxies) == 0 {
		c.Security.TrustForwardedIPForAPIKeyACL = false
	}
	...
	if c.OIDC.Enabled {
		...
		oidcIssuerURL := strings.TrimSpace(c.OIDC.IssuerURL)
		oidcDiscoveryURL := strings.TrimSpace(c.OIDC.DiscoveryURL)
		if oidcIssuerURL == "" && oidcDiscoveryURL == "" {
			return fmt.Errorf("oidc_connect.issuer_url or oidc_connect.discovery_url is required when oidc_connect.enabled=true")
		}
		...
		if oidcIssuerURL != "" {
			if err := ValidateAbsoluteHTTPURL(oidcIssuerURL); err != nil {
				return fmt.Errorf("oidc_connect.issuer_url invalid: %w", err)
			}
		}
		if oidcDiscoveryURL != "" {
			if err := ValidateAbsoluteHTTPURL(oidcDiscoveryURL); err != nil {
	...
	if c.Gateway.OpenAIFallbackResponseHeaderTimeout < 0 {
		return fmt.Errorf("gateway.openai_fallback_response_header_timeout must be non-negative")
	}
	...
	if c.Gateway.AnthropicFirstOutputTimeoutSeconds < 0 || c.Gateway.AnthropicFirstOutputTimeoutSeconds > 600 ||
		(c.Gateway.AnthropicFirstOutputTimeoutSeconds > 0 && c.Gateway.AnthropicFirstOutputTimeoutSeconds < 5) {
		return fmt.Errorf("gateway.anthropic_first_output_timeout_seconds must be 0 or between 5-600 seconds")
	}
	...
	if c.Gateway.OpenAIPool5xxFailureThreshold < 2 {
		return fmt.Errorf("gateway.openai_pool_5xx_failure_threshold must be at least 2")
	}
	if c.Gateway.OpenAIPool5xxFailureWindowSeconds <= 0 {
		return fmt.Errorf("gateway.openai_pool_5xx_failure_window_seconds must be positive")
	}
	if c.Gateway.OpenAIPool5xxSampleIntervalSeconds <= 0 {
		return fmt.Errorf("gateway.openai_pool_5xx_sample_interval_seconds must be positive")
	}
	minimumWindow := int64(c.Gateway.OpenAIPool5xxFailureThreshold-1) * int64(c.Gateway.OpenAIPool5xxSampleIntervalSeconds)
	if minimumWindow >= int64(c.Gateway.OpenAIPool5xxFailureWindowSeconds) {
		return fmt.Errorf("gateway.openai_pool_5xx_sample_interval_seconds is too large for the threshold and failure window")
	}
	if c.Gateway.OpenAIPool5xxCooldownSeconds <= 0 {
		return fmt.Errorf("gateway.openai_pool_5xx_cooldown_seconds must be positive")
	}
	if c.Gateway.OpenAIPool5xxRecoveryProbeEnabled {
		if c.Gateway.OpenAIPool5xxRecoveryScanSeconds <= 0 {
			return fmt.Errorf("gateway.openai_pool_5xx_recovery_scan_seconds must be positive")
		}
		if c.Gateway.OpenAIPool5xxRecoveryProbeSeconds <= 0 {
			return fmt.Errorf("gateway.openai_pool_5xx_recovery_probe_seconds must be positive")
		}
		if c.Gateway.OpenAIPool5xxRecoveryTimeoutSeconds <= 0 {
			return fmt.Errorf("gateway.openai_pool_5xx_recovery_timeout_seconds must be positive")
		}
		if c.Gateway.OpenAIPool5xxRecoveryConcurrency <= 0 || c.Gateway.OpenAIPool5xxRecoveryConcurrency > 32 {
			return fmt.Errorf("gateway.openai_pool_5xx_recovery_concurrency must be between 1 and 32")
		}
		if c.Gateway.OpenAIPool5xxRecoveryBatchSize <= 0 || c.Gateway.OpenAIPool5xxRecoveryBatchSize > 1000 {
			return fmt.Errorf("gateway.openai_pool_5xx_recovery_batch_size must be between 1 and 1000")
		}
	}
	...
	if c.Gateway.MaxAccountSwitchesGemini < 0 {
		return fmt.Errorf("gateway.max_account_switches_gemini must be non-negative")
	}
...
func GetServerAddress() string {
	return GetServerAddressWithDefaultHost("0.0.0.0")
}

// GetSetupServerAddress returns the bind address for the unauthenticated install wizard.
// Defaults to loopback to prevent remote takeover during first-run setup.
// Operators may override via SERVER_HOST / config.yaml when they also set SETUP_TOKEN.
func GetSetupServerAddress() string {
	return GetServerAddressWithDefaultHost("127.0.0.1")
}

// GetServerAddressWithDefaultHost is like GetServerAddress but uses the given default host.
func GetServerAddressWithDefaultHost(defaultHost string) string {
