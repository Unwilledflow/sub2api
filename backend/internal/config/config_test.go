...
func TestLoadDefaultGatewayFailoverIsOptIn(t *testing.T) {
	resetViperWithJWTSecret(t)

	cfg, err := Load()
	require.NoError(t, err)
	require.False(t, cfg.Gateway.FailoverOn400)
	require.Equal(t, 0, cfg.Gateway.MaxAccountSwitches)
	require.Equal(t, 0, cfg.Gateway.MaxAccountSwitchesGemini)
	require.True(t, cfg.Gateway.OpenAIScheduler.StickyEscapeEnabled)
	require.False(t, cfg.Security.ProxyFallback.AllowDirectOnError)
}
...
func TestLoadDefaultOpenAIFallbackResponseHeaderTimeout(t *testing.T) {
	resetViperWithJWTSecret(t)

	cfg, err := Load()
	require.NoError(t, err)
	require.Equal(t, 12, cfg.Gateway.OpenAIFallbackResponseHeaderTimeout)
}

func TestLoadOpenAIFallbackResponseHeaderTimeoutFromEnv(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("GATEWAY_OPENAI_FALLBACK_RESPONSE_HEADER_TIMEOUT", "8")

	cfg, err := Load()
	require.NoError(t, err)
	require.Equal(t, 8, cfg.Gateway.OpenAIFallbackResponseHeaderTimeout)
}

func TestLoadDefaultOpenAIPool5xxCooldown(t *testing.T) {
	resetViperWithJWTSecret(t)

	cfg, err := Load()
	require.NoError(t, err)
	require.Equal(t, 3, cfg.Gateway.OpenAIPool5xxFailureThreshold)
	require.Equal(t, 60, cfg.Gateway.OpenAIPool5xxFailureWindowSeconds)
	require.Equal(t, 10, cfg.Gateway.OpenAIPool5xxSampleIntervalSeconds)
	require.Equal(t, 300, cfg.Gateway.OpenAIPool5xxCooldownSeconds)
	require.True(t, cfg.Gateway.OpenAIPool5xxRecoveryProbeEnabled)
	require.Equal(t, 10, cfg.Gateway.OpenAIPool5xxRecoveryScanSeconds)
	require.Equal(t, 15, cfg.Gateway.OpenAIPool5xxRecoveryProbeSeconds)
	require.Equal(t, 8, cfg.Gateway.OpenAIPool5xxRecoveryTimeoutSeconds)
	require.Equal(t, 4, cfg.Gateway.OpenAIPool5xxRecoveryConcurrency)
	require.Equal(t, 32, cfg.Gateway.OpenAIPool5xxRecoveryBatchSize)
}

func TestLoadOpenAIPool5xxCooldownFromEnv(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("GATEWAY_OPENAI_POOL_5XX_FAILURE_THRESHOLD", "5")
	t.Setenv("GATEWAY_OPENAI_POOL_5XX_FAILURE_WINDOW_SECONDS", "90")
	t.Setenv("GATEWAY_OPENAI_POOL_5XX_SAMPLE_INTERVAL_SECONDS", "15")
	t.Setenv("GATEWAY_OPENAI_POOL_5XX_COOLDOWN_SECONDS", "420")
	t.Setenv("GATEWAY_OPENAI_POOL_5XX_RECOVERY_PROBE_ENABLED", "true")
	t.Setenv("GATEWAY_OPENAI_POOL_5XX_RECOVERY_SCAN_SECONDS", "12")
	t.Setenv("GATEWAY_OPENAI_POOL_5XX_RECOVERY_PROBE_SECONDS", "18")
	t.Setenv("GATEWAY_OPENAI_POOL_5XX_RECOVERY_TIMEOUT_SECONDS", "9")
	t.Setenv("GATEWAY_OPENAI_POOL_5XX_RECOVERY_CONCURRENCY", "3")
	t.Setenv("GATEWAY_OPENAI_POOL_5XX_RECOVERY_BATCH_SIZE", "24")

	cfg, err := Load()
	require.NoError(t, err)
	require.Equal(t, 5, cfg.Gateway.OpenAIPool5xxFailureThreshold)
	require.Equal(t, 90, cfg.Gateway.OpenAIPool5xxFailureWindowSeconds)
	require.Equal(t, 15, cfg.Gateway.OpenAIPool5xxSampleIntervalSeconds)
	require.Equal(t, 420, cfg.Gateway.OpenAIPool5xxCooldownSeconds)
	require.True(t, cfg.Gateway.OpenAIPool5xxRecoveryProbeEnabled)
	require.Equal(t, 12, cfg.Gateway.OpenAIPool5xxRecoveryScanSeconds)
	require.Equal(t, 18, cfg.Gateway.OpenAIPool5xxRecoveryProbeSeconds)
	require.Equal(t, 9, cfg.Gateway.OpenAIPool5xxRecoveryTimeoutSeconds)
	require.Equal(t, 3, cfg.Gateway.OpenAIPool5xxRecoveryConcurrency)
	require.Equal(t, 24, cfg.Gateway.OpenAIPool5xxRecoveryBatchSize)
}
...
func TestLoadOIDCConnectConfigFromLegacyEnv(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("OIDC_ENABLED", "true")
	t.Setenv("OIDC_PROVIDER_NAME", "Legacy SSO")
	t.Setenv("OIDC_CLIENT_ID", "legacy-client")
	t.Setenv("OIDC_CLIENT_SECRET", "legacy-secret")
	t.Setenv("OIDC_WELL_KNOWN", "https://issuer.example.com/.well-known/openid-configuration")
	t.Setenv("OIDC_CALLBACK_URL", "https://sub.example.com/api/v1/auth/oauth/oidc/callback")
	t.Setenv("OIDC_FRONTEND_REDIRECT_URL", "/auth/oidc/callback")
	t.Setenv("OIDC_TOKEN_AUTH_METHOD", "client_secret_basic")
	t.Setenv("OIDC_USE_PKCE", "false")
	t.Setenv("OIDC_VALIDATE_ID_TOKEN", "false")
	t.Setenv("OIDC_CLOCK_SKEW", "90")
	t.Setenv("OIDC_USERINFO_EMAIL_PATH", "profile.email")
	t.Setenv("OIDC_USERINFO_ID_PATH", "profile.id")
	t.Setenv("OIDC_USERINFO_USERNAME_PATH", "profile.username")

	cfg, err := Load()
	require.NoError(t, err)
	require.True(t, cfg.OIDC.Enabled)
	require.Equal(t, "Legacy SSO", cfg.OIDC.ProviderName)
	require.Equal(t, "legacy-client", cfg.OIDC.ClientID)
	require.Equal(t, "legacy-secret", cfg.OIDC.ClientSecret)
	require.Equal(t, "https://issuer.example.com/.well-known/openid-configuration", cfg.OIDC.DiscoveryURL)
	require.Equal(t, "https://sub.example.com/api/v1/auth/oauth/oidc/callback", cfg.OIDC.RedirectURL)
	require.Equal(t, "/auth/oidc/callback", cfg.OIDC.FrontendRedirectURL)
	require.Equal(t, "client_secret_basic", cfg.OIDC.TokenAuthMethod)
	require.False(t, cfg.OIDC.UsePKCE)
	require.False(t, cfg.OIDC.ValidateIDToken)
	require.True(t, cfg.OIDC.UsePKCEExplicit)
	require.True(t, cfg.OIDC.ValidateIDTokenExplicit)
	require.Equal(t, 90, cfg.OIDC.ClockSkewSeconds)
	require.Equal(t, "profile.email", cfg.OIDC.UserInfoEmailPath)
	require.Equal(t, "profile.id", cfg.OIDC.UserInfoIDPath)
	require.Equal(t, "profile.username", cfg.OIDC.UserInfoUsernamePath)
}

func TestLoadOIDCConnectConfigFromServerStyleAliases(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("OIDC_ENABLED", "true")
	t.Setenv("OIDC_DISPLAY_NAME", "Server SSO")
	t.Setenv("OIDC_CLIENTID", "server-client")
	t.Setenv("OIDC_CLIENTSECRET", "server-secret")
	t.Setenv("OIDC_WELLKNOWN_URL", "https://issuer.example.com/.well-known/openid-configuration")
	t.Setenv("OIDC_REDIRECT_URI", "https://sub.example.com/api/v1/auth/oauth/oidc/callback")
	t.Setenv("OIDC_FRONTEND_REDIRECT_URL", "/auth/oidc/callback")
	t.Setenv("OIDC_AUTH_URL", "https://issuer.example.com/oauth2/authorize")
	t.Setenv("OIDC_TOKEN_ENDPOINT", "https://issuer.example.com/oauth2/token")
	t.Setenv("OIDC_USER_INFO_ENDPOINT", "https://issuer.example.com/oauth2/userinfo")
	t.Setenv("OIDC_AUTH_STYLE", "2")

	cfg, err := Load()
	require.NoError(t, err)
	require.True(t, cfg.OIDC.Enabled)
	require.Equal(t, "Server SSO", cfg.OIDC.ProviderName)
	require.Equal(t, "server-client", cfg.OIDC.ClientID)
	require.Equal(t, "server-secret", cfg.OIDC.ClientSecret)
	require.Equal(t, "https://issuer.example.com/.well-known/openid-configuration", cfg.OIDC.DiscoveryURL)
	require.Equal(t, "https://sub.example.com/api/v1/auth/oauth/oidc/callback", cfg.OIDC.RedirectURL)
	require.Equal(t, "https://issuer.example.com/oauth2/authorize", cfg.OIDC.AuthorizeURL)
	require.Equal(t, "https://issuer.example.com/oauth2/token", cfg.OIDC.TokenURL)
	require.Equal(t, "https://issuer.example.com/oauth2/userinfo", cfg.OIDC.UserInfoURL)
	require.Equal(t, "client_secret_basic", cfg.OIDC.TokenAuthMethod)
}

func TestLoadOIDCConnectConfigFromCustomOAuthFieldAliases(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("OIDC_ENABLED", "true")
	t.Setenv("OIDC_CLIENT_ID", "custom-provider-client")
	t.Setenv("OIDC_CLIENT_SECRET", "custom-provider-secret")
	t.Setenv("OIDC_WELL_KNOWN", "https://issuer.example.com/.well-known/openid-configuration")
	t.Setenv("OIDC_CALLBACK_URL", "https://sub.example.com/api/v1/auth/oauth/oidc/callback")
	t.Setenv("OIDC_EMAIL_FIELD", "profile.mail")
	t.Setenv("OIDC_USER_ID_FIELD", "profile.uid")
	t.Setenv("OIDC_USERNAME_FIELD", "profile.login")
	t.Setenv("OIDC_DISPLAY_NAME_FIELD", "profile.display_name")

	cfg, err := Load()
	require.NoError(t, err)
	require.True(t, cfg.OIDC.Enabled)
	require.Equal(t, "profile.mail", cfg.OIDC.UserInfoEmailPath)
	require.Equal(t, "profile.uid", cfg.OIDC.UserInfoIDPath)
	require.Equal(t, "profile.login", cfg.OIDC.UserInfoUsernamePath)
	require.Equal(t, "profile.display_name", cfg.OIDC.UserInfoDisplayPath)
}

func TestLoadOIDCConnectCanonicalEnvOverridesLegacyEnv(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("OIDC_ENABLED", "true")
	t.Setenv("OIDC_CLIENT_ID", "legacy-client")
	t.Setenv("OIDC_CONNECT_CLIENT_ID", "canonical-client")
	t.Setenv("OIDC_CLIENT_SECRET", "legacy-secret")
	t.Setenv("OIDC_WELL_KNOWN", "https://issuer.example.com/.well-known/openid-configuration")
	t.Setenv("OIDC_CALLBACK_URL", "https://sub.example.com/api/v1/auth/oauth/oidc/callback")
	...
	require.NoError(t, err)
	require.True(t, cfg.OIDC.Enabled)
	require.Equal(t, "canonical-client", cfg.OIDC.ClientID)
	require.Equal(t, "legacy-secret", cfg.OIDC.ClientSecret)
}
...
func TestLoadDefaultSecurityToggles(t *testing.T) {
	...
	if cfg.Security.URLAllowlist.AllowInsecureHTTP {
		t.Fatalf("URLAllowlist.AllowInsecureHTTP = true, want false (custom hardened default)")
	}
	if cfg.Security.URLAllowlist.AllowPrivateHosts {
		t.Fatalf("URLAllowlist.AllowPrivateHosts = true, want false (custom hardened default)")
	}
...
func TestValidateOIDCAllowsDiscoveryURLWithoutIssuerURL(t *testing.T) {
	resetViperWithJWTSecret(t)
	...
	cfg.OIDC.ClientSecret = "oidc-secret"
	cfg.OIDC.IssuerURL = ""
	cfg.OIDC.DiscoveryURL = "https://issuer.example.com/.well-known/openid-configuration"
	cfg.OIDC.AuthorizeURL = "https://issuer.example.com/auth"
	cfg.OIDC.TokenURL = "https://issuer.example.com/token"
	cfg.OIDC.JWKSURL = "https://issuer.example.com/jwks"
	cfg.OIDC.RedirectURL = "https://example.com/api/v1/auth/oauth/oidc/callback"
	...
	err = cfg.Validate()
	if err != nil {
		t.Fatalf("Validate() expected discovery-only OIDC config to pass, got: %v", err)
	}
}
...
func TestValidateOIDCPublicClientRequiresPKCE(t *testing.T) {
	resetViperWithJWTSecret(t)
	...
	if !strings.Contains(err.Error(), "oidc_connect.use_pkce") {
		t.Fatalf("Validate() expected oidc_connect.use_pkce error, got: %v", err)
	}
}
...
func TestValidateConfigErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:    "gateway openai fallback response header timeout",
			mutate:  func(c *Config) { c.Gateway.OpenAIFallbackResponseHeaderTimeout = -1 },
			wantErr: "gateway.openai_fallback_response_header_timeout",
		},
		{
			name:    "gateway openai pool 5xx failure threshold",
			mutate:  func(c *Config) { c.Gateway.OpenAIPool5xxFailureThreshold = 0 },
			wantErr: "gateway.openai_pool_5xx_failure_threshold",
		},
		{
			name:    "gateway openai pool 5xx failure window",
			mutate:  func(c *Config) { c.Gateway.OpenAIPool5xxFailureWindowSeconds = 0 },
			wantErr: "gateway.openai_pool_5xx_failure_window_seconds",
		},
		{
			name:    "gateway openai pool 5xx sample interval",
			mutate:  func(c *Config) { c.Gateway.OpenAIPool5xxSampleIntervalSeconds = 0 },
			wantErr: "gateway.openai_pool_5xx_sample_interval_seconds",
		},
		{
			name: "gateway openai pool 5xx unreachable threshold",
			mutate: func(c *Config) {
				c.Gateway.OpenAIPool5xxFailureThreshold = 7
				c.Gateway.OpenAIPool5xxFailureWindowSeconds = 60
				c.Gateway.OpenAIPool5xxSampleIntervalSeconds = 10
			},
			wantErr: "gateway.openai_pool_5xx_sample_interval_seconds",
		},
		{
			name:    "gateway openai pool 5xx cooldown",
			mutate:  func(c *Config) { c.Gateway.OpenAIPool5xxCooldownSeconds = 0 },
			wantErr: "gateway.openai_pool_5xx_cooldown_seconds",
		},
		{
			name:    "gateway openai pool 5xx recovery scan",
			mutate:  func(c *Config) { c.Gateway.OpenAIPool5xxRecoveryScanSeconds = 0 },
			wantErr: "gateway.openai_pool_5xx_recovery_scan_seconds",
		},
		{
			name:    "gateway openai pool 5xx recovery concurrency",
			mutate:  func(c *Config) { c.Gateway.OpenAIPool5xxRecoveryConcurrency = 33 },
			wantErr: "gateway.openai_pool_5xx_recovery_concurrency",
		},
		{
