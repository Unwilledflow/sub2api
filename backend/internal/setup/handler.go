...
// When SETUP_TOKEN is set, mutating endpoints require matching X-Setup-Token / setup_token.
// When SETUP_TOKEN is empty, remote clients (non-loopback) are rejected so the unauthenticated
// wizard cannot be taken over from the network.
func setupGuard() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if errMsg := authorizeSetupMutation(c); errMsg != "" {
			response.Error(c, http.StatusForbidden, errMsg)
			c.Abort()
			return
		}
		c.Next()
...
func authorizeSetupMutation(c *gin.Context) string {
	expected := strings.TrimSpace(os.Getenv("SETUP_TOKEN"))
	if expected != "" {
		...
		if provided == "" || subtle.ConstantTimeCompare([]byte(provided), []byte(expected)) != 1 {
			return "Invalid or missing SETUP_TOKEN (send X-Setup-Token header)"
		}
		return ""
	}
	// No token configured: only allow loopback clients for unauthenticated setup.
	ip := net.ParseIP(c.ClientIP())
	if ip == nil || !ip.IsLoopback() {
		return "Setup wizard is restricted to localhost; set SETUP_TOKEN to allow remote install"
	}
	return ""
}
