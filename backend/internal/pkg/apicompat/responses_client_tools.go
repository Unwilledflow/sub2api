...
func AdaptResponsesClientTools(req map[string]any) (ResponsesClientToolMapping, bool, error) {
	...
	// Preserve native namespaces when no Codex client-only tools need lowering;
	// flattening a namespace-only request would break Codex addressing and the
	// namespace-only body must remain byte-identical (TestAdapt...LeavesNamespaceOnly).
	// When flattening is required, keep the service-owned image_gen namespace
	// native so hasOpenAIImageGenerationTool stays true and the hosted
	// image_generation bridge is not injected alongside it.
	var names map[string]ResponsesNamespaceName
	var flattened bool
	if len(customNames) > 0 || adapter.ToolSearch || discovered {
		names, flattened, err = FlattenResponsesNamespacesExcept(req, map[string]bool{"image_gen": true})
		if err != nil {
			return ResponsesClientToolMapping{}, false, err
		}
	}
	...
	for _, raw := range tools {
		...
		case "custom":
			...
			delete(copy, "format")
			// custom 工具专属字段在 function 语义下无效，残留会触发上游校验
			// 报错（如 Codex CLI 的 defer_loading: "Invalid Value: 'tools.defer_loading'"）。
		...
		case "tool_search":
			...
			changed = true
		default:
			// strip-defer-loading-during-lowering: once tool_search is lowered
			// into a proxy function, strict upstreams reject defer_loading markers
			// that remain on sibling tools ("Deferred tools require
			// tools.tool_search"). Drop the marker; the proxy tool keeps the
			// discoverability the marker was advertising.
			if _, has := tool["defer_loading"]; has {
				stripped := copyClientTool(tool)
				delete(stripped, "defer_loading")
				lowered = append(lowered, stripped)
				changed = true
				continue
			}
			lowered = append(lowered, raw)
