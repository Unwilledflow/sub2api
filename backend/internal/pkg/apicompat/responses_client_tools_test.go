...
func TestAdaptResponsesClientTools_StripsCustomOnlyFieldsWhenLowering(t *testing.T) {
	req := map[string]any{
		"tools": []any{
			map[string]any{"type": "custom", "name": "exec", "format": map[string]any{"type": "grammar"}, "defer_loading": true},
		},
		"input": []any{
	...
	require.NotContains(t, exec, "defer_loading")
	require.NotContains(t, exec, "custom")
}
