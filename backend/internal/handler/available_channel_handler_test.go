...
func TestFilterUserVisibleGroups_NilAllowedKeepsAll(t *testing.T) {
	groups := []service.AvailableGroupRef{
		{ID: 1, Name: "g1", Platform: "anthropic"},
		{ID: 2, Name: "g2", Platform: "openai"},
	}

	visible := filterUserVisibleGroups(groups, nil)
	require.Len(t, visible, 2)
...
func TestMergeUserAvailableChannels_KeepsConfiguredAndAccountBackedModels(t *testing.T) {
	configured := []userAvailableChannel{
		{
			Name: "supergrok-build-official",
			Platforms: []userChannelPlatformSection{{
				Platform:        "openai",
				SupportedModels: []userSupportedModel{{Name: "grok-4.5", Platform: "openai"}},
			}},
		},
	}
	accountBacked := []userAvailableChannel{
		{
			...
			Platforms: []userChannelPlatformSection{{
				Platform:        "openai",
				SupportedModels: []userSupportedModel{{Name: "gpt-5.6-sol", Platform: "openai"}},
			}},
		},
	}

	merged := mergeUserAvailableChannels(configured, accountBacked)

	require.Len(t, merged, 2)
	require.Equal(t, "GPTPRO", merged[0].Name)
	require.Equal(t, "gpt-5.6-sol", merged[0].Platforms[0].SupportedModels[0].Name)
	require.Equal(t, "supergrok-build-official", merged[1].Name)
	require.Equal(t, "grok-4.5", merged[1].Platforms[0].SupportedModels[0].Name)
}
