...
func normalizeCodexImportEntry(entry codexImportEntry) (*codexImportAccount, error) {
	...
	case map[string]any:
		...
		authProvider := firstCodexString(raw,
			[]string{"auth_provider"},
			[]string{"authProvider"},
			[]string{"extra", "auth_provider"},
		)
		if authProvider != "" {
		...
		copyCodexCredentialValue(raw, item.Credentials, "expires_in", []string{"credentials", "expires_in"})
		copyCodexCredentialValue(raw, item.Credentials, "token_type", []string{"credentials", "token_type"})
	...
	setCodexCredentialIfNotEmpty(item.Credentials, "client_id", item.ClientID)
	if item.IDToken != "" {
...
func copyCodexCredentialValue(obj map[string]any, credentials map[string]any, key string, path []string) {
	value, ok := codexPathValue(obj, path)
	if !ok || value == nil {
		return
	}
