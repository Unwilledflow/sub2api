...
type channelMonitorCreateRequest struct {
	...
	ExternalRef      string            `json:"external_ref" binding:"omitempty,max=200"`
	PublicVisible    bool              `json:"public_visible"`
	ManagementMode   string            `json:"management_mode" binding:"omitempty,oneof=external"`
}
...
type channelMonitorExternalResultsRequest struct {
	Results []channelMonitorExternalResultRequest `json:"results" binding:"required,min=1,max=10,dive"`
}

type channelMonitorExternalResultRequest struct {
	Model         string    `json:"model" binding:"required,max=200"`
	Status        string    `json:"status" binding:"required,oneof=operational degraded failed error"`
	LatencyMs     *int      `json:"latency_ms" binding:"omitempty,min=0"`
	PingLatencyMs *int      `json:"ping_latency_ms" binding:"omitempty,min=0"`
...
func channelMonitorToResponse(m *service.ChannelMonitor) *channelMonitorResponse {
	...
	resp := &channelMonitorResponse{
		...
		ManagementMode:      m.ManagementMode,
...
// ExternalResults POST /api/v1/admin/channel-monitors/:id/external-results
func (h *ChannelMonitorHandler) ExternalResults(c *gin.Context) {
	id, ok := ParseChannelMonitorID(c)
	if !ok {
		return
	}
	var req channelMonitorExternalResultsRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.ErrorFrom(c, infraerrors.BadRequest("VALIDATION_ERROR", err.Error()))
		return
	}
	results := make([]*service.CheckResult, 0, len(req.Results))
	for _, item := range req.Results {
		results = append(results, &service.CheckResult{
			Model:         item.Model,
			Status:        item.Status,
	...
	if err := h.monitorService.RecordExternalResults(c.Request.Context(), id, results); err != nil {
		response.ErrorFrom(c, err)
		return
	}
	response.Success(c, gin.H{"recorded": len(results)})
}
