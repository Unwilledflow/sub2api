...
type backgroundAccountTestRunner interface {
...
type AccountHandler struct {
	...
	backgroundTestRunner    backgroundAccountTestRunner
	...
	healthScheduler         *service.AccountHealthScheduler
}
...
func NewAccountHandler(
	...
	return &AccountHandler{
		...
		backgroundTestRunner:    accountTestService,
...
func (h *AccountHandler) Create(c *gin.Context) {
	...
	if err := validateCreateAccountSchedulingFields(req.Priority, req.LoadFactor); err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *AccountHandler) Update(c *gin.Context) {
	...
	if err := validateAccountSchedulingFields(req.Priority, req.LoadFactor); err != nil {
		response.ErrorFrom(c, err)
		return
	}
	if req.RateMultiplier != nil && *req.RateMultiplier < 0 {
...
// BatchDeleteOpenAIOAuth deletes selected OpenAI OAuth accounts.
// POST /api/v1/admin/accounts/openai-oauth/batch-delete
func (h *AccountHandler) BatchDeleteOpenAIOAuth(c *gin.Context) {
	var req struct {
		AccountIDs []int64 `json:"account_ids" binding:"required,min=1,max=1000"`
	}
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	...
	for _, accountID := range accountIDs {
		...
		if account == nil {
			failed = append(failed, gin.H{"account_id": accountID, "error": "account not found"})
			continue
		}
		if account.Platform != service.PlatformOpenAI || account.Type != service.AccountTypeOAuth {
			skipped = append(skipped, gin.H{"account_id": accountID, "reason": "not an OpenAI OAuth account"})
			continue
		}
		candidates = append(candidates, account)
	}

	// Deleting a parent cascades to its shadows, so process selected shadows first.
	sort.SliceStable(candidates, func(i, j int) bool {
		return candidates[i].ParentAccountID != nil && candidates[j].ParentAccountID == nil
	})

	deletedIDs := make([]int64, 0, len(candidates))
	for _, account := range candidates {
		if err := h.adminService.DeleteAccount(c.Request.Context(), account.ID); err != nil {
			failed = append(failed, gin.H{"account_id": account.ID, "error": err.Error()})
			continue
		}
...
// BatchTestOpenAIOAuth tests selected OpenAI OAuth accounts with bounded concurrency.
// POST /api/v1/admin/accounts/openai-oauth/batch-test
func (h *AccountHandler) BatchTestOpenAIOAuth(c *gin.Context) {
	var req struct {
	...
	for index, accountID := range accountIDs {
		results[index].AccountID = accountID
		account := accountByID[accountID]
		switch {
		case account == nil:
			results[index].Error = "account not found"
		case account.Platform != service.PlatformOpenAI || account.Type != service.AccountTypeOAuth:
			results[index].Skipped = true
			results[index].Error = "not an OpenAI OAuth account"
		default:
			candidates = append(candidates, candidate{index: index, accountID: accountID})
		}
	}
	...
	for _, item := range candidates {
		...
		go func() {
			...
			case <-requestCtx.Done():
				results[item.index].Error = requestCtx.Err().Error()
				return
			}

			testCtx, cancel := context.WithTimeout(requestCtx, 90*time.Second)
			defer cancel()
			testResult, testErr := h.backgroundTestRunner.RunTestBackground(testCtx, item.accountID, "")
			if testErr != nil {
				results[item.index].Error = testErr.Error()
				return
			}
			if testResult == nil {
				results[item.index].Error = "test returned no result"
				return
			}
			results[item.index].LatencyMS = testResult.LatencyMs
			results[item.index].Success = testResult.Status == "success"
			if !results[item.index].Success {
				results[item.index].Error = testResult.ErrorMessage
				if results[item.index].Error == "" {
					results[item.index].Error = "account test failed"
				}
			}
		}()
	}
...
func (h *AccountHandler) BatchCreate(c *gin.Context) {
	...
	for index := range req.Accounts {
		if err := service.ValidateOpenAILongContextBillingExtra(req.Accounts[index].Platform, req.Accounts[index].Extra); err != nil {
			response.ErrorFrom(c, fmt.Errorf("accounts[%d]: %w", index, err))
			return
		}
		if err := validateCreateAccountSchedulingFields(req.Accounts[index].Priority, req.Accounts[index].LoadFactor); err != nil {
			response.ErrorFrom(c, fmt.Errorf("accounts[%d]: %w", index, err))
			return
		}
	}
