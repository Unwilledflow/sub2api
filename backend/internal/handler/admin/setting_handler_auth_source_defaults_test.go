...
func TestSettingHandler_UpdateSettings_AllowsOIDCDiscoveryURLWithoutIssuerURL(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repo := &settingHandlerRepoStub{
		values: map[string]string{
			service.SettingKeyPromoCodeEnabled: "true",
	...
	body := map[string]any{
		...
		"oidc_connect_client_id":                "new-api-client",
		"oidc_connect_client_secret":            "replace-with-secret",
		"oidc_connect_issuer_url":               "",
		"oidc_connect_discovery_url":            "https://issuer.example.com/.well-known/openid-configuration",
		"oidc_connect_scopes":                   "openid email profile",
		"oidc_connect_redirect_url":             "https://sub.example.com/api/v1/auth/oauth/oidc/callback",
	...
	require.Equal(t, http.StatusOK, rec.Code)
	require.Equal(t, "", repo.values[service.SettingKeyOIDCConnectIssuerURL])
	require.Equal(t, "https://issuer.example.com/.well-known/openid-configuration", repo.values[service.SettingKeyOIDCConnectDiscoveryURL])
}
