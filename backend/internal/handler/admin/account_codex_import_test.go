...
func TestParseCodexSessionImportEntriesSupportsSub2APIDataExport(t *testing.T) {
	...
	payload := map[string]any{
		...
		"accounts": []any{
			map[string]any{
				"name":     "exported-account-1",
				"platform": "openai",
				"type":     "oauth",
				"credentials": map[string]any{
					"access_token":       "nested-access-token-1",
					"refresh_token":      "nested-refresh-token-1",
					"client_id":          "nested-client-id-1",
					"email":              "nested-1@example.com",
					"chatgpt_account_id": "nested-account-id-1",
					"chatgpt_user_id":    "nested-user-id-1",
					"organization_id":    "nested-org-id-1",
					"plan_type":          "plus",
					"expires_at":         expiresAt.Format(time.RFC3339),
				},
			},
			map[string]any{
				"name":     "exported-account-2",
				"platform": "openai",
				"type":     "oauth",
				"credentials": map[string]any{
					"access_token": "nested-access-token-2",
					"email":        "nested-2@example.com",
	...
	for _, tc := range []struct {
		...
		{name: "direct export", content: payload},
		{name: "data envelope", content: map[string]any{"data": payload}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			first, err := normalizeCodexImportEntry(entries[0])
			if err != nil {
				t.Fatalf("normalize first account error = %v", err)
			}
			if first.Name != "exported-account-1" {
				t.Fatalf("name = %q, want exported-account-1", first.Name)
			}
			if first.AccessToken != "nested-access-token-1" {
				t.Fatalf("access token was not read from credentials")
			}
			if first.RefreshToken != "nested-refresh-token-1" {
				t.Fatalf("refresh token was not read from credentials")
			}
			if first.Credentials["client_id"] != "nested-client-id-1" {
				t.Fatalf("client_id = %v, want nested-client-id-1", first.Credentials["client_id"])
			}
			if first.Email != "nested-1@example.com" || first.AccountID != "nested-account-id-1" || first.UserID != "nested-user-id-1" {
				t.Fatalf("nested identity fields were not preserved")
			}
			if first.Organization != "nested-org-id-1" || first.PlanType != "plus" {
				t.Fatalf("nested account metadata was not preserved")
			}
			if first.TokenExpiresAt == nil || !first.TokenExpiresAt.Equal(expiresAt) {
				t.Fatalf("TokenExpiresAt = %v, want %v", first.TokenExpiresAt, expiresAt)
			}

			second, err := normalizeCodexImportEntry(entries[1])
			if err != nil {
				t.Fatalf("normalize second account error = %v", err)
			}
			if second.AccessToken != "nested-access-token-2" || second.Email != "nested-2@example.com" {
				t.Fatalf("second nested account fields were not preserved")
			}
		})
	}
}
