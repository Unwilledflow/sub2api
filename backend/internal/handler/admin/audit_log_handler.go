...
// WriteStats 暴露异步审计写入丢弃/失败计数，避免静默丢证不可观测。
// GET /api/v1/admin/audit-logs/write-stats
func (h *AuditLogHandler) WriteStats(c *gin.Context) {
	if h == nil || h.auditService == nil {
		response.Success(c, service.AuditLogWriteStats{})
		return
	}
	response.Success(c, h.auditService.Stats())
}
