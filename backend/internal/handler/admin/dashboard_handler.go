...
const dashboardBatchUsageMaxIDs = 200

func dashboardBatchUsageCacheKey(kind string, ids []int64) string {
...
func (h *DashboardHandler) GetBatchUsersUsage(c *gin.Context) {
	...
	if len(userIDs) > dashboardBatchUsageMaxIDs {
		response.BadRequest(c, fmt.Sprintf("user_ids exceeds limit of %d", dashboardBatchUsageMaxIDs))
		return
	}

	// cacheKey 必须包含当日日期，否则跨午夜后 30s 内会复用昨天的 "today_*" 结果。
	// Use a fixed-length hash so unbounded ID lists cannot inflate cache keys.
	cacheKey := dashboardBatchUsageCacheKey("users", userIDs)
...
func (h *DashboardHandler) GetBatchAPIKeysUsage(c *gin.Context) {
	...
	if len(apiKeyIDs) > dashboardBatchUsageMaxIDs {
		response.BadRequest(c, fmt.Sprintf("api_key_ids exceeds limit of %d", dashboardBatchUsageMaxIDs))
		return
	}

	cacheKey := dashboardBatchUsageCacheKey("api_keys", apiKeyIDs)
