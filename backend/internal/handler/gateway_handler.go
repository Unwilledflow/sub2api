...
// SetAdaptivePlanner attaches Adaptive route planner (Anthropic/Gemini parents).
...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	competitiveFirstOutputDeadline := requestStartedAt.Add(competitiveConcurrencyFirstOutput)
	...
	// Anti-Stall PRO + Adaptive leaf plan for Anthropic (and Gemini-on-/v1/messages).
	rootAPIKey := apiKey
	apiKey = h.startGatewayAdaptiveIfParent(
		c.Request.Context(), c, apiKey, reqModel,
		service.AdaptiveRouteProtocolAnthropicMessages, reqLog,
	)
	...
	if platform == service.PlatformGemini {
		...
		for {
			...
			if selection == nil || account == nil {
				reqLog.Warn("gateway.select_account_nil_account",
					zap.String("model", reqModel),
					zap.String("platform", platform),
				)
			...
			if !selection.Acquired {
				...
				if err != nil {
					...
					h.gatewayService.UnregisterGatewaySession(c.Request.Context(), account.ID, sessionKey)
			...
			if err != nil {
				...
				if errors.As(err, &failoverErr) {
					// 流式内容已写入客户端时默认禁止 failover（防止流拼接腐化）；
					// Anti-Stall 开启且 SafeToFailoverAfterWrite / force-leaf 策略时仍可内部切换。
					if service.OpenAICompactKeepaliveAdjustedWrittenSize(c) != writerSizeBeforeForward && !antiStallAllowFailoverAfterWrite(c, failoverErr) {
						submitGeminiUsage(result)
						h.handleFailoverExhausted(c, failoverErr, service.PlatformGemini, true)
						return
					}
					if !antiStallOnUpstreamFailure(c, failoverErr) {
					...
					case FailoverExhausted:
						// Try next Adaptive leaf before failing hard to client.
						if nextKey := h.advanceGatewayAdaptiveLeaf(c.Request.Context(), c, rootAPIKey, reqLog); nextKey != nil {
							apiKey = nextKey
							fs = NewFailoverState(h.maxAccountSwitchesGemini, hasBoundSession)
							continue
	...
	for {
		...
		for {
			...
			competitiveEnabled := competitiveAnthropicEnabled(rootAPIKey, currentAPIKey)
			// 选择支持该模型的账号
			...
			// 3. 获取账号并发槽位（含竞争模式：与普通路径一致先按 WaitPlan 排队）
			accountReleaseFunc := selection.ReleaseFunc
			if !selection.Acquired {
				if selection.WaitPlan == nil {
					markOpsRoutingCapacityLimited(c)
			...
			if !competitiveEnabled && channelMapping.Mapped {
			...
			if competitiveEnabled {
				selectNext := func(ctx context.Context, excludedIDs map[int64]struct{}) (*service.AccountSelectionResult, error) {
					return h.gatewayService.SelectAccountWithLoadAwareness(
						ctx, currentAPIKey.GroupID, sessionKey, reqModel, excludedIDs,
						parsedReq.MetadataUserID, subject.UserID,
					)
				}
				forward := func(ctx context.Context, attemptGin *gin.Context, candidate *service.Account) (*service.ForwardResult, *service.ParsedRequest, error) {
					return h.forwardCompetitiveAnthropicCandidate(
						ctx, attemptGin, candidate, parsedReq, body, channelMapping,
						currentAPIKey.GroupID, hasBoundSession, fs.ForceCacheBilling, reqLog,
					)
				}
				var winnerParsed *service.ParsedRequest
				account, result, winnerParsed, err = h.runCompetitiveAnthropicForward(
					c, competitiveFirstOutputDeadline, currentAPIKey.GroupID, sessionKey, selection, accountReleaseFunc,
					fs.FailedAccountIDs, reqModel, selectNext, forward, reqLog,
				)
				accountReleaseFunc = nil
				if winnerParsed != nil {
					attemptParsedReq = winnerParsed
				}
				if account != nil {
					setOpsSelectedAccount(c, account.ID, account.Platform)
				}
			...
			submitForwardUsage := func(result *service.ForwardResult) {
				...
				mergePendingCompetitiveAnthropicWaste(c, result)
			...
			if err != nil {
				...
				var failoverErr *service.UpstreamFailoverError
				if errors.As(err, &failoverErr) {
					// 流式内容已写入客户端时默认禁止 failover；Anti-Stall 开启且
					// SafeToFailoverAfterWrite / force-leaf 策略时仍可内部切换。
					if service.OpenAICompactKeepaliveAdjustedWrittenSize(c) != writerSizeBeforeForward && !antiStallAllowFailoverAfterWrite(c, failoverErr) {
					...
					switch action {
					case FailoverContinue:
						// Force-leaf errors: skip remaining accounts on this leaf when Adaptive has more leaves.
						if antiStallForceSwitchRecommended(failoverErr) {
							if nextKey := h.advanceGatewayAdaptiveLeaf(c.Request.Context(), c, rootAPIKey, reqLog); nextKey != nil {
			...
			if !competitiveEnabled {
				h.settleOrdinaryAnthropicSticky(c, currentAPIKey.GroupID, sessionKey, account, result, reqLog)
			}
			...
			//   下次请求粘性账号恢复后仍可命中
			if sessionKey != "" && anthropicStickyBindAllowed(c) && (sessionBoundAccountID == 0 || sessionBoundAccountID == account.ID) {
...
func billingErrorDetails(err error) (status int, code, message string, retryAfter int) {
	...
	// 余额预扣/流式续扣失败是确定性的客户端余额不足拒绝（上游已补偿/退款），
	// 必须显式映射 403 billing_error，不能落入下方 unknown→500 兜底。
	if errors.Is(err, service.ErrBalanceWithholdingFailed) {
		msg := pkgerrors.Message(err)
		if msg == "" {
			msg = "Insufficient balance, withholding failed"
		}
		return http.StatusForbidden, "billing_error", msg, 0
	}
	// Unknown / non-sentinel errors are internal failures, not client billing denials.
	// Mapping them to 403 billing_error misleads clients and pollutes ops classification.
	msg := pkgerrors.Message(err)
	if msg == "" || msg == "internal error" {
		logger.L().With(
			zap.String("component", "handler.gateway.billing"),
			zap.Error(err),
		).Error("gateway.billing_error_unmapped_internal")
		msg = "Internal error"
	} else {
		logger.L().With(
			zap.String("component", "handler.gateway.billing"),
	...
	return http.StatusInternalServerError, "api_error", msg, 0
}
