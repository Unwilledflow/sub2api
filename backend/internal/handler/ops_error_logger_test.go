...
func TestLogOpsStreamError_RecordsInBandConcurrencyLimit(t *testing.T) {
	...
	// ours 定制：限流是预期运营信号而非 P1 事故（classifyOpsSeverity 的
	// rate_limit_error 分支恒 P3），此处与生产行为对齐。
	require.Equal(t, "P3", job.entry.Severity)            // 用 IntendedStatus 429 分级
...
func TestNormalizeOpsErrorType(t *testing.T) {
	tests := []struct {
		...
		// Unknown/garbage types are rejected and fall through to code-based or "unknown".
		{"nil literal from upstream", "<nil>", "", "unknown"},
		{"null string", "null", "", "unknown"},
		{"random string", "something_weird", "", "unknown"},
