...
var (
	oauthFragmentBearerTokenPattern = regexp.MustCompile(`(?i)\b(Bearer\s+)([A-Za-z0-9._~+/=-]+)`)
	oauthFragmentSecretKeyPattern   = regexp.MustCompile(`\bsk-[A-Za-z0-9][A-Za-z0-9._-]*\b`)
)
...
func redirectOAuthError(c *gin.Context, frontendCallback string, code string, message string, description string) {
	fragment := url.Values{}
	fragment.Set("error", redactOAuthFragmentValue(code))
	if strings.TrimSpace(message) != "" {
		fragment.Set("error_message", redactOAuthFragmentValue(message))
	}
	if strings.TrimSpace(description) != "" {
		fragment.Set("error_description", redactOAuthFragmentValue(description))
	}
	redirectWithFragment(c, frontendCallback, fragment)
}

func redactOAuthFragmentValue(value string) string {
	redacted := logredact.RedactText(value)
	redacted = oauthFragmentBearerTokenPattern.ReplaceAllString(redacted, `${1}***`)
	redacted = oauthFragmentSecretKeyPattern.ReplaceAllString(redacted, `sk-***`)
	return truncateFragmentValue(redacted)
}
