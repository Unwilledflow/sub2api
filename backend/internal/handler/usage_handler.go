...
// userDashboardStatsCache 缓存用户仪表盘统计（30 秒/用户）。
// 累计聚合扫全量 usage_logs（用户 71 有 19 万行，约 125ms），30s 内重复刷新直接命中。
var userDashboardStatsCache = struct {
	mu      sync.RWMutex
	expires map[int64]time.Time
	payload map[int64]any
}{expires: map[int64]time.Time{}, payload: map[int64]any{}}
...
func (h *UsageHandler) DashboardStats(c *gin.Context) {
	...
	userDashboardStatsCache.mu.RLock()
	exp, hit := userDashboardStatsCache.expires[subject.UserID]
	cached := userDashboardStatsCache.payload[subject.UserID]
	userDashboardStatsCache.mu.RUnlock()
	...
	userDashboardStatsCache.mu.Lock()
	userDashboardStatsCache.expires[subject.UserID] = time.Now().Add(30 * time.Second)
	userDashboardStatsCache.payload[subject.UserID] = stats
	userDashboardStatsCache.mu.Unlock()
