...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	imageIntent := service.IsActiveImageGenerationIntent("/v1/responses", reqModel, body)
	fallbackAttempts := h.buildOpenAIGroupAttempts(c, apiKey, reqModel, service.AdaptiveRouteProtocolOpenAIResponses, true, imageIntent, reqLog)
	defer h.finishAdaptiveOpenAIRequest(c.Request.Context(), c, "request_end", reqLog)
	if imageIntent && !openAIPrimaryGroupAllowsImageGeneration(apiKey, fallbackAttempts) {
		if requestPlatform == service.PlatformGrok || service.IsExplicitImageGenerationIntent("/v1/responses", reqModel, body) {
			h.errorResponse(c, http.StatusForbidden, "permission_error", service.ImageGenerationPermissionMessage())
			return
		}
		strippedBody, changed, stripErr := service.StripOptionalImageGenerationTools(body)
		if stripErr != nil {
		...
		if changed {
			body = strippedBody
			imageIntent = service.IsActiveImageGenerationIntent("/v1/responses", reqModel, body)
			if err := h.resetAdaptiveOpenAISession(c.Request.Context(), c, "image_tools_replanned", reqLog); err != nil {
				h.errorResponse(c, http.StatusServiceUnavailable, "server_error", "Request replanning is temporarily unavailable")
				return
			}
			fallbackAttempts = h.buildOpenAIGroupAttempts(c, apiKey, reqModel, service.AdaptiveRouteProtocolOpenAIResponses, true, imageIntent, reqLog)
		}
	...
	for attemptIndex, fallbackAttempt := range fallbackAttempts {
		...
		if attemptAPIKey.GroupID != nil {
			// Adaptive: one MarkInFlight per leaf before account selection/upstream work.
			// H2 fail-closed: abort attempt if the in-flight fence cannot be persisted.
		...
		nextFallbackGroupID := func() *int64 {
			// Adaptive: never switch leaves after any response bytes are committed.
		...
		if imageIntent && !service.GroupAllowsImageGeneration(fallbackAttempt.Group) {
			if nextGroupID := nextFallbackGroupID(); nextGroupID != nil {
				attemptLog.Warn("openai.group_skipped_image_generation_disabled",
		...
		if err := h.billingCacheService.CheckBillingEligibility(c.Request.Context(), attemptAPIKey.User, attemptAPIKey, attemptAPIKey.Group, subscription, service.QuotaPlatform(c.Request.Context(), attemptAPIKey)); err != nil {
			attemptLog.Info("openai.billing_eligibility_check_failed", zap.Error(err))
			status, code, message, retryAfter := billingErrorDetails(err)
			if retryAfter > 0 {
				c.Header("Retry-After", strconv.Itoa(retryAfter))
			}
		...
		for {
			// Streaming forwarding may detach upstream draining from the client request.
			// Re-check before every replay so a disconnected client starts no new attempt.
			if !openAIRequestAllowsFailoverReplay(c) {
				return
			}
			...
			if err != nil {
				if failoverClientGone(c) {
					attemptLog.Info("openai.account_select_aborted_client_disconnected", zap.Error(err))
					return
				}
				if len(failedAccountIDs) == 0 {
					if errors.Is(err, service.ErrNoAvailableCompactAccounts) {
						if nextGroupID := nextFallbackGroupID(); nextGroupID != nil {
							attemptLog.Warn("openai.group_failover_switching",
								zap.String("reason", "compact_not_supported"),
								zap.Any("next_group_id", nextGroupID),
							)
							break
						}
		...
		if selection == nil || selection.Account == nil {
			...
			if previousResponseID != "" && selection != nil && selection.Account != nil {
				attemptLog.Debug("openai.account_selected_with_previous_response_id", zap.Int64("account_id", selection.Account.ID))
			}
			attemptLog.Debug("openai.account_schedule_decision",
			...
			if previousResponseID != "" && requestPlatform == service.PlatformOpenAI && !account.IsOpenAIApiKey() {
				...
				attemptLog.Debug("openai.account_skipped_http_continuation_unsupported",
			...
			attemptLog.Debug("openai.account_selected", zap.Int64("account_id", account.ID), zap.String("account_name", account.Name))
			...
			if groupCompetitiveConcurrencyEnabled(attemptAPIKey) {
				selectNext := func(ctx context.Context, excludedIDs map[int64]struct{}) (*service.AccountSelectionResult, error) {
					next, _, selectErr := h.gatewayService.SelectAccountWithSchedulerForCapabilityClass(
						ctx, attemptAPIKey.GroupID, previousResponseID, sessionHash, reqModel, excludedIDs,
						service.OpenAIUpstreamTransportAny, requiredCapability, requireCompact, false,
						!imageIntent, schedulerPerformanceClass, requestPlatform,
					)
			...
			forward := func(ctx context.Context, attemptGin *gin.Context, candidate *service.Account) (*service.OpenAIForwardResult, error) {
					attemptBody := h.deriveOpenAIForwardAttemptBody(attemptLog, forwardBody, candidate, &passthroughFailoverState)
					forwardCtx := h.withOpenAIFallbackResponseHeaderTimeout(ctx, apiKey, fallbackAttempts, attemptIndex, true, imageIntent, attemptLog)
					return h.gatewayService.Forward(forwardCtx, attemptGin, candidate, attemptBody)
				}
				...
				attemptBody := h.deriveOpenAIForwardAttemptBody(attemptLog, forwardBody, account, &passthroughFailoverState)
				result, err = func() (*service.OpenAIForwardResult, error) {
					...
					forwardCtx := h.withOpenAIFallbackResponseHeaderTimeout(c.Request.Context(), apiKey, fallbackAttempts, attemptIndex, true, imageIntent, attemptLog)
			...
			h.recordCyberPolicyIfMarked(c, attemptAPIKey, account, subscription, reqModel, shouldRecordStandaloneCyberUsage(err, result != nil), cyberBlockBodyHTTP, clientRequestedUsageFields(c, channelMapping, reqModel, ""), service.HashUsageRequestPayload(body))
			...
			// 入账；failover 错误恒定 result=nil，不会重复计费。终断 handler 返回前必须
			// 先提交 partial usage，避免已交付的语义输出丢失计量。
			submitResponsesUsage := func(res *service.OpenAIForwardResult) {
				if res == nil {
					return
				}
				partialUserAgent := c.GetHeader("User-Agent")
				partialClientIP := ip.GetClientIP(c)
				partialPayloadHash := service.HashUsageRequestPayload(body)
				partialInbound := GetInboundEndpoint(c)
				partialUpstream := resolveOpenAIUpstreamEndpoint(c, account, res)
				partialQuotaPlatform := service.QuotaPlatform(c.Request.Context(), attemptAPIKey)
				partialSessionID := service.ExtractClientSessionID(c)
				partialChannelFields := clientRequestedUsageFields(c, channelMapping, reqModel, res.UpstreamModel)
				h.submitOpenAIUsageRecordTask(c.Request.Context(), res, nil, func(ctx context.Context) {
					if err := h.gatewayService.RecordUsage(ctx, &service.OpenAIRecordUsageInput{
						...
						InboundEndpoint:    partialInbound,
						UpstreamEndpoint:   partialUpstream,
						UserAgent:          partialUserAgent,
						IPAddress:          partialClientIP,
						RequestPayloadHash: partialPayloadHash,
						APIKeyService:      h.apiKeyService,
						QuotaPlatform:      partialQuotaPlatform,
						SessionID:          partialSessionID,
						ChannelUsageFields: partialChannelFields,
						PricingAt:          pricingAt,
					}); err != nil {
						logger.L().With(
							zap.String("component", "handler.openai_gateway.responses"),
							zap.Int64("account_id", account.ID),
						).Error("openai.partial_record_usage_failed", zap.Error(err))
					}
				})
			}
			if err != nil {
				if result != nil && result.ImageCount > 0 {
					attemptLog.Warn("openai.forward_partial_error_with_image_result",
						zap.Int64("account_id", account.ID),
					...
					if errors.As(err, &failoverErr) {
						if failoverClientGone(c) {
							attemptLog.Info("openai.failover_aborted_client_disconnected",
								zap.Int64("account_id", account.ID),
					...
					if openAIFirstOutputFailoverExhausted(failoverErr, &firstOutputTimeoutSwitchCount) {
						...
						if nextGroupID := nextFallbackGroupID(); nextGroupID != nil {
							...
							case adaptiveLeafFailureHold:
								...
								failedAccountIDs = make(map[int64]struct{})
								attemptLog.Info("openai.adaptive_leaf_held",
							...
							case adaptiveLeafFailureSwitch:
								...
								attemptLog.Warn("openai.group_failover_switching",
									...
									zap.Float64("next_health_score", decision.NextHealthScore),
									zap.Int64("account_id", account.ID),
									zap.Int("upstream_status", failoverErr.StatusCode),
									zap.Any("next_group_id", nextGroupID),
								)
						...
						if failoverErr.RetryableOnSameAccount {
							...
							if sameAccountRetryCount[account.ID] < retryLimit {
								...
								attemptLog.Warn("openai.pool_mode_same_account_retry",
									zap.Int64("account_id", account.ID),
									zap.Int("upstream_status", failoverErr.StatusCode),
					...
					if maxAccountSwitches <= 0 || switchCount >= maxAccountSwitches {
						...
						if h.gatewayService.ShouldStopOpenAIOAuth429Failover(account, failoverErr.StatusCode, switchCount, &oauth429FailoverState) {
							// Anti-Stall: intercept OAuth 429 storm — switch leaf instead of
							// returning 429 to the client while leaf budget remains.
							if nextGroupID := nextFallbackGroupID(); nextGroupID != nil {
								if anti := service.AntiStallSessionFromGin(c); anti != nil && anti.Config().Enabled && !anti.ShouldFailHard() {
									...
									attemptLog.Warn("openai.anti_stall_pro_429_leaf_switch",
										zap.Int64("account_id", account.ID),
						...
						attemptLog.Warn("openai.upstream_failover_switching", failoverSwitchFields...)
						continue
					}
					h.gatewayService.ReportOpenAIAccountModelScheduleResultWithClass(account.ID, reqModel, schedulerPerformanceClass, false, nil)
					...
					if shouldLogOpenAIForwardFailureAsWarn(c, wroteFallback) {
						attemptLog.Warn("openai.forward_failed", fields...)
						return
					}
					attemptLog.Error("openai.forward_failed", fields...)
					return
				}
			}
			if result != nil {
				...
				h.gatewayService.ReportOpenAIAccountModelScheduleResultWithClass(account.ID, reqModel, schedulerPerformanceClass, openAIForwardSucceededForScheduling(result), result.FirstTokenMsForScheduling())
				...
				h.gatewayService.ReportOpenAIAccountModelScheduleResultWithClass(account.ID, reqModel, schedulerPerformanceClass, openAIForwardSucceededForScheduling(result), nil)
			}
			...
			// Passive Adaptive leaf health: record success before async usage settle.
			if openAIForwardSucceededForScheduling(result) {
			...
			attemptLog.Debug("openai.request_completed",
				zap.Int64("account_id", account.ID),
				zap.Int("switch_count", switchCount),
			)
			return
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		writerSizeBeforeForward := service.OpenAICompactKeepaliveAdjustedWrittenSize(c)
		var result *service.OpenAIForwardResult
		if groupCompetitiveConcurrencyEnabled(apiKey) {
			selectNext := func(ctx context.Context, excludedIDs map[int64]struct{}) (*service.AccountSelectionResult, error) {
				next, _, selectErr := h.gatewayService.SelectAccountWithSchedulerForCapabilityClass(
					ctx, apiKey.GroupID, "", sessionHash, currentRoutingModel, excludedIDs,
			...
			forward := func(ctx context.Context, attemptGin *gin.Context, candidate *service.Account) (*service.OpenAIForwardResult, error) {
				return h.gatewayService.ForwardAsAnthropic(ctx, attemptGin, candidate, forwardBody, promptCacheKey, defaultMappedModel)
			}
			account, result, err = h.runCompetitiveOpenAIForward(
				c, apiKey.GroupID, sessionHash, selection, accountReleaseFunc, failedAccountIDs,
				reqModel, schedulerPerformanceClass, selectNext, forward, reqLog,
			)
			if account != nil {
				setOpsSelectedAccount(c, account.ID, account.Platform)
			}
		...
		cyberBlocked := service.GetOpsCyberPolicy(c) != nil
		h.gatewayService.RecordOpenAIUpstreamSuccess(c.Request.Context(), account, result, cyberBlocked)
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	schedulerPerformanceClass := service.OpenAISchedulerPerformanceClass(firstMessage, reqModel)
	...
	// Explicit image requests require native Responses capability; passive image_gen catalogs do not.
	requiredCapability := service.OpenAIEndpointCapabilityChatCompletions
	...
	for {
		...
		hooks := &service.OpenAIWSIngressHooks{
			...
			AfterTurn: func(turn int, result *service.OpenAIForwardResult, turnErr error) {
				...
				cyberBlocked := service.GetOpsCyberPolicy(c) != nil
				h.gatewayService.RecordOpenAIUpstreamSuccess(ctx, account, result, cyberBlocked)
			h.submitOpenAIUsageRecordTask(ctx, result, nil, func(taskCtx context.Context) {
...
func (h *OpenAIGatewayHandler) submitOpenAIUsageRecordTask(
	...
	adaptiveSession *openAIAdaptiveSession,
	task service.UsageRecordTask,
) {
	if adaptiveSession != nil && task != nil {
		settlementTask := task
		task = func(ctx context.Context) {
			adaptiveSession.stopAdaptiveBillingHeartbeat()
			settlementTask(ctx)
		}
	}
	if adaptiveSession != nil || result != nil && result.ImageCount > 0 {
		h.submitMandatoryUsageRecordTask(parent, task)
		return
	}
