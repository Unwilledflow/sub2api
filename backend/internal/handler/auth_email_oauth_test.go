...
func TestEmailOAuthCallbackExistingEmailRequiresBindLoginWhenInvitationEnabled(t *testing.T) {
	// ours 定制：已验证 OAuth 邮箱命中本地已有账号但缺少 identity 时，
	// 不再静默自动绑定登录，而是走 bind_login pending 会话要求密码绑定
	// （emailOAuthPendingKind → emailOAuthPendingBindLogin）。
	handler, client := newOAuthPendingFlowTestHandler(t, true)
	ctx := context.Background()
	...
	require.Equal(t, "oauth_account_bind_required", completion["error"])

	// 未通过密码绑定前不得创建 OAuth identity。
...
func (r *oauthEmailAffiliateRepoStub) AccrueQuota(context.Context, int64, int64, float64, int, *int64, float64) (bool, float64, error) {
...
func (r *oauthEmailAffiliateRepoStub) ClawbackQuotaForOrder(context.Context, int64) (float64, error) {
