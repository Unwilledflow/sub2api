...
func normalizeOpsErrorType(errType string, code string) string {
	...
	default:
		// Unknown types stay "unknown" so the ops board does not mislabel them as platform api_error.
...
func classifyOpsPhase(errType, message, code string) string {
	...
	case "api_error":
		...
		return "internal"
	case "unknown":
		// Unclassified errors are not assumed to be platform-internal.
		return "upstream"
	default:
		return "internal"
	}
...
func classifyOpsSeverity(errType string, status int) string {
	...
	case "invalid_request_error", "authentication_error", "permission_error", "forbidden_error", "not_found_error", "model_not_found", "billing_error", "subscription_error":
		return "P3"
	case "rate_limit_error":
		// Business / upstream rate limits are expected operational signals, not P1 outages.
		return "P3"
	case "unknown":
		return "P2"
	}
	...
	if status == 429 {
		// Generic 429 without a classified type: treat as business throttle (P3), not platform P1.
		return "P3"
	}
	if status >= 400 {
		return "P2"
