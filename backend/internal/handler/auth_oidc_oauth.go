...
var (
	oidcOAuthCallbackReplayTTL   = 30 * time.Second
	oidcOAuthCallbackReplayGroup singleflight.Group
	oidcOAuthCallbackReplayStore = newOIDCOAuthCallbackReplayCache(oidcOAuthCallbackReplayTTL)
)
...
type oidcOAuthCapturedResponse struct {
	Status int
	Header http.Header
	Body   []byte
}

type oidcOAuthCallbackReplayEntry struct {
	response  oidcOAuthCapturedResponse
	expiresAt time.Time
}

type oidcOAuthCallbackReplayCache struct {
	mu      sync.Mutex
	ttl     time.Duration
	entries map[string]oidcOAuthCallbackReplayEntry
}

func newOIDCOAuthCallbackReplayCache(ttl time.Duration) *oidcOAuthCallbackReplayCache {
	return &oidcOAuthCallbackReplayCache{
		ttl:     ttl,
		entries: make(map[string]oidcOAuthCallbackReplayEntry),
	}
}

func (c *oidcOAuthCallbackReplayCache) get(key string) (oidcOAuthCapturedResponse, bool) {
	if c == nil || strings.TrimSpace(key) == "" {
		return oidcOAuthCapturedResponse{}, false
	}
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *oidcOAuthCallbackReplayCache) set(key string, response oidcOAuthCapturedResponse) {
	if c == nil || strings.TrimSpace(key) == "" || !response.replayable() {
		return
	}
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	c.entries[key] = oidcOAuthCallbackReplayEntry{
		response:  response.clone(),
		expiresAt: now.Add(c.ttl),
	}
}

func (r oidcOAuthCapturedResponse) clone() oidcOAuthCapturedResponse {
	cloned := oidcOAuthCapturedResponse{
		Status: r.Status,
	...
	if len(r.Body) > 0 {
		cloned.Body = append([]byte(nil), r.Body...)
	}
	return cloned
}

func (r oidcOAuthCapturedResponse) replayable() bool {
	if r.Status != http.StatusFound {
		return false
	}
	location := strings.TrimSpace(r.Header.Get("Location"))
	if location == "" {
		return false
	}
...
func (r oidcOAuthCapturedResponse) writeTo(c *gin.Context) {
	if c == nil {
		return
	}
	for key, values := range r.Header {
...
func oidcOAuthCallbackReplayKey(c *gin.Context, cfg config.OIDCConnectConfig) string {
	if c == nil {
		return ""
	}
	...
	if code == "" || state == "" || browserSessionKey == "" {
		return ""
	}
	sum := sha256.Sum256([]byte(
		strings.TrimSpace(cfg.ClientID) + "\x00" +
			strings.TrimSpace(cfg.TokenURL) + "\x00" +
			strings.TrimSpace(cfg.RedirectURL) + "\x00" +
			state + "\x00" +
			code + "\x00" +
			browserSessionKey,
	))
...
func (h *AuthHandler) captureOIDCOAuthCallback(c *gin.Context, cfg config.OIDCConnectConfig) oidcOAuthCapturedResponse {
	recorder := httptest.NewRecorder()
	capturedContext, _ := gin.CreateTestContext(recorder)
	capturedContext.Request = c.Request.Clone(c.Request.Context())
	capturedContext.Params = append(gin.Params(nil), c.Params...)
	if len(c.Keys) > 0 {
		capturedContext.Keys = make(map[string]any, len(c.Keys))
		for key, value := range c.Keys {
			capturedContext.Keys[key] = value
		}
	}

	h.handleOIDCOAuthCallbackWithConfig(capturedContext, cfg)

	result := recorder.Result()
	captured := oidcOAuthCapturedResponse{
		Status: recorder.Code,
		Header: make(http.Header, len(result.Header)),
		Body:   append([]byte(nil), recorder.Body.Bytes()...),
	}
	for key, values := range result.Header {
		captured.Header[key] = append([]string(nil), values...)
	}
	return captured
}
...
func (h *AuthHandler) OIDCOAuthCallback(c *gin.Context) {
	...
	replayKey := oidcOAuthCallbackReplayKey(c, cfg)
	if replayKey == "" {
		h.handleOIDCOAuthCallbackWithConfig(c, cfg)
		return
	}

	if cached, ok := oidcOAuthCallbackReplayStore.get(replayKey); ok {
		cached.writeTo(c)
		return
	}

	result, _, _ := oidcOAuthCallbackReplayGroup.Do(replayKey, func() (any, error) {
		captured := h.captureOIDCOAuthCallback(c, cfg)
		oidcOAuthCallbackReplayStore.set(replayKey, captured)
		return captured, nil
	})
	if captured, ok := result.(oidcOAuthCapturedResponse); ok {
		captured.writeTo(c)
		return
	}
...
func (h *AuthHandler) handleOIDCOAuthCallback(c *gin.Context) {
	cfg, cfgErr := h.getOIDCOAuthConfig(c.Request.Context())
	if cfgErr != nil {
		response.ErrorFrom(c, cfgErr)
		return
...
func (h *AuthHandler) handleOIDCOAuthCallbackWithConfig(c *gin.Context, cfg config.OIDCConnectConfig) {
	...
	if err != nil {
		...
		if errors.As(err, &exchangeErr) && exchangeErr != nil {
			log.Printf(
				...
				oidcRedactProviderBodyForLog(exchangeErr.Body, 2048),
			)
...
func oidcRedactProviderBodyForLog(body string, maxLen int) string {
	return truncateLogValue(logredact.RedactText(body), maxLen)
}
...
func oidcParseUserInfo(body string, cfg config.OIDCConnectConfig) *oidcUserInfoClaims {
	...
	claims.DisplayName = firstNonEmpty(
		getGJSON(body, cfg.UserInfoDisplayPath),
		getGJSON(body, "name"),
