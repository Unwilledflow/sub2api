...
func TestOIDCRedactsProviderBodyBeforeLogging(t *testing.T) {
	body := `{"error":"invalid_client","access_token":"secret-access","refresh_token":"secret-refresh","id_token":"secret-id","client_secret":"secret-client"}`

	out := oidcRedactProviderBodyForLog(body, 2048)

	require.Contains(t, out, "invalid_client")
	require.NotContains(t, out, "secret-access")
	require.NotContains(t, out, "secret-refresh")
	require.NotContains(t, out, "secret-id")
	require.NotContains(t, out, "secret-client")
	require.Contains(t, out, `"access_token":"***"`)
	require.Contains(t, out, `"refresh_token":"***"`)
	require.Contains(t, out, `"id_token":"***"`)
	require.Contains(t, out, `"client_secret":"***"`)
}
...
func TestOIDCOAuthCallbackReplaysDuplicateCallbackWithoutReusingCode(t *testing.T) {
	tokenCalls := 0
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/token":
			...
			if tokenCalls > 1 {
				...
				_, _ = w.Write([]byte(`{"error":"invalid_grant","error_description":"Invalid or expired authorization code"}`))
				return
			}
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"access_token":"oidc-access","token_type":"Bearer","expires_in":3600}`))
		case "/userinfo":
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"sub":"oidc-replay-subject","preferred_username":"oidc_replay","name":"OIDC Replay","email":"replay@example.com"}`))
		default:
			http.NotFound(w, r)
		}
	}))
	...
	makeRequest := func() *http.Request {
		req := httptest.NewRequest(http.MethodGet, "/api/v1/auth/oauth/oidc/callback?code=oidc-replay-code&state=state-replay", nil)
		req.AddCookie(encodedCookie(oidcOAuthStateCookieName, "state-replay"))
		req.AddCookie(encodedCookie(oidcOAuthRedirectCookie, "/dashboard"))
		req.AddCookie(encodedCookie(oidcOAuthIntentCookieName, oauthIntentLogin))
		req.AddCookie(encodedCookie(oauthPendingBrowserCookieName, "browser-replay"))
		return req
	}

	firstRecorder := httptest.NewRecorder()
	firstCtx, _ := gin.CreateTestContext(firstRecorder)
	firstCtx.Request = makeRequest()
	handler.OIDCOAuthCallback(firstCtx)

	secondRecorder := httptest.NewRecorder()
	secondCtx, _ := gin.CreateTestContext(secondRecorder)
	secondCtx.Request = makeRequest()
	handler.OIDCOAuthCallback(secondCtx)

	require.Equal(t, http.StatusFound, firstRecorder.Code)
	require.Equal(t, http.StatusFound, secondRecorder.Code)
	require.Equal(t, "/auth/oidc/callback", firstRecorder.Header().Get("Location"))
	require.Equal(t, firstRecorder.Header().Get("Location"), secondRecorder.Header().Get("Location"))
	require.NotNil(t, findCookie(firstRecorder.Result().Cookies(), oauthPendingSessionCookieName))
	require.NotNil(t, findCookie(secondRecorder.Result().Cookies(), oauthPendingSessionCookieName))
	require.Equal(t, 1, tokenCalls)

	pendingCount, err := client.PendingAuthSession.Query().Count(context.Background())
	require.NoError(t, err)
...
func TestTryOIDCVerifiedEmailFastPathTakesPriorityOverForcedEmailSignup(t *testing.T) {
	...
	identity := service.PendingAuthIdentityKey{
		ProviderType:    "oidc",
		ProviderKey:     "https://issuer.example.com",
		ProviderSubject: "fast-path-force-email-bypass",
	}
	completed := handler.tryOIDCVerifiedEmailFastPath(
		c,
		"/auth/oidc/callback",
	...
	createdUser, err := client.User.Query().Where(dbuser.EmailEQ("force-email@example.com")).Only(context.Background())
	require.NoError(t, err)
	require.Equal(t, "oidc-subject-10001", createdUser.Username)
	...
	identityCount, err := client.AuthIdentity.Query().
		Where(
			...
			authidentity.ProviderTypeEQ("oidc"),
			authidentity.ProviderKeyEQ("https://issuer.example.com"),
			authidentity.ProviderSubjectEQ("fast-path-force-email-bypass"),
		).
