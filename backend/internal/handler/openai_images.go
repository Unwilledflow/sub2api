...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	fallbackAttempts := h.buildOpenAIFallbackGroupAttempts(c.Request.Context(), apiKey, false, true, reqLog)
	if !openAIFallbackAttemptsAllowImageGeneration(fallbackAttempts) {
	...
	for attemptIndex, fallbackAttempt := range fallbackAttempts {
		if fallbackAttempt.Fallback {
			var ok bool
			fallbackAttempt, ok = h.resolveOpenAIFallbackGroupAttempt(requestCtx, apiKey, fallbackAttempt, false, reqLog)
			if !ok {
		...
		nextFallbackGroupID := func() *int64 {
			return h.nextUsableOpenAIFallbackGroupID(requestCtx, apiKey, fallbackAttempts, attemptIndex, false, true, attemptLog)
		}
		if !service.GroupAllowsImageGeneration(fallbackAttempt.Group) {
			if nextGroupID := nextFallbackGroupID(); nextGroupID != nil {
				attemptLog.Warn("openai.images.group_skipped_image_generation_disabled",
		...
		channelMapping, _ := h.gatewayService.ResolveChannelMappingAndRestrict(c.Request.Context(), attemptAPIKey.GroupID, routingModel)
		...
		if err := h.billingCacheService.CheckBillingEligibility(c.Request.Context(), attemptAPIKey.User, attemptAPIKey, attemptAPIKey.Group, subscription, service.QuotaPlatform(c.Request.Context(), attemptAPIKey)); err != nil {
			attemptLog.Info("openai.images.billing_eligibility_check_failed", zap.Error(err))
		...
		for {
			attemptLog.Debug("openai.images.account_selecting", zap.Int("excluded_account_count", len(failedAccountIDs)))
			...
			if err != nil {
				if failoverClientGone(c) {
					attemptLog.Info("openai.images.account_select_aborted_client_disconnected", zap.Error(err))
					return
				}
				attemptLog.Warn("openai.images.account_select_failed",
					zap.Error(err),
					zap.Int("excluded_account_count", len(failedAccountIDs)),
				)
				if len(failedAccountIDs) == 0 {
					if nextGroupID := nextFallbackGroupID(); nextGroupID != nil {
						attemptLog.Warn("openai.images.group_failover_switching",
							zap.String("reason", "account_select_failed"),
							zap.Any("next_group_id", nextGroupID),
						)
						break
					}
					cls := classifyNoAccountErrorFromGin(c, h.gatewayService, attemptAPIKey, routingModel, clientRequestModel, service.PlatformOpenAI)
			...
			attemptLog.Debug("openai.images.account_schedule_decision",
			...
			attemptLog.Debug("openai.images.account_selected", zap.Int64("account_id", account.ID), zap.String("account_name", account.Name))
			setOpsSelectedAccount(c, account.ID, account.Platform)

			accountReleaseFunc, slotResult := h.acquireResponsesAccountSlot(c, attemptAPIKey.GroupID, sessionHash, selection, parsed.Stream, &streamStarted, attemptLog)
			...
			result, err := func() (*service.OpenAIForwardResult, error) {
				...
				forwardCtx := h.withOpenAIFallbackResponseHeaderTimeout(requestCtx, apiKey, fallbackAttempts, attemptIndex, false, true, attemptLog)
			...
			if err != nil {
				if result != nil && result.ImageCount > 0 {
					...
					if errors.As(err, &imageUpstreamErr) {
						...
						h.gatewayService.ReportOpenAIAccountModelScheduleResult(account.ID, routingModel, !retryableServerError, nil)
					...
					if errors.As(err, &failoverErr) {
						h.gatewayService.ReportOpenAIAccountScheduleResult(account, openAIAccountScheduleModel(c, account, requestModel, false, result), false, nil, err)
						h.gatewayService.ReportOpenAIAccountModelScheduleResult(account.ID, routingModel, false, nil)
		...
		if result != nil {
				...
				h.gatewayService.ReportOpenAIAccountModelScheduleResult(account.ID, routingModel, true, result.FirstTokenMsForScheduling())
			} else {
				h.gatewayService.ReportOpenAIAccountScheduleResult(account, openAIAccountScheduleModel(c, account, requestModel, false, result), true, nil)
				h.gatewayService.ReportOpenAIAccountModelScheduleResult(account.ID, routingModel, true, nil)
			}
			...
			attemptLog.Debug("openai.images.request_completed",
				zap.Int64("account_id", account.ID),
				zap.Int("switch_count", switchCount),
			)
			return
