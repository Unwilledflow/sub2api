...
const (
	...
	// maxSelectionExhaustedRetries 单账号分组 503 选号耗尽退避重试次数上限，
	// 防止上游真挂时以固定间隔无限重试打爆上游。
	maxSelectionExhaustedRetries = 5
...
type FailoverState struct {
	...
	profitVetoCount int
	// selectionExhaustedRetries 503 选号耗尽退避重试次数，用于上限保护，
	// 避免上游真挂时以固定间隔无限重试。
...
func (s *FailoverState) HandleSelectionExhausted(ctx context.Context) FailoverAction {
	...
	if s.LastFailoverErr != nil &&
		...
		// 503 退避重试次数上限：上游真挂时避免以固定间隔无限重试打爆上游。
		s.selectionExhaustedRetries++
		if s.selectionExhaustedRetries > maxSelectionExhaustedRetries {
			logger.FromContext(ctx).Warn("gateway.failover_selection_exhausted_retry_limit",
