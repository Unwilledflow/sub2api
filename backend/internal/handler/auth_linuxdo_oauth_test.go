...
func TestRedirectOAuthErrorRedactsSecretsInFragment(t *testing.T) {
	...
	redirectOAuthError(
		...
		"provider_error",
		"invalid_client client_secret=secret-client",
		"access_token=secret-access Bearer secret-bearer sk-secret-key refresh_token=secret-refresh",
	)

	require.Equal(t, http.StatusFound, recorder.Code)
	location := recorder.Header().Get("Location")
	require.Contains(t, location, "/auth/linuxdo/callback#")
	require.NotContains(t, location, "secret-client")
	require.NotContains(t, location, "secret-access")
	require.NotContains(t, location, "secret-bearer")
	require.NotContains(t, location, "sk-secret-key")
	require.NotContains(t, location, "secret-refresh")

	fragment := parseOAuthRedirectFragment(t, location)
	require.Equal(t, "provider_error", fragment.Get("error"))
	require.Contains(t, fragment.Get("error_message"), "client_secret=***")
	require.Contains(t, fragment.Get("error_description"), "access_token=***")
	require.Contains(t, fragment.Get("error_description"), "Bearer ***")
	require.Contains(t, fragment.Get("error_description"), "sk-***")
}
