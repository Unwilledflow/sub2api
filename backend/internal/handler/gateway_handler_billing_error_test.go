...
func TestBillingErrorDetails_UnknownErrorMapsTo500(t *testing.T) {
	status, code, msg, _ := billingErrorDetails(errors.New("db connection lost"))
	require.Equal(t, http.StatusInternalServerError, status)
	require.Equal(t, "api_error", code)
	require.NotEmpty(t, msg)
}
