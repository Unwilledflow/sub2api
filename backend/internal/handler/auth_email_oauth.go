...
func (h *AuthHandler) emailOAuthCallbackWithProfile(
	...
	pendingKind, err := h.emailOAuthPendingKind(c.Request.Context(), input)
	if err != nil {
		redirectOAuthError(c, frontendCallback, infraerrors.Reason(err), infraerrors.Message(err), "")
		return
	}
	switch pendingKind {
	case emailOAuthPendingRegistration:
		...
		return
	case emailOAuthPendingBindLogin:
		if pendingErr := h.createEmailOAuthBindLoginPendingSession(c, provider, frontendCallback, redirectTo, profile); pendingErr != nil {
			redirectOAuthError(c, frontendCallback, infraerrors.Reason(pendingErr), infraerrors.Message(pendingErr), "")
			return
		}
		redirectToFrontendCallback(c, frontendCallback)
	...
	if err != nil {
		...
		if errors.Is(err, service.ErrOAuthAccountBindRequired) {
			if pendingErr := h.createEmailOAuthBindLoginPendingSession(c, provider, frontendCallback, redirectTo, profile); pendingErr != nil {
				redirectOAuthError(c, frontendCallback, infraerrors.Reason(pendingErr), infraerrors.Message(pendingErr), "")
				return
			}
...
const (
	emailOAuthPendingNone         = ""
	emailOAuthPendingRegistration = "registration"
	emailOAuthPendingBindLogin    = "bind_login"
)

func (h *AuthHandler) emailOAuthPendingKind(ctx context.Context, input service.EmailOAuthIdentityInput) (string, error) {
	client := h.entClient()
	if client == nil {
	...
	if identityUser != nil {
		if !strings.EqualFold(strings.TrimSpace(identityUser.Email), email) {
			return "", infraerrors.Conflict("AUTH_IDENTITY_EMAIL_MISMATCH", "oauth identity belongs to a different email")
		}
		return emailOAuthPendingNone, nil
	}
	if _, err := findUserByNormalizedEmail(ctx, client, email); err != nil {
		if errors.Is(err, service.ErrUserNotFound) {
			return emailOAuthPendingRegistration, nil
		}
		return "", err
	}
	// Verified OAuth email matches an existing local account without this identity:
	// require password bind-login instead of silent auto-bind.
	return emailOAuthPendingBindLogin, nil
}
...
func (h *AuthHandler) emailOAuthShouldCreatePendingRegistration(ctx context.Context, input service.EmailOAuthIdentityInput) (bool, error) {
	kind, err := h.emailOAuthPendingKind(ctx, input)
	if err != nil {
		return false, err
	}
	return kind == emailOAuthPendingRegistration, nil
}
...
func (h *AuthHandler) createEmailOAuthBindLoginPendingSession(
	c *gin.Context,
	provider string,
	frontendCallback string,
	redirectTo string,
	...
	if client := h.entClient(); client != nil {
		if existing, findErr := findUserByNormalizedEmail(c.Request.Context(), client, email); findErr == nil && existing != nil && existing.ID > 0 {
			id := existing.ID
			targetUserID = &id
		}
	}
	...
	completionResponse := map[string]any{
		...
		"error":                     "oauth_account_bind_required",
		"choice_reason":             "existing_email_requires_bind",
		"adoption_required":         false,
