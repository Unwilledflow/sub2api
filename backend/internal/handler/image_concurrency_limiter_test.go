...
func TestOpenAIGatewayHandlerResponses_DisabledGroupStripsOptionalImageTools(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := `{
		"model":"gpt-5.5",
		"input":[
	...
	newImagePermissionTestHandler().Responses(c)

	require.NotEqual(t, http.StatusForbidden, rec.Code)
	require.NotContains(t, rec.Body.String(), service.ImageGenerationPermissionMessage())
}

func TestOpenAIGatewayHandlerResponses_DisabledGroupRejectsExplicitImageRequests(t *testing.T) {
	gin.SetMode(gin.TestMode)
	tests := []struct {
		name string
		body string
	}{
		{
			...
			body: `{"model":"gpt-5.4","input":"draw","tools":[{"type":"image_generation"}],"tool_choice":{"type":"image_generation"}}`,
		},
		{
			name: "image model",
...
func newImagePermissionTestHandler() *OpenAIGatewayHandler {
