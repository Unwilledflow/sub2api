...
func APIKeyFromService(k *service.APIKey) *APIKey {
	...
	out := &APIKey{
		...
		AntiStallTier:                  service.NormalizeAntiStallTier(k.AntiStallTier),
		AdaptivePassiveFailoverEnabled: k.AdaptivePassiveFailoverEnabled,
		BlockOpenAIFast:                k.BlockOpenAIFast,
		Status:                         k.Status,
...
func groupFromServiceBase(g *service.Group) Group {
	return Group{
		...
		Platform:                        g.Platform,
		IsAdaptiveParent:                g.IsAdaptiveParent,
		UserVisible:                     g.UserVisible,
		RateMultiplier:                  g.RateMultiplier,
		RateMultiplierMin:               g.RateMultiplierMin,
		RateMultiplierMax:               g.RateMultiplierMax,
...
func usageLogFromServiceUser(l *service.UsageLog) UsageLog {
	...
	outputTokensPerSecond := deriveOutputTokensPerSecond(l, requestType)
	return UsageLog{
		ID:                        l.ID,
		UserID:                    l.UserID,
		APIKeyID:                  l.APIKeyID,
		...
		AdaptiveBaseCost:          l.AdaptiveBaseCost,
		AdaptiveManagementFeeCost: l.AdaptiveManagementFeeCost,
		AdaptiveParentGroupID:     l.AdaptiveParentGroupID,
		RoutedGroupID:             l.RoutedGroupID,
		AdaptiveAttemptNo:         l.AdaptiveAttemptNo,
		AdaptivePricingSnapshotID: l.AdaptivePricingSnapshotID,
		AdaptiveReservationID:     l.AdaptiveReservationID,
		AdaptiveSettlementStatus:  l.AdaptiveSettlementStatus,
		...
		AdaptiveParentGroup:       GroupFromServiceShallow(l.AdaptiveParentGroup),
		RoutedGroup:               GroupFromServiceShallow(l.RoutedGroup),
		Subscription:              UserSubscriptionFromService(l.Subscription),
	}
}

// outputRateWindowCap bounds plausible per-stream generation speed. When the
// first-token window rate exceeds it, the recorded first_token_ms is degenerate:
// buffered or reasoning streams only surface visible output at the end of the
// stream, so duration-first_token collapses toward zero and the window rate
// explodes. In that case we fall back to end-to-end throughput instead.
const outputRateWindowCap = 1500.0

// deriveOutputTokensPerSecond excludes the first-token interval and only
// reports complete streaming rows, so sync and partial responses remain clear.
func deriveOutputTokensPerSecond(l *service.UsageLog, requestType service.RequestType) *float64 {
	if l == nil || (requestType != service.RequestTypeStream && requestType != service.RequestTypeWSV2) ||
		l.OutputTokens <= 0 || l.DurationMs == nil || l.FirstTokenMs == nil ||
		*l.DurationMs <= *l.FirstTokenMs || *l.FirstTokenMs < 0 {
		return nil
	}
	windowSeconds := float64(*l.DurationMs-*l.FirstTokenMs) / 1000
	if windowSeconds <= 0 {
		return nil
	}
	rate := float64(l.OutputTokens) / windowSeconds
	if rate > outputRateWindowCap {
		totalSeconds := float64(*l.DurationMs) / 1000
		if totalSeconds <= 0 {
			return nil
		}
		rate = float64(l.OutputTokens) / totalSeconds
	}
	return &rate
}
