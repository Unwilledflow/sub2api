...
func registerAuditLogRoutes(admin *gin.RouterGroup, h *handler.Handlers, _ middleware.StepUpAuthMiddleware) {
	...
	// write-stats is read-only observability for drop/write-fail counters (13-H4).
	auditLogs.GET("/write-stats", h.Admin.AuditLog.WriteStats)
	{
...
func registerAdaptiveRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	// Anti-Stall PRO (抗中断) — stream hold-back + drip recovery module.
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.GET("/health-scheduler/settings", h.Admin.Account.GetHealthSchedulerPolicy)
		accounts.PUT("/health-scheduler/settings", h.Admin.Account.UpdateHealthSchedulerPolicy)
		...
		accounts.POST("/openai-oauth/batch-delete", h.Admin.Account.BatchDeleteOpenAIOAuth)
		accounts.POST("/openai-oauth/batch-test", h.Admin.Account.BatchTestOpenAIOAuth)
...
func registerChannelMonitorRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	monitors := admin.Group("/channel-monitors")
	{
		...
		monitors.POST("/:id/external-results", h.Admin.ChannelMonitor.ExternalResults)
