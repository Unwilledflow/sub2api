...
func registerRoutes(
	...
	// 通用路由（健康检查、状态等）
	routes.RegisterCommonRoutesWithOptions(r, routes.CommonRouteOptions{
		ReadinessChecks: []routes.ReadinessCheck{
			routes.DatabaseReadinessCheck(db),
			routes.RedisReadinessCheck(redisClient),
		},
	})
