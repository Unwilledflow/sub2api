...
func TestAPIKeyAuthForwardsUserScopedOpenAIFastPolicyToUpstream(t *testing.T) {
		...
		Rules: []service.OpenAIFastPolicyRule{
			{
				...
				Scope:          service.BetaPolicyScopeAPIKey,
				UserIDs:        []int64{44},
	...
	apiKeys := map[string]*service.APIKey{
		...
		"key-user-44": newOpenAIFastPolicyForwardingAPIKey(3, "key-user-44", 44, groupID, group),
	}
	...
	explicitPriorityBody := `{"model":"gpt-5","stream":false,"service_tier":"priority","input":"hi"}`
	send("key-user-42", explicitPriorityBody)
	send("key-user-43", explicitPriorityBody)
	send("key-user-44", `{"model":"gpt-5","stream":false,"input":"hi"}`)
