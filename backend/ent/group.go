...
type Group struct {
	...
	// Race up to three accounts and commit the first valid response byte
	CompetitiveConcurrency bool `json:"competitive_concurrency,omitempty"`
	// 是否启用高峰时段倍率
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldCompetitiveConcurrency:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field competitive_concurrency", values[i])
			} else if value.Valid {
				_m.CompetitiveConcurrency = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("competitive_concurrency=")
	builder.WriteString(fmt.Sprintf("%v", _m.CompetitiveConcurrency))
	builder.WriteString(", ")
