...
func (APIKey) Fields() []ent.Field {
	return []ent.Field{
		...
		field.JSON("fallback_group_ids", []int64{}).
			Optional().
			Comment("User-selected fallback group IDs for this API key"),
		field.Bool("block_openai_fast").
			Default(true).
			Comment("Strip OpenAI fast/priority service tier and prevent priority injection for this API key"),
		...
		field.Float("reserved_quota_usd").
			SchemaType(map[string]string{dialect.Postgres: "decimal(24,10)"}).
			Default(0).
			Comment("Quota held by active Adaptive requests"),
		...
		field.Float("reserved_usage_5h_usd").
			SchemaType(map[string]string{dialect.Postgres: "decimal(24,10)"}).
			Default(0).
			Comment("5h spend held by active Adaptive requests"),
		field.Float("reserved_usage_1d_usd").
			SchemaType(map[string]string{dialect.Postgres: "decimal(24,10)"}).
			Default(0).
			Comment("1d spend held by active Adaptive requests"),
		field.Float("reserved_usage_7d_usd").
			SchemaType(map[string]string{dialect.Postgres: "decimal(24,10)"}).
			Default(0).
			Comment("7d spend held by active Adaptive requests"),
		// Window start times
