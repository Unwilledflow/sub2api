...
func (UsageLog) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("actual_cost").
			Default(0).
			SchemaType(map[string]string{dialect.Postgres: "decimal(20,10)"}),
		field.Float("adaptive_base_cost").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "decimal(20,10)"}),
		field.Float("adaptive_management_fee_cost").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "decimal(20,10)"}),
		field.Float("adaptive_total_cost").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "decimal(20,10)"}),
		field.Float("adaptive_uncapped_base_cost").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "decimal(20,10)"}),
		field.Float("adaptive_platform_overage_cost").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "decimal(20,10)"}),
		field.Int64("adaptive_parent_group_id").
			Optional().
			Nillable(),
		field.Int64("routed_group_id").
			Optional().
			Nillable(),
		field.Int("adaptive_attempt_no").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "smallint"}),
		field.String("adaptive_pricing_snapshot_id").
			MaxLen(128).
			Optional().
			Nillable(),
		field.String("adaptive_reservation_id").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "uuid"}),
		field.String("adaptive_evidence_hash").
			MaxLen(64).
			Optional().
			Nillable(),
		field.String("adaptive_settlement_status").
			MaxLen(16).
			Optional().
			Nillable(),
