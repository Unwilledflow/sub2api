...
type UserSubscription struct {
	...
	// ReservedUsageUsd holds the value of the "reserved_usage_usd" field.
	ReservedUsageUsd float64 `json:"reserved_usage_usd,omitempty"`
...
func (*UserSubscription) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case usersubscription.FieldDailyUsageUsd, usersubscription.FieldWeeklyUsageUsd, usersubscription.FieldMonthlyUsageUsd, usersubscription.FieldReservedUsageUsd:
...
func (_m *UserSubscription) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usersubscription.FieldReservedUsageUsd:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field reserved_usage_usd", values[i])
			} else if value.Valid {
				_m.ReservedUsageUsd = value.Float64
			}
...
func (_m *UserSubscription) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("reserved_usage_usd=")
	builder.WriteString(fmt.Sprintf("%v", _m.ReservedUsageUsd))
	builder.WriteString(", ")
