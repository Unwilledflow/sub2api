<template>
  <aside
    ...
    :class="[
      sidebarCollapsed ? 'w-[76px]' : 'w-[17.5rem]',
  ...
  >
    ...
    <div class="sidebar-header" :class="{ 'sidebar-header-collapsed': sidebarCollapsed }">
      ...
      <router-link
        :to="homePath"
        class="brand-logo-frame sidebar-logo h-10 w-10 transition-opacity hover:opacity-90"
        @click="handleMenuItemClick(homePath)"
      >
        <img v-if="settingsLoaded" :src="siteLogo || '/logo-v2.svg'" alt="Logo" class="h-full w-full object-contain" />
      </router-link>
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/models', label: t('nav.modelMarketplace'), icon: SparklesIcon, hideInSimpleMode: true, featureFlag: flagAvailableChannels },
  ...
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/adaptive-groups', label: t('nav.adaptiveGroups'), icon: AdaptiveIcon, hideInSimpleMode: true },
    {
      ...
      children: [
        ...
        { path: '/admin/channels/group-monitor', label: t('nav.groupMonitor'), icon: SignalIcon },
      ],
    },
    { path: '/admin/subscriptions', label: t('nav.subscriptions'), icon: CreditCardIcon, hideInSimpleMode: true },
    { path: '/admin/accounts', label: t('nav.accounts'), icon: GlobeIcon },
    { path: '/admin/oauth-accounts', label: t('nav.oauthAccounts'), icon: KeyIcon },
